...
def filtered_tickers(tickers):
    """개선된 조건에 맞는 티커 필터링 - 계층적 기회 포착 시스템 (최적화 버전)"""
       
    # 결과 저장 리스트 초기화 (UnboundLocalError 방지)
    filtered_tickers = []
    selected_details = []
    
    # 🛡️ 방어적 프로그래밍: 안전한 병렬 처리를 위한 헬퍼 함수들
    def safe_executor_shutdown(executor, timeout=5):
        """ThreadPoolExecutor 안전한 종료"""
        try:
            executor.shutdown(wait=True, cancel_futures=True)
        except Exception as e:
            print(f"⚠️ Executor 종료 중 오류 (무시됨): {e}")
            try:
                # 강제 종료 시도
    ...
    def safe_future_result(future, default_value=None, timeout=10):
        """Future 결과를 안전하게 가져오기"""
        try:
            return future.result(timeout=timeout)
        except TimeoutError:
            print("⚠️ 작업 타임아웃 (기본값 반환)")
            return default_value
        except Exception as e:
            print(f"⚠️ Future 결과 처리 오류 (기본값 반환): {e}")
            return default_value
    
    # === 🌍 시장 전체 심리 분석 (완전 안전한 병렬 처리) ===
    market_tickers = [
        'KRW-BTC', 'KRW-ETH', 'KRW-XRP', 'KRW-ADA', 'KRW-DOGE',
        'KRW-SOL', 'KRW-AVAX', 'KRW-DOT', 'KRW-MATIC', 'KRW-LINK'
    ]
    
    def get_market_rsi_safe(ticker):
        """완전 안전한 시장 RSI 데이터 수집"""
        try:
            # 인터프리터 종료 체크
            if not hasattr(sys, 'ps1'):  # 대화형 세션이 아닌 경우
                return None
                
            rsi_data = get_rsi(ticker, 14, interval=min5)
            if rsi_data is not None and not rsi_data.empty and len(rsi_data) > 0:
                result = float(rsi_data.iloc[-1])
                return result if 0 < result < 100 else None
        except KeyboardInterrupt:
            print(f"🛑 사용자 중단: {ticker}")
            return None
        except SystemExit:
            print(f"🛑 시스템 종료: {ticker}")
            return None
        except Exception as e:
            # 모든 예외를 조용히 처리
            return None
    ...
    try:
        ...
        # 순차 처리로 대체 (안정성 우선)
        for ticker in market_tickers[:5]:  # 상위 5개만 사용하여 속도 향상
            try:
                result = get_market_rsi_safe(ticker)
                if result is not None:
                    market_rsi_values.append(result)
                time.sleep(0.1)  # 안전한 간격
                
                # 충분한 데이터 확보 시 조기 종료
                if len(market_rsi_values) >= 3:
                    break
            except Exception:
                continue
        
        market_data_success = len(market_rsi_values) >= 2
        
    except Exception as e:
        print(f"⚠️ 시장 데이터 수집 실패, 기본값 사용: {e}")
        market_rsi_values = [50.0, 50.0]  # 안전한 기본값
        market_data_success = False
    
    # 시장 RSI 계산 (견고한 기본값 제공)
    if market_rsi_values:
        market_rsi = float(np.mean(market_rsi_values))
        market_volatility = float(np.std(market_rsi_values)) if len(market_rsi_values) > 1 else 10.0
    else:
        market_rsi = 50.0  # 중립 상태로 기본값 설정
        market_volatility = 10.0
        print("⚠️ 시장 데이터 없음 - 중립 상태로 분석 진행")
    
    # ✨ 개선된 적응형 시장 상태 분석 ✨
    market_conditions = [
        (25, "극과매도", "패닉셀링-적극포착", 1.0, 2.0, [120, 100, 80]),
        (35, "과매도", "하락끝-다단계포착", 0.9, 1.7, [130, 110, 90]),
        (45, "약과매도", "조정구간-균형포착", 0.7, 1.3, [140, 120, 100]),
        (55, "중립", "안정구간-표준포착", 0.6, 1.0, [145, 125, 105]),
        (65, "강세중립", "상승추세-균형포착", 0.7, 1.1, [140, 120, 100]),
        (75, "과매수", "상승중-신중포착", 0.5, 0.8, [150, 130, 110]),
        (100, "극과매수", "과열-완벽조건만", 0.3, 0.6, [170, 150, 130])
    ]
    ...
    perfect_threshold, good_threshold, decent_threshold = [145, 125, 105]
    
    for threshold, status, strategy, aggression, multiplier, thresholds in market_conditions:
        if market_rsi < threshold:
            market_status = status
            strategy_note = strategy
            buy_aggression = aggression
            opportunity_multiplier = multiplier
            perfect_threshold, good_threshold, decent_threshold = thresholds
            break
    ...
    if market_volatility > 15:
        ...
        perfect_threshold = max(70, perfect_threshold - 10)
        good_threshold = max(60, good_threshold - 10)
        decent_threshold = max(50, decent_threshold - 10)
    
    print(f"🌍 시장심리: RSI {market_rsi:.1f} ({market_status}) | {strategy_note}")
    print(f"📊 변동성: {market_volatility:.1f} ({volatility_note}) | 데이터품질: {'✅' if market_data_success else '⚠️'}")
    print(f"🎯 기준점: 완벽{perfect_threshold} | 우수{good_threshold} | 괜찮음{decent_threshold}")
    print("=" * 80)
    
    # === 🔍 개별 코인 분석 (완전 안전한 순차 처리) ===
    prediction_summary = {'SURGE': 0, 'UP': 0, 'DOWN': 0, 'CRASH': 0, 'NEUTRAL': 0}
    all_candidates = []
    
    def analyze_single_ticker_safe(ticker):
        """완전 안전한 개별 티커 분석"""
        try:
            # 인터프리터 상태 체크
            if not hasattr(sys, 'ps1') and threading.current_thread() != threading.main_thread():
                return None
            ...
            if df is None or current_price is None or len(df) < 20:
                return None
            ...
            try:
                ...
                if rsi_data is not None and not rsi_data.empty and len(rsi_data) > 0:
                    ...
                    if len(rsi_data) >= 3:
                        rsi_values = rsi_data.values[-3:]
                        rsi_momentum = rsi_values[-1] > rsi_values[-2] >= rsi_values[-3]
            except Exception:
                pass
            
            # 2. 볼린저 밴드 분석 (안전한 기본값)
            bb_position = 0.5  # 중립 위치
            band_expanding = False
            
            try:
                bb_window = 12 if market_rsi < 30 else (30 if market_rsi > 70 else 20)
                bb_std = 1.6 if market_rsi < 30 else (2.2 if market_rsi > 70 else 2.0)
                
                bands_df = get_bollinger_bands(ticker, interval=min5, window=bb_window, std_dev=bb_std)
                if bands_df is not None and not bands_df.empty:
                    upper_band = float(bands_df['Upper_Band'].iloc[-1])
                    lower_band = float(bands_df['Lower_Band'].iloc[-1])
                    ...
                    if upper_band > lower_band > 0:
                        ...
                        band_expanding = band_diff_ratio > 0.05
            except Exception:
                # 볼린저 밴드 실패 시 간단한 계산
                try:
                    sma20 = np.mean(closes[-20:])
                    bb_position = 0.3 if current_price < sma20 else 0.7
                except Exception:
                    bb_position = 0.5
            ...
            try:
                divergence_result = get_rsi_bul_diver(ticker)
                has_divergence = bool(divergence_result.get('is_bullish_divergence', False)) if divergence_result else False
            except Exception:
                has_divergence = False
            ...
            prediction = 'NEUTRAL'
            try:
                pred_result = predict_price_direction(ticker)
                if pred_result and len(pred_result) >= 1:
                    prediction = str(pred_result[0]) if pred_result[0] in ['SURGE', 'UP', 'DOWN', 'CRASH', 'NEUTRAL'] else 'NEUTRAL'
            except Exception:
                prediction = 'NEUTRAL'
            ...
            # RSI 기준 조정
            adjusted_rsi_s = rsi_buy_s * (0.9 if market_rsi < 35 else 1.05 if market_rsi > 65 else 0.95)
            adjusted_rsi_e = rsi_buy_e * (1.1 if market_rsi < 35 else 0.95 if market_rsi > 65 else 1.05)
            
            # 기본 조건 체크
            is_rsi_good = adjusted_rsi_s < current_rsi < adjusted_rsi_e
            is_position_good = bb_position < (0.4 + 0.15 * buy_aggression)
            
            # 모멘텀 분석 (안전한 처리)
            is_momentum_ok = True  # 기본값
            try:
                if len(closes) >= 4:
                    price_change_5min = (current_price - closes[-2]) / closes[-2]
                    price_change_15min = (current_price - closes[-4]) / closes[-4]
                    is_momentum_ok = price_change_5min > -0.015 and price_change_15min > -0.025
            except Exception:
                is_momentum_ok = True
            
            # 거래량 분석 (안전한 처리)
            is_volume_good = True  # 기본값
            try:
                if len(volumes) >= 10:
                    vol_recent = np.mean(volumes[-3:])
                    vol_avg = np.mean(volumes[-10:])
                    if vol_avg > 0:
                        is_volume_good = vol_recent > vol_avg * (1.03 + buy_aggression * 0.02)
            except Exception:
                is_volume_good = True
            ...
            # 핵심 지표 점수 (70점 만점)
            if is_rsi_good: base_score += 25
            if is_position_good: base_score += 20
            if band_expanding: base_score += 15
            if has_divergence: base_score += 30  # 가장 중요한 지표
            
            # 보조 지표 점수 (30점 만점)
            if is_momentum_ok: base_score += 12
            if is_volume_good: base_score += 8
            if rsi_momentum: base_score += 10
            
            # 예측 보너스/페널티 (±25점)
            prediction_bonus = {'SURGE': 25, 'UP': 12, 'NEUTRAL': 0, 'DOWN': -12, 'CRASH': -25}.get(prediction, 0)
            
            # 시장 환경 보너스 (±15점)
            market_bonus = 0
            if market_rsi < 30 and prediction in ['SURGE', 'UP']:
                market_bonus = 15
            elif market_rsi > 70 and prediction in ['DOWN', 'CRASH']:
                market_bonus = -15
            elif market_rsi < 35 and prediction == 'SURGE':
                market_bonus = 8
            
            # 최종 점수 계산
            final_score = (base_score + prediction_bonus + market_bonus) * opportunity_multiplier
            ...
            safety_passed = sum(safety_conditions) >= 3
            
            if safety_passed:
                # 핵심 조건 체크
                core_conditions = [is_rsi_good, is_position_good, has_divergence, prediction in ['SURGE', 'UP']]
                core_score = sum(core_conditions)
                
                if final_score >= perfect_threshold and core_score >= 3:
                    buy_decision, buy_tier, buy_reason = True, "🏆 PERFECT", "완벽한 조건 충족"
                elif final_score >= good_threshold and core_score >= 2:
                    buy_decision, buy_tier, buy_reason = True, "🥇 GOOD", "우수한 조건 충족"
                elif final_score >= decent_threshold and buy_aggression >= 0.6 and core_score >= 1:
                    buy_decision, buy_tier, buy_reason = True, "🥈 DECENT", "괜찮은 조건 충족"
            
            # 🔥 특수 상황 (극과매도 + 다이버전스)
            if not buy_decision and market_rsi < 25 and has_divergence and prediction != 'CRASH':
                buy_decision, buy_tier, buy_reason = True, "🔥 SPECIAL", "극과매도+다이버전스"
                final_score = max(final_score, decent_threshold + 5)
            
            # 예측 카운트 업데이트
            prediction_summary[prediction] = prediction_summary.get(prediction, 0) + 1
            ...
            # 모든 예외를 조용히 처리하고 None 반환
            return None
    
    # === 🚀 순차 처리로 모든 티커 분석 (ThreadPool 완전 제거) ===
    print(f"🔄 {len(tickers)}개 티커 순차 분석 시작...")
    ...
    for i, ticker in enumerate(tickers):
        try:
            ...
            if i % 5 == 0 or i == len(tickers) - 1:
                ...
                print(f"📊 진행률: {progress:.1f}% ({i+1}/{len(tickers)})")
            
            result = analyze_single_ticker_safe(ticker)
            if result is not None:
                analyzed_count += 1
                all_candidates.append(result)
                
                if result['buy_decision']:
                    filtered_tickers.append(result['ticker'])
                    selected_details.append(result)
                    
                    # 실시간 결과 출력
                    filtering_time = datetime.now().strftime('%H:%M:%S')
                    print(f"✅ **{result['buy_tier']}**: {result['ticker']} (점수: {result['final_score']:.0f}) - {filtering_time}")
                    print(f"   └ RSI:{result['current_rsi']:.1f} | 위치:{result['bb_position']:.1%} | 예측:{result['prediction']} | 다이버전스:{'✅' if result['has_divergence'] else '❌'}")
                    print("-" * 60)
            ...
            print(f"⚠️ {ticker} 분석 중 오류 (계속 진행): {str(e)[:50]}")
            continue
    ...
    # 매수 기회가 부족할 때만 기준 완화
    if total_selected == 0 and total_analyzed >= 5:
        print("🔄 **기회 부족 감지** - 기준 완화 적용...")
        
        # 점수순 정렬
        all_candidates.sort(key=lambda x: x['final_score'], reverse=True)
        
        # 완화된 기준 (20% 하향)
        emergency_threshold = decent_threshold * 0.8
        
        for candidate in all_candidates[:2]:  # 상위 2개만 검토
            if (candidate['final_score'] >= emergency_threshold and 
                candidate['prediction'] not in ['DOWN', 'CRASH'] and
                candidate['current_rsi'] < 70 and
                candidate['safety_passed']):
                
                print(f"🚨 **긴급 기회**: {candidate['ticker']} (점수: {candidate['final_score']:.0f})")
                filtered_tickers.append(candidate['ticker'])
                candidate['buy_tier'] = "🆘 EMERGENCY"
                candidate['buy_reason'] = "기준 완화 적용"
                selected_details.append(candidate)
    ...
    print("=" * 100)
    print(f"🎯 **분석 완료**: {total_analyzed}개 분석 → {len(filtered_tickers)}개 선정 ({selection_rate:.1f}%)")
    print(f"📊 **시장 환경**: {market_status} (RSI: {market_rsi:.1f}) | 품질: {'양호' if market_data_success else '제한적'}")
    print(f"🔮 **예측 분포**: 급상승{prediction_summary.get('SURGE', 0)} | 상승{prediction_summary.get('UP', 0)} | 중립{prediction_summary.get('NEUTRAL', 0)} | 하락{prediction_summary.get('DOWN', 0)} | 폭락{prediction_summary.get('CRASH', 0)}")
    
    if len(filtered_tickers) > 0:
        print(f"🎉 **매수 기회 {len(filtered_tickers)}개 발견!**")
        
        # 간단한 요약 메시지
        summary_msg = f"🎉 매수 기회 {len(filtered_tickers)}개 발견!\n"
        summary_msg += f"시장: {market_status} (RSI: {market_rsi:.1f})\n"
        
        for detail in selected_details:
            summary_msg += f"{detail['ticker']} {detail['buy_tier']} (점수: {detail['final_score']:.0f})\n"
        
        summary_msg += f"{datetime.now().strftime('%m/%d %H:%M:%S')} 완료"
        ...
        if all_candidates:
            ...
            print(f"🥈 **최고점수**: {best['ticker']} ({best['final_score']:.0f}점) - 조건 미달")
        
        print("💡 현재 매수 조건을 만족하는 기회가 없습니다.")
        
        # 기준점 정보 제공
        if perfect_threshold >= 150:
            print("💡 현재 높은 기준점이 적용되고 있습니다.")
        elif buy_aggression < 0.5:
            print("💡 시장 과열로 신중한 접근이 적용되고 있습니다.")
