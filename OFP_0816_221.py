...
def analyze_optimal_band_margin(ticker, lookback_periods=150, target_periods=[5, 10], min_gain_threshold=0.02):
    """
    과거 데이터 분석을 통한 최적 밴드마진 도출 함수
    
     Parameters:
    - ticker: 분석할 티커 (예: 'KRW-BTC')
    - lookback_periods: 분석할 과거 데이터 개수 (기본 150개, 최대 200개)
    - target_periods: 미래 수익률 계산 기간들 (5봉 후, 10봉 후)
    - min_gain_threshold: 의미있는 상승으로 간주할 최소 수익률 (기본 2%)
    ...
    try:
        ...
        # 1. 200개 한도 내에서 데이터 수집 (분석용 + 지표계산용 여유분)
        total_needed = min(lookback_periods + 50, 200)  # 최대 200개로 제한
        df = pyupbit.get_ohlcv(ticker, interval="minute5", count=total_needed)
        if df is None or len(df) < lookback_periods:
            return None
            
        # 2. 각 시점별 조건-결과 데이터 생성
        analysis_data = []
        
        for i in range(30, min(len(df) - max(target_periods), lookback_periods + 30)):  # 200개 한도 준수
            try:
                # 현재 시점 데이터
                ...
                # 볼린저밴드 계산 (20일, 2 표준편차)
                close_prices = current_slice['close'].values
                ...
                # RSI 계산 (14일) - 기존 함수 활용
                try:
                    rsi_data = get_rsi(ticker, 14, interval="minute5")
                    if rsi_data is not None and len(rsi_data) > 0:
                        rsi = rsi_data.iloc[-1]  # 최신 RSI 값 사용
                    else:
                        # 백업: 내장 함수로 직접 계산
                        rsi = calculate_rsi_for_analysis(close_prices, 14)
                except:
                    # 기존 함수 호출 실패시 백업 방식 사용
                    rsi = calculate_rsi_for_analysis(close_prices, 14)
                
                # 거래량 지표
                volumes = current_slice['volume'].values
                vol_sma_5 = np.mean(volumes[-5:]) if len(volumes) >= 5 else volumes[-1]
                ...
                # 가격 모멘텀
                price_momentum = (close_prices[-1] / close_prices[-5] - 1) if len(close_prices) >= 5 else 0
                
                # === 종속변수들 계산 (미래 수익률) ===
                future_returns = {}
                for period in target_periods:
                    if i + period < len(df):
                        future_price = df['close'].iloc[i + period]
                        return_rate = (future_price / current_price) - 1
                        future_returns[f'return_{period}'] = return_rate
                        future_returns[f'is_profitable_{period}'] = return_rate > min_gain_threshold
                    else:
                        break
                
                # 데이터 추가
                if future_returns:  # 미래 데이터가 있는 경우만
                    data_point = {
        ...
        # 3. DataFrame 변환 및 통계 분석 (최소 데이터 체크)
        if len(analysis_data) < 30:  # 200개 한도에서 최소 30개 확보
            print(f"❌ {ticker}: 분석 데이터 부족 ({len(analysis_data)}개, 최소 30개 필요)")
            return None
        ...
        for period in target_periods:
            ...
            # 상관관계 계산
            correlations = {
                'band_width_ratio': stats.pearsonr(df_analysis['band_width_ratio'], df_analysis[return_col])[0],
                'bb_position': stats.pearsonr(df_analysis['bb_position'], df_analysis[return_col])[0],
                'rsi': stats.pearsonr(df_analysis['rsi'], df_analysis[return_col])[0],
                'volume_ratio': stats.pearsonr(df_analysis['volume_ratio'], df_analysis[return_col])[0],
                'price_momentum': stats.pearsonr(df_analysis['price_momentum'], df_analysis[return_col])[0]
            }
            ...
            # 5. 최적 임계값 탐색 (ROC 곡선 이용)
            X = df_analysis[['band_width_ratio', 'bb_position', 'rsi', 'volume_ratio', 'price_momentum']]
            y = df_analysis[profit_col].astype(int)
            ...
            try:
                ...
                optimal_prob_threshold = thresholds[optimal_idx]
                
                # 밴드폭 기준 최적 임계값 계산
                successful_trades = df_analysis[df_analysis[profit_col] == True]
                failed_trades = df_analysis[df_analysis[profit_col] == False]
                
                if len(successful_trades) > 3 and len(failed_trades) > 3:  #200개 한도에 맞춘 최소 기준
                    # 성공한 거래들의 밴드폭 분포 분석
                    success_band_mean = successful_trades['band_width_ratio'].mean()
                    success_band_std = successful_trades['band_width_ratio'].std()
                    
                    # 최적 밴드폭 임계값 (성공 확률 70% 이상 구간)
                    band_threshold = success_band_mean - 0.5 * success_band_std
                    
                    optimal_thresholds[period] = {
                        'band_width_threshold': max(0.01, band_threshold),  # 최소값 보장
                        'probability_threshold': optimal_prob_threshold,
                        'success_rate': len(successful_trades) / len(df_analysis),
                        'roc_auc': auc(fpr, tpr),
                        'feature_importance': dict(zip(['band_width', 'bb_position', 'rsi', 'volume', 'momentum'], 
                                                     lr.coef_[0]))
                    }
                    
                    print(f"  🎯 최적 밴드폭 임계값: {band_threshold:.4f}")
                    print(f"  📊 성공률: {len(successful_trades)/len(df_analysis)*100:.1f}%")
        ...
        # 기간별 가중 평균으로 최종 임계값 계산
        weights = {5: 0.7, 10: 0.3}  # 단기에 더 높은 가중치
        final_threshold = sum(optimal_thresholds[period]['band_width_threshold'] * weights.get(period, 1) 
                            for period in optimal_thresholds.keys() if period in weights)
        final_threshold /= sum(weights.get(period, 1) for period in optimal_thresholds.keys() if period in weights)
...
def trade_buy(ticker):
    ...
    for b in balances:
        if b['currency'] not in ["KRW", "QI", "ONX", "ETHF", "ETHW", "PURSE"]:
            try:
                asset_ticker = f"KRW-{b['currency']}"
                current_price = upbit.get_current_price(asset_ticker)
                asset_value = float(b['balance']) * current_price
                
                if asset_value >= 10000:
                    significant_assets_count += 1
            except Exception as e:
                # 가격 조회 실패 시 무시하고 다음 자산으로
                print(f"매수 로직 / 가격 조회 실패: {asset_ticker}, 에러: {e}")

    # === 2. 보유 자산 수에 따른 매수 금액 결정 ===
    buy_size = 0
    
    # 1만원 이상 보유한 자산이 1개 이상일 때: 보유 원화 전량 매수
    if significant_assets_count >= 1:
        print("➡️ 1만원 이상 보유 자산이 1개 이상. 보유 원화 전량 매수 로직 적용.")
        buy_size = krw * 0.9995  # 수수료 고려
        
    # 1만원 이상 보유한 자산이 없을 때: 보유 원화 절반 매수
    else:
        print("➡️ 1만원 이상 보유 자산 없음. 보유 원화 절반 매수 로직 적용.")
        buy_size = krw * 0.5
