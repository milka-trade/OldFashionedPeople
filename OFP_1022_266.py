...
def trade_sell(ticker):
    ...
    if position_info:
        ...
        expected_gain = position_info['expected_gain']        # 예: 3.2%
        ...
        # ═══════════════════════════════════════════════════════════
        """
        기존: target_profit = expected_gain × 0.8 (너무 보수적)
        개선: target_profit = max(1.0, expected_gain × 0.9) (공격적)
        
        원칙:
        1. 최소 1.0% 보장 (너무 낮은 목표 방지)
        2. 기대값이 높으면 90%까지 추구 (80% → 90%)
        3. 손익비 높으면 추가 상향 (최대 110%)
        """
        
        # 기본 목표: 기대값의 90% (기존 80%보다 상향)
        base_target = expected_gain * 0.9
        
        # 최소 1.0% 보장
        target_profit = max(1.0, base_target)
        
        # 손익비에 따른 추가 상향
        if profit_loss_ratio >= 4.0:  # 매우 높은 손익비
            target_profit = max(1.0, expected_gain * 1.0)  # 기대값 100%
        elif profit_loss_ratio >= 3.5:  # 높은 손익비
            target_profit = max(1.0, expected_gain * 0.95)  # 기대값 95%
        
        print(f"📊 [{ticker}] 개별전략: 손절{dynamic_stop_loss:.1f}% | 목표{target_profit:.1f}% (기대:{expected_gain:.1f}%) | 손익비{profit_loss_ratio:.1f} | {risk_level}")
    else:
        # 정보가 없으면 기존 기준 사용 (호환성)
        dynamic_stop_loss = -4.0
        target_profit = 1.0  # 최소 1.0% 고정
        expected_gain = 1.5
        profit_loss_ratio = 2.0
        risk_level = "기본"
        
        # print(f"📊 [{ticker}] 기본전략: 손절{dynamic_stop_loss:.1f}% | 목표{target_profit:.1f}%")
    ...
    개선점:
    - 기존: 고정 min_rate 사용
    - 신규: 최소 1.0% + 기대값의 90~100% 목표
    - 손익비 고려: 높은 손익비 = 더 오래 홀딩
    """
    
    # 최소 수익률 미달 시 대기 (목표의 40% = 더 일찍 감시 시작)
    if profit_rate < target_profit * 0.4:
        return None
    ...
    # ═══════════════════════════════════════════════════════════
    """
    목적: 중기 추세 파악으로 추가 상승 여력 판단
    - 5분봉 = 단기 매도 시점
    - 15분봉 = 중기 추세 확인 (추가 상승 가능성)
    """
    
    df_15m = pyupbit.get_ohlcv(ticker, interval="minute15", count=50)
    time.sleep(0.5)
    
    has_15m_data = df_15m is not None and len(df_15m) >= 20
    ...
    # 손익비에 따른 목표 수익률 조정 (기존 유지하되 더 공격적)
    if profit_loss_ratio >= 4.0:  # 매우 높은 손익비
        adjusted_target = target_profit * 1.15  # 목표 15% 상향
    elif profit_loss_ratio >= 3.0:  # 높은 손익비
        adjusted_target = target_profit * 1.08  # 목표 8% 상향
    else:  # 일반 손익비
    ...
    for attempt in range(max_attempts):
        ...
        if attempt % 5 == 0:
            ...
            # 15분봉도 주기적 업데이트
            if has_15m_data:
                df_15m_live = pyupbit.get_ohlcv(ticker, interval="minute15", count=50)
                time.sleep(0.5)
                if df_15m_live is not None and len(df_15m_live) >= 20:
                    closes_15m = df_15m_live['close'].values
                    volumes_15m = df_15m_live['volume'].values
        
        # ═══════════════════════════════════════════════════════════
        # 🔥 [개선] 15분봉 추가 상승 여력 분석
        # ═══════════════════════════════════════════════════════════
        """
        목적: 목표 달성 후에도 추가 상승 가능성 체크
        
        홀딩 조건:
        1. 15분봉 BB 중단 이하 (상승 여력)
        2. 15분봉 RSI 70 미만 (과매수 아님)
        3. 15분봉 거래량 증가 (매수세 강함)
        4. 손익비가 높을수록 더 적극적 홀딩
        """
        
        continue_uptrend = False
        uptrend_reason = ""
        
        if has_15m_data and closes_15m is not None:
            # 15분봉 BB 분석
            _, _, _, bb_pos_15m, _ = calculate_bb(closes_15m, 20)
            
            # 15분봉 RSI 분석
            rsi_15m = calculate_rsi(closes_15m, 14)
            
            # 15분봉 거래량 추세 (최근 5개 평균 vs 이전 10개 평균)
            recent_vol_15m = volumes_15m[-5:].mean() if len(volumes_15m) >= 5 else 0
            prev_vol_15m = volumes_15m[-15:-5].mean() if len(volumes_15m) >= 15 else recent_vol_15m
            vol_increasing_15m = recent_vol_15m > prev_vol_15m * 1.1
            
            # 15분봉 가격 모멘텀 (최근 상승 중인지)
            price_momentum_15m = (closes_15m[-1] - closes_15m[-3]) / closes_15m[-3] if len(closes_15m) >= 3 else 0
            ...
            # 조건 1: BB 위치 (50점)
            if bb_pos_15m < 0.50:  # 중단 이하
                uptrend_score += 50
                reasons.append(f"BB15m중단({bb_pos_15m*100:.0f}%)")
            elif bb_pos_15m < 0.65:  # 중단 근처
                uptrend_score += 30
                reasons.append(f"BB15m중간({bb_pos_15m*100:.0f}%)")
            
            # 조건 2: RSI (30점)
            if rsi_15m < 60:  # 과매수 아님
                uptrend_score += 30
                reasons.append(f"RSI15m정상({rsi_15m:.0f})")
            elif rsi_15m < 70:  # 약간 높음
                uptrend_score += 15
                reasons.append(f"RSI15m주의({rsi_15m:.0f})")
            
            # 조건 3: 거래량 증가 (20점)
            if vol_increasing_15m and recent_vol_15m > 0:
                vol_ratio_15m = recent_vol_15m / prev_vol_15m if prev_vol_15m > 0 else 1
                uptrend_score += 20
                reasons.append(f"거래량증가({vol_ratio_15m:.1f}x)")
            
            # 조건 4: 가격 모멘텀 (20점)
            if price_momentum_15m > 0.005:  # 0.5% 이상 상승
                uptrend_score += 20
                reasons.append(f"모멘텀+({price_momentum_15m*100:.1f}%)")
            elif price_momentum_15m > 0:
                uptrend_score += 10
                reasons.append(f"모멘텀중립")
            
            # ─────────── 손익비 기반 홀딩 기준 조정 ───────────
            # 손익비가 높을수록 더 낮은 점수에서도 홀딩
            if profit_loss_ratio >= 4.0:
                hold_threshold = 60  # 60점 이상
            elif profit_loss_ratio >= 3.5:
                hold_threshold = 70  # 70점 이상
            elif profit_loss_ratio >= 3.0:
                hold_threshold = 80  # 80점 이상
            else:
                hold_threshold = 90  # 90점 이상 (엄격)
            
            if uptrend_score >= hold_threshold:
                continue_uptrend = True
                uptrend_reason = ", ".join(reasons)
        
        # 즉시 매도 판단 (기존 함수 활용 - adjusted_target 사용)
        decision = should_sell_now(profit_rate, closes, volumes, cur_price, adjusted_target)
        
        # ═══════════════════════════════════════════════════════════
        # 🔥 [개선] 15분봉 기반 홀딩 우선순위 추가
        # ═══════════════════════════════════════════════════════════
        """
        우선순위:
        1. 급락 신호 → 즉시 매도 (decision['urgency'] == 'HIGH')
        2. 목표 미달 → 홀딩
        3. 목표 달성 + 15분봉 상승 여력 → 홀딩 (신규)
        4. 목표 달성 + 매도 신호 → 매도
        """
        
        # 급락 신호면 무조건 매도 (기존 로직 유지)
        if decision['sell'] and decision['urgency'] == 'HIGH':
            sell_order = upbit.sell_market_order(ticker, buyed_amount)
            remove_position_info(ticker)
        ...
        # 목표 달성 + 15분봉 상승 여력 있음 → 홀딩 (신규 로직)
        if profit_rate >= adjusted_target * 0.9 and continue_uptrend:
            print(f"[홀딩유지] {ticker} | {profit_rate:+.2f}% | 15분봉 상승여력: {uptrend_reason}")
            time.sleep(0.5)
            continue  # 매도하지 않고 다음 루프
        
        # 간결한 출력
        _, _, _, bb_pos, _ = calculate_bb(closes, 20)
        uptrend_status = f"상승여력O" if continue_uptrend else "상승여력X"
        
        print(f"[매도감시] {ticker} {attempt+1}/{max_attempts} | "
              f"{profit_rate:+.2f}% (목표:{adjusted_target:.1f}%) | BB:{bb_pos*100:.0f}% | "
              f"{uptrend_status} | {'매도!' if decision['sell'] else '홀딩'}")
        time.sleep(0.5)
    ...
    if df_final is not None and len(df_final) >= 20:
        ...
        # 15분봉 최종 데이터
        df_15m_final = pyupbit.get_ohlcv(ticker, interval="minute15", count=50)
        time.sleep(0.5)
        
        strong_15m_uptrend = False
        
        if df_15m_final is not None and len(df_15m_final) >= 20:
            closes_15m_final = df_15m_final['close'].values
            _, _, _, bb_pos_15m_final, _ = calculate_bb(closes_15m_final, 20)
            rsi_15m_final = calculate_rsi(closes_15m_final, 14)
            
            # 15분봉이 강한 상승 추세인지 확인
            if bb_pos_15m_final < 0.60 and rsi_15m_final < 65:
                strong_15m_uptrend = True
        
        # 목표 수익률 달성 여부 (조정된 목표 기준)
        if profit_rate >= adjusted_target:
            # ─────────── 홀딩 조건 강화 (15분봉 추가) ───────────
            # 손익비에 따른 홀딩 기준
            if profit_loss_ratio >= 3.5:
                hold_threshold_5m = 5   # 더 공격적
            elif profit_loss_ratio >= 3.0:
                hold_threshold_5m = 6   # 중간
            else:
                hold_threshold_5m = 7   # 보수적
            
            # 조건 1: 5분봉 상승세 (기존)
            has_5m_uptrend = (uptrend_final and uptrend_final['strength'] >= hold_threshold_5m)
            
            # 조건 2: 15분봉 상승세 (신규)
            has_strong_trend = has_5m_uptrend or strong_15m_uptrend
            
            # BB 하단 + 강한 상승세 → 홀딩
            if bb_pos_final < 0.30 and has_strong_trend:
                trend_info = "5분+15분 상승" if (has_5m_uptrend and strong_15m_uptrend) else \
                            "15분 상승여력" if strong_15m_uptrend else uptrend_final['reason']
                
                msg = f"🤝 **[시간종료-홀딩]** {ticker}\n"
                msg += f"수익: {profit_rate:.2f}% (목표: {adjusted_target:.1f}%) | BB:{bb_pos_final*100:.0f}%\n"
                msg += f"사유: {trend_info} (추가상승대기)"
                print(msg)
                send_discord_message(msg)
                return None
