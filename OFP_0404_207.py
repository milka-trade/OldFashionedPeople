...
def filtered_tickers(tickers):
    ...
    for t in tickers:
        try:
            ...
            count_below_lower_band = sum(1 for i in range(len(lower_band)) if df_low[i] < min(lower_band[i], last_ema5))
            ...
            filtering_message += f"[cond3: {low_boliinger}] LB: {lower_band[-1]:,.2f} or ema5: {last_ema5:,.2f} > df_low: {df_low[-1]:,.2f} \n"
...
def trade_buy(ticker):
    ...
    rsi_rising = rsi[-2] < rsi[-1] and rsi_buy_s < rsi[-1] < rsi_buy_e2
...
def trade_sell(ticker):
    ...
    if profit_rate >= min_rate:
        while attempts < max_attempts:       
            ...
            if profit_rate >= max_rate or (upper_boliinger and upper_price) or middle_price:
                sell_order = upbit.sell_market_order(ticker, buyed_amount)
