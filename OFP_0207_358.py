...
def calculate_coin_status_for_report(ticker):
    ...
    Returns:
        dict: {
            'daily_status': '양봉' or '음봉',
            'daily_emoji': '🟢' or '🔴',
            'daily_change': 등락률 (%),
            'rise_from_low': 저가대비 상승률 (%),
            'drop_from_high': 고가대비 하락률 (%),
            'power_emoji': '⚡' or '💀' or '➡️',
            'power_label': '강세' or '약세' or '중립',
            'p1_pass': bool, 'p1_reason': str,
            'p2_pass': bool, 'p2_reason': str,
            'p3_pass': bool, 'p3_reason': str,
            'final_signal': 'BUY' or '대기' or '부족',
            'phase_str': '✅✅✅' 등
        }
    """
    try:
        ...
        if df_15m is not None and len(df_15m) >= 20:
            ...
            if not position_check['is_downtrend']:
                p2_pass = True
                p2_reason = f"BB{position_check['bb_position']:.0f}%"
            else:
        ...
        if watchlist_coins:
            ...
            for ticker in watchlist_coins:
                ...
                if status['final_signal'] == 'BUY':
                    line2 = f"\n └ {status['phase_str']} **BUY** ({status['p3_reason']})"
                elif status['reject_phase']:
                    line2 = f"\n └ {status['phase_str']} {status['final_signal']} ({status['reject_phase']}:{status['reject_detail']})"
                else:
                    line2 = f"\n └ {status['phase_str']} {status['final_signal']}"
