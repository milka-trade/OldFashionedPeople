...
"""
═══════════════════════════════════════════════════════════════════════
BB Bounce Hunter v37.0 — Crypto Sell-Side Optimization Edition
"고점에서 팔고, 시간대를 가려서, 다시 좋은 시점에 사라"
═══════════════════════════════════════════════════════════════════════

v36 → v37 핵심 변경 (광현님 요청 — 매도 로직 전면 개선):

  [1] KST 시간대 4구간 매도 정책 차등화
      - GLOBAL_PEAK    (22:00~02:00): 글로벌 변동성 최고 → 트레일링 1.3x
      - KOREA_MORNING  (09:00~12:00): 한국 단발성 펌프 → 빠른 익절 0.7x
      - LOW_LIQUIDITY  (12:00~17:00): 저거래량 → 타이트 0.8x
      - EU_RAMP        (17:00~22:00): 유럽 점진 확대 → 표준 1.0x
      - OVERNIGHT_LATE (02:00~09:00): 미국 후반→새벽 → 표준 1.0x

  [2] ATR 기반 변동성 정규화 트레일링
      - trail_pct = max(MIN, k × ATR_pct(15m, 14)) × 시간대 × 단계
      - 코인별 변동성 자동 조정 (XRP/SOL 차별화)

  [3] 보유 단계 4분류 시스템
      - FRESH  (0~30분):    안정화 유예, 절대손절만
      - EARLY  (30분~6h):   단기 급등 보호 + 가속손절 + 2-Bar 손절
      - MID    (6h~24h):    트레일링 + RSI반전 풀가동
      - MATURE (24h+):      추세 이탈 5조건 모두 활성

  [4] 단기 천장 다중 신호 합의 (Top Signal Consensus)
      - 1H BB Position > 90%
      - 1H RSI > 75
      - 15m StochRSI %K 데드크로스 (90+ 영역)
      - 1H 음봉 + 윗꼬리 60%+
      → 3개 이상 합의 시 즉시 익절 (수익 +2%+ 조건)

  [5] 단기 급등 보호 (Rally Guard) 부활 (PHASE_EARLY 활성)
      - 2시간 내 +3% 도달 → peak 대비 -1.5%에 매도
      - 6시간 내 +5% 도달 → peak 대비 -2.0%에 매도

  [6] RSI 과매수 반전 D+0 활성화
      - v34에서 폐기되었던 로직 부활 (수익 +2%+ 조건)
      - 시간대별 RSI 임계값 차등 (GLOBAL_PEAK 78, KOREA_MORNING 72)

  [7] -3% 2-Bar 확정 손절 (칼손절 방지)
      - 5분봉 2봉 연속 종가가 -3% 이하 → 진짜 손절
      - 1봉만 -3% → 다음 봉 회복 유예 (-5% 절대손절은 즉시)

  [8] BTC 시스템 리스크 감지 (Market Crash Guard)
      - BTC 1H봉 -3% 이상 하락 → 보유 코인 전체 즉시 청산
      - BTC 5분봉 3연속 -1%+ 하락 → 동일 처리

  [9] 매도 사유별 차등 재진입 쿨다운
      - 익절: 30분 / 트레일링: 60분 / 손절: 240분 / 추세종료: 1440분
      - BTC_CRASH: 360분 (시스템 위기 회피)

  [10] 매도 사유 컨텍스트 로깅 강화
      - reason에 [세션|단계|ATR|TR] 포함 → 사후 분석 가능

설계 철학:
  v36 = "추세 끝까지 보유" (단순 시간 비차별)
  v37 = "고점에서 팔고, 시간대 가려서, 변동성에 적응하며" (정밀 매도)

학술 근거:
  - Hansen, Kim, Kimbrough (2021) — 비트코인 시간대별 변동성 주기성
  - Amberdata 2023 BTC/USDT 분석 — 시간대별 변동성 분산
  - Le Beau & Elder — Chandelier Exit (ATR × Multiplier 표준)
  - Snorrason & Yusupov (2009) — 변동성장에서 트레일링 우월성
...
DEBUG_MODE = True
TEST_MODE = False
VERSION = "37.0 SELL_SIDE_OPTIMIZED — Time-Adaptive + ATR-Normalized + Phase-Tiered"
...
# ═══════════════════════════════════════════════════════════════════════
# ★ v37 핵심: 시간대 적응 + ATR 정규화 + 단계별 차등 매도 파라미터
# ═══════════════════════════════════════════════════════════════════════

# ── [v37 NEW] KST 시간대 정책 (5개 세션) ──
TIME_ZONE_POLICY = {
    "GLOBAL_PEAK":    {"start": (22, 0), "end": (2, 0),
                       "trail_mult": 1.3, "rsi_exit_th": 78.0,
                       "rally_guard_enabled": True},
    "KOREA_MORNING":  {"start": (9, 0),  "end": (12, 0),
                       "trail_mult": 0.7, "rsi_exit_th": 72.0,
                       "rally_guard_enabled": True},
...
# ── [v37 NEW] ATR 기반 변동성 정규화 트레일링 ──
ATR_TRAIL_K_MULT = 3.0          # trail_pct = k × ATR_pct
ATR_TRAIL_MIN_PCT = 2.0         # 최소 트레일 갭 (%)
ATR_TRAIL_MAX_PCT = 8.0         # 최대 트레일 갭 (%)
ATR_PERIOD = 14                 # ATR 계산 기간

# ── [v37 NEW] 보유 단계 정의 (시간 단위) ──
PHASE_FRESH_MAX_HOURS  = 0.5    # 0~30분: 안정화 유예
PHASE_EARLY_MAX_HOURS  = 6.0    # 30분~6h: 단기 급등 보호
PHASE_MID_MAX_HOURS    = 24.0   # 6h~24h: 트레일링+RSI 활성
# 24h+ = PHASE_MATURE (추세 이탈 5조건 풀가동)

# ── [v37 NEW] 단기 천장 다중 신호 합의 ──
TOP_SIGNAL_BB_TH = 90.0         # BB Position 임계
TOP_SIGNAL_RSI_TH = 75.0        # RSI 임계
TOP_SIGNAL_STOCH_K_TH = 90.0    # StochRSI %K 임계
TOP_SIGNAL_UPPER_WICK_RATIO = 0.60  # 윗꼬리 비율 임계
TOP_SIGNAL_MIN_AGREE = 3        # 최소 합의 신호 수
TOP_SIGNAL_MIN_PROFIT = 2.0     # 최소 발화 수익률

# ── [v37 NEW] 단기 급등 보호 (Rally Guard) ──
RALLY_GUARD_2H_PROFIT_TH = 3.0  # 2시간 내 +3% 도달 시 활성
RALLY_GUARD_2H_DRAWDOWN = 1.5   # peak 대비 -1.5% 시 매도
RALLY_GUARD_6H_PROFIT_TH = 5.0  # 6시간 내 +5% 도달 시 활성
RALLY_GUARD_6H_DRAWDOWN = 2.0   # peak 대비 -2.0% 시 매도

# ── [v37 NEW] 2-Bar 확정 손절 (칼손절 방지) ──
TWO_BAR_CONFIRM_LOSS_PCT = -3.0  # -3% 도달 시 2봉 확정 필요

# ── [v37 NEW] BTC 시스템 리스크 감지 ──
BTC_CRASH_1H_PCT = -3.0         # BTC 1H봉 -3% 이상 → 시스템 위기
BTC_CRASH_5M_CONSECUTIVE = 3    # BTC 5분봉 N연속 -1% 이상

# ── [v37 NEW] 매도 사유별 재진입 쿨다운 (분) ──
COOLDOWN_PROFIT_TAKE   = 30      # 익절
COOLDOWN_TRAILING      = 60      # 트레일링
COOLDOWN_RSI_REVERT    = 45      # RSI 과매수 반전
COOLDOWN_TOP_SIGNAL    = 45      # 단기 천장 합의
COOLDOWN_RALLY_GUARD   = 60      # 급등 보호
COOLDOWN_STOPLOSS      = 240     # 손절
COOLDOWN_TREND_END     = 1440    # 추세 종료
COOLDOWN_BTC_CRASH     = 360     # BTC 시스템 위기

# ── D+0 안전장치 (v36 유지) ──
INTRADAY_STOP_LOSS_PCT = -3.0          # 당일 -3% 손절 (2-Bar 확정 후)
...
{Colors.BOLD}{Colors.CYAN}{'='*68}
  CRYPTO SELL-SIDE OPTIMIZED HUNTER {VERSION}
  ★★★ v37.0: 고점에서 팔고, 시간대 가려서, 변동성에 적응하며

  ★ 마켓 와이드 스크리너: 업비트 KRW 200+ 코인 매시 정각 스크리닝
  ★ EMA 추세 매수 (v36 유지): 4H EMA 정배열 + 1H BB과매도 + 15m 체크리스트

  ★★ v37 매도 로직 (Sell-Side Specialist CoT):
     [1] KST 시간대 4구간 정책 차등 (GLOBAL_PEAK / KOREA_MORNING / LOW_LIQ / EU_RAMP)
     [2] ATR 기반 변동성 정규화 트레일링 (코인별 자동 조정)
     [3] 보유 단계 4분류 (FRESH/EARLY/MID/MATURE) 차등 적용
     [4] 단기 천장 다중 신호 합의 (BB+RSI+StochDX+윗꼬리)
     [5] Rally Guard 부활 (EARLY/MID 단기 급등 보호)
     [6] RSI 과매수 반전 D+0 활성 (시간대별 임계값 차등)
     [7] -3% 2-Bar 확정 손절 (칼손절 방지)
     [8] BTC 시스템 리스크 감지 (1H -3% / 5분봉 3연속 -1%)
     [9] 매도 사유별 차등 재진입 쿨다운 (30~1440분)

{'='*68}
  Thread 1: 매수 ({BUY_THREAD_INTERVAL}초) │ EMA자격 통과 후보 평가
  Thread 2: 매도 ({SELL_THREAD_INTERVAL}초) │ v37 Specialist CoT (5단계)
...
def _classify_kst_session(now=None):
    ...
    # KOREA_MORNING: 09:00~12:00
    if 9 * 60 <= hm < 12 * 60:
        return "KOREA_MORNING", TIME_ZONE_POLICY["KOREA_MORNING"]
...
def _classify_holding_phase(elapsed_hours):
    """보유 시간을 4개 단계로 분류

    Returns:
        'FRESH' | 'EARLY' | 'MID' | 'MATURE'
    """
    if elapsed_hours < PHASE_FRESH_MAX_HOURS:
        return "FRESH"
...
class TrendSellEngine:
    """
    [v37 매도 의사결정 체계 — Sell-Side Specialist CoT]

    STEP 1: 시장 컨텍스트 (시간대 + BTC 리스크 + ATR)
      - KST 5개 세션 분류 (GLOBAL_PEAK / KOREA_MORNING / LOW_LIQUIDITY / EU_RAMP / OVERNIGHT_LATE)
      - BTC 1H -3% 또는 5분봉 3연속 -1% 감지 → 시스템 위기
      - 15분 ATR_pct 계산 → 변동성 정규화 트레일링 갭

    STEP 2: 보유 단계 분류
      - FRESH  (0~30분):    안정화 유예, 절대손절만
      - EARLY  (30분~6h):   가속손절 + 2-Bar 손절 + Rally Guard
      - MID    (6h~24h):    + 트레일링 + RSI 반전
      - MATURE (24h+):      + EMA50 하회 + 데드크로스

    STEP 3: 손절 트리 (최우선)
      ① 절대 손절 -5% (모든 PHASE)
      ② BTC 시스템 위기 (FRESH 제외)
      ③ 가속 손절 (EARLY, 5분봉 3연속음봉 + RSI<25)
      ④ -3% 2-Bar 확정 손절 (EARLY/MID, 칼손절 방지)

    STEP 4: 익절 트리
      ⑤ 단기 천장 다중 신호 합의 (BB+RSI+StochDX+윗꼬리, 수익 +2%+)
      ⑥ RSI 과매수 반전 (D+0 포함, 시간대별 임계값 차등)
      ⑦ Rally Guard (EARLY/MID, 시간대 활성 시)
      ⑧ 동적 트레일링 (MID/MATURE, ATR 정규화)
      ⑨ EMA 추세 종료 (MATURE만, EMA50 하회 2봉 또는 데드크로스)

    STEP 5: 매도 사유 카테고리 + 차등 쿨다운
      - 익절(30) / 천장(45) / 트레일(60) / 손절(240) / 추세종료(1440)

    ★ 시간 청산 코드 0건 (광현님 핵심 지시 유지)
    ★ 모든 매도 신호에 [세션|단계|ATR|TR] 컨텍스트 로깅
    """
    ...
    def check_sell_signal(self, ticker: str, current_price: float) -> dict:
        """★ v37 매도 신호 평가 — Sell-Side Specialist CoT

        의사결정 흐름:
          STEP 1: 시장 컨텍스트 (시간대 + ATR + 1H BB)
          STEP 2: 보유 단계 분류 (FRESH/EARLY/MID/MATURE)
          STEP 3: 손절 트리 (절대/시스템/2-Bar/가속) — 최우선
          STEP 4: 익절 트리 (천장합의/RSI반전/RallyGuard/트레일링/추세종료)
          STEP 5: 매도 사유 카테고리 라벨링 (sell_category)
        """
        with self._lock:
        ...
        # STEP 2: 보유 단계 분류
        # ════════════════════════════════════════════════════════════
        phase = _classify_holding_phase(elapsed_hours)

        # 동적 트레일링 갭 사전 계산 (로깅용)
        dynamic_trail = _calc_dynamic_trail_pct(atr_pct, session_policy, phase)

        # 컨텍스트 정보를 포함한 base dict
        ctx_str = f"[{session_name}|{phase}"
        if atr_pct is not None:
            ctx_str += f"|ATR{atr_pct:.2f}%|TR{dynamic_trail:.1f}%"
        ctx_str += "]"
        ...
        # ── 3-3. FRESH (0~30분): 안정화 유예, 절대손절만 적용 ──
        if phase == "FRESH":
            return base

        # ── 3-4. 가속 손절 (EARLY, 5분봉 3연속음봉 + RSI<25) ──
        ...
        # ── 3-5. -3% 2-Bar 확정 손절 (EARLY/MID, 칼손절 방지) ──
        ...
        # ── 4-2. RSI 과매수 반전 (D+0 EARLY 포함, 시간대별 임계값) ──
        ...
        # ── 4-3. 단기 급등 보호 (Rally Guard) — EARLY/MID, 시간대 활성 시 ──
        if phase in ("EARLY", "MID") and session_policy.get("rally_guard_enabled", False):
            # 2시간 내 +3% 초과 후 -1.5% drawdown
            if elapsed_hours <= 2.0 and peak_pnl >= RALLY_GUARD_2H_PROFIT_TH:
                if drawdown >= RALLY_GUARD_2H_DRAWDOWN:
                    return {**base, "signal": True,
                            "reason": (f"급등보호2H(고점{peak_pnl:+.1f}%→낙{drawdown:.1f}%,"
                                       f"{profit_pct:+.2f}%) {ctx_str}"),
                            "sell_category": "RALLY_GUARD"}
            # 6시간 내 +5% 초과 후 -2.0% drawdown
            if elapsed_hours <= 6.0 and peak_pnl >= RALLY_GUARD_6H_PROFIT_TH:
                if drawdown >= RALLY_GUARD_6H_DRAWDOWN:
                    return {**base, "signal": True,
                            "reason": (f"급등보호6H(고점{peak_pnl:+.1f}%→낙{drawdown:.1f}%,"
                                       f"{profit_pct:+.2f}%) {ctx_str}"),
                            "sell_category": "RALLY_GUARD"}

        # ── 4-4. 변동성 정규화 트레일링 (MID/MATURE, 수익 +2%+) ──
        if phase in ("MID", "MATURE") and peak_pnl >= 2.0:
    ...
    try:
        with trade_lock:
            ...
            with held_coins_lock:
                if ticker in held_coins:
                    del held_coins[ticker]
            # ★ v37: 매도 사유 카테고리 추가 (재진입 쿨다운 차등 적용용)
            recent_sells[ticker] = {
                'time': datetime.now(),
...
def sell_thread_worker():
    ...
    """
    print(f"{Colors.YELLOW}[Thread 2] v37 매도 스레드 시작 ({SELL_THREAD_INTERVAL}초 주기){Colors.ENDC}")
    print(f"{Colors.YELLOW}  ├ Sell-Side Specialist CoT (5단계){Colors.ENDC}")
    ...
    print(f"{Colors.YELLOW}[Thread 2] v37 매도 스레드 종료{Colors.ENDC}")
# ═══════════════════════════════════════════════════════════════════════
