...
BB BOUNCE HUNTER v25.1 — 패치 파일
  수정된 함수만 포함 (기존 v25.0 파일에 해당 함수를 교체하여 사용)

  수정 내역:
  [CRITICAL] sync_held_coins_with_exchange()
      - avg_buy_price = 0 케이스 강제 처리 (current_price 대체)
      - 관리 코인(FIXED_STABLE_COINS)은 잔고 > 0이면 무조건 held_coins 등록
      - 비관리 코인도 5천원 이상이면 등록 (기존 동일)
      - 초기 자산 현황 Discord 상세 보고 추가

  [CRITICAL] buy_thread_worker()
      - 루프 최상단 KRW 잔고 사전 확인 추가 (7코인 스캔 전)
      - 현금 < 5000원 → 매수 스캔 전체 skip → BUY_SLEEP_WHEN_FULL 대기
      - 현금 부족 메시지 주기적 터미널 출력 + 주기적 Discord 알림

  [NEW] send_startup_asset_report()
      - 봇 시작 시 전체 자산 현황 Discord 전송
      - 동기화된 코인 상세 + KRW 잔고 + 총 자산 + 매수 가능 여부

  [MODIFIED] main()
      - sync 완료 후 send_startup_asset_report() 호출
      - Discord 시작 알림에 현금 부족 시 매수차단 안내 추가
"""
...
def _send_sync_discord_report(synced_coins, skipped_coins, unmanaged_coins):
    ...
    try:
        ...
        for c in synced_coins:
            ...
            pft = c['profit_pct']
            pft_str = f"{pft:+.2f}%"
            price_warn = f" ⚠️[{c['price_source']}]" if 'avg=0' in c.get('price_source','') or '임시' in c.get('price_source','') else ""
            coin_lines += (f"\n  {managed_tag} **{c['currency']}** "
                          f"`{c['balance']:.4f}개` @ `{c['avg_price']:,.0f}원`"
                          f" → `{c['cur_price']:,.0f}원` **{pft_str}**{price_warn}")

        buy_status = f"✅ 매수 가능 (`{krw_balance:,.0f}원`)" if can_buy else f"🚫 매수 불가 (`{krw_balance:,.0f}원 < 5,000원`) → 매도 전용 모드"

        msg = (
            f"\n⚙️ **보유 코인 동기화 완료**\n\n"
            f"💰 **자산 현황**\n"
            f"├ 총자산: `{total_assets:,.0f}원`\n"
            f"├ 코인: `{total_coin_value:,.0f}원` ({len(synced_coins)}종)\n"
        ...
        if skipped_coins:
            msg += f"\n⏭️ 스킵: `{', '.join(skipped_coins)}`\n"

        msg += (
            f"\n⚠️ **주의**: 동기화 코인은 봇 시작 전 매수된 것입니다.\n"
            f"보유 시간이 부정확할 수 있으므로 손절 기준이 즉시 적용됩니다.\n\n"
...
# ============================================================================
# [2/4] send_startup_asset_report()  ← 신규 함수 (SECTION 15 뒤에 추가)
# ============================================================================
...
def send_startup_asset_report():
    ...
    try:
        ...
        if can_buy:
            ...
            print(f"\n  {Colors.YELLOW}🚫 현금 부족 ({krw_balance:,.0f}원 < 5,000원)")
            print(f"  → 매도 + 모니터링만 실행 (매수 스캔 자동 차단){Colors.ENDC}")
...
def buy_thread_worker():
    """
    [v25.1 수정] Thread 1: 매수 스레드

    핵심 수정:
    ② KRW 잔고 사전 확인 — 7코인 스캔 전에 현금 체크.
       현금 < 5000원이면 스캔 전체 skip → BUY_SLEEP_WHEN_FULL 대기.
       이로써 "코인만 있고 현금 없음" 상태에서 불필요한 API 호출 0건.
    ...
    print(f"{Colors.GREEN}  └ [NEW] KRW < 5,000원 시 매수 스캔 자동 차단{Colors.ENDC}")

    iteration         = 0
    last_no_cash_log  = 0    # 현금부족 Discord 마지막 전송 시각 (Unix ts)
    ...
    while not stop_event.is_set():
        try:
            ...
            # ② [v25.1 신규] KRW 잔고 사전 확인 ─────────────────────────────
            #    7코인 스캔(API 7회) 전에 현금 먼저 체크.
            #    현금 < 5000원 → 전체 스캔 skip, 30초 대기.
            try:
            ...
            if krw_balance_now < 5000:
                ...
                if DEBUG_MODE and iteration % 6 == 0:
                    with held_coins_lock:
                        holding_info = [
                            f"{t.replace('KRW-','')}({((get_current_price(t) or info['buy_price']) - info['buy_price']) / info['buy_price'] * 100:+.1f}%)"
                            for t, info in held_coins.items()
                        ]
                    holding_str = ', '.join(holding_info) if holding_info else "없음"
                    print(f"{Colors.YELLOW}[BUY] 🚫 매수불가 — 현금 {krw_balance_now:,.0f}원 < 5,000원"
                          f" | 보유: {holding_str} | 매도 대기 중{Colors.ENDC}")
            ...
            update_market_grade()
            if DEBUG_MODE and iteration % 6 == 0:
                print(f"{Colors.MAGENTA}[BUY] KRW:{krw_balance_now:,.0f}원 | {get_grade_display_str()}{Colors.ENDC}")
            ...
            for ticker in FIXED_STABLE_COINS:
                ...
                if sig['signal']:
                    ...
                    print(f"  💵 가용현금: {krw_balance_now:,.0f}원")
...
def _send_no_cash_discord_alert(krw_balance: float, num_held: int, managed_count: int):
    """현금 부족 시 10분마다 Discord에 알림 (buy_thread_worker 내부용)"""
    try:
        # 보유 코인 평가액 계산
        total_coin_value = 0.0
        coin_details = ""
        with held_coins_lock:
            for ticker, info in held_coins.items():
                try:
                    ...
                    coin_details += f"\n  └ **{ticker.replace('KRW-','')}** `{cur_price:,.0f}원` {pft:+.2f}% ({coin_val:,.0f}원)"
                except Exception:
                    pass

        msg = (
            f"\n⏸️ **매수 일시 차단** — 현금 부족\n\n"
            f"💰 현금(KRW): `{krw_balance:,.0f}원` (최소 5,000원 필요)\n"
            f"📦 보유 코인: `{managed_count}/{MAX_HOLDINGS}개` (평가액 `{total_coin_value:,.0f}원`)"
            f"{coin_details}\n\n"
            f"코인이 매도되어 현금이 5,000원 이상이 되면 매수가 자동 재개됩니다.\n\n"
            f"⏰ {datetime.now().strftime('%H:%M:%S')}\n"
        )
        send_discord_message(msg)
    except Exception as e:
        if DEBUG_MODE:
            print(f"{Colors.RED}[NoCash Alert Error] {e}{Colors.ENDC}")
...
def main():
    ...
    # 5. Discord 시작 알림 (v25.1: 현금 상태 표시 추가)
    buy_mode_str = (
        f"✅ 매수 활성 (`{init_krw:,.0f}원` 보유)"
        if can_buy_now
        else f"🚫 매수 차단 (`{init_krw:,.0f}원` < 5,000원) → 매도·모니터링 전용"
    )
    start_msg = (
        f"\n**🤖 봇 시작** `v{VERSION}`\n\n"
        ...
        f"**★ 시장 등급 시스템:**\n"
        f"🔴 HIGH(BBW≥4%): BB≤30% 손절-2.5% 강제+2.5%\n"
        f"🟡 MID (BBW2~4%): BB≤25% 손절-2.5% 강제+2.5%\n"
        f"🔵 LOW (BBW<2%): BB≤20% 손절-2.0% 강제+2.0%\n\n"
        f"⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    )
