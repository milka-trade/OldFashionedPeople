...
# 상승장 감지: 7코인 평균 등락 + 양봉 비율 복합 스코어
TREND_BULL_SCORE_MIN     = 0.52   # 시장 상승 스코어 최소치 (0~1 범위)
# 개별 코인 BB 위치 허용 구간 (너무 낮으면 반등장으로 분류, 너무 높으면 고점)
TREND_BB_POS_MIN         = 22     # BB 하단 제외 (반등 로직 담당)
TREND_BB_POS_MAX         = 58     # BB 상단 근접 제외 (고점 진입 방지)
# RSI 유효 범위
TREND_RSI_MIN            = 42     # 과매도 제외
TREND_RSI_MAX            = 72     # 과매수 제외
# BB 폭 최소치 (횡보 제외)
TREND_BBW_MIN            = 0.8    # 최소 변동성 확인
# 진입 점수 최소치 (5점 만점)
TREND_SCORE_MIN          = 3
# Watchlist 유효 시간(분) — 추세는 급락보다 오래 지속되므로 더 길게
TREND_WATCH_WINDOW_MIN   = 45
# 스캐너 실행 주기(초) — 급락 스캐너와 동일
TREND_SNAPSHOT_INTERVAL  = 900
# 추세 진입 후 너무 올라간 경우 차단 (감지 후 이 % 이상 상승 시 늦은 진입으로 차단)
TREND_MAX_ADVANCE_PCT    = 3.0
# 추세 진입 전용 가속 손절 RSI 임계값 (반등 진입보다 높게 설정 — 모멘텀 소진 빠름)
...
def score_15m_bounce(df):
    """
    ★ v27.1: 15분봉 반등 점수 (최대 5점)
    +1: RSI 2봉 연속 상승
    +1: SRSI K 상향돌파 D (골든크로스)
    +1: 양봉 + 종가 > 직전 종가
    +1: 현재가 > 3봉전 저가 (하락 멈춤 확인)
    +1: ★신규 — 2연속 양봉 반등 확인
         (현재봉 양봉 + 직전봉 양봉 + 현재봉 종가 > 직전봉 종가)
         BB 하단 저점 구간에서 2봉 연속 반등 방향성 확인으로 안전 진입
    """
    try:
        score = 0
        details = []
        ...
        # ★ 5) v27.1 신규 — 2연속 양봉 반등 확인
        # 조건: 현재봉 양봉 AND 직전봉 양봉 AND 현재봉 종가 > 직전봉 종가
        # 의미: BB 하단에서 2봉 연속 매수세 우위 → 반등 방향성 확인 안전 진입
        if c['is_bull'] and p1['is_bull'] and c['close'] > p1['close']:
            score += 1
            details.append(f"2연속양봉↑({p1['close']:.0f}→{c['close']:.0f})")
