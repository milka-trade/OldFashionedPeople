...
def send_discord_message(content):
    ...
    if not DISCORD_WEBHOOK_URL:
        print("[경고] Discord Webhook URL이 설정되지 않았습니다.")
        return
    try:
...
def analyze_buy_signal(ticker):
    ...
    try:
        ...
        result = {
            ...
            'volatility': bb_width,
            'reasons': all_reasons,
            'should_buy': total_score >= base_threshold and expected_value >= 0.8
        }
        
        return result
        
    except Exception as e:
        print(f"[매수 신호 분석 오류] {ticker}: {e}")
        return None
...
def main():
    """메인 트레이딩 루프"""
    print("="*60)
    print("Fortress Hunter v8.2 시작")
    print("8개 전략 코인 집중 모니터링")
    print("지능형 손절 시스템 / 맥락 이해 / 일시적 조정 허용")
    print("="*60)
    print(f"모니터링 코인: {', '.join(STRATEGIC_COINS)}")
    print("="*60)
    
    send_discord_message(f"""
🚀 **Fortress Hunter v8.2 시작**

📊 **모니터링 코인 (8개)**
{', '.join(STRATEGIC_COINS)}

✨ **v8.2 핵심 개선**
- 8개 전략 코인 집중 모니터링
- pandas_ta 의존성 제거 (안정성 향상)
- 고정 손절선 제거
- 지능형 손절 시스템 도입
- 맥락을 이해하는 손절 판단
- 일시적 조정 vs 진짜 추세 전환 구분
- 추세 강도에 따른 적응형 관리
...
""")
    ...
    while True:
        try:
            loop_count += 1
            print(f"\n[검색 #{loop_count}] {datetime.now().strftime('%H:%M:%S')}")
            ...
            if len(held_coins) < 3:
                ...
                for ticker in tickers:
                    ...
                    if analysis and analysis['should_buy']:
                        print(f"[기회 발견] {ticker} | {analysis['total_score']}점 | EV {analysis['expected_value']:.2f}% | 추세 {analysis['trend_strength']:.0f}점")
