...
def detect_market_condition(df_daily, df_15m):
    ...
    [v7.9 변경사항]
    - CRASH 모드 완전 제거 (급락장에서는 매수 안함)
    - SURGE(급등) / NORMAL(평균) 2가지로 단순화
    ...
    try:
        ...
        # ========================================
        # [v7.9 제거] CRASH (급락장) 판단 완전 삭제
        # ========================================
        # 기존 CRASH 판단 로직 전체 제거
        # 급락장에서는 Daily Momentum Gate가 매수를 차단함
        ...
        result['confidence'] = 100 - surge_score
...
def evolution_77_buy_signal(df_15m, ticker):
    """
    [v7.9] MOMENTUM GATE - 일봉 모멘텀 확인 후 매수
    
    [v7.9 핵심 변경사항]
    1. Daily Momentum Gate 최상위 추가 (일봉 양봉 필수)
    2. CRASH_REVERSAL 모드 완전 제거
    3. 급락장에서는 매수 안함 (게이트에서 차단)
    
    매수 모드 (2가지로 단순화):
    1. SURGE_PULLBACK: 급등장에서 눌림목 매수
    2. NORMAL_BOTTOM: 평균장에서 하단 반등
    3. MOMENTUM_BREAK: 돌파 모멘텀 매수
    ...
    try:
        ...
        # ========================================
        # [v7.9 신규] Step 0: Daily Momentum Gate (최상위 필터)
        # ========================================
        gate_pass, gate_reason, daily_info = check_daily_momentum_gate(ticker)
        
        base_response['daily_bb'] = daily_info.get('daily_bb', 50)
        
        if not gate_pass:
            base_response['reason'] = f"[GATE] {gate_reason}"
            base_response['mode'] = 'GATE_BLOCKED'
            return base_response
        ...
        # --- NORMAL (평균장) ---
        # [v7.9] CRASH 조건 완전 제거 - 급락장에서는 Gate에서 이미 차단됨
        else:
        ...
        reason_lines.append(f"일봉: 양봉 +{daily_info.get('daily_change', 0):.2f}%")
        reason_lines.extend(score_result['reasons'][:3])
        ...
        import traceback
        if DEBUG_MODE:
            print(f"[v79 Buy Signal Error] {e}")
            traceback.print_exc()
...
def check_daily_momentum_gate(ticker):
    """
    [v7.9 신규] 일봉 모멘텀 게이트 - 매수 전제조건 검증
    
    핵심 원칙: "일봉 양봉 확인 없이는 절대 매수 안함"
    
    폭락장에서 연속 손실을 방지하기 위한 최상위 필터
    - 일봉이 양봉이어야 함 (close > open)
    - BB 위치별 최소 등락률 충족 필요
    - 연속 음봉 N개 이상이면 매수 금지
    ...
    try:
        # 게이트 비활성화 시 통과
        if not DAILY_MOMENTUM_GATE_ENABLED:
            return (True, "게이트 비활성화", {
                'daily_bb': 50.0,
                'daily_change': 0.0,
                'is_bullish': True,
                'consecutive_bear': 0
        ...
        # 캐시 체크
        cache_key = f"{ticker}_momentum_gate"
        cached = get_cached_data(cache_key, DAILY_GATE_CACHE_TTL)
        ...
        if df_daily is None or len(df_daily) < 20:
            # 데이터 부족 시 매수 차단 (안전 우선)
            result = (False, "일봉 데이터 부족 (매수 차단)", {
                'daily_bb': 50.0,
                'daily_change': 0.0,
                'is_bullish': False,
                'consecutive_bear': 0
        ...
        if df_daily is None:
            result = (False, "일봉 지표 계산 실패 (매수 차단)", {
                'daily_bb': 50.0,
                'daily_change': 0.0,
                'is_bullish': False,
                'consecutive_bear': 0
        ...
        if not is_bullish:
            result = (
                False,
                f"일봉 음봉 (등락률 {daily_change:+.2f}%) → 매수 차단",
                daily_info
            )
            set_cached_data(cache_key, result)
            return result
        ...
        # 조건 2: 연속 음봉 체크
        if consecutive_bear >= DAILY_GATE_MAX_CONSECUTIVE_BEAR:
            result = (
                False,
                f"연속 음봉 {consecutive_bear}개 (최근 반등, 추세 확인 필요) → 매수 차단",
                daily_info
            )
            set_cached_data(cache_key, result)
            return result
        
        # 조건 3: BB 위치별 최소 등락률 확인
        if daily_bb <= DAILY_GATE_BB_EXTREME_LOW:
            # 극저점 (BB 0-15%): 강한 반등 필요
            min_change = DAILY_GATE_MIN_CHANGE_EXTREME
            zone_name = "극저점"
        elif daily_bb <= DAILY_GATE_BB_LOW:
            # 저점 (BB 16-30%): 반등 확인
            min_change = DAILY_GATE_MIN_CHANGE_LOW
            zone_name = "저점"
        elif daily_bb <= DAILY_GATE_BB_MID:
            # 하단 (BB 31-50%): 최소 회복
            min_change = DAILY_GATE_MIN_CHANGE_MID
            zone_name = "하단"
        elif daily_bb <= DAILY_GATE_BB_NEUTRAL:
            # 중립 (BB 51-70%): 양봉이면 거의 OK
            min_change = DAILY_GATE_MIN_CHANGE_NEUTRAL
            zone_name = "중립"
        else:
            # 상단 (BB 71-100%): 양봉이면 OK
            min_change = DAILY_GATE_MIN_CHANGE_HIGH
            zone_name = "상단"
        
        if daily_change < min_change:
            result = (
                False,
                f"일봉 {zone_name}구간(BB {daily_bb:.0f}%) 등락률 부족 ({daily_change:+.2f}% < +{min_change}%) → 매수 차단",
                daily_info
            )
            set_cached_data(cache_key, result)
            return result
        ...
        result = (
            True,
            f"일봉 양봉 +{daily_change:.2f}% (BB {daily_bb:.0f}%, {zone_name}) → 매수 허용",
            daily_info
        )
        set_cached_data(cache_key, result)
        return result
        ...
        if DEBUG_MODE:
            print(f"{Colors.RED}[Momentum Gate Error] {ticker}: {e}{Colors.ENDC}")
        # 오류 시 매수 차단 (안전 우선)
        return (False, f"게이트 검증 오류: {e}", {
            'daily_bb': 50.0,
            'daily_change': 0.0,
            'is_bullish': False,
            'consecutive_bear': 0
...
def check_daily_bb_filter(ticker, market_condition):
    """
    [v7.9 수정] 일봉 BB 기반 고가매수 방지 필터
    
    [v7.9 변경사항]
    - CRASH 모드 필터 완화 로직 제거
    - 모든 시장 상황에서 동일한 필터 적용
    ...
    Returns:
        tuple: (매수가능여부, 사유, 일봉BB, 등락률)
    """
    try:
        # [v7.9 제거] CRASH 모드 필터 완화 로직 삭제
        # 기존: if condition == 'CRASH': return (True, "급락장 필터 완화", 0, 0)
        # → 급락장에서도 동일한 필터 적용
        
        # 캐시 체크
        cache_key = f"{ticker}_daily_bb_check_v79"
        cached = get_cached_data(cache_key, DAILY_BB_CACHE_TTL)
