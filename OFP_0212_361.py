...
VERSION = "11.1 BOUNCE_HUNTER"

print(f"\n{Colors.BOLD}{Colors.CYAN}{'='*60}")
print(f"EVOLUTION {VERSION}")
print(f"{'='*60}")
print(f"{Colors.GREEN}[v11.1] BOUNCE HUNTER - BB폭 일봉상태별 차등 적용{Colors.ENDC}")
print(f"   [핵심] 매수3Phase ↔ 매도3Step 대칭 구조")
print(f"   [매수] 일봉양봉 → BB하단(폭 차등) → 반등5지표")
print(f"   [v11.1] 양봉일(≥0.5%) BB폭 ≥{V10_BB_WIDTH_BULLISH}% / 비양봉 ≥{V10_BB_WIDTH_BEARISH}%")
print(f"   [매도] 손절→극과매수→소진7지표(동적임계치)→동적트레일링")
print(f"   [기존] SRSI/MACD/SwingHigh/LowerHigh 매도 활용")
print(f"")
...
def calculate_coin_status_for_report(ticker):
    ...
    try:
        ...
        # Phase 2: 15분봉 BB 위치 + Higher Low [v11.1 - 일봉상태 전달]
...
def detect_downtrend_15m(df_15m, daily_candle_type='unknown'):
    """
    [v11.1] Phase 2: 15분봉 위치 필터 + Higher Low 검증
    
    ⚠️ 함수명 유지, 역할 확장:
    기존: BB 하단 근처인지 확인 (BB폭 일률 적용)
    변경: 일봉 캔들 상태별 BB폭 차등 적용 [v11.1]
    ...
    BB폭 차등 기준:
    - daily_candle_type == 'bullish': BB폭 ≥ V10_BB_WIDTH_BULLISH (1.5%)
    - 그 외(weak_bullish/recent_bullish/unknown): BB폭 ≥ V10_BB_WIDTH_BEARISH (2.5%)
    
    조건:
    1. BB 폭: 일봉 상태별 차등 적용 [v11.1 변경]
    2. BB 위치: 5% ~ 35%
    3. RSI: ≥ 20
    4. Higher Low: 직전 파동 저점보다 현재 저점이 높을 것
    
    Args:
        df_15m: 15분봉 DataFrame (지표 포함)
        daily_candle_type: 일봉 캔들 유형 ('bullish'/'weak_bullish'/'recent_bullish'/'unknown')
    """
    try:
