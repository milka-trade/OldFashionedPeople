...
DYNAMIC_PRECHECK_SURGE = True              # SURGE 사전 적합성 체크 (등락3~12%+BB40~85%)
...
SURGE_RSI_DROP = 4                  # RSI 급락 기준 (pt)
SURGE_RSI_DROP_PFT = 0.0            # RSI 급락 최소 수익

SURGE_SL_TIGHT = -1.5
SURGE_SL_WIDE = -2.0
SURGE_SL_BB_THRESHOLD = 60
SURGE_TRAIL_ACT = 1.0               # [v15.0] 1.5 → 1.0
SURGE_TRAIL_DIST = 0.5              # [v15.0] 0.8 → 0.5
SURGE_EXTREME_BB = 88               # [v15.0] 92 → 88
SURGE_EXTREME_MIN_PFT = 0.8
SURGE_CONSEC_BEAR = 2
SURGE_CONSEC_BEAR_PFT = 0.3
SURGE_RSI_DROP = 4                  # [v15.0] 5 → 4
SURGE_RSI_DROP_PFT = 0.0
...
# [v15.0] SURGE 5분봉 강화
# ========================================
SURGE_5M_ENABLED = True
SURGE_5M_BULL_MIN = 2                # 5분봉 최근3봉 중 최소 양봉수
SURGE_5M_BB_MID_MIN = 50             # 5분봉 BB 최소 위치 (%)
SURGE_5M_SELL_CONSEC_BEAR = 2        # 5분봉 연속음봉 매도
SURGE_5M_SELL_RSI_DROP = 4           # 5분봉 RSI 급락 기준 (pt)
SURGE_5M_SELL_BB_EXIT = 55           # 5분봉 BB 하락 탈출선 (%)
...
# ╔══════════════════════════════════════════════════════════════╗
# ║  PART 2: 전역 변수 추가                                      ║
# ║  삽입 위치: 316행 아래 (daily_losing_trades 다음)             ║
# ╚══════════════════════════════════════════════════════════════╝
    ...
    try:
        ...
        for ticker in all_tickers:
            ...
            try:
                ...
                if match_type is None and DYNAMIC_PRECHECK_SURGE:
                    # SURGE 가능: 등락 3~12% + BB 40~85%
                    if (SURGE_DAILY_CHANGE_MIN <= change_pct <= SURGE_DAILY_CHANGE_MAX and
                        SURGE_15M_BB_MIN <= bb_pos <= SURGE_15M_BB_MAX):
                        match_type = 'SURGE'
...
def surge_buy_signal(df_15m, df_5m, ticker):
    ...
    try:
        ...
        # ===== [v15.0] Phase S2.5: 5분봉 실시간 모멘텀 확인 =====
        if SURGE_5M_ENABLED and df_5m is not None and len(df_5m) >= 5:
            c5 = df_5m.iloc[-1]
            s25_pass = 0
            s25_total = 3
            
            # ① 최근 3봉 중 양봉 수
            bull_5m = sum(1 for i in range(-3, 0) 
                        if len(df_5m)+i >= 0 and df_5m.iloc[i]['close'] > df_5m.iloc[i]['open'])
            if bull_5m >= SURGE_5M_BULL_MIN:
                s25_pass += 1
            
            # ② 5분봉 RSI 상승 중
            r5_cur = c5.get('rsi', 50)
            r5_prv = df_5m.iloc[-2].get('rsi', 50) if len(df_5m) >= 2 else 50
            if r5_cur > 50 and r5_cur > r5_prv:
                s25_pass += 1
            
            # ③ 5분봉 BB 중간선 이상
            bb5 = c5.get('bb_position', 50)
            if bb5 >= SURGE_5M_BB_MID_MIN:
                s25_pass += 1
            
            if s25_pass < 2:
                base['reason'] = f'SURGE S2.5:5분봉모멘텀{s25_pass}/{s25_total}(양{bull_5m} RSI{r5_cur:.0f} BB{bb5:.0f}%)'
                return base
            
            if DEBUG_MODE:
                print(f"  🚀 [{coin}] SURGE S2.5통과: 5분봉 {s25_pass}/{s25_total} (양{bull_5m} RSI{r5_cur:.0f} BB{bb5:.0f}%)")
        
        # ===== Phase S3: 모멘텀 강도 =====
        mom = calculate_surge_momentum_score(df_15m)
        req = SURGE_MOMENTUM_MIN
        if bb >= SURGE_15M_BB_STRONG:
            req = SURGE_MOMENTUM_STRONG
...
def evolution_80_sell_signal(df, buy_price, buy_time=None, held_info=None, df_5m=None):
    """
    [v15.0] BOUNCE + SURGE 통합 매도
    
    buy_mode='SURGE_V13' → SURGE 전용 (타이트 손절/트레일링 + 5분봉 급락감지)
    그 외               → 기존 v11.0 소진점수제
    
    [v15.0 변경] df_5m 파라미터 추가 (SURGE 5분봉 매도용)
    """
    try:
        ...
        if buy_mode == 'SURGE_V13':
            ...
            # ===== [v15.0] S-2.5: 5분봉 급락 감지 (수익 중일 때만) =====
            if SURGE_5M_ENABLED and df_5m is not None and len(df_5m) >= 5 and profit_pct > 0:
                c5 = df_5m.iloc[-1]
                p5 = df_5m.iloc[-2]
                
                # 5분봉 연속 음봉 + 수익구간 → 즉시 탈출
                cb5 = 0
                for i in range(-1, -4, -1):
                    if len(df_5m)+i >= 0 and df_5m.iloc[i]['close'] < df_5m.iloc[i]['open']:
                        cb5 += 1
                        ...
                        break
                if cb5 >= SURGE_5M_SELL_CONSEC_BEAR:
                    return {**base_response, 'signal': True,
                            'reason': f'SURGE 5분봉{cb5}연속음봉(수익{profit_pct:.2f}%)'}
                
                # 5분봉 RSI 급락 감지
                r5_n = c5.get('rsi', 50)
                r5_p = p5.get('rsi', 50)
                r5_drop = r5_p - r5_n
                if r5_drop >= SURGE_5M_SELL_RSI_DROP:
                    return {**base_response, 'signal': True,
                            'reason': f'SURGE 5분봉RSI급락({r5_p:.0f}→{r5_n:.0f} 수익{profit_pct:.2f}%)'}
                
                # 5분봉 BB 급락 → 모멘텀 이탈
                bb5 = c5.get('bb_position', 50)
                if bb5 < SURGE_5M_SELL_BB_EXIT and profit_pct >= 0.3:
                    return {**base_response, 'signal': True,
                            'reason': f'SURGE 5분봉BB이탈({bb5:.0f}%<{SURGE_5M_SELL_BB_EXIT}% 수익{profit_pct:.2f}%)'}
        ...
        if DEBUG_MODE:
            print(f"{Colors.RED}[v15.0 Sell Error] {e}{Colors.ENDC}")
            traceback.print_exc()
...
def evolution_76_sell_signal(df, buy_price, buy_time=None, held_info=None):
    """레거시 wrapper - v15.0으로 리다이렉트"""
    return evolution_80_sell_signal(df, buy_price, buy_time, held_info, df_5m=None)

def evolution_70_sell_signal(df, buy_price):
    """레거시 wrapper - v15.0으로 리다이렉트"""
    return evolution_80_sell_signal(df, buy_price, df_5m=None)
...
def buy_thread_worker():
    """
    [v15.0] BOUNCE + SURGE 듀얼 매수 스레드
    
    v15.0 변경:
    - 보유수 체크를 가장 먼저 (API 호출 절약)
    - 08:00~09:15 매수 차단
    - 2개 보유 시 30초 sleep으로 API 효율화
    """
    print(f"{Colors.CYAN}[Thread 1] v15.0 BOUNCE+SURGE 매수 스레드 시작 ({BUY_THREAD_INTERVAL}초 주기){Colors.ENDC}")
    if DYNAMIC_COIN_ENABLED:
    ...
    while not stop_event.is_set():
        try:
            ...
            if current_holdings >= MAX_HOLDINGS:
                ...
                time.sleep(BUY_SLEEP_WHEN_FULL)  # [v15.0] 30초 sleep
                continue
            ...
            for ticker in active_coins:
                ...
                if surge_signal['signal']:
                    ...
                    print(f"\n{Colors.MAGENTA}{'='*50}")
                    print(f"[SURGE SIGNAL] {coin_name} 급등 모멘텀 매수!{dyn_tag}")
                    print(f"{'='*50}{Colors.ENDC}")
                    ...
                    if success:
                        print(f"{Colors.MAGENTA}[BUY] {coin_name} SURGE 매수 완료!{dyn_tag}{Colors.ENDC}")
                    else:
...
"""
            ...
            for ticker in tickers:
                ...
                # ===== [v15.0] SURGE 코인은 5분봉도 수집 =====
                df_5m_sell = None
                if held_info_copy.get('buy_mode') == 'SURGE_V13' and SURGE_5M_ENABLED:
                    df_5m_sell = get_extended_candles_5m(ticker, count=V80_CANDLES_5M_COUNT)
                
                # [v15.0] df_5m 전달
                sell_signal = evolution_80_sell_signal(df_15m, buy_price, buy_time, held_info_copy, df_5m=df_5m_sell)
