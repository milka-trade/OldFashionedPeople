...
V19_BB_TOUCH_THRESHOLD = 25         # "터치" 판정 BB Position (%)
...
def calculate_reversal_score(df_5m, df_15m=None, time_mode='DAY'):
    """
    [v21.0] Phase 3: 5분봉 기반 반등 트리거 (듀얼 타임프레임)
    
    v21.0 핵심 변경:
    - 신규 시그널 3개 추가 (BB Reclaim, RSI Acceleration, Capitulation Reversal)
    - 필수조건에 BB Reclaim 추가
    
    5분봉 반등 신호 (7+2+3 = 12개):
    ① RSI 상승 전환
    ② Stochastic RSI 골든 크로스
    ③ MACD 히스토그램 개선
    ④ 양봉 확인
    ⑤ 거래량 확인
    ⑥ BB Position 상승 전환 (v19.0)
    ⑦ RSI Bullish Divergence (15분봉 보너스)
    +(NIGHT) ⑧V-Reversal ⑨하단꼬리
    ⑩★ BB Reclaim (신규: BB<10% → 10%+ 복귀)
    ⑪★ RSI Acceleration (신규: RSI 3p+ 급등)
    ⑫★ Capitulation Reversal (신규: 거래량 2x + 양봉)
    """
    try:
        ...
        # ========================================
        # ⑩★ [v21.0 신규] BB Reclaim (BB 하단 이탈 후 복귀)
        # 직전 캔들이 BB 10% 이하 → 현재 캔들이 10% 이상으로 복귀
        # ========================================
        ...
        # ========================================
        # ⑪★ [v21.0 신규] RSI Acceleration (RSI 3p+ 급등)
        # RSI 30 이하에서 한 캔들에 3포인트 이상 급상승
        # ========================================
        ...
        # ========================================
        # ⑫★ [v21.0 신규] Capitulation Reversal (거래량 스파이크 + 양봉)
        # 거래량 MA20 대비 2배 이상 + 양봉 = 패닉 셀링 후 매집 시작
        # ========================================
        ...
        return {
            ...
            'bb_reclaim': bb_reclaim_detected,       # [v21.0 신규]
            'rsi_accel': rsi_accel_detected,          # [v21.0 신규]
            'capitulation': capitulation_detected     # [v21.0 신규]
        }
        
    except Exception as e:
        if DEBUG_MODE:
            print(f"{Colors.RED}[v21.0 Bounce Score Error] {e}{Colors.ENDC}")
        return {
...
def evolution_80_buy_signal(df_15m, df_5m, ticker):
    """
    [v21.0] DUAL TIMEFRAME BOUNCE HUNTER - 시간대별 다중 지표 반등 매수 신호
    
    v21.0 핵심 변경:
    - df_5m 정상 활용 (기존: None 전달 버그 수정)
    - calculate_reversal_score에 df_5m/df_15m 올바르게 전달
    - df_5m 없을 시 15분봉 fallback (완전 차단 제거)
    - Phase 2 파라미터 완화 (BB 상한, RSI 상한, 터치이력 등)
    
    3-Phase 구조:
    Phase 0: 시간대 모드 + 시장 레짐 판별
    Phase 1: 일봉 양봉 필터 (대폭 완화)
    Phase 2: 15분봉 BB 하단 위치 (완화)
    Phase 3: 5분봉 실시간 반등 신호 (핵심!)
    """
    try:
        ...
        # ★★★ [v21.0] 5분봉 데이터 fallback 처리 ★★★
        # 기존: df_5m None이면 즉시 return (매수 100% 차단)
        # 변경: df_5m None이면 15분봉을 5분봉 대체로 사용 (fallback)
        use_5m_fallback = False
        if df_5m is None or len(df_5m) < 20:
            df_5m = df_15m  # 15분봉을 fallback으로 사용
            use_5m_fallback = True
            if DEBUG_MODE:
                coin_name = ticker.replace('KRW-', '')
                print(f"  ⚠️ [{coin_name}] 5분봉 부족 → 15분봉 fallback")
        ...
        if DEBUG_MODE:
            ...
            fb_tag = '(FB)' if use_5m_fallback else ''
            print(f"  {regime_emoji}{time_emoji} [{coin_name}] 레짐: {regime_info['details']} | 모드: {time_mode}{fb_tag}")
        
        # ========================================
        # Phase 1: 일봉 양봉 필터 [v21.0 파라미터 완화 적용]
        # ========================================
        ...
        # ========================================
        # Phase 2: 15분봉 BB 하단 위치 확인 [v21.0 파라미터 완화 적용]
        # ========================================
        ...
        # ★★★ [v21.0 BUG FIX] df_5m/df_15m 올바르게 전달 ★★★
        # 기존: calculate_reversal_score(df_15m, time_mode=time_mode)
        #   → df_15m이 df_5m 자리에 전달됨 (잘못된 데이터)
        # 수정: calculate_reversal_score(df_5m, df_15m=df_15m, time_mode=time_mode)
        # ========================================
        ...
        if time_mode == 'NIGHT':
            required_signals = V19_5M_BOUNCE_MIN_NIGHT          # 2
        elif REGIME_ENABLED:
            if regime == 'BULLISH':
                required_signals = V19_5M_BOUNCE_MIN_BULL       # 3
            elif regime == 'BEARISH':
                required_signals = V19_5M_BOUNCE_MIN_BEAR       # 4
            else:
                required_signals = V19_5M_BOUNCE_MIN_NEUT       # 3
        else:
            required_signals = V10_BOUNCE_MIN_SIGNALS             # 3
        
        # ★★★ [v21.0] fallback 모드에서 요구 신호수 -1 (15분봉은 반응이 느리므로) ★★★
        if use_5m_fallback and required_signals > 2:
            required_signals = max(2, required_signals - 1)
        ...
        if not reversal['bounce_confirmed'] or reversal['score'] < required_signals:
            ...
            if not mandatory and V10_BOUNCE_MANDATORY_ONE:
                if time_mode == 'NIGHT':
                    reason_parts.append("필수(RSI↑or양봉orV반등orBBReclaim)미충족")
                else:
                    reason_parts.append("필수(RSI↑or양봉orBBReclaim)미충족")
        ...
        return {
            'signal': True,
            'reason': f"{regime_tag}{time_tag}{fb_tag} BOUNCE! BB{bb_position:.0f}% 폭{bb_width:.1f}% | {bounce_detail} | {daily_info}",
        ...
        if DEBUG_MODE:
            print(f"{Colors.RED}[v21.0 Buy Signal Error] {e}{Colors.ENDC}")
            traceback.print_exc()
...
def buy_thread_worker():
    """
    [Thread 1] v21.0 BOUNCE+NIGHT 매수 스레드
    ...
    v21.0 변경: df_5m을 evolution_80_buy_signal에 정상 전달
    """
    print(f"{Colors.CYAN}[Thread 1] v21.0 BOUNCE+NIGHT 매수 스레드 시작 ({BUY_THREAD_INTERVAL}초 주기){Colors.ENDC}")
    if DYNAMIC_COIN_ENABLED:
        ...
        try:
            ...
            for ticker in active_coins:
                ...
                if buy_signal['signal']:
                    ...
                    print(f"  🎯 모드: {buy_signal.get('mode', 'BOUNCE_V21')}")
