...
def filtered_tickers(tickers):
    ...
    for t in tickers:
        try:
            ...
            count_below_lower_band = sum(1 for i in range(len(lower_band15)) if df_close[i] < min(lower_band[i] * 1.03, last_ema5))
            count_below_lower_band15 = sum(1 for i in range(len(lower_band15)) if df15_close[i] < min(lower_band15[i] * 1.05, last_ema15))
            ...
            filtering_message += f"[cond3: {low_boliinger}] LowBoliinger: {low_boliinger} / LB * 0.3%: {lower_band[-1] * 1.003:,.3f} or ema: {last_ema5:,.3f} > df_close: {df_close[-1]:,.3f} \n"
            # filtering_message += f"[cond4: {low_boliinger15}] LowBoliinger15: {low_boliinger} LB15 * 0.5%: {lower_band15[-1] * 1.005:,.3f} or ema: {last_ema15:,.3f}  > df15_close: {df15_close[-1]:,.3f} \n"
            # filtering_message += f"[cond4: {srsi_d_risingS}] srsi_d: {srsi_d_risingS} / {srsi_value_s} < srsi_d: {srsi_dS[-2]:,.3f} >> {srsi_dS[-1]:,.3f} < {srsi_value_e} / srsi_k: {srsi_kS[-2]:,.3f} >> {srsi_kS[-1]:,.3f} \n"
            # filtering_message += f"[cond5: {low_band_slope_decreasing}] LBSlopes: {low_band_slope_decreasing} / {slopes_2 * 0.95:,.3f} >> {slopes_1:,.3f} \n \n"
            
            filtering_message4 = f"[cond4: {low_boliinger15}] LowBoliinger15: {low_boliinger} LB15 * 0.5%: {lower_band15[-1] * 1.005:,.3f} or ema: {last_ema15:,.3f}  > df15_close: {df15_close[-1]:,.3f} \n"
            filtering_message5 = f"[cond5: {srsi_d_risingS}] srsi_d: {srsi_d_risingS} / {srsi_value_s} < srsi_d: {srsi_dS[-2]:,.3f} >> {srsi_dS[-1]:,.3f} < {srsi_value_e} / srsi_k: {srsi_kS[-2]:,.3f} >> {srsi_kS[-1]:,.3f} \n"
            filtering_message6 = f"[cond6: {low_band_slope_decreasing}] LBSlopes: {low_band_slope_decreasing} / {slopes_2 * 0.95:,.3f} >> {slopes_1:,.3f} \n \n"
    ...
    try:
        ...
        for ticker in all_tickers:
            if ticker in selected_tickers and ticker not in held_coins:
                ...
                df_15 = pyupbit.get_ohlcv(ticker, interval="minute60", count=3)
                time.sleep(second)
...
def trade_sell(ticker):
    ...
    upper_price = (upper_boliinger and srsi_d[2] > 0.95)
        ...
        if profit_rate >= min_rate:
            while attempts < max_attempts:       
                ...
                if profit_rate >= max_rate or upper_price :
                    ...
                    sellmsg += f"upper_Bol: {upper_boliinger} / 0.95 < srsi_d: {srsi_d[1]:,.2f} >> {srsi_d[2]:,.2f} < srsi_k: {srsi_k[1]:,.2f} >> {srsi_k[2]:,.2f} \n \n"
                    print(sellmsg)
...
trade_msg += f'최소: {min_rate}%   최대: {max_rate}%   시도: {sell_time}회   손절: {cut_rate1}% ~ {cut_rate2}% \n \n'
print(trade_msg)
