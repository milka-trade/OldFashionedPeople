...
print(f"  매수: BB≤{BUY_BB_MAX}% + 폭≥{BUY_BB_WIDTH_MIN}% + 반등{BUY_BOUNCE_MIN}개+")
print(f"  매도: 손절{SELL_STOP_LOSS}% | 강제+{SELL_FORCE_PROFIT}% | 안전+{SELL_SAFE_PROFIT}% | 트레일{SELL_TRAIL_ACTIVATION}%/-{SELL_TRAIL_DISTANCE}%")
print(f"{'='*60}")
...
def buy_signal(ticker):
    ...
    try:
        ...
        if bb_pos > BUY_BB_MAX:
            base['reason'] = f"BB{bb_pos:.0f}%>{BUY_BB_MAX}%"
            return base
        if bb_width < BUY_BB_WIDTH_MIN:
            base['reason'] = f"BB폭{bb_width:.1f}%<{BUY_BB_WIDTH_MIN}%(횡보)"
            return base

        # Step 3: 반등 확인 (3개 중 2개+)
        bounce_signals = 0
        bounce_details = []
        ...
        return {
            'signal': True,
            'reason': f"BB{bb_pos:.0f}% 폭{bb_width:.1f}% | {detail_str} | {daily_reason}",
            'entry_price': current['close'],
            'bb_position': bb_pos,
...
def sell_signal(df, buy_price, buy_time=None, held_info=None):
    ...
    Step 1: 손절 (-2.5%) → 즉시 매도
    Step 2: 강제 익절 (+2.5%) → 즉시 매도
    Step 3: 안전 익절 (+1.2% AND RSI↓ OR BB≥60%) → 매도
    Step 4: 트레일링 (1.5% 활성화, 고점 -0.8%) → 매도
    """
    try:
        ...
        if buy_time:
            ...
            if elapsed_sec < BUY_MIN_HOLD_SEC:
                min_hold_active = True

        # Step 1: 손절 (최소 보유 무시)
        if profit_pct <= SELL_STOP_LOSS:
            return {**base, 'signal': True,
                    'reason': f'손절({profit_pct:.2f}%≤{SELL_STOP_LOSS}%)'}

        # 최소 보유 시간 중 나머지 스킵
        if min_hold_active:
            remaining = int((BUY_MIN_HOLD_SEC - elapsed_sec) / 60) + 1
            base['reason'] = f'최소보유 대기({remaining}분, 수익{profit_pct:.2f}%)'
            return base

        # Step 2: 강제 익절
        if profit_pct >= SELL_FORCE_PROFIT:
            return {**base, 'signal': True,
                    'reason': f'강제익절({profit_pct:.2f}%≥{SELL_FORCE_PROFIT}%)'}

        # Step 3: 안전 익절
        if profit_pct >= SELL_SAFE_PROFIT:
            rsi_dropping = current['rsi'] < prev['rsi']
            bb_high = bb_pos >= SELL_SAFE_BB_MIN
            if rsi_dropping or bb_high:
                reasons = []
                if rsi_dropping:
                    reasons.append(f"RSI↓{current['rsi']:.0f}")
                if bb_high:
                    reasons.append(f"BB{bb_pos:.0f}%≥{SELL_SAFE_BB_MIN}%")
                return {**base, 'signal': True,
                        'reason': f'안전익절({profit_pct:.2f}%, {"+".join(reasons)})'}

        # Step 4: 트레일링
        if profit_pct >= SELL_TRAIL_ACTIVATION and held_info:
            peak_price = held_info.get('peak_price', buy_price)
            if peak_price > 0:
                drawdown = (peak_price - current_price) / peak_price * 100
                if drawdown >= SELL_TRAIL_DISTANCE:
                    peak_profit = ((peak_price - buy_price) / buy_price) * 100
                    return {**base, 'signal': True,
                            'reason': (f'트레일링(고점{peak_profit:.1f}%→{profit_pct:.1f}%, '
                                       f'-{drawdown:.1f}%≥{SELL_TRAIL_DISTANCE}%)')}
        ...
        base['reason'] = f'홀드(수익{profit_pct:.2f}%, BB{bb_pos:.0f}%{trail_info})'
        return base
...
**전략:** BB≤{BUY_BB_MAX}% + 반등{BUY_BOUNCE_MIN}개 → 익절{SELL_SAFE_PROFIT}%+ / 손절{SELL_STOP_LOSS}%
