...
# -*- coding: utf-8 -*-
"""
BB Bounce Hunter v32.0 업그레이드 코드
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
이 파일은 v31→v32 업그레이드에 필요한 코드를 포함합니다.
...
# ── v32.0 신규: 시간대별 공격도 파라미터 ──────────────────────────────────
# 시간대별 BB 상한 조정 및 최소점수 조정
TIMEZONE_AGGRESSION = {
    # (시작시, 종료시): (BB 상한 보정값, 최소점수 보정값, 라벨)
    'afternoon': {'start': 12, 'end': 18, 'bb_adjust': +5, 'score_adjust': -1, 'label': '오후적극'},
    'evening':   {'start': 18, 'end': 22, 'bb_adjust':  0, 'score_adjust':  0, 'label': '저녁표준'},
    'night':     {'start': 22, 'end': 30, 'bb_adjust':  0, 'score_adjust':  0, 'label': '야간표준'},  # 30=06시
    'dawn':      {'start':  0, 'end':  6, 'bb_adjust': -5, 'score_adjust':  0, 'label': '새벽보수'},
    'morning':   {'start':  6, 'end': 12, 'bb_adjust': -5, 'score_adjust': +1, 'label': '오전방어'},
}
...
{Colors.BOLD}{Colors.CYAN}{'='*60}
  ...
  ★★★ v32.0: 탄력 트레일링 + 시간대별 전략 + 동적 목표
  ★ 3-Tier 직접 진입: A(BB<25) B(BB<35) C(반전포인트)
  ★ 5분봉 3점 체크리스트 + 볼륨 필터 의무화
...
{'='*60}
  Thread 1: 매수 ({BUY_THREAD_INTERVAL}초) │ 직접스캔+시간대전략
...
def get_timezone_aggression():
    """
    ★ v32.0 신규: 현재 KST 시간대에 따른 공격도 파라미터 반환.
 
    실증 근거:
      - 오후(12-18시) 진입 → 8h 보유: 1%승률 88.6% (최강)
      - 저녁(18-22시) 진입 → 4h 보유: 1%승률 50.0%, 평균↑1.50%
      - 새벽(00-06시) BB<25 한정: 1%도달률 57~86%
      - 오전(06-12시): 가장 보수적 (반등 확률 최저)
 
    반환: {'bb_adjust': int, 'score_adjust': int, 'label': str, 'hold_hours_hint': int}
    """
    kst_hour = datetime.now().hour  # 서버가 KST라고 가정
    # UTC 서버인 경우: kst_hour = (datetime.now().hour + 9) % 24
 
    if 12 <= kst_hour < 18:
        return {'bb_adjust': +5, 'score_adjust': -1, 'label': '오후적극', 'hold_hours_hint': 12}
    elif 18 <= kst_hour < 22:
        return {'bb_adjust': 0, 'score_adjust': 0, 'label': '저녁표준', 'hold_hours_hint': 8}
    elif kst_hour >= 22 or kst_hour < 6:
        return {'bb_adjust': 0, 'score_adjust': 0, 'label': '야간표준', 'hold_hours_hint': 16}
    elif 0 <= kst_hour < 6:
        return {'bb_adjust': -5, 'score_adjust': 0, 'label': '새벽보수', 'hold_hours_hint': 16}
    else:  # 06~12
        return {'bb_adjust': -5, 'score_adjust': +1, 'label': '오전방어', 'hold_hours_hint': 8}
...
def buy_signal(ticker):
    """
    ★★★ v32.0 매수 신호 — 시간대별 전략 + 5분봉 3점 체크리스트 + 볼륨 필터
 
    v31 대비 변경사항:
    ① 시간대별 공격도 조절 (BB 상한 동적 조정)
    ② 5분봉 확인: 1/2 느슨 → 2/3 필수 (3점 체크리스트)
    ③ 볼륨 필터 의무화 (vol_ratio > 1.0)
    ④ BBW 최소값 강화 (0.8→1.0%)
    ...
    try:
        ...
        # 야간 모멘텀 트랙 (v30 유지)
        # ════════════════════════════════════════
        kst_hour = datetime.now().hour  # KST 서버 가정
        ...
        if (bb_pos <= tier_a_bb_max and
            ...
            # Tier-A는 강한 조건 → 5분봉 체크 선택적, 볼륨도 선택적
            bonus_str = ""
            if day_bonus > 0:
        ...
        if (DIRECT_TIER_C_ENABLED and
            ...
            not prev['is_bull'] and    # ★ 직전봉 음봉 필수 (반전 포인트)
            vol_ok):                   # ★ v32: 볼륨 필수
 
            # 5분봉 체크 (통과 못하면 15분봉 점수로 보완)
        ...
        if (bb_pos <= tier_b_bb_max and
            ...
            # ★ v32: 5분봉 3점 체크리스트 필수 (2/3)
            if not confirm_5m_pass and DIRECT_5M_CONFIRM:
                # 5분봉 미통과 → 15분봉 점수로 보완 시도
                score_15m, details_15m = score_15m_bounce(df_15m)
