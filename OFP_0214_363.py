...
# ========================================
# [v13.0] SURGE_RIDER 급등 모멘텀 매수
# ========================================
# Phase S1: 일봉 급등 확인
SURGE_DAILY_CHANGE_MIN = 3.0        # 당일 최소 등락률 (%)
SURGE_DAILY_CHANGE_MAX = 12.0       # 당일 최대 등락률 (%) - 극단 제외
SURGE_3DAY_MIN = -2.0               # 3일 누적 최소 (%) - V자 반전 허용
SURGE_DAILY_RSI_MIN = 40            # 일봉 RSI 하한
SURGE_DAILY_RSI_MAX = 80            # 일봉 RSI 상한

# Phase S2: 15분봉 모멘텀 구간
SURGE_15M_BB_MIN = 40               # BB 하한 (%)
SURGE_15M_BB_MAX = 85               # BB 상한 (%)
SURGE_15M_BB_STRONG = 75            # 이 이상 시 조건 강화
SURGE_15M_BB_WIDTH_MIN = 1.5        # BB 폭 최소 (%)
SURGE_15M_RSI_MIN = 45              # 15분봉 RSI 하한
SURGE_15M_RSI_MAX = 78              # 15분봉 RSI 상한

# Phase S3: 모멘텀 강도 (6개 지표)
SURGE_MOMENTUM_MIN = 4              # 최소 충족 (6개 중)
SURGE_MOMENTUM_STRONG = 5           # BB 75%+ 시 필요
SURGE_VOLUME_MIN = 1.5              # 거래량 최소 (MA20 대비)
SURGE_VOLUME_STRONG = 2.0           # 강구간 거래량

# SURGE 매도 (시간제한 없음)
SURGE_SL_TIGHT = -1.5               # 기본 손절 (%)
SURGE_SL_WIDE = -2.0                # BB≥60% 손절 (%)
SURGE_SL_BB_THRESHOLD = 60          # 손절 분기 BB (%)
SURGE_TRAIL_ACT = 1.5               # 트레일링 시작 (%)
SURGE_TRAIL_DIST = 0.8              # 트레일링 드로다운 (%)
SURGE_EXTREME_BB = 92               # 극과매수 BB
SURGE_EXTREME_MIN_PFT = 0.8         # 극과매수 최소 수익
SURGE_CONSEC_BEAR = 2               # 연속 음봉 탈출
SURGE_CONSEC_BEAR_PFT = 0.3         # 연속 음봉 최소 수익
SURGE_RSI_DROP = 5                  # RSI 급락 기준 (pt)
SURGE_RSI_DROP_PFT = 0.0            # RSI 급락 최소 수익
...
VERSION = "12.0 REGIME_HUNTER"

print(f"\n{Colors.BOLD}{Colors.CYAN}{'='*60}")
print(f"EVOLUTION {VERSION}")
print(f"{'='*60}")
print(f"{Colors.GREEN}[v12.0] REGIME HUNTER - 시장레짐기반 매수조건 차등{Colors.ENDC}")
print(f"   [신규] Phase 0: 시장레짐판별 (일봉+3일추세+ADX+양봉빈도)")
print(f"   [🟢BULL] BB5~42% 폭≥1.3% 반등2/5 (공격적)")
print(f"   [🟡NEUT] BB5~35% 폭≥1.8% 반등3/5 (표준)")
print(f"   [🔴BEAR] BB3~25% 폭≥2.5% 반등3/5+ADX/OBV필터 (보수적)")
print(f"   [매도] 변경없음: 손절→극과매수→소진7지표→동적트레일링")
print(f"   [안전] REGIME_ENABLED=False로 즉시 v11.1 복원 가능")
print(f"")
...
def calculate_coin_status_for_report(ticker):
    """
    [v13.0] 보고서용 코인 상태 - BOUNCE+SURGE 통합
    """
    try:
        # === 일봉 ===
        df_daily = get_candles_daily(ticker, count=50)
        ...
        if df_daily is not None and len(df_daily) >= 1:
            ...
            td = df_daily.iloc[-1]
            do, dc, dh, dl = td['open'], td['close'], td['high'], td['low']
            
            d_emoji = '🟢' if dc >= do else '🔴'
            d_change = ((dc - do) / do * 100) if do > 0 else 0
            if df_di is not None:
                d_rsi = df_di.iloc[-1].get('rsi', 50)
            
            rise = ((dc - dl) / dl * 100) if dl > 0 else 0
            drop = ((dh - dc) / dh * 100) if dh > 0 else 0
            if rise > drop * 2:
                pwr = '⚡'
            elif drop > rise * 2:
                pwr = '👀'
        ...
        ri = determine_market_regime(ticker, df_15m)
        regime = ri['regime']
        r_scr = ri['score']
        r_emo = {'BULLISH':'🟢','NEUTRAL':'🟡','BEARISH':'🔴'}.get(regime,'⚪')
        ...
        if df_15m is not None and len(df_15m) >= 20:
            ...
            rsi15 = c.get('rsi', 50)
            vol15 = c.get('volume_ratio', 1.0)
        
        # === BOUNCE 판정 ===
        b_res = 'P1❌'
        b_dtl = ''
        ds = check_daily_safety_filter(ticker, regime=regime)
        if ds['safe']:
            dt = ds.get('daily_candle_type', 'unknown')
            if df_15m is not None and len(df_15m) >= 20:
                pc = detect_downtrend_15m(df_15m, daily_candle_type=dt, regime=regime)
                if not pc['is_downtrend']:
                    rv = calculate_reversal_score(df_15m)
                    p3s = format_reversal_detail(rv)
                    if REGIME_ENABLED:
                        rq = {'BULLISH':REGIME_BULL_REVERSAL_MIN,
                              'BEARISH':REGIME_BEAR_REVERSAL_MIN}.get(regime, REGIME_NEUT_REVERSAL_MIN)
                    else:
                        rq = V10_BOUNCE_MIN_SIGNALS
                    if rv['bounce_confirmed'] and rv['score'] >= rq:
                        b_res = '✅✅✅BUY'
                        b_dtl = p3s
                    else:
                        b_res = '✅✅P3❌'
        ...
        s_res = '-'
        s_dtl = ''
        if d_change >= SURGE_DAILY_CHANGE_MIN:
            if df_15m is not None and len(df_15m) >= 20:
                s2 = (SURGE_15M_BB_MIN <= bb15 <= SURGE_15M_BB_MAX and
                      bw15 >= SURGE_15M_BB_WIDTH_MIN and
                      SURGE_15M_RSI_MIN <= rsi15 <= SURGE_15M_RSI_MAX)
                if s2:
                    mm = calculate_surge_momentum_score(df_15m)
                    rq2 = SURGE_MOMENTUM_STRONG if bb15 >= SURGE_15M_BB_STRONG else SURGE_MOMENTUM_MIN
                    if mm['confirmed'] and mm['score'] >= rq2:
                        s_res = '✅✅✅BUY'
                        s_dtl = f'{mm["score"]}/6'
                    else:
                        s_res = '✅✅S3❌'
                        s_dtl = f'{mm["score"]}/{rq2}'
                else:
                    s_res = '✅S2❌'
                    if bb15 < SURGE_15M_BB_MIN:
                        s_dtl = f'BB{bb15:.0f}<{SURGE_15M_BB_MIN}'
                    elif bb15 > SURGE_15M_BB_MAX:
                        s_dtl = f'BB{bb15:.0f}>{SURGE_15M_BB_MAX}'
                    else:
                        s_dtl = 'RSI/폭'
            else:
                s_res = '✅S2❌'
        ...
        return {
            'd_emoji': d_emoji, 'd_change': d_change, 'd_rsi': d_rsi, 'pwr': pwr,
            'regime': regime, 'r_emo': r_emo, 'r_scr': r_scr,
            'bb15': bb15, 'bw15': bw15, 'rsi15': rsi15, 'vol15': vol15,
            'b_res': b_res, 'b_dtl': b_dtl,
            's_res': s_res, 's_dtl': s_dtl,
            'final': final,
            # 레거시 호환 (기존 코드에서 참조할 수 있는 필드)
            'daily_status': '양봉' if d_change >= 0 else '음봉',
            'daily_emoji': d_emoji, 'daily_change': d_change,
            'rise_from_low': 0, 'drop_from_high': 0,
            'power_emoji': pwr, 'power_label': '',
            'p1_pass': ds['safe'] if ds else False,
            'p1_reason': '', 'p2_pass': False, 'p2_reason': '',
            'p3_pass': False, 'p3_reason': '',
            'final_signal': final, 'phase_str': '',
        ...
        return {
            'd_emoji': '⚪', 'd_change': 0, 'd_rsi': 50, 'pwr': '❓',
            'regime': 'NEUTRAL', 'r_emo': '⚪', 'r_scr': 0,
            'bb15': 50, 'bw15': 0, 'rsi15': 50, 'vol15': 1.0,
            'b_res': '오류', 'b_dtl': str(e)[:15],
...
def send_enhanced_statistics_report():
    """
    [v13.0] 매시각 보고서 - 코인별 지표+판정 한눈에
    
    ① 자산+성과  ② 보유코인(수익/BB/RSI/시간/모드)
    ③ 관심코인: 레짐+일봉+15분봉지표 / B:BOUNCE S:SURGE → 결론
    """
    try:
        portfolio = get_enhanced_portfolio_status()
        now = datetime.now()
            ...
            for ci in portfolio['coins']:
                ...
                with held_coins_lock:
                    if tk in held_coins:
                        ...
                        bm = held_coins[tk].get('buy_mode', '')
                        mtag = "🚀S" if 'SURGE' in bm else "📈B"
        ...
        watch = [c for c in FIXED_STABLE_COINS if c not in hs]
        ...
        if watch:
            ...
            for tk in watch:
                ...
                l1 = (
                    f"\n{st['d_emoji']}**{cn}** "
                    f"{st['r_emo']}{st['regime']}({st['r_scr']}) "
                    f"일`{st['d_change']:+.1f}%` "
                    f"BB`{st['bb15']:.0f}%`폭`{st['bw15']:.1f}` "
                    f"RSI`{st['rsi15']:.0f}` V`{st['vol15']:.1f}x`"
                )
                
                # 2줄: BOUNCE+SURGE 판정
                bs = f"B:{st['b_res']}"
                if st['b_dtl'] and 'BUY' not in st['b_res']:
                    bs += f"({st['b_dtl'][:20]})"
                
                ss = ""
                if st['s_res'] != '-':
                    ss = f" S:{st['s_res']}"
                    if st['s_dtl'] and 'BUY' not in st['s_res']:
                        ss += f"({st['s_dtl']})"
                
                fe = '🟢' if '매수' in st['final'] else '⚪'
                l2 = f"\n └ {bs}{ss} → {fe}**{st['final']}**"
...
def check_daily_safety_filter(ticker, regime='NEUTRAL'):
    """
    [v11.1] Phase 1: 일봉 양봉 필터 (대세 상승 확인)
    
    v11.1 변경: daily_candle_type 반환 추가
    - 'bullish': 양봉 + 등락률 ≥ 0.5% (적극적 BB폭 적용)
    - 'weak_bullish': 양봉 + 등락률 < 0.5% (보수적 BB폭 적용)
    - 'recent_bullish': 오늘 음봉이지만 최근 3일중 2일 양봉 (보수적 BB폭 적용)
    ...
    try:
        ...
        # [v12.0 변경] 레짐별 연속 음봉 허용 기준
        if REGIME_ENABLED:
            if regime == 'BEARISH':
                max_consecutive_bear = REGIME_BEAR_CONSECUTIVE_BEAR_MAX  # 2
            elif regime == 'BULLISH':
                max_consecutive_bear = REGIME_BULL_CONSECUTIVE_BEAR_MAX  # 3
            else:
                max_consecutive_bear = REGIME_NEUT_CONSECUTIVE_BEAR_MAX  # 3
        else:
            max_consecutive_bear = V10_DAILY_CONSECUTIVE_BEAR_MAX  # 3 (기존)
        ...
        # ========================================
        # 모든 체크 통과 → daily_candle_type 결정 [v11.1 신규]
        # ========================================
...
def calculate_surge_momentum_score(df_15m):
    """
    [v13.0] SURGE Phase S3: 모멘텀 강도 평가 (6점 만점)
    
    ① RSI>50 AND 상승   ② S-RSI K>40 AND K>D
    ③ MACD히스토그램 양수/증가  ④ 현재봉 양봉
    ⑤ 거래량 ≥ MA20×1.5    ⑥ 최근3봉 중 2봉+ 양봉
    """
    try:
        if df_15m is None or len(df_15m) < 5:
            return {'score': 0, 'details': [], 'signals': {},
                    'confirmed': False, 'mandatory_met': False, 'vol_ratio': 1.0}
        ...
        # ① RSI > 50 AND 상승
        rsi_n, rsi_p = cur['rsi'], prv['rsi']
        rsi_ok = rsi_n > 50 and rsi_n > rsi_p
        signals['rsi_momentum'] = rsi_ok
        if rsi_ok:
            score += 1
            details.append(f"①RSI↑{rsi_n:.0f}")
        
        # ② S-RSI K>40 AND K>D
        sk = cur.get('stoch_rsi_k', 50)
        sd = cur.get('stoch_rsi_d', 50)
        stoch_ok = sk > 40 and sk > sd
        signals['stoch_rsi'] = stoch_ok
        if stoch_ok:
            score += 1
            details.append(f"②SK{sk:.0f}>D{sd:.0f}")
        
        # ③ MACD 양수 OR 증가
        mh = cur.get('macd_hist', 0)
        mh_p = prv.get('macd_hist', 0)
        macd_ok = (mh > 0) or (mh > mh_p)
        signals['macd'] = macd_ok
        if macd_ok:
            score += 1
            details.append(f"③MACD{'+'if mh>0 else '↑'}")
        
        # ④ 양봉
        bull = cur['close'] > cur['open']
        signals['bullish'] = bull
        if bull:
            score += 1
            chg = ((cur['close'] - cur['open']) / cur['open']) * 100
            details.append(f"④양봉+{chg:.1f}%")
        
        # ⑤ 거래량 ≥ MA20 × 1.5
        vr = cur.get('volume_ratio', 1.0)
        vol_ok = vr >= SURGE_VOLUME_MIN
        signals['volume'] = vol_ok
        if vol_ok:
            score += 1
            details.append(f"⑤Vol{vr:.1f}x")
        
        # ⑥ 최근 3봉 중 2봉+ 양봉
        bc = sum(1 for i in range(-3, 0) if len(df_15m)+i >= 0
                 and df_15m.iloc[i]['close'] > df_15m.iloc[i]['open'])
        con_ok = bc >= 2
        signals['consecutive'] = con_ok
        if con_ok:
            score += 1
            details.append(f"⑥연양{bc}/3")
        
        mandatory = bull or rsi_ok  # ④ 또는 ① 중 1개 필수
        confirmed = score >= SURGE_MOMENTUM_MIN and mandatory
        
        return {'score': score, 'details': details, 'signals': signals,
                'confirmed': confirmed, 'mandatory_met': mandatory,
                'vol_ratio': vr}
    except Exception as e:
        if DEBUG_MODE:
            print(f"{Colors.RED}[Surge Momentum Error] {e}{Colors.ENDC}")
        return {'score': 0, 'details': [], 'signals': {},
                'confirmed': False, 'mandatory_met': False, 'vol_ratio': 1.0}
...
def surge_buy_signal(df_15m, df_5m, ticker):
    """
    [v13.0] SURGE_RIDER - 급등 모멘텀 매수 신호
    
    Phase S1: 일봉 3~12% 급등 + RSI 40~80
    Phase S2: 15분봉 BB 40~85% + 폭≥1.5% + RSI 45~78
    Phase S3: 모멘텀 6지표 중 4개+ (BB≥75%면 5개+)
    
    기존 BOUNCE와 완전 독립 - execute_buy() 호환 signal dict 반환
    """
    try:
        cur = df_15m.iloc[-1] if df_15m is not None and len(df_15m) > 0 else None
        
        # execute_buy()가 참조하는 모든 필드 포함
        base = {
            'signal': False, 'reason': '', 'confidence': 0,
            'entry_price': cur['close'] if cur is not None else 0,
            'bb_position': cur['bb_position'] if cur is not None else 50,
            'bb_width_pct': cur['bb_width'] if cur is not None else 2.0,
            'mode': 'SURGE_V13',
            'bb_width_zone': 'SURGE',
            'target_profit': 2.0,
            'market_condition': 'NORMAL',
            'score': 0, 'daily_change': 0, 'reversal_score': 0,
            'regime': 'NEUTRAL', 'regime_score': 0, 'regime_details': '',
            'daily_bb': 50
        }
        
        if df_15m is None or len(df_15m) < 20:
            base['reason'] = 'SURGE: 15분봉부족'
            return base
        
        coin = ticker.replace('KRW-', '')
        
        # ===== Phase S1: 일봉 급등 확인 =====
        df_d = get_candles_daily(ticker, count=50)
        if df_d is None or len(df_d) < 20:
            base['reason'] = 'SURGE S1:일봉부족'
            return base
        df_d = add_indicators(df_d)
        if df_d is None:
            base['reason'] = 'SURGE S1:지표오류'
            return base
        
        td = df_d.iloc[-1]
        d_open, d_close, d_rsi = td['open'], td['close'], td['rsi']
        dc = ((d_close - d_open) / d_open * 100) if d_open > 0 else 0
        base['daily_change'] = dc
        
        if dc < SURGE_DAILY_CHANGE_MIN:
            base['reason'] = f'SURGE S1:등락{dc:+.1f}%<{SURGE_DAILY_CHANGE_MIN}%'
            return base
        if dc > SURGE_DAILY_CHANGE_MAX:
            base['reason'] = f'SURGE S1:극등{dc:+.1f}%>{SURGE_DAILY_CHANGE_MAX}%'
            return base
        
        # 3일 누적
        td3 = dc
        if len(df_d) >= 3:
            td3 = ((df_d.iloc[-1]['close'] - df_d.iloc[-3]['open']) / df_d.iloc[-3]['open'] * 100)
        if td3 < SURGE_3DAY_MIN:
            base['reason'] = f'SURGE S1:3일{td3:+.1f}%<{SURGE_3DAY_MIN}%'
            return base
        
        if d_rsi < SURGE_DAILY_RSI_MIN or d_rsi > SURGE_DAILY_RSI_MAX:
            base['reason'] = f'SURGE S1:RSI{d_rsi:.0f}범위외({SURGE_DAILY_RSI_MIN}~{SURGE_DAILY_RSI_MAX})'
            return base
        
        if DEBUG_MODE:
            print(f"  🚀 [{coin}] SURGE S1통과: +{dc:.1f}% 3일{td3:+.1f}% RSI{d_rsi:.0f}")
        ...
        if bb < SURGE_15M_BB_MIN or bb > SURGE_15M_BB_MAX:
            base['reason'] = f'SURGE S2:BB{bb:.0f}%범위외({SURGE_15M_BB_MIN}~{SURGE_15M_BB_MAX})'
            return base
        if bw < SURGE_15M_BB_WIDTH_MIN:
            base['reason'] = f'SURGE S2:BB폭{bw:.1f}%<{SURGE_15M_BB_WIDTH_MIN}%'
            return base
        if r15 < SURGE_15M_RSI_MIN or r15 > SURGE_15M_RSI_MAX:
            base['reason'] = f'SURGE S2:RSI{r15:.0f}범위외({SURGE_15M_RSI_MIN}~{SURGE_15M_RSI_MAX})'
            return base
        
        if DEBUG_MODE:
            print(f"  🚀 [{coin}] SURGE S2통과: BB{bb:.0f}% 폭{bw:.1f}% RSI{r15:.0f}")
        
        # ===== Phase S3: 모멘텀 강도 =====
        mom = calculate_surge_momentum_score(df_15m)
        req = SURGE_MOMENTUM_MIN  # 4
        if bb >= SURGE_15M_BB_STRONG:
            req = SURGE_MOMENTUM_STRONG  # 5
            if mom.get('vol_ratio', 1.0) < SURGE_VOLUME_STRONG:
                base['reason'] = f'SURGE S3:BB{bb:.0f}%강구간 Vol{mom["vol_ratio"]:.1f}x<{SURGE_VOLUME_STRONG}x'
                return base
        
        if not mom['confirmed'] or mom['score'] < req:
            dtl = ','.join(mom['details']) if mom['details'] else '없음'
            base['reason'] = f'SURGE S3:모멘텀{mom["score"]}/{req}[{dtl}]'
            return base
        
        # ===== 전 Phase 통과 → 매수! =====
        conf = min(100, 50 + mom['score'] * 8 + int(dc * 3))
        vtag = ""
        if td3 < 0 and dc >= SURGE_DAILY_CHANGE_MIN:
            conf = min(100, conf + 5)
            vtag = " V자반전"
        
        sdtl = ','.join(mom['details'][:4])
        
        return {
            'signal': True,
            'reason': f'🚀SURGE! +{dc:.1f}% BB{bb:.0f}% | {sdtl}{vtag}',
            'confidence': conf,
            ...
            'bb_width_zone': 'SURGE',
            'target_profit': 2.0,
            'market_condition': 'SURGE_CONFIRMED',
            'score': mom['score'],
            'daily_change': dc,
            'reversal_score': mom['score'],
            'daily_bb': td.get('bb_position', 50),
            'daily_rsi': d_rsi,
            'regime': 'BULLISH',
            'regime_score': 5,
            'regime_details': f'SURGE(+{dc:.1f}%)'
        }
        
    except Exception as e:
        if DEBUG_MODE:
            print(f"{Colors.RED}[SURGE Signal Error] {e}{Colors.ENDC}")
            traceback.print_exc()
        return {
            'signal': False, 'reason': f'SURGE오류:{e}', 'confidence': 0,
            'entry_price': 0, 'bb_position': 50, 'bb_width_pct': 2.0,
            'mode': 'SURGE_V13', 'bb_width_zone': 'ERROR', 'target_profit': 2.0,
            'market_condition': 'ERROR', 'score': 0, 'daily_change': 0,
            'reversal_score': 0, 'daily_bb': 50,
            'regime': 'NEUTRAL', 'regime_score': 0, 'regime_details': ''
        }

def evolution_80_sell_signal(df, buy_price, buy_time=None, held_info=None):
    """
    [v13.0] BOUNCE + SURGE 통합 매도 (시간제한 없음)
    
    buy_mode='SURGE_V13' → SURGE 전용 (타이트 손절/트레일링)
    그 외               → 기존 v11.0 소진점수제 (변경 없음)
    """
    try:
        ...
        # buy_mode 분기
        # =======================================
        buy_mode = held_info.get('buy_mode', '') if held_info else ''
        
        if buy_mode == 'SURGE_V13':
            # ===== SURGE 전용 매도 =====
            is_bear = current['close'] < current['open']
            rsi_n = current['rsi']
            rsi_p = prev['rsi']
            
            # S-1: 손절 (BB 기반 차등)
            sl = SURGE_SL_WIDE if bb_position >= SURGE_SL_BB_THRESHOLD else SURGE_SL_TIGHT
            if profit_pct <= sl:
                return {**base_response, 'signal': True,
                        'reason': f'SURGE손절({profit_pct:.2f}%<={sl}%)'}
            
            # S-2: 극과매수 익절
            if bb_position >= SURGE_EXTREME_BB and is_bear and profit_pct >= SURGE_EXTREME_MIN_PFT:
                return {**base_response, 'signal': True,
                        'reason': f'SURGE극과매수(BB{bb_position:.0f}%+음봉 수익{profit_pct:.2f}%)'}
            
            # S-3: 연속 음봉 탈출
            cb = 0
            for i in range(-1, -4, -1):
                if len(df)+i >= 0 and df.iloc[i]['close'] < df.iloc[i]['open']:
                    cb += 1
                else:
                    break
            if cb >= SURGE_CONSEC_BEAR and profit_pct >= SURGE_CONSEC_BEAR_PFT:
                return {**base_response, 'signal': True,
                        'reason': f'SURGE연속음봉({cb}봉 수익{profit_pct:.2f}%)'}
            
            # S-4: RSI 급락
            rd = rsi_p - rsi_n
            if rd >= SURGE_RSI_DROP and profit_pct >= SURGE_RSI_DROP_PFT:
                return {**base_response, 'signal': True,
                        'reason': f'SURGE RSI급락({rsi_p:.0f}→{rsi_n:.0f} 수익{profit_pct:.2f}%)'}
            
            # S-5: 트레일링 스탑
            if profit_pct >= SURGE_TRAIL_ACT and held_info:
                pk = held_info.get('peak_price', buy_price)
                if pk > 0 and current_price > 0:
                    dd = (pk - current_price) / pk * 100
                    pp = ((pk - buy_price) / buy_price) * 100
                    if dd >= SURGE_TRAIL_DIST:
                        return {**base_response, 'signal': True,
                                'reason': f'SURGE트레일링(고점{pp:.1f}%→현재{profit_pct:.1f}% -{dd:.1f}%)'}
            
            # S-6: 홀드
            base_response['reason'] = f'SURGE홀드(수익{profit_pct:.2f}% BB{bb_position:.0f}% RSI{rsi_n:.0f})'
            return base_response
        
        # ===== 기존 BOUNCE 매도 (v11.0 동일) =====
        ...
        if DEBUG_MODE:
            print(f"{Colors.RED}[v13.0 Sell Error] {e}{Colors.ENDC}")
            traceback.print_exc()
...
def buy_thread_worker():
    """
    [v13.0] BOUNCE + SURGE 듀얼 매수 스레드
    우선순위: BOUNCE > SURGE
    """
    print(f"{Colors.CYAN}[Thread 1] v13.0 BOUNCE+SURGE 매수 스레드 시작 ({BUY_THREAD_INTERVAL}초 주기){Colors.ENDC}")
                ...
                if buy_signal['signal']:
                    ...
                    with held_coins_lock:
                        if len(held_coins) >= MAX_HOLDINGS:
                            ...
                            break
                    time.sleep(0.5)
                    continue  # BOUNCE 시도 후 SURGE 스킵
                
                # ===== [2순위] SURGE 매수 (BOUNCE 미충족 시만) =====
                surge_signal = surge_buy_signal(df_15m, df_5m, ticker)
                
                if surge_signal['signal']:
                    coin_name = ticker.replace('KRW-', '')
                    print(f"\n{Colors.MAGENTA}{'='*50}")
                    print(f"[SURGE SIGNAL] {coin_name} 급등 모멘텀 매수!")
                    print(f"{'='*50}{Colors.ENDC}")
                    print(f"  🚀 모멘텀: {surge_signal.get('score', 0)}/6 | 일봉 +{surge_signal.get('daily_change', 0):.1f}%")
                    print(f"  📈 BB 위치: {surge_signal['bb_position']:.1f}%")
                    print(f"  💰 진입가: {surge_signal['entry_price']:,.0f}원")
                    print(f"  🔑 신뢰도: {surge_signal['confidence']}%")
                    print(f"  📝 사유: {surge_signal['reason']}")
                    print(f"{Colors.MAGENTA}{'='*50}{Colors.ENDC}\n")
                    
                    success = execute_buy(ticker, surge_signal)
                    if success:
                        print(f"{Colors.MAGENTA}[BUY] {coin_name} SURGE 매수 완료!{Colors.ENDC}")
                    else:
                        print(f"{Colors.RED}[BUY] {coin_name} SURGE 매수 실패{Colors.ENDC}")
    ...
    print(f"{Colors.CYAN}[Thread 1] v13.0 BOUNCE+SURGE 매수 스레드 종료{Colors.ENDC}")
