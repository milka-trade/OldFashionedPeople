...
# ========================================
# [v8.1] 반등 초기 감지 및 동적 기준 파라미터
# ========================================

# 반등 초기 보너스
V81_RECOVERY_BONUS_ENABLED = True      # 반등 초기 보너스 활성화
V81_RECOVERY_BONUS_SCORE = 25          # 반등 초기 보너스 점수
V81_RECOVERY_RSI_MAX = 45              # 반등 초기 RSI 상한
V81_RECOVERY_BB_MAX = 35               # 반등 초기 BB 상한
...
V81_CRASH_DEPTH_THRESHOLD = -2.5       # 급락 기준 (%)
V81_CRASH_RECOVERY_BB_MIN = 10         # 반등 BB 하한
V81_CRASH_RECOVERY_BB_MAX = 35         # 반등 BB 상한
V81_CRASH_RECOVERY_STRENGTH = 85       # 트리거 강도

# ================================================================================
# [v9.1] VOLATILITY HUNTER Parameters (신규 추가)
# ================================================================================

# ========================================
# [v9.1] 일봉 안전 필터
# ========================================
V91_DAILY_SURGE_LIMIT = 3.0          # 당일 급등 제한 (%)
V91_DAILY_CRASH_LIMIT = -3.0         # 당일 급락 기준 (%)
V91_DAILY_CONSECUTIVE_BEAR_MAX = 3   # 최대 연속 음봉 허용

# ========================================
# [v9.1] 15분봉 하락 추세 감지
# ========================================
V91_DOWNTREND_RSI_DROP_COUNT = 3     # RSI 연속 하락 횟수
V91_DOWNTREND_LOWER_LOW_COUNT = 3    # 저점 연속 하락 횟수
V91_BB_BREAKDOWN_THRESHOLD = -2      # BB 하단 돌파 기준 (%, 음수)

# ========================================
# [v9.1] BB 폭 기반 진입 조건
# ========================================
V91_BB_WIDTH_ABSOLUTE_MIN = 1.5      # 절대 금지 기준
V91_BB_WIDTH_CAUTION_MIN = 2.0       # 신중 매수 기준
V91_BB_WIDTH_OPTIMAL_MIN = 3.0       # 최적 매수 기준
V91_BB_WIDTH_OPTIMAL_MAX = 5.0       # 최적 매수 상한
V91_BB_WIDTH_HIGH_VOLATILITY = 5.0   # 고변동성 기준

# BB 폭 < 1.5%: 절대 금지
# BB 폭 1.5-2.0%: 조건부 허용 (아래 조건)
V91_LOW_WIDTH_BB_POSITION_MAX = 10   # BB 위치 10% 이하만
V91_LOW_WIDTH_REVERSAL_MIN = 28      # 반등 점수 28점 이상

# BB 폭 2.0-3.0%: 신중 매수
V91_WIDTH_LOW_BB_POSITION_MAX = 15   # BB 위치 상한
V91_WIDTH_LOW_REVERSAL_MIN = 25      # 반등 점수 하한

# BB 폭 3.0-5.0%: 적극 매수 (최적)
V91_WIDTH_MID_BB_POSITION_MAX = 25   # BB 위치 상한
V91_WIDTH_MID_REVERSAL_MIN = 20      # 반등 점수 하한

# BB 폭 5.0%+: 조건부 매수 (고변동성)
V91_WIDTH_HIGH_BB_POSITION_MAX = 20  # BB 위치 상한
V91_WIDTH_HIGH_REVERSAL_MIN = 28     # 반등 점수 하한
V91_WIDTH_HIGH_HIGHER_LOW_REQUIRED = True  # 저점 상승 필수

# ========================================
# [v9.1] 반등 확인 점수 (30점 만점)
# ========================================
V91_REVERSAL_RSI_RISING = 10         # RSI 상승 전환 점수
V91_REVERSAL_BULLISH_CANDLE = 10     # 양봉 출현 점수
V91_REVERSAL_VOLUME_INCREASE = 5     # 거래량 증가 점수 (1.2배+)
V91_REVERSAL_HIGHER_LOW = 5          # 저점 상승 점수
V91_REVERSAL_MIN_ITEMS = 2           # 최소 충족 항목 수

# ========================================
# [v9.1] 목표 수익 (BB 폭별)
# ========================================
V91_TARGET_WIDTH_LOW = 1.5           # 폭 2-3%: 목표 수익 (%)
V91_TARGET_WIDTH_MID = 2.5           # 폭 3-5%: 목표 수익 (%)
V91_TARGET_WIDTH_HIGH = 2.0          # 폭 5%+: 목표 수익 (%)
V91_MIN_PROFIT_WIDTH_LOW = 1.0       # 폭 2-3%: 최소 익절 (%)
V91_MIN_PROFIT_WIDTH_MID = 1.5       # 폭 3-5%: 최소 익절 (%)
V91_MIN_PROFIT_WIDTH_HIGH = 1.2      # 폭 5%+: 최소 익절 (%)

# ========================================
# [v9.1] 트레일링 스탑
# ========================================
V91_TRAILING_ACTIVATION_RATIO = 0.7  # 목표의 70%에서 활성화
V91_TRAILING_BB_MULTIPLIER = 0.25    # BB폭 × 0.25 = 트레일링 거리
V91_TRAILING_MIN_DISTANCE = 0.8      # 최소 트레일링 거리 (%)

# ========================================
# [v9.1] 손절
# ========================================
V91_STOP_LOSS_PCT = -2.5             # 손절률 (%)

# ========================================
# [v9.1] 하락 선행 감지 (매도용)
# ========================================
V91_DECLINE_RSI_DIVERGENCE = 2       # RSI 다이버전스 점수
V91_DECLINE_VOLUME_DROP = 2          # 거래량 급감 + 음봉 점수
V91_DECLINE_CONSECUTIVE_BEAR_5M = 2  # 5분봉 연속 음봉 점수
V91_DECLINE_BB_REJECTION = 2         # BB 상단 이탈 후 복귀 점수
V91_DECLINE_DRAWDOWN = 1             # 고점 대비 하락 점수
V91_DECLINE_THRESHOLD = 4            # 익절 권장 임계값
...
print(f"{'='*60}")
print(f"{Colors.GREEN}[v9.1] VOLATILITY HUNTER - BB 폭 기반 최적 매수{Colors.ENDC}")
print(f"   [핵심] BB 폭이 핵심! 하단 터치보다 폭+위치 조합")
print(f"   [매수] 일봉안전→하락추세감지→BB폭조건→반등확인")
print(f"   [매도] 선행하락감지→동적트레일링→목표익절")
print(f"")
print(f"{Colors.YELLOW}BB 폭별 매수 조건{Colors.ENDC}")
print(f"   < 1.5%: 매수 금지 (횡보장)")
print(f"   1.5-2.0%: 조건부 (BB≤10%, 반등≥28점)")
print(f"   2.0-3.0%: 신중 (BB≤15%, 반등≥25점)")
print(f"   3.0-5.0%: 최적 (BB≤25%, 반등≥20점)")
print(f"   5.0%+: 고변동성 (BB≤20%, 반등≥28점, HL필수)")
print(f"")
...
def generate_market_summary():
    """
    [v8.0] 모멘텀 기반 시장 분석 요약
    
    정시 보고서의 관심코인 섹션 생성용
    (send_enhanced_statistics_report에서 직접 처리하므로 이 함수는 단순화)
    ...
    try:
        ...
        for ticker in FIXED_STABLE_COINS:
            ...
            if analysis:
                # 모멘텀 정보 추가
                momentum = calculate_momentum_for_report(ticker)
        ...
        for coin in analysis_results:
            ...
            grade = momentum.get('grade', '?')
            reason = momentum.get('reason_str', '')
            ...
            message += f"\n{emoji} **{coin_name}** {daily_warning}모멘텀{grade}({reason})"
            message += f" | BB`{coin['bb_position']:.0f}%` RSI`{coin['rsi']:.0f}`"
...
def send_enhanced_statistics_report():
    """
    [v8.0] 개선된 정시 보고서 - 모멘텀 기반
    
    표시 정보:
    - 1줄: 시각 + 총자산 + 코인가치(수익률) + 현금
    - 보유코인: 모멘텀 상태 + 수익률 + 가격정보
    - 관심코인: 모멘텀 상태 + BB/RSI
    
    제거된 정보:
    - 버전명, 가동시간, 금일 거래건수, 누적 거래
    """
    try:
        portfolio = get_enhanced_portfolio_status()
        ...
        profit_emoji = "📈" if coin_profit_pct >= 0 else "📉"
        
        header = f"""⏰ {current_time.strftime('%H:%M:%S')}
💰 총 {portfolio['total_assets']:,.0f}원 | 코인 {portfolio['total_coin_value']:,.0f}원({coin_profit_pct:+.1f}%) | 현금 {portfolio['krw_balance']:,.0f}원"""
        ...
        if portfolio['coins']:
            ...
            for coin_info in portfolio['coins']:
                ...
                profit_emoji_coin = "📈" if profit_pct >= 0 else "📉"
                ...
                holdings_text += f"\n├ **{coin_name}** {momentum['emoji']}모멘텀{momentum['grade']}({momentum['reason_str']}) {profit_emoji_coin}`{profit_pct:+.2f}%`"
                holdings_text += f"\n│  └ {buy_price:,.0f}→{current_price:,.0f}원 | BB`{bb_pos:.0f}%` RSI`{rsi:.0f}` | ⏱{hold_duration}"
        else:
        ...
        for ticker in watchlist_coins:
            ...
            if analysis:
                ...
                # 일봉 BB 경고
                daily_warning = ""
                if analysis.get('daily_bb_position') is not None:
                    daily_bb = analysis['daily_bb_position']
                    if daily_bb >= DAILY_BB_HIGH_FILTER:
                        daily_warning = "⚠️"
                    elif daily_bb <= 30:
                        daily_warning = "🔥"
                
                watchlist_text += f"\n├ {momentum['emoji']} **{coin_name}** {daily_warning}모멘텀{momentum['grade']}({momentum['reason_str']}) BB`{bb_pos:.0f}%` RSI`{rsi:.0f}`"
            else:
                watchlist_text += f"\n├ ⚪ **{coin_name}** 데이터없음"
...
def calculate_momentum_strength(df_15m, df_daily=None):
    """
    [v8.1] 복합 모멘텀 강도 측정 (0-100점) + 반등 초기 보너스
    
    핵심: "오르고 있다"가 아니라 "오를 힘이 있다"를 측정
    
    [v8.1 추가] 반등 초기 보너스:
    - RSI 상승 중 + RSI < 45 + 양봉 + BB < 35% → +25점
    ...
    Returns:
        dict: {
            'score': 총점 (0-125, 100점 초과 가능),
            'details': 점수 획득 사유 리스트,
            'rsi': 현재 RSI,
            'volume_ratio': 거래량 비율,
            'bb_width': BB 폭,
            'is_recovery_start': 반등 초기 여부
        }
    """
    try:
        score = 0
        ...
        if df_15m is None or len(df_15m) < 20:
            return {'score': 0, 'details': ['데이터 부족'], 'rsi': 50, 'volume_ratio': 1.0, 'bb_width': 2.0, 'is_recovery_start': False}
        ...
        # ========================================
        # 5. [v8.1 신규] 반등 초기 보너스 (25점)
        # ========================================
        bb_position = current['bb_position']
        is_recovery_start = False
        
        if V81_RECOVERY_BONUS_ENABLED:
            # 반등 초기 조건:
            # - RSI 상승 중
            # - RSI가 아직 낮음 (< 45)
            # - 현재 양봉
            ...
            is_recovery_start = (
                rsi_rising_1 and
                rsi_now < V81_RECOVERY_RSI_MAX and
                is_bullish and
                bb_position < V81_RECOVERY_BB_MAX
            )
            
            if is_recovery_start:
                score += V81_RECOVERY_BONUS_SCORE
                details.append(f"반등초기보너스 RSI{rsi_now:.0f}↑ BB{bb_position:.0f}% (+{V81_RECOVERY_BONUS_SCORE})")
        ...
        return {
            ...
            'is_recovery_start': is_recovery_start,
            'bb_position': bb_position
        }
...
def check_daily_safety_filter(ticker):
    ...
    체크 항목:
    1. 당일 등락률 +3% 이상 급등 → 매수 금지
    2. 당일 등락률 -3% 이상 급락 + 반등 미확인 → 매수 금지
    3. 3일 연속 음봉 → 매수 금지
    ...
    Returns:
        dict: {
            'safe': 안전 여부 (True/False),
            'daily_change': 당일 등락률,
            'reason': 판단 사유,
            'is_surge': 급등 여부,
            'is_crash': 급락 여부,
            'consecutive_bear': 연속 음봉 수
        }
    """
    try:
        # 일봉 데이터 조회
        ...
        # 체크 1: 급등 체크 (+3% 이상)
        # ========================================
        is_surge = daily_change >= V91_DAILY_SURGE_LIMIT
        
        if is_surge:
            return {
                'safe': False,
                'daily_change': daily_change,
                'reason': f'일봉 급등 +{daily_change:.1f}% (추격매수 금지)',
                'is_surge': True,
                'is_crash': False,
                'consecutive_bear': 0
            }
        ...
        # 체크 2: 급락 체크 (-3% 이상)
        # ========================================
        is_crash = daily_change <= V91_DAILY_CRASH_LIMIT
        
        if is_crash:
            # 급락이지만 반등 확인되면 허용 (15분봉에서 체크)
            # 여기서는 일단 경고만
            return {
                'safe': True,  # 조건부 허용 (15분봉에서 반등 확인 필요)
                'daily_change': daily_change,
                'reason': f'일봉 급락 {daily_change:.1f}% (반등 확인 필요)',
                'is_surge': False,
                'is_crash': True,
                'consecutive_bear': 0
            }
        ...
        if consecutive_bear >= V91_DAILY_CONSECUTIVE_BEAR_MAX:
            return {
                'safe': False,
                'daily_change': daily_change,
                'reason': f'일봉 {consecutive_bear}일 연속 음봉 (추세 하락)',
                'is_surge': False,
                'is_crash': False,
        ...
        return {
            'safe': True,
            'daily_change': daily_change,
            'reason': f'일봉 안전 (당일 {daily_change:+.1f}%)',
            'is_surge': False,
            'is_crash': is_crash,
            'consecutive_bear': consecutive_bear
        }
...
def detect_downtrend_15m(df_15m):
    """
    [v9.1] 15분봉 하락 추세 감지 - 폭락 회피용
    
    하락 추세 조건 (택1 충족 시 하락 추세):
    1. RSI 3연속 하락
    2. 저점 3연속 하락 (Lower Low)
    3. BB 하단 돌파 (이탈)
    ...
    Returns:
        dict: {
            'is_downtrend': 하락 추세 여부,
            'reason': 판단 사유,
            'rsi_dropping': RSI 하락 여부,
            'lower_lows': 저점 하락 여부,
            'bb_breakdown': BB 이탈 여부
        }
    """
    try:
        ...
        # ========================================
        rsi_dropping = True
        for i in range(-1, -V91_DOWNTREND_RSI_DROP_COUNT, -1):
            if len(df_15m) + i - 1 >= 0:
                current_rsi = df_15m.iloc[i]['rsi']
        ...
        if rsi_dropping:
            return {
                'is_downtrend': True,
                'reason': f'RSI {V91_DOWNTREND_RSI_DROP_COUNT}연속 하락 (하락 추세)',
                'rsi_dropping': True,
                'lower_lows': False,
                'bb_breakdown': False
            }
        ...
        # ========================================
        # 체크 2: 저점 연속 하락 (Lower Low)
        # ========================================
        lower_lows = True
        for i in range(-1, -V91_DOWNTREND_LOWER_LOW_COUNT, -1):
            if len(df_15m) + i - 1 >= 0:
                current_low = df_15m.iloc[i]['low']
                prev_low = df_15m.iloc[i-1]['low']
        ...
        if lower_lows:
            return {
                'is_downtrend': True,
                'reason': f'저점 {V91_DOWNTREND_LOWER_LOW_COUNT}연속 하락 (Lower Low)',
                'rsi_dropping': False,
                'lower_lows': True,
                'bb_breakdown': False
            }
        ...
        # BB 위치가 음수 = 하단 이탈
        bb_breakdown = bb_position <= V91_BB_BREAKDOWN_THRESHOLD
        
        if bb_breakdown:
            return {
                'is_downtrend': True,
                'reason': f'BB 하단 돌파 (BB {bb_position:.1f}%)',
                'rsi_dropping': False,
                'lower_lows': False,
                'bb_breakdown': True
            }
        ...
        if DEBUG_MODE:
            print(f"{Colors.RED}[Downtrend Detection Error] {e}{Colors.ENDC}")
        return {
            'is_downtrend': False,
            'reason': f'체크 오류: {e}',
            'rsi_dropping': False,
...
def calculate_reversal_score(df_15m):
    """
    [v9.1] 반등 확인 점수 계산 (30점 만점)
    
    점수 항목:
    1. RSI 상승 전환 (현재 > 이전): +10점
    2. 양봉 출현: +10점
    3. 거래량 증가 (평균 대비 1.2배+): +5점
    4. 저점 상승 (Higher Low): +5점
    ...
    try:
        ...
        if rsi_rising:
            score += V91_REVERSAL_RSI_RISING
            items_met += 1
            details.append(f"RSI상승 {rsi_prev:.0f}→{rsi_now:.0f} (+{V91_REVERSAL_RSI_RISING})")
        ...
        if is_bullish:
            score += V91_REVERSAL_BULLISH_CANDLE
            items_met += 1
            change_pct = ((current['close'] - current['open']) / current['open']) * 100
            details.append(f"양봉 +{change_pct:.2f}% (+{V91_REVERSAL_BULLISH_CANDLE})")
        ...
        if volume_up:
            score += V91_REVERSAL_VOLUME_INCREASE
            items_met += 1
            details.append(f"거래량 {vol_ratio:.1f}x (+{V91_REVERSAL_VOLUME_INCREASE})")
        
        # ========================================
        # 항목 4: 저점 상승 - Higher Low (+5점)
        # ========================================
        low_now = current['low']
        low_prev = prev['low']
        low_prev2 = prev2['low']
        higher_low = low_now > min(low_prev, low_prev2)
        
        if higher_low:
            score += V91_REVERSAL_HIGHER_LOW
            items_met += 1
            details.append(f"저점상승 HL (+{V91_REVERSAL_HIGHER_LOW})")
        ...
        if DEBUG_MODE:
            print(f"{Colors.RED}[Reversal Score Error] {e}{Colors.ENDC}")
        return {
            'score': 0,
            'items_met': 0,
            'details': [f'오류: {e}'],
...
def detect_decline_signals(df_15m, df_5m=None, held_info=None):
    ...
    Args:
        ...
        df_5m: 5분봉 DataFrame (선택)
        held_info: 보유 정보 dict
    
    Returns:
        dict: {
            'score': 총점,
            'should_exit': 익절 권장 여부 (4점 이상),
            'details': 상세 내역 리스트
        }
    """
    try:
        ...
        if price_higher and rsi_lower:
            score += V91_DECLINE_RSI_DIVERGENCE
            details.append(f"RSI다이버전스 (+{V91_DECLINE_RSI_DIVERGENCE})")
        ...
        if vol_ratio < 0.7 and is_bearish:
            score += V91_DECLINE_VOLUME_DROP
            details.append(f"거래량급감+음봉 {vol_ratio:.1f}x (+{V91_DECLINE_VOLUME_DROP})")
        
        # ========================================
        # 지표 3: 5분봉 연속 음봉 4개+ (+2점)
        # ========================================
        if df_5m is not None and len(df_5m) >= 5:
            ...
            if bearish_5m_count >= 4:
                score += V91_DECLINE_CONSECUTIVE_BEAR_5M
                details.append(f"5분봉{bearish_5m_count}연속음봉 (+{V91_DECLINE_CONSECUTIVE_BEAR_5M})")
        
        # ========================================
        # 지표 4: BB 상단 이탈 후 복귀 (+2점)
        # ========================================
        if held_info:
            ...
            if peak_bb >= 95 and current_bb < 85:
                score += V91_DECLINE_BB_REJECTION
                details.append(f"BB상단이탈복귀 {peak_bb:.0f}%→{current_bb:.0f}% (+{V91_DECLINE_BB_REJECTION})")
        
        # ========================================
        # 지표 5: 고점 대비 -1.5% 이상 하락 (+1점)
        # ========================================
        if held_info:
            ...
            if peak_price > 0:
                ...
                if drawdown >= 1.5:
                    score += V91_DECLINE_DRAWDOWN
                    details.append(f"고점대비-{drawdown:.1f}% (+{V91_DECLINE_DRAWDOWN})")
        
        # ========================================
        # 판정
        # ========================================
        should_exit = score >= V91_DECLINE_THRESHOLD
        
        return {
            'score': score,
            'should_exit': should_exit,
            'details': details,
            'threshold': V91_DECLINE_THRESHOLD
        }
...
def calculate_momentum_for_report(ticker):
    """
    [v8.0] 보고서용 모멘텀 강도 계산
    
    Discord 정시 보고서에 표시할 모멘텀 정보 생성
    ...
    Returns:
        dict: {
            ...
            'emoji': '🟢'/'🟡'/'🔴',
            'score': 모멘텀 점수 (0-100),
            'reasons': 판단 사유 리스트 (최대 3개),
            'reason_str': 판단 사유 문자열
        }
    """
    try:
        # 15분봉 데이터 수집
        ...
        # 모멘텀 강도 계산 (기존 함수 활용)
        momentum = calculate_momentum_strength(df_15m)
        ...
        for detail in details[:3]:
            # "+점수" 부분 제거하고 핵심만 추출
            reason = detail.split('+')[0].strip()
            # 길이 제한
            if len(reason) > 10:
                reason = reason[:10]
            short_reasons.append(reason)
        
        reason_str = '+'.join(short_reasons) if short_reasons else '신호없음'
        ...
        if DEBUG_MODE:
            print(f"{Colors.RED}[Momentum Report Error] {ticker}: {e}{Colors.ENDC}")
        return {
            'grade': '?',
            'emoji': '⚪',
            'score': 0,
...
def detect_entry_trigger(df_15m, df_5m=None):
    """
    [v8.1] 최적 매수 타이밍 트리거 감지 + CRASH_RECOVERY 추가
    ...
    4. 연속 양봉: 3개 이상 연속 양봉
    5. [v8.1 신규] CRASH_RECOVERY: 급락 후 반등 감지
    ...
    try:
        ...
        # ========================================
        # [v8.1 신규] 트리거 0: CRASH_RECOVERY (최우선 확인)
        # 급락 후 반등 감지 - 가장 수익성 높은 패턴
        # ========================================
        if len(df_15m) >= V81_CRASH_LOOKBACK_CANDLES:
            recent_candles = df_15m.tail(V81_CRASH_LOOKBACK_CANDLES)
            high_point = recent_candles['high'].max()
            low_point = recent_candles['low'].min()
            
            if high_point > 0:
                crash_depth = (low_point - high_point) / high_point * 100
                
                # 급락 발생 여부
                had_crash = crash_depth <= V81_CRASH_DEPTH_THRESHOLD
                
                # 현재 BB 위치
                bb_position = current['bb_position']
                bb_in_range = V81_CRASH_RECOVERY_BB_MIN <= bb_position <= V81_CRASH_RECOVERY_BB_MAX
                
                # RSI 상승 중
                rsi_rising = current['rsi'] > prev['rsi']
                
                # 저점 상승 (higher low)
                higher_low = current['low'] > min(prev['low'], prev2['low'])
                
                if had_crash and is_current_bullish and rsi_rising and bb_in_range and higher_low:
                    triggers.append({
                        'type': 'CRASH_RECOVERY',
                        'strength': V81_CRASH_RECOVERY_STRENGTH,
                        'reason': f'급락{crash_depth:.1f}%후반등 RSI↑ 저점상승 BB{bb_position:.0f}%'
                    })
        ...
        if V80_TRIGGER_PULLBACK_CONFIRM:
            ...
            bounce_strong = current['close'] > prev['high']
...
def evolution_80_buy_signal(df_15m, df_5m, ticker):
    """
    [v9.1] VOLATILITY HUNTER - BB 폭 기반 최적 매수 신호
    
    [v9.1 핵심 변경사항]
    1. BB 폭이 핵심 (하단 터치보다 중요)
    2. 3중 폭락 회피 시스템
    3. 반등 확인 점수제
    4. 일봉 급등(+3%)/급락(-3%) 필터
    
    매수 조건:
    - 1단계: 일봉 안전 필터 (급등/급락/연속음봉 체크)
    - 2단계: 15분봉 하락 추세 감지 (폭락 회피)
    - 3단계: BB 폭 기반 진입 조건
    - 4단계: 반등 확인 필수
    ...
    try:
        ...
        # ========================================
        # Step 1: 일봉 안전 필터 (3중 체크)
        # ========================================
        daily_safety = check_daily_safety_filter(ticker)
        base_response['daily_change'] = daily_safety['daily_change']
        
        if not daily_safety['safe']:
            base_response['reason'] = daily_safety['reason']
            base_response['market_condition'] = 'DAILY_UNSAFE'
            return base_response
        
        # ========================================
        # Step 2: 15분봉 하락 추세 감지 (폭락 회피)
        # ========================================
        downtrend = detect_downtrend_15m(df_15m)
        
        if downtrend['is_downtrend']:
            base_response['reason'] = f"하락추세감지: {downtrend['reason']}"
            base_response['market_condition'] = 'DOWNTREND'
            return base_response
        
        # ========================================
        # Step 3: BB 폭 + 위치 조건 체크
        # ========================================
        bb_width = current['bb_width']
        bb_position = current['bb_position']
        ...
        base_response['reversal_score'] = reversal_score
        ...
        # ----------------------------------------
        # Zone 1: BB 폭 < 1.5% → 절대 금지
        # ----------------------------------------
        if bb_width < V91_BB_WIDTH_ABSOLUTE_MIN:
            base_response['reason'] = f"BB폭 부족 {bb_width:.1f}% < {V91_BB_WIDTH_ABSOLUTE_MIN}% (횡보장)"
            base_response['bb_width_zone'] = 'FORBIDDEN'
            return base_response
        
        # ----------------------------------------
        # Zone 2: BB 폭 1.5-2.0% → 조건부 허용
        # ----------------------------------------
        elif bb_width < V91_BB_WIDTH_CAUTION_MIN:
            zone_name = 'LOW_CAUTION'
            required_bb_max = V91_LOW_WIDTH_BB_POSITION_MAX  # 10%
            required_reversal = V91_LOW_WIDTH_REVERSAL_MIN   # 28점
            
            if bb_position <= required_bb_max and reversal_score >= required_reversal:
                buy_allowed = True
            else:
                base_response['reason'] = f"BB폭 {bb_width:.1f}% 조건부구간: BB위치 {bb_position:.0f}%>{required_bb_max}% 또는 반등 {reversal_score}<{required_reversal}점"
                base_response['bb_width_zone'] = zone_name
                return base_response
        
        # ----------------------------------------
        # Zone 3: BB 폭 2.0-3.0% → 신중 매수
        # ----------------------------------------
        elif bb_width < V91_BB_WIDTH_OPTIMAL_MIN:
            zone_name = 'LOW'
            required_bb_max = V91_WIDTH_LOW_BB_POSITION_MAX  # 15%
            required_reversal = V91_WIDTH_LOW_REVERSAL_MIN   # 25점
            
            if bb_position <= required_bb_max and reversal_score >= required_reversal:
                buy_allowed = True
            else:
                base_response['reason'] = f"신중매수 구간: BB {bb_position:.0f}%>{required_bb_max}% 또는 반등 {reversal_score}<{required_reversal}점"
                base_response['bb_width_zone'] = zone_name
                return base_response
        
        # ----------------------------------------
        # Zone 4: BB 폭 3.0-5.0% → 적극 매수 (최적)
        # ----------------------------------------
        elif bb_width <= V91_BB_WIDTH_OPTIMAL_MAX:
            zone_name = 'OPTIMAL'
            required_bb_max = V91_WIDTH_MID_BB_POSITION_MAX  # 25%
            required_reversal = V91_WIDTH_MID_REVERSAL_MIN   # 20점
            
            if bb_position <= required_bb_max and reversal_score >= required_reversal:
                buy_allowed = True
            else:
                base_response['reason'] = f"최적구간 조건미달: BB {bb_position:.0f}%>{required_bb_max}% 또는 반등 {reversal_score}<{required_reversal}점"
                base_response['bb_width_zone'] = zone_name
                return base_response
        
        # ----------------------------------------
        # Zone 5: BB 폭 5.0%+ → 조건부 매수 (고변동성)
        # ----------------------------------------
        else:
            zone_name = 'HIGH_VOLATILITY'
            required_bb_max = V91_WIDTH_HIGH_BB_POSITION_MAX  # 20%
            required_reversal = V91_WIDTH_HIGH_REVERSAL_MIN   # 28점
            
            # 추가 조건: 저점 상승 필수
            if V91_WIDTH_HIGH_HIGHER_LOW_REQUIRED and not reversal['higher_low']:
                base_response['reason'] = f"고변동성 구간: 저점상승(HL) 필수 미충족"
                base_response['bb_width_zone'] = zone_name
                return base_response
            ...
            if bb_position <= required_bb_max and reversal_score >= required_reversal:
                ...
                base_response['reason'] = f"고변동성 구간: BB {bb_position:.0f}%>{required_bb_max}% 또는 반등 {reversal_score}<{required_reversal}점"
                base_response['bb_width_zone'] = zone_name
                return base_response
        
        # ========================================
        # Step 4: 반등 확인 최소 항목 체크
        # ========================================
        if reversal['items_met'] < V91_REVERSAL_MIN_ITEMS:
            base_response['reason'] = f"반등신호 부족: {reversal['items_met']}/{V91_REVERSAL_MIN_ITEMS}개 항목 충족"
            return base_response
        ...
        if buy_allowed:
            # 신뢰도 계산
            confidence = min(100, 50 + reversal_score + int(bb_width * 5))
            
            # 목표 수익 결정 (BB 폭 기반)
            if zone_name in ['LOW_CAUTION', 'LOW']:
                target_profit = V91_TARGET_WIDTH_LOW
            elif zone_name == 'OPTIMAL':
                target_profit = V91_TARGET_WIDTH_MID
            else:
                target_profit = V91_TARGET_WIDTH_HIGH
            
            # 사유 조합
            reversal_detail = '+'.join(reversal['details'][:2]) if reversal['details'] else ''
            
            return {
                'signal': True,
                'reason': f"BB폭{bb_width:.1f}%[{zone_name}] | BB{bb_position:.0f}% | 반등{reversal_score}점({reversal_detail})",
                ...
                'daily_change': daily_safety['daily_change'],
                'reversal_score': reversal_score,
                'bb_width_zone': zone_name,
                'target_profit': target_profit,
                'reversal_details': reversal['details']
            }
        
        # 기본 반환 (도달하지 않아야 함)
        base_response['reason'] = '조건 미충족'
        return base_response
        
    except Exception as e:
        if DEBUG_MODE:
            print(f"{Colors.RED}[v9.1 Buy Signal Error] {e}{Colors.ENDC}")
            traceback.print_exc()
...
def evolution_80_sell_signal(df, buy_price, buy_time=None, held_info=None):
    """
    [v9.1] VOLATILITY HUNTER - 최적 매도 신호
    
    [v9.1 핵심 변경사항]
    1. 선행 하락 감지 (4점 이상 시 익절)
    2. BB 폭 기반 동적 트레일링 스탑
    3. BB 폭별 목표 수익 차등 적용
    4. 상승 중 홀드 원칙 강화
    
    매도 트리거:
    1. 손절: -2.5%
    2. 선행 하락 감지: 4점 이상 + 수익 1%+ → 익절
    3. 목표 수익 도달 + 트레일링 스탑
    4. 극과매수 익절: BB 95%+ & 음봉 & 수익 1.5%+
    ...
    try:
        ...
        # Step 1: 손절 체크 (무조건 실행)
        # ========================================
        if profit_pct <= V91_STOP_LOSS_PCT:
            return {
                **base_response,
                'signal': True,
                'reason': f'STOP_LOSS ({profit_pct:.2f}% <= {V91_STOP_LOSS_PCT}%)'
            }
        ...
        # ========================================
        # 진입 시 BB 폭 기반 목표 수익 결정
        entry_bb_width = held_info.get('entry_bb_width', bb_width) if held_info else bb_width
        
        if entry_bb_width < V91_BB_WIDTH_OPTIMAL_MIN:
            target_profit = V91_TARGET_WIDTH_LOW  # 1.5%
            min_profit = V91_MIN_PROFIT_WIDTH_LOW  # 1.0%
        elif entry_bb_width <= V91_BB_WIDTH_OPTIMAL_MAX:
            target_profit = V91_TARGET_WIDTH_MID  # 2.5%
            min_profit = V91_MIN_PROFIT_WIDTH_MID  # 1.5%
        else:
            target_profit = V91_TARGET_WIDTH_HIGH  # 2.0%
            min_profit = V91_MIN_PROFIT_WIDTH_HIGH  # 1.2%
        
        # 트레일링 활성화 조건: 목표의 70% 도달
        trailing_activation = target_profit * V91_TRAILING_ACTIVATION_RATIO
        
        if profit_pct >= trailing_activation and held_info:
            peak_price = held_info.get('peak_price', buy_price)
            
            if peak_price > 0:
                # 동적 트레일링 거리 계산
                trailing_distance = max(V91_TRAILING_MIN_DISTANCE, bb_width * V91_TRAILING_BB_MULTIPLIER)
        ...
        if DEBUG_MODE:
            print(f"{Colors.RED}[v9.1 Sell Signal Error] {e}{Colors.ENDC}")
            traceback.print_exc()
