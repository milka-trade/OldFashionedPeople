...
def filtered_tickers(tickers):
    ...
    # 🎯 핵심 개선: 최종 선정 리스트를 초기화하여 UnboundLocalError 방지
    filtered_tickers = []
    # 선정된 코인들의 상세 정보를 저장하는 리스트 초기화
    selected_details = []
    ...
    # 병렬 처리로 시장 데이터 수집 속도 개선
    def get_rsi_single(ticker):
        try:
            rsi_data = get_rsi(ticker, 14, interval=min5)
            if not rsi_data.empty:
                return rsi_data.values[-1]
        except Exception as e:
            print(f"❌ 시장 데이터 오류: {ticker} - {e}")
            return None
    
    with ThreadPoolExecutor(max_workers=5) as executor:
        results = list(executor.map(get_rsi_single, market_tickers))
    
    market_rsi_data = [r for r in results if r is not None]
    ...
    for t in tickers:
        try:
            ...
            if market_rsi < 35:
                adjusted_rsi_s, adjusted_rsi_e = base_rsi_s * 0.9, base_rsi_e * 1.1
            elif market_rsi > 65:
                adjusted_rsi_s, adjusted_rsi_e = base_rsi_s * 1.05, base_rsi_e * 0.95
            else:
                adjusted_rsi_s, adjusted_rsi_e = base_rsi_s * 0.95, base_rsi_e * 1.05
                
            is_rsi_good = adjusted_rsi_s < current_rsi < adjusted_rsi_e
            is_rsi_perfect = adjusted_rsi_s * 1.05 < current_rsi < adjusted_rsi_e * 0.95
            ...
            vol_ma3, vol_ma10, vol_ma20 = volumes[-3:].mean(), volumes[-10:].mean(), volumes[-20:].mean()
            ...
            price_sma5, price_sma10, price_sma20 = closes[-5:].mean(), closes[-10:].mean(), closes[-20:].mean()
            ...
            if has_divergence: decent_core_score += 50
            ...
            buy_decision, buy_tier, buy_reason, final_score = False, "", "", 0
            
            # 🏆 1순위: 완벽한 기회 검증
            if perfect_final_score >= perfect_threshold:
                if sum([is_band_expanding and is_perfect_band_position,
                        is_rsi_perfect or (is_rsi_good and rsi_momentum_strong),
                        is_perfect_position,
                        prediction in ['SURGE', 'UP']]) >= 3:
                    buy_decision, buy_tier, buy_reason, final_score = True, "🏆 PERFECT", "완벽한 조건 충족", perfect_final_score
            
            # 🥇 2순위: 우수한 기회 검증
            if not buy_decision and good_final_score >= good_threshold:
                if sum([is_band_expanding, is_rsi_good, is_good_position or is_near_lower_band,
                        has_divergence, prediction != 'CRASH', is_volume_good or trend_good]) >= 4:
                    buy_decision, buy_tier, buy_reason, final_score = True, "🥇 GOOD", "우수한 조건 충족", good_final_score
            
            # 🥈 3순위: 괜찮은 기회 검증
            if not buy_decision and decent_final_score >= decent_threshold and buy_aggression >= 0.6:
                if sum([is_band_expanding or is_near_middle_band, is_rsi_good or rsi_momentum,
                        is_good_position or bb_position < 0.5, prediction not in ['DOWN', 'CRASH'],
                        is_momentum_ok, trend_ok]) >= 4:
                    buy_decision, buy_tier, buy_reason, final_score = True, "🥈 DECENT", "괜찮은 조건 충족", decent_final_score

            # 🔥 보너스: 특수 상황
            if (not buy_decision and market_rsi < 25 and has_divergence and 
                prediction != 'CRASH' and bounce_good):
                buy_decision, buy_tier, buy_reason, final_score = True, "🔥 SPECIAL", "극과매도+다이버전스 특수조건", max(perfect_final_score, good_final_score, decent_final_score)
            ...
            if buy_decision:
                # 🎯 핵심 개선: 매수 결정 시, 즉시 리스트에 추가
                filtered_tickers.append(t)
                selected_details.append(candidate_info)
                
                filtering_message = f"<<[{filteringTime}] {t}>>\n[등급] {buy_tier} | {buy_reason}\n"
    ...
    if total_selected == 0 and total_analyzed >= 10:
        ...
        for candidate in all_candidates[:3]:
            ...
            if (max_score >= emergency_threshold and 
                ...
                candidate['ticker'] not in filtered_tickers): # 이미 추가되지 않았는지 확인
                
                emergency_message = f"🚨 **긴급 기회 포착**: {candidate['ticker']}\n"
                ...
                print(f"🚨 긴급 기회 확보: {candidate['ticker']} (점수: {max_score:.0f})")
                print(emergency_message)
                print("-" * 80)
                
                filtered_tickers.append(candidate['ticker'])
                selected_details.append(candidate) # 상세 정보 추가
                if len(filtered_tickers) >= 1: # 최대 1개만 긴급 확보
                    break
    ...
    for detail in selected_details:
        tier_key = detail['buy_tier'].split()[1] if ' ' in detail['buy_tier'] else 'SPECIAL'
    ...
    if total_selected > 0:
        ...
        for detail in selected_details:
            discord_msg += f"**{detail['ticker']}** {detail['buy_tier']}\n"
            discord_msg += f"├ 점수: {detail['final_score']:.0f}점 | 예측: {detail['prediction']}\n"
            discord_msg += f"├ RSI: {detail['current_rsi']:.1f} | 위치: {detail['bb_position']:.1%}\n"
