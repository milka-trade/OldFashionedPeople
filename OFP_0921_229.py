...
def filtered_tickers(tickers):
    """개선된 조건에 맞는 티커 필터링 - 기회 포착 최적화 + 유연한 매수 전략"""
    filtered_tickers = []
    ...
    # 🎯 개선된 시장 상태 분석 - 기회 포착 강화
    if market_rsi < 25:
        market_status = "극과매도(공격매수)"
        strategy_note = "패닉셀링-적극진입"
        buy_aggression = 0.95  # 더 공격적
        opportunity_multiplier = 1.8  # 기회 확대
    elif market_rsi < 35:
        market_status = "과매도(기회매수)"  
        strategy_note = "하락끝-기회포착"
        buy_aggression = 0.8  # 공격적
        opportunity_multiplier = 1.5
    elif market_rsi < 45:
        market_status = "약과매도(신중매수)"
        strategy_note = "조정구간-선별매수"
        buy_aggression = 0.6  # 보통
        opportunity_multiplier = 1.2
    elif market_rsi > 75:
        market_status = "극과매수(초단타만)"
        strategy_note = "과열-단타기회만"
        buy_aggression = 0.3  # 보수적
        opportunity_multiplier = 0.8
    elif market_rsi > 65:
        market_status = "과매수(되돌림매수)"
        strategy_note = "상승중되돌림-빠른익절"
        buy_aggression = 0.5  # 보수적
        opportunity_multiplier = 0.9
    elif market_rsi > 55:
        market_status = "강세중립(트렌드매수)"
        strategy_note = "상승추세-트렌드따라"
        buy_aggression = 0.7  # 약간 공격적
        opportunity_multiplier = 1.1
    else:
        market_status = "중립(균형매수)"
        strategy_note = "안정구간-표준조건"
        buy_aggression = 0.6  # 표준
        opportunity_multiplier = 1.0
    
    # 변동성에 따른 조정
    if market_volatility > 15:  # 고변동성 시장
        buy_aggression *= 0.9  # 약간 보수적
        volatility_note = "고변동성"
        opportunity_multiplier *= 1.1  # 변동성 = 기회
    elif market_volatility < 5:  # 저변동성 시장
        buy_aggression *= 1.1  # 더 공격적
        volatility_note = "저변동성"
        opportunity_multiplier *= 0.9
    else:
        volatility_note = "정상변동성"
    
    print(f"🌍 시장심리: RSI {market_rsi:.1f} ({market_status}) | {strategy_note}")
    print(f"📊 변동성: {market_volatility:.1f} ({volatility_note}) | 매수공격도: {buy_aggression:.1f} | 기회배율: {opportunity_multiplier:.1f}")
    ...
    for t in tickers:
        try:
            ...
            # ✨ 시장 상황별 볼린저 밴드 최적화 ✨
            if market_rsi < 30:
                bb_window, bb_std = 12, 1.6  # 더 민감하게
            elif market_rsi > 70:
                bb_window, bb_std = 30, 2.2  # 더 보수적으로
            else:
                bb_window, bb_std = 20, 2.0  # 표준 설정
            ...
            # 1. 🔧 적응형 밴드 확장 조건 (기존 문제점 해결)
            base_margin = optimized_band_margin * (1.5 - buy_aggression * 0.3)  # 공격적일수록 완화
            is_band_expanding = band_diff_ratio > base_margin
            # 🆕 대안 조건: 밴드 확장이 아니어도 중간밴드 근처면 OK
            is_near_middle_band = abs(current_price - middle_band) / middle_band < 0.02
            band_condition = is_band_expanding or (is_near_middle_band and buy_aggression > 0.6)
            
            # 2. 🔧 적응형 RSI 범위 (더 유연하게)
            base_rsi_s, base_rsi_e = rsi_buy_s, rsi_buy_e
            if market_rsi < 35:  # 과매도 시장
                adjusted_rsi_s = base_rsi_s * 0.85  # 더 완화
                adjusted_rsi_e = base_rsi_e * 1.15  # 더 확대
            elif market_rsi > 65:  # 과매수 시장
                adjusted_rsi_s = base_rsi_s * 1.05  # 약간 강화
                adjusted_rsi_e = base_rsi_e * 0.95  # 약간 축소
            else:
                adjusted_rsi_s = base_rsi_s * 0.95  # 기본값도 약간 완화
                adjusted_rsi_e = base_rsi_e * 1.05
            
            is_rsi_good = adjusted_rsi_s < current_rsi < adjusted_rsi_e
            # 🆕 RSI 대안 조건: 상승 모멘텀 있으면 범위 밖이어도 고려
            rsi_momentum = len(rsi_values) >= 3 and rsi_values[-1] > rsi_values[-2] >= rsi_values[-3]
            rsi_condition = is_rsi_good or (rsi_momentum and 25 < current_rsi < 75)
            ...
            base_position_threshold = 0.4 + (0.2 * buy_aggression)  # 더 유연한 기준
            is_good_position = bb_position < base_position_threshold
            # 🆕 위치 대안: 하단 밴드 근처면 위치 무관하게 OK
            is_near_lower_band = bb_position < 0.2
            position_condition = is_good_position or is_near_lower_band
            
            # ✨ 향상된 보조 지표들 ✨
            
            # 5분간 가격 변화율 (더 관대하게)
            price_change_5min = (current_price - closes[-2]) / closes[-2]
            is_short_momentum = price_change_5min > -0.01  # -1%까지 허용
            
            # 최근 캔들 패턴 (더 유연하게)
            recent_3_candles = closes[-3:]
            is_rising_pattern = recent_3_candles[-1] >= recent_3_candles[-3] * 0.995  # 단순 비교
            
            # 거래량 분석 강화
            vol_ma3 = volumes[-3:].mean()
            vol_ma10 = volumes[-10:].mean()
            vol_ma20 = volumes[-20:].mean()
            is_volume_spike = vol_ma3 > vol_ma10 * (1.05 + buy_aggression * 0.1)
            is_volume_trend_up = vol_ma10 > vol_ma20 * 1.02
            ...
            short_trend_ok = current_price > price_sma5 * (0.98 - buy_aggression * 0.01)
            mid_trend_ok = price_sma5 > price_sma10 * (0.98 - buy_aggression * 0.01)
            long_trend_ok = price_sma10 > price_sma20 * 0.99
            
            # 변동성 및 바운스 시그널
            recent_range = (highs[-5:].max() - lows[-5:].min()) / closes[-5:].mean()
            good_volatility = 0.01 < recent_range < (0.15 + buy_aggression * 0.05)
            
            recent_low = lows[-10:].min()
            bounce_signal = current_price > recent_low * (1.005 - buy_aggression * 0.002)
            
            # 🎯 새로운 가중치 기반 점수 시스템
            
            # === 핵심 조건 점수 (0~100) ===
            core_score = 0
            if band_condition: core_score += 25
            if rsi_condition: core_score += 25  
            if position_condition: core_score += 20
            if has_divergence: core_score += 30  # 다이버전스는 높은 가중치
            
            # === 보조 조건 점수 (0~100) ===
            support_score = 0
            if is_short_momentum: support_score += 15
            if is_rising_pattern: support_score += 10
            if is_volume_spike: support_score += 15
            if is_volume_trend_up: support_score += 10
            if short_trend_ok: support_score += 15
            if mid_trend_ok: support_score += 10
            if long_trend_ok: support_score += 5
            if good_volatility: support_score += 10
            if bounce_signal: support_score += 10
            ...
            # === 예측 보정 점수 ===
            prediction_bonus = 0
            if prediction == 'SURGE': prediction_bonus = 30
            elif prediction == 'UP': prediction_bonus = 15
            elif prediction == 'NEUTRAL': prediction_bonus = 5
            elif prediction == 'DOWN': prediction_bonus = -10
            elif prediction == 'CRASH': prediction_bonus = -25
            
            # 🎯 최종 점수 계산
            total_score = core_score + support_score + prediction_bonus
            
            # 기회 배율 적용
            adjusted_score = total_score * opportunity_multiplier
            
            # 🚀 개선된 5단계 매수 결정 로직
            filteringTime = datetime.now().strftime('%m/%d %H시%M분%S초')
            filtering_message = f"<<[{filteringTime}] {t}>>\n"
            filtering_message += f"[점수] 핵심: {core_score}/100 | 보조: {support_score}/100 | 예측: {prediction_bonus} | 총점: {adjusted_score:.1f}\n"
            filtering_message += f"[조건] 밴드: {band_condition} | RSI: {rsi_condition}({current_rsi:.1f}) | 위치: {position_condition}({bb_position:.1%})\n"
            filtering_message += f"[예측] {prediction}({prediction_score}) | 공격도: {buy_aggression:.1f} | 기회배율: {opportunity_multiplier:.1f}\n"
            
            buy_decision = False
            buy_reason = ""
            
            # 🏆 레벨 1: 완벽한 기회 (총점 120+ 또는 핵심 90+ & 상승예측)
            if adjusted_score >= 120 or (core_score >= 90 and prediction in ['SURGE', 'UP']):
                buy_decision = True
                buy_reason = "🏆 **완벽한 기회** (최고점수/완벽조건)"
                
            # 🎯 레벨 2: 우수한 기회 (총점 100+ 또는 핵심 70+ & 급상승예측)
            elif adjusted_score >= 100 or (core_score >= 70 and prediction == 'SURGE'):
                buy_decision = True
                buy_reason = "🎯 **우수한 기회** (높은점수/급상승예측)"
                
            # 🚀 레벨 3: 좋은 기회 (총점 85+ 또는 핵심 60+ & 상승예측 & 고공격도)
            elif adjusted_score >= 85 or (core_score >= 60 and prediction == 'UP' and buy_aggression >= 0.7):
                buy_decision = True
                buy_reason = "🚀 **좋은 기회** (양호점수/상승예측/공격시장)"
                
            # 📈 레벨 4: 괜찮은 기회 (총점 75+ 또는 특수조건)
            elif adjusted_score >= 75:
                buy_decision = True
                buy_reason = "📈 **괜찮은 기회** (기준점수 달성)"
                
            # 🔥 레벨 5: 기회 포착 (극공격 시장에서 완화 조건)
            elif buy_aggression >= 0.8:  # 극공격 시장
                # 완화된 조건: 핵심 50+ & 보조 30+ & 비하락예측
                if core_score >= 50 and support_score >= 30 and prediction not in ['DOWN', 'CRASH']:
                    buy_decision = True
                    buy_reason = "🔥 **극공격시장 기회포착** (완화조건/비하락예측)"
                # 또는 다이버전스 + 바운스 시그널
                elif has_divergence and bounce_signal and prediction != 'CRASH':
                    buy_decision = True
                    buy_reason = "🔥 **다이버전스+바운스** (극공격시장 특수조건)"
                    
            # 🌪️ 레벨 6: 패닉 매수 (극과매도 시장 특별 조건)
            elif market_rsi < 25 and buy_aggression >= 0.9:
                # 최소 조건: RSI 적정 + 바운스 + 비폭락예측
                if current_rsi < 65 and bounce_signal and prediction != 'CRASH':
                    buy_decision = True
                    buy_reason = "🌪️ **패닉 매수** (극과매도시장/최소조건)"
            
            # 매수 결정시에만 디스코드 메시지 발송 및 리스트 추가
            if buy_decision:
                final_message = filtering_message + buy_reason
                send_discord_message(final_message)
                filtered_tickers.append(t)
                print(f"✅ 매수 선정: {t} - {buy_reason}")
            else:
                # 점수가 높지만 매수하지 않은 경우 로그
                if adjusted_score >= 60:
                    print(f"⚠️  아쉬운 기회: {t} (점수: {adjusted_score:.1f}) - 조건 미달")
    ...
    summary_msg = f"📊 **분석 완료**: {total_analyzed}개 분석 → {total_selected}개 선택 ({selection_rate:.1f}%)"
    ...
    summary_msg += f"\n⚡ **시장 상태**: {market_status} (공격도: {buy_aggression:.1f}, 기회배율: {opportunity_multiplier:.1f})"
    
    if total_selected > 0:
        send_discord_message(summary_msg)
        print(f"🎉 매수 대상 발견: {summary_msg}")
    else:
        print(f"🔍 매수 기회 없음: {summary_msg}")
        # 기회가 없을 때 시장 상황 안내
        if buy_aggression < 0.5:
            print("💡 시장이 과열상태입니다. 조정 후 재진입을 권장합니다.")
        else:
            print("💡 현재 조건에 맞는 기회가 없습니다. 잠시 후 다시 확인하세요.")
        ...
        for ticker in batch:
            try:
                ...
                if today_volatility > 0.10:  # 고변동성: 하락 여유는 주되 상승은 제한
                    price_range = (0.85, 1.049)  # -15% ~ +4.9%
                elif today_volatility < 0.03:  # 저변동성: 안정적 범위
                    price_range = (0.98, 1.045)  # -2% ~ +4.5%
                else:  # 일반 변동성: 균형잡힌 범위
                    price_range = (0.92, 1.048)  # -8% ~ +4.8%
