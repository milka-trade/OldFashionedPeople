...
def trade_buy(ticker=None):
    ...
    if ticker is None:
        ...
        for t in candidates:
            ...
            # [필터 4] BB 전환 시그널 필수
                ...
                has_bb_signal = (
                    (slope and (slope['is_flattening'] or slope['slope_change'] > 0.2)) or
                    (reversal and reversal['is_reversing'])
                )
    ...
    slope = ind.get('bb_slope')
    if slope:
        status = "✅ 완만화" if slope['is_flattening'] else "진행중"
        print(f"  └─ BB기울기: 이전({slope['prev_slope']:.2f}) → 최근({slope['recent_slope']:.2f}) "
              f"변화({slope['slope_change']:.2f}) [{status}]")
    
    reversal = ind.get('price_reversal')
    if reversal:
        status = "✅ 반등중" if reversal['is_reversing'] else "대기"
        print(f"  └─ 가격반등: 모멘텀({reversal['price_momentum']*100:.2f}%) "
              f"거래량({reversal['volume_surge']:.1f}x) [{status}]")
    ...
    if alignment:
        pos = alignment['tf_positions']
        status = "✅ 정렬" if alignment['is_aligned'] else "미정렬"
        print(f"  └─ BB정렬: 5m({pos['5m']*100:.0f}%) 15m({pos['15m']*100:.0f}%) "
              f"30m({pos['30m']*100:.0f}%) [{status}]")
    ...
    print(f"\n  └─ 안전 점수: {passed_count}/5")
    
    # # ===== STEP 5: 최종 매수 조건 검증 (혁신적 가중치 시스템) =====
    # print("\n[STEP 5] 최종 매수 조건 검증")
    
    # conditions = {
    #     '신호 강도': selected_score >= 65,
    #     '안전 검증': passed_count >= 4,
    #     '일봉 필터': ind['daily_change_from_open'] <= 0.5,
    #     'RSI 범위': 10 < ind['rsi_5m'] < 40,
    #     'BB 위치': ind['bb_5m_pos'] < 0.25,
    #     'BB 전환': (slope and slope['is_flattening']) or (reversal and reversal['is_reversing']),
    #     '반등 가능성': rebound and rebound['probability'] >= 0.65
    # }
    ...
    # ===== STEP 5: 최종 매수 조건 검증 (혁신적 가중치 시스템) =====
    print("\n[STEP 5] 최종 매수 조건 검증 (동적 가중치)")

    # 🔥 [필수 조건] 절대 타협 불가 (3개)
    # 이 조건들은 반드시 충족해야 매수 가능
    critical_conditions = {
        '반등확률 65%+': rebound and rebound['probability'] >= 0.65,
        'BB전환 확인': (slope and slope['is_flattening']) or (reversal and reversal['is_reversing']),
        'BB위치 35% 미만': ind['bb_5m_pos'] < 0.35  # 기존 25%에서 35%로 완화
    }
    ...
    if rsi_5m < 20:
        rsi_points = 30
        rsi_detail = f"극과매도({rsi_5m:.0f})"
    elif rsi_5m < 25:
        rsi_points = 25
        rsi_detail = f"과매도({rsi_5m:.0f})"
    elif rsi_5m < 30:
        rsi_points = 20
        rsi_detail = f"과매도권({rsi_5m:.0f})"
    elif rsi_5m < 35:
        rsi_points = 15
        rsi_detail = f"약과매도({rsi_5m:.0f})"
    elif rsi_5m < 40:
        rsi_points = 10
        rsi_detail = f"중립하단({rsi_5m:.0f})"
    else:
        rsi_points = 0
        rsi_detail = f"보통({rsi_5m:.0f})"

    optional_score += rsi_points
    print(f"  └─ RSI: {rsi_points}/30점 [{rsi_detail}]")
    ...
    optional_score += day_points
    print(f"  └─ 일봉안정성: {day_points}/25점 [{day_detail} {intraday:+.1f}%]")

    # [3] 안전 검증 (25점)
    safety_checks = {
        'RSI극단': 10 < ind['rsi_5m'] < 90,
        'BB범위': -0.2 < ind['bb_5m_pos'] < 1.2,
        'EMA지지': current_price > ind['ema_26'] * 0.70,
        '급등락방지': abs(ind.get('price_reversal', {}).get('price_momentum', 0)) < 0.20,
        'BB전환': slope and (slope['is_flattening'] or slope['slope_change'] > 0.15)
    }

    safety_passed = sum(safety_checks.values())
    safety_points = int(safety_passed / 5 * 25)  # 5개 중 통과 개수에 비례
    optional_score += safety_points
    print(f"  └─ 안전검증: {safety_points}/25점 [{safety_passed}/5개 통과]")

    for check_name, passed in safety_checks.items():
        if not passed:
            print(f"      └─ ⚠️ {check_name} 미통과")

    # [4] 신호 강도 (20점)
    # 기존 신호 점수를 20점 만점으로 변환
    ...
    optional_score += signal_points
    print(f"  └─ 신호강도: {signal_points}/20점 [{signal_detail} {selected_score:.0f}점]")

    # 총점 계산
    print(f"\n【총점】 {optional_score}/100점")

    # 🔥 [매수 결정] 동적 임계값
    # 반등 확률에 따라 필요 점수 조정
    if rebound['probability'] >= 0.75:
        # 반등 확률 매우 높음 → 50점만 있어도 매수
        required_score = 50
        grade = "S급 기회"
    elif rebound['probability'] >= 0.70:
        # 반등 확률 높음 → 55점 필요
        required_score = 55
        grade = "A급 기회"
    else:
    ...
    print(f"  └─ 필요점수: {required_score}점 (반등확률 {rebound['probability']*100:.0f}% 기준)")
    print(f"  └─ 기회등급: {grade}")

    # 최종 매수 가능 여부
    can_buy = optional_score >= required_score

    if can_buy:
        print(f"\n  └─ 🟢 매수 조건 충족! ({optional_score}점 ≥ {required_score}점)")
        send_discord_message(f"🟢 {selected_ticker} 매수조건 충족 ({optional_score}점)")
    else:
        print(f"\n  └─ 🔴 점수 부족 ({optional_score}점 < {required_score}점)")
        send_discord_message(f"🔴 {selected_ticker} 점수부족 ({optional_score}/{required_score}점)")
        return "Score insufficient", None

    # 🔥 [추가 보너스] BB 극하단 특별 조건
    # BB 위치가 20% 미만이고 반등 확률 65% 이상이면 점수 무시하고 매수
    if ind['bb_5m_pos'] < 0.20 and rebound['probability'] >= 0.65:
        print("\n  └─ 🔥 특별 조건: BB 극하단 감지!")
        print(f"      └─ BB위치 {ind['bb_5m_pos']*100:.0f}% < 20%")
        print(f"      └─ 반등확률 {rebound['probability']*100:.0f}% ≥ 65%")
        print("      └─ 점수 무시하고 즉시 매수!")
        can_buy = True
        send_discord_message(f"🔥 {selected_ticker} BB극하단 특별매수!")
    ...
    for attempt in range(1, 3):
        try:
            ...
            success_msg = f"\n{'='*60}\n"
            success_msg += f"{emoji} {grade} 매수 성공! (BB 전환점 포착)\n"
            success_msg += f"{'='*60}\n"
            ...
            if slope and slope['is_flattening']:
                success_msg += f"  └─ BB완만화 (기울기 변화: {slope['slope_change']:.2f})\n"
            
            if reversal and reversal['is_reversing']:
                success_msg += f"  └─ 가격반등 (모멘텀: {reversal['price_momentum']*100:.1f}%)\n"
            
            if rebound:
                success_msg += f"  └─ 반등확률: {rebound['probability']*100:.0f}% (예상수익: {rebound['expected_gain']:.1f}%)\n"
            
            success_msg += f"\n총자산: {total_asset:,.0f}원 (암호화폐: {crypto_value/total_asset*100:.1f}%)\n"
            success_msg += f"{'='*60}"
