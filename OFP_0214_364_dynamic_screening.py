...
def buy_thread_worker():
    """
    [v14.0] BOUNCE + SURGE 듀얼 매수 스레드
    
    변경사항:
    - FIXED_STABLE_COINS → get_active_coin_list() (고정 7 + 동적 N)
    - 동적 코인 스크리닝은 DYNAMIC_SCREEN_INTERVAL 주기로 자동 갱신
    
    우선순위: BOUNCE > SURGE (기존 동일)
    """
    print(f"{Colors.CYAN}[Thread 1] v14.0 BOUNCE+SURGE 매수 스레드 시작 ({BUY_THREAD_INTERVAL}초 주기){Colors.ENDC}")
    if DYNAMIC_COIN_ENABLED:
