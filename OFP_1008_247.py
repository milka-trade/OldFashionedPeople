...
def trade_buy(ticker=None):
    ...
    def analyze_ticker_integrated(ticker_symbol):
        ...
        try:
            ...
            if None in [df_1m, df_5m, df_15m, df_30m, df_1h, df_1d, current_price]:
                return {'valid': False}
            ...
            bb_5m_l, bb_5m_m, bb_5m_u, bb_5m_pos, bb_5m_width = calculate_bb(c_5m, 20)
            bb_15m_l, bb_15m_m, bb_15m_u, bb_15m_pos, bb_15m_width = calculate_bb(c_15m, 20)
            bb_30m_l, bb_30m_m, bb_30m_u, bb_30m_pos, bb_30m_width = calculate_bb(c_30m, 20)
            bb_1h_l, bb_1h_m, bb_1h_u, bb_1h_pos, bb_1h_width = calculate_bb(c_1h, 20)
...
def trade_sell(ticker):
    ...
    핵심 개선:
    - BB 위치 기반 홀딩/매도 강화
    - 시간 종료 시에도 BB 상단일 때만 매도
    - 하단~중단에서는 추가 홀딩으로 기회비용 극대화
    
    전략:
    1. 최소수익률 기준 엄격 적용
    2. BB 상단(70% 이상) → 하락 위험 → 매도 우선
    3. BB 중단(30~70%) → 관망 → 신호 확인 후 매도
    4. BB 하단(30% 미만) → 상승 여력 → 추가 홀딩
    """
    import numpy as np
    import time
    ...
    def analyze_crash_probability(df_5m, df_15m, current_price, avg_buy_price, profit_rate):
        ...
        velocity_1 = (middle_3 - older_3) / older_3  # 과거→중간 속도
        velocity_2 = (recent_3 - middle_3) / middle_3  # 중간→최근 속도
        acceleration = velocity_2 - velocity_1  # 가속도
        
        # 음의 가속도 = 하락 가속 (위험!)
        if acceleration < -0.02:  # -2% 이상 가속
            score += 25
            factors.append(f"급락가속(가속도{acceleration*100:.1f}%)")
        elif acceleration < -0.01:
            score += 15
            factors.append(f"하락가속(가속도{acceleration*100:.1f}%)")
        elif velocity_2 < -0.03:  # 단순 급락
            score += 10
            factors.append(f"급락중(속도{velocity_2*100:.1f}%)")
        ...
        if vol_ratio > 2.0 and price_change_recent < -0.02:
            score += 20
            factors.append(f"공포매도(거래량{vol_ratio:.1f}x+급락)")
        elif vol_ratio > 1.5 and price_change_recent < -0.01:
            score += 12
            factors.append(f"매도압력증가")
        ...
        if bb_pos < -0.1:
            ...
            score += 8
            factors.append(f"BB하단근접")
        
        # BB 폭 급확대 (변동성 증가)
        if bb_width > 8.0:
            score += 5
            factors.append(f"고변동성(BB폭{bb_width:.1f}%)")
        ...
        if df_15m is not None and len(df_15m) >= 10:
            ...
            if trend_15m < -0.05:  # 15분봉 5% 이상 하락
                score += 15
                factors.append(f"15분봉급락({trend_15m*100:.1f}%)")
            elif trend_15m < -0.03:
                score += 8
                factors.append(f"15분봉하락세")
        ...
        if current_low < support_level * 0.98:  # 지지선 하향 돌파
            score += 5
            factors.append(f"지지선붕괴")
    ...
    def analyze_bb_sell_signal(current_price, closes, volumes):
        ...
        전략:
        - BB 상단(70%+): 과열, 매도 강추천
        - BB 중단(30~70%): 중립, 추가 신호 필요
        - BB 하단(30% 미만): 저평가, 홀딩 강추천
        
        Returns:
            {
                'bb_position': float,      # BB 위치 0-1
                'sell_urgency': str,       # 'HIGH' / 'MEDIUM' / 'LOW'
                'should_hold': bool,       # 홀딩 권장 여부
                'reason': str              # 판단 근거
            }
        """
        if len(closes) < 20:
        ...
        # 🎯 BB 위치별 판단
        if bb_position >= 0.70:  # 상단 (과열)
            urgency = 'HIGH'
            should_hold = False
            reason = f"BB상단{bb_position*100:.0f}%(과열)"
            
            # 추가 확인: RSI도 과열이면 더 강력한 매도 신호
            if rsi > 70:
                urgency = 'CRITICAL'
                reason += f"+RSI{rsi:.0f}(극과열)"
        
        elif bb_position >= 0.50:  # 중상단 (주의)
            urgency = 'MEDIUM'
            should_hold = False
            ...
            if price_momentum > 0.01 and rsi < 65:
                ...
                reason = f"BB중상단{bb_position*100:.0f}%(주의)"
        
        elif bb_position >= 0.30:  # 중단 (중립)
            urgency = 'LOW'
            
            # 하락 추세면 관망, 상승 추세면 홀딩
            if price_momentum < -0.01:
                should_hold = False
                reason = f"BB중단{bb_position*100:.0f}%+하락추세"
            else:
                should_hold = True
            ...
            urgency = 'NONE'
            should_hold = True
            reason = f"BB하단{bb_position*100:.0f}%(상승여력큼)"
            
            # 단, 극심한 하락 중이면 예외
            if price_momentum < -0.03 and vol_surge:
                urgency = 'MEDIUM'
                should_hold = False
                reason = f"BB하단이나급락중(모멘텀{price_momentum*100:.1f}%)"
    ...
    if profit_rate < min_rate:
        ...
        # 🚨 손실 구간 폭락 확률 체크 (새로운 로직!)
        if profit_rate < 0:  # 손실 구간
            # 15분봉 추가 수집
            ...
            if df_5m_loss is not None and len(df_5m_loss) >= 30:
                ...
                if crash_analysis:
                    print(f"\n🚨 폭락 위험 분석:")
                    print(f"   폭락 확률: {crash_analysis['crash_probability']*100:.0f}%")
                    print(f"   위험 등급: {crash_analysis['risk_level']}")
                    print(f"   위험 요인: {', '.join(crash_analysis['factors'][:3])}")
                    
                    # 🔥 -3% 이상 손실 + 폭락 확률 70% 이상 → 손절
                    if profit_rate <= -3.0 and crash_analysis['should_cut']:
                        sell_order = upbit.sell_market_order(ticker, buyed_amount)
                        stoploss_msg = f"🛑 **[지능형손절]**: [{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f}\n"
                        stoploss_msg += f"폭락확률: {crash_analysis['crash_probability']*100:.0f}% ({crash_analysis['risk_level']})\n"
                        stoploss_msg += f"위험요인: {', '.join(crash_analysis['factors'])}\n"
                        stoploss_msg += f"판단: 추가 하락 방지 위한 선제 손절"
                        print(stoploss_msg)
                        send_discord_message(stoploss_msg)
                        return sell_order
                    
                    # -5% 이상 손실 + 폭락 확률 55% 이상 → 손절 (기준 완화)
                    elif profit_rate <= -5.0 and crash_analysis['crash_probability'] >= 0.55:
                        sell_order = upbit.sell_market_order(ticker, buyed_amount)
                        emergency_msg = f"🚨 **[긴급손절]**: [{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f}\n"
                        emergency_msg += f"폭락확률: {crash_analysis['crash_probability']*100:.0f}% ({crash_analysis['risk_level']})\n"
                        emergency_msg += f"사유: 대형 손실 + 추가 하락 위험 고조"
                        print(emergency_msg)
    ...
    # 🚨 폭락 위험 분석 (수익 구간에서도 체크)
    crash_analysis = analyze_crash_probability(
        df_5m, df_15m, cur_price, avg_buy_price, profit_rate
    )
    
    if crash_analysis:
        print(f"\n📊 폭락 위험도: {crash_analysis['crash_probability']*100:.0f}% ({crash_analysis['risk_level']})")
        if crash_analysis['factors']:
            print(f"   요인: {', '.join(crash_analysis['factors'][:3])}")
    
    # 🔥 수익 구간에서도 폭락 위험 높으면 조기 매도
    if crash_analysis and crash_analysis['risk_level'] == 'CRITICAL':
        if min_rate <= profit_rate < min_rate * 1.3:  # 최소~1.3배 구간
            sell_order = upbit.sell_market_order(ticker, buyed_amount)
            panic_msg = f"⚠️ **[폭락위험조기매도]**: [{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f}\n"
            panic_msg += f"폭락확률: {crash_analysis['crash_probability']*100:.0f}% (CRITICAL)\n"
            panic_msg += f"판단: 소폭 수익 구간이지만 대폭락 예상으로 선제 매도"
            print(panic_msg)
            send_discord_message(panic_msg)
            return sell_order
    ...
    if bb_analysis:
        print(f"[{ticker}] BB 분석: {bb_analysis['reason']} | "
              f"긴급도: {bb_analysis['sell_urgency']} | "
              f"홀딩권장: {'예' if bb_analysis['should_hold'] else '아니오'}")
    ...
    if bb_analysis and bb_analysis['should_hold']:
        hold_bonus += 2  # BB 하단~중단에서는 홀딩 우대
    
    adjusted_required_score = required_score + hold_bonus
    
    should_sell_technical = sell_strength >= adjusted_required_score
    signal_text = " + ".join(signals) + f" (강도:{sell_strength}/{adjusted_required_score}, 반등:{recovery_prob:.1%})"
    ...
    while attempts < max_attempts:
        ...
        if df_5m_live is not None and len(df_5m_live) >= 30:
            ...
            bb_analysis_live = bb_analysis  # 이전 분석 재사용
        
        print(f"[{ticker}] 시도 {attempts + 1}/{max_attempts} | 수익률: {profit_rate:.2f}% | "
              f"신호: {sell_strength}/{adjusted_required_score} | "
              f"BB: {bb_analysis_live['reason'] if bb_analysis_live else 'N/A'}")
        ...
        if consecutive_no_change >= 8 and profit_rate >= min_rate * 1.5:
            if bb_analysis_live and bb_analysis_live['should_hold']:
                print(f"   ⏸️ 정체 감지했으나 BB 하단으로 홀딩: {bb_analysis_live['reason']}")
            else:
                sell_order = upbit.sell_market_order(ticker, buyed_amount)
                stagnant_msg = f"⏸️ **[정체매도]**: [{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f}\n"
                stagnant_msg += f"사유: 8틱 연속 가격정체, BB: {bb_analysis_live['reason']}"
                print(stagnant_msg)
                send_discord_message(stagnant_msg)
                return sell_order
        
        # [4] 🔥 BB 상단 + 긴급 매도 (새로운 조건!)
        if bb_analysis_live and bb_analysis_live['sell_urgency'] == 'CRITICAL':
            if profit_rate >= min_rate * 1.1:  # 최소 1.1배 이상
                sell_order = upbit.sell_market_order(ticker, buyed_amount)
                critical_msg = f"🚨 **[BB긴급매도]**: [{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f}\n"
                critical_msg += f"사유: {bb_analysis_live['reason']} (극과열)"
                print(critical_msg)
                send_discord_message(critical_msg)
                return sell_order
    ...
    if profit_rate >= min_rate:
        ...
        if bb_analysis_final and bb_analysis_final['should_hold']:
            hold_msg = f"🤝 **[시간종료-홀딩지속]**: [{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f}\n"
            hold_msg += f"사유: {bb_analysis_final['reason']} - 상승 여력 있어 추가 홀딩\n"
            hold_msg += f"반등확률: {recovery_prob:.1%}"
            print(hold_msg)
        ...
        # BB 상단이면 매도
            ...
            sell_order = upbit.sell_market_order(ticker, buyed_amount)
            final_msg = f"⏰ **[시간종료-BB상단매도]**: [{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f}\n"
            final_msg += f"BB 분석: {bb_analysis_final['reason']}\n"
            final_msg += f"판단: 하락 위험으로 안전 매도"
            print(final_msg)
        ...
        hold_msg += f"사유: 최소수익률 미달 (목표: {min_rate:.1f}% 이상)\n"
        if bb_analysis_final:
            hold_msg += f"BB: {bb_analysis_final['reason']}\n"
        hold_msg += f"반등확률: {recovery_prob:.1%}"
        print(hold_msg)
        send_discord_message(hold_msg)
