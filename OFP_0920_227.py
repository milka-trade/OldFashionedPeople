...
def filtered_tickers(tickers):
    ...
    for t in tickers:
        try:
            ...
            # ✨ 최종 매수 결정 로직 - 조건 강화 (UP 또는 SURGE 예측만 매수) ✨
            ...
            # 🌟 조건 3: 필수조건만 + 급상승 예측 또는 과매도시 상승예측 매수
            elif essential_conditions:
                if prediction == 'SURGE':
                    send_discord_message(filtering_message + "🌟 **급상승 예측! 필수조건만으로도 매수**")
                    filtered_tickers.append(t)
                
                elif prediction == 'UP' and market_rsi < 40:
                    # 과매도 상황에서만 추가 기회
                    send_discord_message(filtering_message + "🔍📉 **필수조건+과매도시장+상승예측 매수**")
                    filtered_tickers.append(t)
                    # 일반 시장에서 UP, 그리고 NEUTRAL, DOWN, CRASH는 모두 매수 제외
        ...
        for ticker in all_tickers:
            ...
            if ticker not in held_coins:
                try:  # ✅ 개별 티커별 예외 처리
                    ...
                    if df is not None and 'open' in df.columns and not df.empty:
                        try:
