...
# -*- coding: utf-8 -*-
"""
BB Bounce Hunter v31.0 — 직접 스캔 방식 + 다층 진입 (감시목록 게이트 제거)
 
★ v31.0 핵심 개선 (XRP 30일 5분/15분/일봉 실증분석 기반):
 
  [매수 — 구조적 혁신]
  1. ★★★ 감시목록 의존 구조 제거 — 직접 스캔 방식으로 전환
     - v30 문제: buy_signal()이 호출되려면 급락 감시목록 등록 필수
     - 급락(-2%+)은 30일간 18봉(0.6%)만 발생 → 기회의 99%를 놓침
     - v31: 매 사이클(10초)마다 7코인 직접 조건 스캔
  2. 3-Tier 직접 진입 시스템 (데이터 기반 최적화)
     - Tier-A (BB<25%): 양봉+RSI↑ → 2%승률 49%, RR 1.50 (고승률)
     - Tier-B (BB<35%): 양봉+RSI↑+SRSI>D+BBW≥0.8 → 2%승률 37%, RR 1.22
     - Tier-C (BB<35%+직전음봉): 반전 포인트 확인 → 2%승률 39%
  3. 급락 스캐너: 보조 보너스 역할로 전환 (감시목록→보너스 점수)
  4. TME 추세 스캐너: 상승장 보너스 역할로 전환
  5. 야간 모멘텀/크래시 리커버리: 기존 로직 유지
 
  [매도 — v30 유지]
  6. 과매수 조기 익절 (RSI>70+BB>70) 유지
  7. 5분봉 SRSI 데드크로스 유지
 
★ 데이터 근거 (XRP 15분봉 30일 2,880봉 실증):
  - v30 감시목록 경유: 최근 7일 4봉낙폭≤-2% → 0건 (거래 불능)
  - Tier-A BB<25+양봉+RSI↑: 188건, 2%승률 37.8%, 손실률 12.8%
  - Tier-B BB<35+복합확인: 190건, 2%승률 37.4%, 일평균 6.3건
  - 직접 스캔 방식: 최근 3일에도 하루 4~5건 신호 발생 확인
"""
...
# ── ★ v31.0: 직접 스캔 진입 파라미터 (감시목록 게이트 제거) ──────────
# Tier-A: 고승률 경로 (BB 하단 깊은 진입)
DIRECT_TIER_A_BB_MAX   = 25      # BB Position 상한
DIRECT_TIER_A_RSI_MAX  = 45      # RSI 상한
DIRECT_TIER_A_RSI_MIN  = 15      # RSI 하한 (극단적 과매도 제외)

# Tier-B: 표준 경로 (BB 중하단 복합 확인)
DIRECT_TIER_B_BB_MAX   = 35      # BB Position 상한
DIRECT_TIER_B_BBW_MIN  = 0.8     # BB Width 최소 (극협보합 제외)
DIRECT_TIER_B_RSI_MAX  = 50      # RSI 상한
DIRECT_TIER_B_RSI_MIN  = 15      # RSI 하한

# Tier-C: 반전 포인트 (Tier-B + 직전봉 음봉 후 반전)
# Tier-B 조건 + 직전봉 음봉 필수 (전환점 확인)
DIRECT_TIER_C_ENABLED  = True

# 5분봉 확인 (선택 필터 — 타이밍 정밀화)
DIRECT_5M_CONFIRM      = True    # 5분봉 확인 활성화

# 급락 보너스 (PlungeScore 높을 때 점수 가산)
PLUNGE_BONUS_THRESHOLD = 3.0     # 이 점수 이상이면 보너스 +1
PLUNGE_BONUS_SCORE     = 1       # 가산 점수

# TME 상승장 보너스 (bull_score 높을 때 기준 완화)
TME_BONUS_THRESHOLD    = 0.55    # 이 스코어 이상이면 BB 기준 완화
TME_BONUS_BB_EXTEND    = 5       # BB 상한 확장 폭

# ── v31.0: 매수 차단 쿨다운 (동일 코인 연속 진입 방지) ──
SAME_COIN_COOLDOWN_MIN = 15      # 동일 코인 매수 후 최소 대기 (분)

# ── ★ v30.0 신규: 3-Tier 낙폭 진입 파라미터 (v31: 보조 참고용으로 유지) ──
...
def buy_signal(ticker):
    """
    ★★★ v31.0 매수 신호 — 직접 스캔 방식 (감시목록 게이트 제거)
    ...
    ② 데이터 수집 + 기본 지표 계산
    ③ ★ 직접 진입 조건 평가 (3-Tier):
       - Tier-A (BB<25%): 양봉+RSI↑ — 고승률 49%
       - Tier-B (BB<35%): 양봉+RSI↑+SRSI>D+BBW≥0.8 — 표준 37%
       - Tier-C (BB<35%+직전음봉): 반전 포인트 — 39%
    ④ 보너스 가산: 급락스코어 / 일봉컨텍스트 / 5분봉확인
    ⑤ 크래시 리커버리 (기존 유지)
    ════════════════════════════════════════════════════════
    """
    try:
        ...
        # ── 5분봉 확인 (선택) ──
        confirm_5m = True  # 기본값 (5분봉 없으면 통과)
        confirm_5m_tag = ""
        if DIRECT_5M_CONFIRM and df_5m is not None and len(df_5m) >= 3:
            c5 = df_5m.iloc[-1]
            p5 = df_5m.iloc[-2]
            # 5분봉 양봉 or RSI 상승이면 확인
            if c5['rsi'] > p5['rsi'] or c5['is_bull']:
                confirm_5m = True
                confirm_5m_tag = " 5m✓"
            else:
                confirm_5m = False
        ...
        # Tier-A: 고승률 경로 (BB < 25%)
        # 조건: 양봉 + RSI↑ + RSI 15~45
        # 백테스트: 2%승률 49%, RR 1.50
        # ────────────────────────────────────────
        tier_a_bb_max = DIRECT_TIER_A_BB_MAX + tme_bb_extend
        if (bb_pos <= tier_a_bb_max and
            is_bull_candle and
            rsi_rising and
            DIRECT_TIER_A_RSI_MIN <= rsi <= DIRECT_TIER_A_RSI_MAX):
 
            # Tier-A는 강한 조건이므로 5분봉 확인 선택적 (없어도 진입)
            bonus_str = ""
            if day_bonus > 0:
            ...
            return {
                'signal': True,
                'reason': (f"[Tier-A고승률][{grade}] BB{bb_pos:.0f}% RSI{rsi:.0f}↑ "
                           f"BBW{bb_width:.1f}%{bonus_str}{confirm_5m_tag}"),
                'entry_price': current['close'],
                'bb_position': bb_pos,
                'bb_width_pct': bb_width,
                'market_grade': grade,
        ...
        # 조건: Tier-B + 직전봉이 음봉 (전환점)
        # 백테스트: 2%승률 39%, 직전 음봉→양봉 = 더 강한 반전
        # ────────────────────────────────────────
        tier_b_bb_max = DIRECT_TIER_B_BB_MAX + tme_bb_extend + (5 if day_ctx['is_low_bb'] else 0)
        ...
        if (DIRECT_TIER_C_ENABLED and
            ...
            not prev['is_bull']):  # ★ 직전봉 음봉 필수 (반전 포인트)
 
            # 5분봉 확인 가능하면 더 좋지만 필수는 아님
            bonus_str = ""
            ...
            return {
                'signal': True,
                'reason': (f"[Tier-C반전][{grade}] BB{bb_pos:.0f}% RSI{rsi:.0f}↑ "
                           f"SRSI{srsi_k:.0f}>D BBW{bb_width:.1f}% "
                           f"직전음봉→양봉{bonus_str}{confirm_5m_tag}"),
                'entry_price': current['close'],
                'bb_position': bb_pos,
                'bb_width_pct': bb_width,
                'market_grade': grade,
        ...
        # ────────────────────────────────────────
        # Tier-B: 표준 경로 (BB < 35%)
        # 조건: 양봉 + RSI↑ + SRSI K>D + BBW≥0.8
        # 백테스트: 2%승률 37%, RR 1.22, 일평균 6.3건
        # ────────────────────────────────────────
        if (bb_pos <= tier_b_bb_max and
            ...
            rsi_rising and
            srsi_golden and
            DIRECT_TIER_B_RSI_MIN <= rsi <= DIRECT_TIER_B_RSI_MAX):
 
            # Tier-B는 5분봉 확인 권장
            if not confirm_5m and DIRECT_5M_CONFIRM:
                # 5분봉 미확인 시 추가 조건: 15분봉 점수 기반 보완
                score_15m, details_15m = score_15m_bounce(df_15m)
                if score_15m + total_bonus < 3:
                    base['reason'] = (f"Tier-B 5분봉미확인+15m점수부족 "
                                      f"BB{bb_pos:.0f}% RSI{rsi:.0f} 15m:{score_15m}점[{grade}]")
                    # 크래시 리커버리로 폴스루 (아래)
                    pass
                else:
                    bonus_str = ""
                    ...
                    return {
                        'signal': True,
                        'reason': (f"[Tier-B표준][{grade}] BB{bb_pos:.0f}% RSI{rsi:.0f}↑ "
                                   f"SRSI{srsi_k:.0f}>D BBW{bb_width:.1f}% "
                                   f"15m보완({score_15m}점:{d15}){bonus_str}"),
                        'entry_price': current['close'],
                        'bb_position': bb_pos,
                        'bb_width_pct': bb_width,
                        'market_grade': grade,
        ...
        base['reason'] = f"조건미충족[{grade}] {' '.join(fail_parts)}"
        return base
        ...
        try:
            ...
            if DEBUG_MODE and iteration % 6 == 0:
                ...
                print(f"{Colors.MAGENTA}[BUY] KRW:{krw_balance_now:,.0f}원 | "
                      ...
                      f"급락보너스:{wl_count}종 추세보너스:{tme_count}종{Colors.ENDC}")

            # ══════════════════════════════════════════════
            # ★★★ v31.0: 직접 스캔 방식 — 7코인 순차 검토
            # 감시목록 게이트 없이 매 사이클 buy_signal() 직접 호출
            # ══════════════════════════════════════════════
            for ticker in FIXED_STABLE_COINS:
            ...
            # ══════════════════════════════════════════════
            # ★ v31.0: TME 추세 경로도 직접 호출 (감시 보너스 참조용)
            # 급락 반등 경로에서 이미 직접 스캔하므로,
            # TME는 bull_score 높은 상승장에서 추가 기회 포착용
            # ══════════════════════════════════════════════
            with held_coins_lock:
                managed_count = sum(1 for info in held_coins.values() if info.get('managed', True))
            if managed_count >= MAX_HOLDINGS:
...
def main():
    ...
    start_msg = (
        ...
        f"**★ v31.0 핵심 개선:**\n"
        f"🎯 직접 스캔 방식 (감시목록 게이트 제거)\n"
        f"📊 3-Tier 진입: A(BB<25) B(BB<35) C(반전포인트)\n"
