...
NIGHT_HL_POSITION_MAX = 60                 # HL 60%까지 허용 (DAY: 레짐별 30~50)
...
def check_daily_safety_filter(ticker, regime='NEUTRAL', time_mode='DAY'):
    """
    [v18.0] Phase 1: 일봉 양봉 필터 + HL Position 필터
    
    v18.0 설계 철학: "NIGHT에서 문을 넓게 열어 기회 확보"
    - DAY: 기존 v16.0 로직 100% 유지
    - NIGHT: 양봉 필터 완화 → 음봉 상태에서도 BB 하단 V자 반등 기회 포착
    - 극단 위험(급등/급락, 연속 3일 음봉)은 시간대 관계없이 차단
    ...
    try:
        ...
        # ========================================
        # 체크 1: 당일 등락률 범위 (DAY/NIGHT 동일 - 극단 위험 차단)
        # ========================================
        ...
        # [v18.0] 시간대별 연속 음봉 기준 결정
        if time_mode == 'NIGHT':
            max_consecutive_bear = NIGHT_CONSECUTIVE_BEAR_MAX    # 3 (통일 완화)
        elif REGIME_ENABLED:
            if regime == 'BEARISH':
        ...
        # ========================================
        # 체크 3: 양봉 조건
        # [v18.0] NIGHT 모드: 당일 양봉 필수 스킵 + 최근 양봉수 완화
        # ========================================
        ...
        if time_mode == 'NIGHT':
            # ───────────────────────────────────────
            # 🌙 NIGHT 모드: 양봉 필터 대폭 완화
            # Phase 1에서 문을 넓게 열어 기회 확보
            # (Phase 2의 BB≤25% 강화로 리스크 관리)
            # ───────────────────────────────────────
        ...
        # ========================================
        # [v16.0] 체크 4: HL Position 필터
        # [v18.0] NIGHT 모드: 완화된 HL 기준
        # ========================================
        if hl_filter_active:
            if time_mode == 'NIGHT':
                hl_max = NIGHT_HL_POSITION_MAX                  # 60% (완화)
            elif REGIME_ENABLED:
                if regime == 'BULLISH':
            ...
            if hl_position > hl_max:
                mode_tag = f"NIGHT" if time_mode == 'NIGHT' else regime
                return {**base_info, 'safe': False, 'is_bullish': is_today_bullish,
                        'reason': f'HL고점필터: HL{hl_position:.0f}%>{hl_max}% (고가근처, {mode_tag})',
                        'daily_candle_type': 'hl_filtered'}
        
        # ========================================
        # 모든 체크 통과 → daily_candle_type 결정
        # [v18.0] NIGHT 모드: 전용 타입 부여
        # ========================================
        if time_mode == 'NIGHT':
            # 🌙 NIGHT: 양봉/음봉 구분 없이 통과 허용
            if is_today_bullish:
                daily_candle_type = 'night_bullish'
                bullish_reason = f"🌙NIGHT양봉{daily_change:+.1f}%"
            else:
                daily_candle_type = 'night_permissive'           # ← 핵심: 음봉도 허용
...
def detect_downtrend_15m(df_15m, daily_candle_type='unknown', regime='NEUTRAL', time_mode='DAY'):
    """
    [v18.0] Phase 2: 15분봉 BB 하단 위치 확인
    
    v18.0 설계 철학: "NIGHT에서 진입 위치는 정확하게 (BB 하단 only)"
    - DAY: 기존 v16.0 로직 100% 유지
    - NIGHT: BB 상한 25% (하단 근처에서만 매수)
             BB 폭 1.2% (소폭 완화)
             RSI 20 (DAY와 동일, 과매도 보호)
             Higher Low 비활성 (V자 반등 특화)
    
    ★ Phase 1에서 문을 열었으므로, Phase 2에서 좁은 문으로 필터링
    → "충분히 내려온 곳에서만 매수" = 손절 리스크 최소화
    """
    try:
        ...
        if time_mode == 'NIGHT':
            # ───────────────────────────────────────
            # 🌙 NIGHT 모드: BB 하단 근처 강화 기준
            # BB 25% 이하에서만 매수 → 충분히 떨어진 곳
            # ───────────────────────────────────────
            bb_min = NIGHT_BB_MIN                    # 3%  (극단 하단 보호)
            bb_max = NIGHT_BB_MAX                    # 25% ★ 핵심: 하단 근처만 허용
            required_bb_width = NIGHT_BB_WIDTH_MIN   # 1.2% (소폭 완화)
            rsi_min = NIGHT_RSI_MIN                  # 20   (DAY와 동일)
            width_label = 'NIGHT'
        elif REGIME_ENABLED and regime == 'BULLISH':
            bb_min = REGIME_BULL_BB_MIN
            bb_max = REGIME_BULL_BB_MAX
            required_bb_width = REGIME_BULL_BB_WIDTH
            rsi_min = REGIME_BULL_RSI_MIN
            width_label = 'BULL'
        elif REGIME_ENABLED and regime == 'BEARISH':
            bb_min = REGIME_BEAR_BB_MIN
            bb_max = REGIME_BEAR_BB_MAX
            required_bb_width = REGIME_BEAR_BB_WIDTH
            rsi_min = REGIME_BEAR_RSI_MIN
            width_label = 'BEAR'
        elif REGIME_ENABLED and regime == 'NEUTRAL':
            bb_min = REGIME_NEUT_BB_MIN
            bb_max = REGIME_NEUT_BB_MAX
            required_bb_width = REGIME_NEUT_BB_WIDTH
            rsi_min = REGIME_NEUT_RSI_MIN
            width_label = 'NEUT'
        else:
        ...
        if bb_position > bb_max:
            return {
                **base, 'is_downtrend': True,
                'reason': f'BB {bb_position:.0f}% > {bb_max}% ({"아직 하락중, 더 내려와야" if time_mode == "NIGHT" else "이미 반등 진행"})'
            }
        ...
        # ========================================
        # 체크 4: Higher Low 검증
        # [v18.0] NIGHT 모드: 비활성 (V자 반등은 HL 불필요)
        # ========================================
        use_higher_low = V10_HIGHER_LOW_ENABLED
        if time_mode == 'NIGHT':
            use_higher_low = NIGHT_HIGHER_LOW_ENABLED    # False
        ...
        # ========================================
        # 체크 5: BEARISH 추가 필터 (ADX+OBV)
        # [v18.0] NIGHT 모드: 비활성 (V자 반등 기회 확보)
        # ========================================
        use_bear_adx = REGIME_BEAR_ADX_FILTER
        if time_mode == 'NIGHT':
            use_bear_adx = NIGHT_BEAR_ADX_FILTER         # False
        
        if REGIME_ENABLED and regime == 'BEARISH' and use_bear_adx:
...
def calculate_reversal_score(df_15m, time_mode='DAY'):
    """
    [v18.0] Phase 3: 다중 지표 반등 신호 확인
    
    v18.0 변경: time_mode 파라미터 추가
    - DAY: 기존 5+1개 신호 유지
    - NIGHT: ⑦V-Reversal, ⑧하단꼬리 신호 추가 + 필수조건 확장
    ...
    NIGHT 추가 신호:
    ⑦ V-Reversal: 직전 연속 음봉 후 현재 양전환
    ⑧ 하단꼬리: 캔들 하단꼬리가 실체의 N배 이상
    """
    try:
        ...
        # ========================================
        # ⑥ RSI Bullish Divergence (보너스 - DAY/NIGHT 공통)
        # ========================================
        rsi_divergence_detected = False
        if RSI_DIVERGENCE_ENABLED and len(df_15m) >= RSI_DIVERGENCE_LOOKBACK:
            try:
                lookback_df = df_15m.tail(RSI_DIVERGENCE_LOOKBACK)
                price_lows = []
                rsi_at_lows = []
                ...
                if len(price_lows) >= 2:
                    if price_lows[-1] < price_lows[-2] and rsi_at_lows[-1] > rsi_at_lows[-2]:
                        ...
                        signals_met += 1
                        details.append("⑥RSI다이버전스(가격↓RSI↑)")
            except:
                pass
        signals['rsi_divergence'] = rsi_divergence_detected
        
        # ========================================
        # [v18.0 신규] ⑦ V-Reversal 감지 (NIGHT 전용)
        # 직전 N개 연속 음봉 후 현재 양전환 (BB 하단 근처에서 발생)
        # ========================================
        v_reversal_detected = False
        if time_mode == 'NIGHT' and NIGHT_V_REVERSAL_ENABLED and len(df_15m) >= NIGHT_V_REVERSAL_MIN_BEARS + 1:
            try:
                consecutive_bears = 0
                for i in range(2, min(6, len(df_15m))):   # 직전 최대 5개 캔들 체크
                    prev_candle = df_15m.iloc[-i]
                    if prev_candle['close'] < prev_candle['open']:
                        consecutive_bears += 1
                    else:
                        break
                
                # 조건: 직전 N개 연속 음봉 + (현재 양봉 OR 종가 반등 중)
                current_bullish_or_turning = (
                    is_bullish or                                         # 양봉 확정
                    (current['close'] > prev['close'])                    # 종가 반등 중
                )
        ...
        # ========================================
        # [v18.0 신규] ⑧ 하단꼬리(Lower Shadow) 감지 (NIGHT 전용)
        # 캔들 하단꼬리가 실체의 N배 이상 → 강한 매수세 유입
        # ========================================
        ...
        # ========================================
        # 필수 조건 체크
        # [v18.0] NIGHT: V-Reversal도 필수 충족 가능
        # ========================================
        if time_mode == 'NIGHT':
            mandatory_met = rsi_rising or is_bullish or v_reversal_detected
        else:
        ...
        return {
            ...
            'volume_up': volume_ok,
            'higher_low': False,
            'v_reversal': v_reversal_detected,          # [v18.0 신규]
            'lower_shadow': lower_shadow_detected       # [v18.0 신규]
        }
...
def evolution_80_buy_signal(df_15m, df_5m, ticker):
    ...
    try:
        ...
        if time_mode == 'NIGHT':
            required_signals = NIGHT_REVERSAL_MIN_SIGNALS         # 2 (통일 완화)
        elif REGIME_ENABLED:
            if regime == 'BULLISH':
...
def evolution_80_sell_signal(df, buy_price, buy_time=None, held_info=None, df_5m=None):
    ...
    try:
        ...
        # Step 3: 모멘텀 소진 점수제
        # [v17.0 신규] Step 2.5: Quick Profit Lock (빠른 수익 확보)
        if QPL_ENABLED and profit_pct > 0:
            ...
            # Tier3: 수익 3%+ → BB 70% 이상이면 즉시 매도
            if profit_pct >= QPL_TIER3_PROFIT and bb_position >= 70:
                return {**base_response, 'signal': True,
                        'reason': f'QPL_T3 (수익{profit_pct:.2f}%≥{QPL_TIER3_PROFIT}%, BB{bb_position:.0f}%≥70%)'}
            
            # Tier2: 수익 2%+ → 음봉+RSI하락이면 즉시 매도
...
def buy_thread_worker():
    """
    [v18.0] BOUNCE 전용 매수 스레드
    
    v18.0 변경:
    - 시작 시 NIGHT 모드 파라미터 출력
    - 매수 신호 로그에 시간대 모드 표시
    - 기존 08:00~09:15 매수 차단 유지
    """
    print(f"{Colors.CYAN}[Thread 1] v18.0 BOUNCE+NIGHT 매수 스레드 시작 ({BUY_THREAD_INTERVAL}초 주기){Colors.ENDC}")
    if DYNAMIC_COIN_ENABLED:
    ...
    # [v18.0 신규] NIGHT 모드 파라미터 출력
    if TIMEZONE_MODE_ENABLED:
        print(f"{Colors.CYAN}  └ 🌙NIGHT: {NIGHT_MODE_START_HOUR:02d}:{NIGHT_MODE_START_MINUTE:02d}~{NIGHT_MODE_END_HOUR:02d}:{NIGHT_MODE_END_MINUTE:02d} | BB≤{NIGHT_BB_MAX}% 폭≥{NIGHT_BB_WIDTH_MIN}% RSI≥{NIGHT_RSI_MIN} 신호≥{NIGHT_REVERSAL_MIN_SIGNALS}{Colors.ENDC}")
        ...
        try:
            ...
            for ticker in active_coins:
                ...
                if buy_signal['signal']:
                    ...
                    print(f"\n{Colors.CYAN}{'='*50}")
                    print(f"[BUY SIGNAL] {coin_name} 모멘텀 매수!{time_tag}{dyn_tag}")
                    print(f"{'='*50}{Colors.ENDC}")
                    print(f"  📊 반등 신호: {buy_signal.get('reversal_score', 0)}개")
                    print(f"  🎯 모드: {buy_signal.get('mode', 'BOUNCE_V18')}")
                    ...
                    time.sleep(2)
                    with held_coins_lock:
                        managed_cnt = sum(1 for v in held_coins.values() if v.get('managed', True))
