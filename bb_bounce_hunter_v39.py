...
"""
═══════════════════════════════════════════════════════════════════════
BB Bounce Hunter v39.0 — Distance-Adaptive BB Upper Exit Edition
"BB 상단 위 거리에 비례한 가속 매도로 조기매도와 함정매도 동시 회피"
═══════════════════════════════════════════════════════════════════════

v38 → v39 핵심 변경 (광현님 요청 — BB 상단 위 거리별 가속 매도):

  [광현님 핵심 지시 정확 반영]
  - "BB 상단 터치 즉시 매도는 조기매도라 원하지 않음"
  - "BB 상단을 뚫고 올라가면 그때부터 매도 시점을 민감하게 모니터링"
  - "BB 상단에서 멀리 올라갈수록 매도를 빨리 하도록"

  [TOKAMAK 30일 데이터 검증 결과]
  - BB 상단 위 0.5%까지 = 다음 봉 평균 -0.56% (즉시매도 가치 낮음)
  - BB 상단 위 0.5~1.0% = 다음 봉 저점 -1.79% (1.0% drawdown 매도 효과적)
  - BB 상단 위 1.0~2.0% = 다음 봉 저점 -1.87% (0.6% drawdown 매도 효과적)
  - BB 상단 위 2.0~3.5% = 다음 봉 저점 -5.16% (0.3% drawdown 즉시 매도)
  - BB 상단 위 3.5%+ = 다음 봉 저점 -10.77% (0.0% drawdown = 즉시 매도)

  [v39 신규 사양: Distance-Adaptive Exit (DAE)]

  [1] BB 상단 위 거리별 5단계 가속 매도 매트릭스
      - Tier 1: 위 0.0~0.3% → 감시만 (매도 X)
      - Tier 2: 위 0.3~0.7% → 1.5% drawdown에 매도
      - Tier 3: 위 0.7~1.5% → 1.0% drawdown에 매도
      - Tier 4: 위 1.5~2.5% → 0.6% drawdown에 매도
      - Tier 5: 위 2.5~4.0% → 0.3% drawdown에 매도
      - Tier 6: 위 4.0%+    → 즉시 매도 (0% drawdown)

  [2] BB 상단 위 zone 진입 시 monitoring mode 활성
      - peak_above_upper_pct (zone 내 최고 BB 상단 위 거리) 추적
      - peak에서 drawdown 발생 시 매도 발화

  [3] 시간대별 활성/비활성 (Band-Walking 보호)
      - VOLATILITY_SPIKE_KOREA (09시): 비활성
      - KST 16:00~17:00: 비활성 (Band-Walking 시간대)
      - 그 외: 활성

  [4] 1H + 15분 듀얼 컨펌
      - 1H BB Position > 95% (큰 그림 천장)
      - 15분 BB 상단 위 거리 (정밀 매도 트리거)
      - RSI 1H > 65 (모멘텀 컨펌, 너무 낮으면 가짜 신호)

  [5] FRESH 단계 5분 유예 (호가 안정화 보호)

  [6] 매도 사유 카테고리 추가 (BB_DAE_T2 ~ BB_DAE_T6)

설계 철학:
  v38 = "분할 매도 + Ratchet Stop + Spike 보호"
  v39 = v38 + "BB 상단 위 거리별 가속 매도" 결합

학술/통계 근거:
  - John Bollinger (2010) — %B 지표로 가격의 BB 상대 위치 정량화
  - Larry Connors — High Probability Trading (Chapter 5: %B 전략)
  - TOKAMAK 30일 자체 분석 — 거리별 mean reversion 폭/속도 입증
...
DEBUG_MODE = True
TEST_MODE = False
VERSION = "39.0 DISTANCE_ADAPTIVE_EXIT — BB Upper Distance-Adaptive Acceleration"
...
# ═══════════════════════════════════════════════════════════════════════
# ★ v39 핵심: BB 상단 위 거리별 가속 매도 (Distance-Adaptive Exit)
# v38 (변동성 폭발 시간대 + 분할매도 + Ratchet) + v39 (DAE) 통합
# ═══════════════════════════════════════════════════════════════════════

# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-A] KST 시간대 정책 (v38 유지: 6개 세션)
# ──────────────────────────────────────────────────────────────────────
TIME_ZONE_POLICY = {
    ...
    "GLOBAL_PEAK":    {"start": (22, 0), "end": (2, 0),
                       ...
                       "fresh_grace_min": 45,
                       "spike_bar_protect": True},
    "KOREA_MORNING_LATE": {"start": (10, 0), "end": (12, 0),
...
BUY_TIME_GATES = {
    ...
    "POST_SPIKE_BONUS": {
        ...
        "action": "BONUS",
        "bb_position_max_relax": 10,
    },
    "LOW_LIQ_CAUTION": {
        "start": (12, 0), "end": (14, 0),
        "action": "CAUTION",
        "vol_ratio_min_extra": 0.3,
    },
}

# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-C] 분할 매도 (Laddered Exit) — v38 유지
# ──────────────────────────────────────────────────────────────────────
LADDERED_EXIT_ENABLED = True
LADDERED_TIER1_PROFIT = 2.5
LADDERED_TIER1_RATIO  = 0.30
LADDERED_TIER2_PROFIT = 5.0
LADDERED_TIER2_RATIO  = 0.30

# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-D] Ratchet Stop (v38 유지)
# ──────────────────────────────────────────────────────────────────────
RATCHET_STOP_TIERS = [
    (12.0, 6.5),
    (8.0,  3.5),
...
# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-E] 변동성 폭발봉 / Squeeze (v38 유지)
# ──────────────────────────────────────────────────────────────────────
SPIKE_BAR_ATR_MULTIPLE = 2.0
SPIKE_BAR_DELAY_SEC = 300
SPIKE_BAR_RECOVERY_PCT = 1.0

SQUEEZE_BREAK_BB_WIDTH_TH = 15.0
SQUEEZE_BREAK_BB_POS_TH = 90.0
SQUEEZE_BREAK_TRAIL_MULT = 2.0

# ══════════════════════════════════════════════════════════════════════
# ★★★ [SECTION 2-F] v39 핵심: BB 상단 위 거리별 가속 매도 (DAE) ★★★
# ══════════════════════════════════════════════════════════════════════
# 광현님 핵심 지시:
#  - "BB 상단 터치 즉시 매도는 조기매도라 원하지 않음"
#  - "BB 상단을 뚫고 올라가면 매도 시점을 민감하게 모니터링"
#  - "BB 상단에서 멀리 올라갈수록 매도를 빨리 하도록"
#
# TOKAMAK 30일 데이터로 통계 입증된 매트릭스:
DAE_ENABLED = True

# 거리별 매도 임계: (peak_above_upper_pct ≥ 거리 → drawdown 임계)
# peak_above_upper_pct = peak이 BB 상단에서 % 위로 멀어진 정도
# drawdown_pct = peak에서 % 하락 시 매도
DAE_TIERS = [
    # (거리 임계 %, drawdown 임계 %, 카테고리 태그)
    (4.0,  0.0, "T6"),   # 4.0%+ 위 → 즉시 매도 (next bar -10.77% 가능)
    (2.5,  0.3, "T5"),   # 2.5%+ 위 → 0.3% drawdown (next bar -5.16%)
    (1.5,  0.6, "T4"),   # 1.5%+ 위 → 0.6% drawdown (next bar -1.87%)
    (0.7,  1.0, "T3"),   # 0.7%+ 위 → 1.0% drawdown (next bar -1.79%)
    (0.3,  1.5, "T2"),   # 0.3%+ 위 → 1.5% drawdown
    # (0.0,  None, "T1") = 감시만, 매도 X
]

# 듀얼 컨펌 임계
DAE_1H_BB_POS_TH = 95.0       # 1H BB Position 최소 (큰 그림 천장 컨펌)
DAE_RSI_MIN = 60.0            # 1H RSI 최소 (60 미만이면 가짜 신호로 무시)
DAE_RSI_STRONG = 75.0         # 1H RSI 75+ → 임계 한 단계 좁힘 (강한 천장)

# 최소 발화 수익률 (전체 매수가 대비)
DAE_MIN_PROFIT_PCT = 1.0      # 수익 +1% 이상에서만 DAE 평가 (손실 매도 방지)

# FRESH 단계 5분 호가 안정화 유예
DAE_FRESH_GRACE_SEC = 300

# 시간대별 활성/비활성 (Band-Walking 보호)
DAE_TIME_ENABLE = {
    "VOLATILITY_SPIKE_KOREA": False,  # KST 09시 = band-walking 가능
    "GLOBAL_PEAK":            True,
    "KOREA_MORNING_LATE":     True,
    "LOW_LIQUIDITY":          True,
    "EU_RAMP":                True,   # 단, 16:00~17:00은 별도 차단
    "OVERNIGHT_LATE":         True,
}
# 분 단위 추가 차단 시간대 ((sh, sm, eh, em))
DAE_BLOCK_HOURS = [
    (16, 0, 17, 0),   # KST 16:00~17:00 (Band-Walking 시간대)
]

# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-G] ATR 변동성 정규화 트레일링 (v37 유지)
# ──────────────────────────────────────────────────────────────────────
ATR_TRAIL_K_MULT = 3.0
ATR_TRAIL_MIN_PCT = 2.0
ATR_TRAIL_MAX_PCT = 8.0
ATR_PERIOD = 14

# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-H] 보유 단계 정의 (FRESH는 시간대별 차등)
# ──────────────────────────────────────────────────────────────────────
PHASE_FRESH_MAX_HOURS  = 0.5
PHASE_EARLY_MAX_HOURS  = 6.0
PHASE_MID_MAX_HOURS    = 24.0

# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-I] 단기 천장 다중 신호 합의 (v37 유지)
# ──────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-J] Rally Guard (레거시, v38은 Ratchet Stop이 우선)
# ──────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-K] 2-Bar 확정 손절 (v37 유지)
# ──────────────────────────────────────────────────────────────────────
TWO_BAR_CONFIRM_LOSS_PCT = -3.0

# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-L] BTC 시스템 리스크 감지 (v37 유지)
# ──────────────────────────────────────────────────────────────────────
BTC_CRASH_1H_PCT = -3.0
BTC_CRASH_5M_CONSECUTIVE = 3

# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-M] 매도 사유별 재진입 쿨다운 (v39: DAE 카테고리 추가)
# ──────────────────────────────────────────────────────────────────────
COOLDOWN_PARTIAL_TAKE  = 0       # 분할 매도 (잔여 보유)
COOLDOWN_PROFIT_TAKE   = 30
COOLDOWN_TRAILING      = 60
COOLDOWN_RSI_REVERT    = 45
COOLDOWN_TOP_SIGNAL    = 45
COOLDOWN_RALLY_GUARD   = 60
COOLDOWN_RATCHET_STOP  = 60
COOLDOWN_SPIKE_RECOVERY = 30
COOLDOWN_STOPLOSS      = 240
COOLDOWN_TREND_END     = 1440
COOLDOWN_BTC_CRASH     = 360

# ★ v39 NEW: BB DAE 카테고리 쿨다운 (거리 멀수록 짧은 쿨다운)
COOLDOWN_BB_DAE_T6     = 30      # 즉시 매도 (가장 강한 천장) → 빠른 재진입
COOLDOWN_BB_DAE_T5     = 30
COOLDOWN_BB_DAE_T4     = 45
COOLDOWN_BB_DAE_T3     = 60
COOLDOWN_BB_DAE_T2     = 60

# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-N] D+0 안전장치 (v36 유지)
# ──────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────
# [SECTION 2-O] D+1 추세 이탈 매도 (v36 유지, MATURE 단계)
# ──────────────────────────────────────────────────────────────────────
...
# ★ v39 변경 요약:
#   [v38 → v39 추가]
#   + DAE_ENABLED, DAE_TIERS (5단계 거리별 가속 매도 매트릭스)
#   + DAE_1H_BB_POS_TH, DAE_RSI_MIN, DAE_RSI_STRONG (듀얼 컨펌)
#   + DAE_MIN_PROFIT_PCT (최소 발화 수익)
#   + DAE_FRESH_GRACE_SEC (호가 안정화 5분)
#   + DAE_TIME_ENABLE, DAE_BLOCK_HOURS (시간대 차등)
#   + COOLDOWN_BB_DAE_T2~T6 (거리별 차등 쿨다운)
#   [v38에서 유지]
#   - TIME_ZONE_POLICY 6 sessions, BUY_TIME_GATES, LADDERED_EXIT, RATCHET_STOP
#   - SPIKE_BAR, SQUEEZE_BREAK, ATR_TRAIL, BTC_CRASH
...
{Colors.BOLD}{Colors.CYAN}{'='*68}
  CRYPTO DISTANCE-ADAPTIVE EXIT HUNTER {VERSION}
  ★★★ v39.0: BB 상단 위 거리에 비례한 가속 매도

  ★ 마켓 와이드 스크리너: 업비트 KRW 200+ 코인 매시 정각 스크리닝

  ★★ v38 유지: 매수 시간대 게이트
     [BLOCK]   KST 08:30~09:30 (변동성 폭발 직전)
     [BONUS]   KST 09:30~10:30 (BB 임계 완화, 폭락 후 진입)
     [CAUTION] KST 12:00~14:00 (슬리피지 회피)

  ★★★ v39 매도 로직 (BB 상단 위 거리별 가속 매도):
     [DAE_T2] 위 0.3%+ → 1.5% drawdown 매도
     [DAE_T3] 위 0.7%+ → 1.0% drawdown 매도
     [DAE_T4] 위 1.5%+ → 0.6% drawdown 매도
     [DAE_T5] 위 2.5%+ → 0.3% drawdown 매도
     [DAE_T6] 위 4.0%+ → 즉시 매도

  ★★ v38 유지: 분할 매도 + Ratchet Stop + Spike Bar + Squeeze
     [1] 분할 매도 +2.5%/+5% (30%/30%)
     [2] Ratchet Stop (peak+3% → stop+0.5% 등)
     [3] 변동성 폭발봉 5분 유예
     [4] 시간대 정책 6분류 + FRESH 차등 유예

  ★★ v39 DAE 시간대 비활성:
     - VOLATILITY_SPIKE_KOREA (KST 09시): Band-Walking 보호
     - KST 16:00~17:00: Band-Walking 보호

{'='*68}
  Thread 1: 매수 ({BUY_THREAD_INTERVAL}초) │ EMA자격 + 시간대 게이트
  Thread 2: 매도 ({SELL_THREAD_INTERVAL}초) │ v39 Sell-Side (분할+DAE+Ratchet)
...
def get_cooldown_for_category(sell_category):
    """★ v39 변경: BB_DAE_T2~T6 추가 (BB 상단 위 거리별 차등 쿨다운)"""
    return {
        "PARTIAL_TAKE":   COOLDOWN_PARTIAL_TAKE,
        "PROFIT_TAKE":    COOLDOWN_PROFIT_TAKE,
        "TOP_SIGNAL":     COOLDOWN_TOP_SIGNAL,
        "RSI_REVERT":     COOLDOWN_RSI_REVERT,
        "RALLY_GUARD":    COOLDOWN_RALLY_GUARD,
        "RATCHET_STOP":   COOLDOWN_RATCHET_STOP,
        "SPIKE_RECOVERY": COOLDOWN_SPIKE_RECOVERY,
        "TRAILING":       COOLDOWN_TRAILING,
        "STOPLOSS":       COOLDOWN_STOPLOSS,
        "TREND_END":      COOLDOWN_TREND_END,
        "BTC_CRASH":      COOLDOWN_BTC_CRASH,
        # ★ v39 NEW: BB 상단 위 거리별 가속 매도 카테고리
        "BB_DAE_T2":      COOLDOWN_BB_DAE_T2,    # 0.3%+ 위, 1.5% drawdown
        "BB_DAE_T3":      COOLDOWN_BB_DAE_T3,    # 0.7%+ 위, 1.0% drawdown
        "BB_DAE_T4":      COOLDOWN_BB_DAE_T4,    # 1.5%+ 위, 0.6% drawdown
        "BB_DAE_T5":      COOLDOWN_BB_DAE_T5,    # 2.5%+ 위, 0.3% drawdown
        "BB_DAE_T6":      COOLDOWN_BB_DAE_T6,    # 4.0%+ 위, 즉시 매도
...
# ═══════════════════════════════════════════════════════════════════════
# ★ [v39 NEW] BB 상단 위 거리별 가속 매도 (Distance-Adaptive Exit) 헬퍼
# ═══════════════════════════════════════════════════════════════════════

def _is_dae_blocked_hour(now=None):
    """v39: 분 단위 BB DAE 매도 차단 시간대 (KST 16:00~17:00 등) 판정"""
    if now is None:
        now = datetime.now()
    hm = now.hour * 60 + now.minute
    for sh, sm, eh, em in DAE_BLOCK_HOURS:
        start_min = sh * 60 + sm
        end_min = eh * 60 + em
        if start_min <= hm < end_min:
            return True
...
def is_dae_active(session_name, now=None):
    """v39: 시간대 + 분 단위 BB DAE 매도 활성 여부

    Returns:
        (active: bool, reason: str)
    """
    if not DAE_ENABLED:
        return False, "DAE_disabled"
    if not DAE_TIME_ENABLE.get(session_name, True):
        return False, f"{session_name}_band_walk"
    if _is_dae_blocked_hour(now):
        return False, "blocked_hour_16_17"
    return True, "OK"


def _calc_above_upper_pct(price, bb_upper):
    """가격이 BB 상단에서 % 위에 있는 정도 (양수=위, 음수=아래/내부)"""
    if bb_upper is None or bb_upper <= 0:
        return None
    try:
        return (float(price) - float(bb_upper)) / float(bb_upper) * 100
    except (TypeError, ValueError):
        return None


def get_dae_threshold(peak_above_upper_pct, rsi_1h=None):
    """v39: peak이 BB 상단 위 거리별 drawdown 매도 임계값 반환

    Args:
        peak_above_upper_pct: peak이 BB 상단에서 % 위
        rsi_1h: 1H RSI (75+ 시 임계 한 단계 좁힘)

    Returns:
        (drawdown_pct: float | None, tier_tag: str)
        - None = 매도 X (감시만)
    """
    if peak_above_upper_pct is None or peak_above_upper_pct < 0:
        return None, ""

    # DAE_TIERS는 거리 큰 순서로 정렬되어 있음
    for distance_th, dd_th, tier_tag in DAE_TIERS:
        if peak_above_upper_pct >= distance_th:
            # ★ RSI 강한 천장 보너스: RSI 75+ 시 한 단계 더 좁힘
            if rsi_1h is not None and rsi_1h >= DAE_RSI_STRONG:
                # 임계를 70%로 좁힘 (예: 1.0 → 0.7)
                dd_th = dd_th * 0.7
            return dd_th, tier_tag

    return None, ""


def evaluate_dae_signal(ticker, profit_pct, current_price, peak_above_upper_pct,
                         get_candles_15m_fn, get_candles_1h_fn):
    """v39: BB 상단 위 거리별 가속 매도 신호 평가

    Args:
        ticker: 코인 티커
        profit_pct: 현재 수익률 (전체 매수가 대비)
        current_price: 현재가
        peak_above_upper_pct: zone 내 peak이 BB 상단에서 % 위로 멀어진 거리
        get_candles_*_fn: 캔들 조회 함수

    Returns:
        dict | None
        - {'signal': True, 'tier': 'T6', 'dd_threshold': 0.0, 'reason': '...'}
        - None = 매도 X
    """
    # 1. 최소 수익률 검증
    if profit_pct < DAE_MIN_PROFIT_PCT:
        return None

    # 2. 듀얼 컨펌: 1H BB Position + RSI
    df_1h = get_candles_1h_fn(ticker, count=3)
    if df_1h is None or len(df_1h) < 1:
        return None

    last_1h = df_1h.iloc[-1]
    bb_pos_1h = float(last_1h.get('bb_position', 50))
    rsi_1h = float(last_1h.get('rsi', 50))

    # 1H BB Position 95% 미만 = 큰 그림 천장 아님 → 매도 보류
    if bb_pos_1h < DAE_1H_BB_POS_TH:
        return None

    # RSI 60 미만 = 모멘텀 부족, 가짜 신호 → 매도 보류
    if rsi_1h < DAE_RSI_MIN:
        return None

    # 3. 거리별 임계값 조회
    dd_threshold, tier_tag = get_dae_threshold(peak_above_upper_pct, rsi_1h)
    if dd_threshold is None:
        return None  # 거리 부족, 감시만

    # 4. peak 대비 현재 drawdown 계산
    # peak_price를 알아야 정확. 여기서는 caller가 drawdown_from_peak를 전달해야 함
    # 따라서 이 함수는 임계값만 반환, drawdown 비교는 caller에서

    return {
        'tier': tier_tag,
        'dd_threshold': dd_threshold,
        'peak_above_upper_pct': peak_above_upper_pct,
        'bb_pos_1h': bb_pos_1h,
        'rsi_1h': rsi_1h,
        'category': f"BB_DAE_{tier_tag}",
    }
...
class TrendSellEngine:
    """
    [v39 매도 의사결정 체계 — BB 상단 위 거리별 가속 매도 통합]

    STEP 1: 시간대 분류 (v38 유지: 6 sessions)
    ...
    STEP 2: 보유 단계 분류 (v38 유지)
      - FRESH (시간대별 30~60분), EARLY (~6h), MID (~24h), MATURE (24h+)

    STEP 3: 시장 컨텍스트 (v38 유지: Squeeze + Spike Bar)

    STEP 4: 손절 트리 (최우선, v38 유지)
      ① 절대 -5%, ② BTC 위기, ③ 가속 손절, ④ -3% 2-Bar

    STEP 5: 분할 매도 (Laddered Exit, v38 유지)
      ⑤ TIER1 +2.5% 30%, ⑥ TIER2 +5.0% 30%

    STEP 5b: ★ v39 NEW — BB 상단 위 거리별 가속 매도 (DAE)
      ⑤b BB 상단 위 zone 진입 → peak_above_upper_pct 추적
         - T6: 위 4.0%+ → 즉시 매도 (다음 봉 -10.77% 가능)
         - T5: 위 2.5%+ → 0.3% drawdown
         - T4: 위 1.5%+ → 0.6% drawdown
         - T3: 위 0.7%+ → 1.0% drawdown
         - T2: 위 0.3%+ → 1.5% drawdown
         - T1 (위 0~0.3%): 감시만 (조기매도 회피)
      듀얼 컨펌: 1H BB Pos > 95% AND 1H RSI > 60
      RSI 75+ 시 임계 30% 좁힘 (강한 천장 보너스)
      시간대 비활성: VOLATILITY_SPIKE_KOREA(09시), KST 16:00~17:00
      FRESH 5분 유예 (호가 안정화)

    STEP 6: Spike Bar 유예 (v38 유지)

    STEP 7: 익절 트리 (v38 유지)
      ⑦ Ratchet Stop, ⑧ 천장 합의, ⑨ RSI 반전, ⑩ 트레일링, ⑪ EMA 추세

    STEP 8: 매도 사유 카테고리 + 매도 비율
      - PARTIAL_TAKE (0.30, 쿨다운 0)
      - BB_DAE_T2~T6 (1.0, 쿨다운 30~60)
      - RATCHET_STOP / STOPLOSS / 기타

    ★ 광현님 핵심 지시 충족:
      - "BB 상단 터치 즉시 매도(조기매도) 회피" → Tier 1 감시만
      - "BB 상단 뚫고 올라가면 민감히 모니터링" → in_above_zone 추적
      - "거리 멀수록 빨리 매도" → 5단계 가속 매트릭스
    """
    ...
    def register(self, ticker: str, buy_price: float, buy_time: float = None,
        ...
        """★ v39 변경: dae_state 추가 (BB 상단 위 zone 추적)
           ★ v38 유지: buy_bb_width, partial_exit_state, last_spike_time
        """
        if buy_time is None:
            buy_time = time.time()
        with self._lock:
            self.targets[ticker] = {
                ...
                "sold": False,
                # ★ v38 신규 필드
                "buy_bb_width": buy_bb_width,
                "partial_exit_state": {
                    "tier1_done": False,
                    "tier2_done": False,
                },
                "last_spike_time": 0,
                # ★ v39 신규 필드: BB 상단 위 zone 추적
                "dae_state": {
                    "in_above_zone": False,           # 현재 BB 상단 위에 있는가
                    "peak_above_upper_pct": 0.0,      # zone 내 peak이 BB 상단에서 % 위
                    "peak_price_in_zone": 0.0,        # zone 내 최고가
                    "zone_entry_time": 0,             # zone 진입 시각
                },
            }
    ...
    def check_sell_signal(self, ticker: str, current_price: float) -> dict:
        ...
        # ════════════════════════════════════════════════════════════
        # STEP 5b: ★ v39 NEW — BB 상단 위 거리별 가속 매도 (DAE)
        # 광현님 요청: "BB 상단 위로 멀리 올라갈수록 매도 빠르게"
        # ════════════════════════════════════════════════════════════
        dae_signal = self._evaluate_dae(ticker, current_price, profit_pct,
                                         elapsed_sec, session_name, ctx_str, info)
        if dae_signal is not None:
            return {**base, **dae_signal}
    ...
    def _evaluate_dae(self, ticker, current_price, profit_pct,
                       elapsed_sec, session_name, ctx_str, info):
        """★ v39 NEW: BB 상단 위 거리별 가속 매도 평가 (Distance-Adaptive Exit)
        ...
        의사결정 흐름:
          1) DAE 활성 여부 (시간대 + 차단 시간)
          2) FRESH 5분 호가 안정화 유예
          3) 최소 수익 (+1.0%) 검증
          4) 1H BB Position > 95% AND RSI > 60 듀얼 컨펌
          5) 현재 가격이 BB 상단 위에 있는지 확인
             - YES: peak_above_upper_pct 갱신, drawdown 평가
             - NO: zone 종료 처리 (peak 리셋)
          6) peak_above_upper_pct에 따른 drawdown 임계 조회
          7) 현재 drawdown ≥ 임계 → 매도 발화

        Returns:
            dict (sell signal) or None
        """
        # 1. DAE 활성 여부
        active, reason = is_dae_active(session_name)
        if not active:
            return None

        # 2. FRESH 호가 안정화 유예
        if elapsed_sec < DAE_FRESH_GRACE_SEC:
            return None
        ...
        # 4. 듀얼 컨펌 (1H BB + RSI)
        df_1h = get_candles_1h(ticker, count=3)
        if df_1h is None or len(df_1h) < 1:
            return None
        last_1h = df_1h.iloc[-1]
        bb_pos_1h = float(last_1h.get('bb_position', 50))
        ...
        if bb_pos_1h < DAE_1H_BB_POS_TH or rsi_1h < DAE_RSI_MIN:
            return None

        # 5. 현재 가격의 BB 상단 위 거리 (15분봉 기준)
        df_15m = get_candles_15m(ticker, count=3)
        if df_15m is None or len(df_15m) < 1:
            return None
        last_15m = df_15m.iloc[-1]
        bb_upper_15m = last_15m.get('bb_upper')
        if bb_upper_15m is None or pd.isna(bb_upper_15m) or bb_upper_15m <= 0:
            return None

        # current_price가 BB 상단에서 % 위
        current_above = _calc_above_upper_pct(current_price, bb_upper_15m)
        # 봉의 high도 평가 (실시간 WS의 경우 current_price가 high 근처일 수 있음)
        last_high = float(last_15m['high'])
        high_above = _calc_above_upper_pct(last_high, bb_upper_15m)

        # 6. dae_state 갱신 (zone 진입/peak 갱신/zone 종료)
        with self._lock:
            if ticker not in self.targets:
                return None
            dae_state = self.targets[ticker].setdefault("dae_state", {
                "in_above_zone": False,
                "peak_above_upper_pct": 0.0,
                "peak_price_in_zone": 0.0,
                "zone_entry_time": 0,
            })

            # zone 진입 판정 (current 또는 high 중 높은 것 기준)
            effective_above = max(
                current_above if current_above is not None else -999,
                high_above if high_above is not None else -999,
            )

            if effective_above > 0:
                # zone 안에 있음
                if not dae_state["in_above_zone"]:
                    dae_state["in_above_zone"] = True
                    dae_state["zone_entry_time"] = time.time()
                    dae_state["peak_above_upper_pct"] = effective_above
                    dae_state["peak_price_in_zone"] = max(current_price, last_high)
                    print(f"{Colors.YELLOW}[DAE감시시작] {ticker} BB상단위 진입 "
                          f"({effective_above:.2f}% 위){Colors.ENDC}")
                else:
                    # peak 갱신
                    if effective_above > dae_state["peak_above_upper_pct"]:
                        dae_state["peak_above_upper_pct"] = effective_above
                        dae_state["peak_price_in_zone"] = max(current_price, last_high)
            else:
                # zone 밖으로 나옴 → zone 종료 (단, drawdown 평가는 한 번 더)
                if dae_state["in_above_zone"] and (current_above is None or current_above < -0.5):
                    print(f"{Colors.YELLOW}[DAE감시종료] {ticker} BB상단아래로 복귀{Colors.ENDC}")
                    dae_state["in_above_zone"] = False
                    dae_state["peak_above_upper_pct"] = 0.0
                    dae_state["peak_price_in_zone"] = 0.0
                    return None

            peak_above = dae_state["peak_above_upper_pct"]
            peak_price_zone = dae_state["peak_price_in_zone"]

        # 7. 거리별 drawdown 임계 조회
        dd_threshold, tier_tag = get_dae_threshold(peak_above, rsi_1h)
        if dd_threshold is None:
            # 거리 부족 (Tier 1 = 0~0.3%) → 감시만, 매도 X
            return None

        # 8. 현재 drawdown 계산 (peak_price_zone 대비)
        if peak_price_zone <= 0:
            return None
        drawdown_from_peak = (peak_price_zone - current_price) / peak_price_zone * 100

        # 9. 매도 발화 평가
        if drawdown_from_peak >= dd_threshold:
            return {
                "signal": True,
                "sell_ratio": 1.0,  # BB DAE는 전량 매도
                "reason": (f"BB_DAE_{tier_tag}(상단위{peak_above:.2f}%피크→낙{drawdown_from_peak:.2f}% "
                           f"≥{dd_threshold:.1f}%, BB1H{bb_pos_1h:.0f}% RSI{rsi_1h:.0f}, "
                           f"수익{profit_pct:+.2f}%) {ctx_str}"),
                "sell_category": f"BB_DAE_{tier_tag}",
                "dae_tier": tier_tag,
                "dae_peak_above": peak_above,
                "dae_drawdown": drawdown_from_peak,
            }
...
def sell_thread_worker():
    ...
    """
    print(f"{Colors.YELLOW}[Thread 2] v39 매도 스레드 시작 ({SELL_THREAD_INTERVAL}초 주기){Colors.ENDC}")
    print(f"{Colors.YELLOW}  ├ Sell-Side (분할+DAE+Ratchet+Spike){Colors.ENDC}")
    ...
    print(f"{Colors.YELLOW}[Thread 2] v39 매도 스레드 종료{Colors.ENDC}")
# ═══════════════════════════════════════════════════════════════════════
