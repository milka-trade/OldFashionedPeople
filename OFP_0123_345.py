...
def evolution_77_buy_signal(df_15m, ticker):
    """
    v7.7 통합 매수 신호 (BOTTOM REVERSAL 우선)
    
    우선순위:
    1. BOTTOM REVERSAL (일봉 BB ≤30%)
    2. NORMAL (기존 v7.5 로직)
    ...
    Returns:
        dict: {
            ...
            'bb_position': BB 위치,
            'bb_width_pct': BB 폭%,
            'mode': 'EXTREME_BOTTOM' or 'BOTTOM' or 'NORMAL'
        }
    """
    try:
        if len(df_15m) < 20:
        ...
        # ========================================
        # Step 2: BOTTOM REVERSAL 체크 (우선순위 1)
        # ========================================
        if df_daily is not None:
            zone_info = get_bottom_reversal_zone(df_daily, df_15m)
            
            if zone_info is not None:
                # BOTTOM REVERSAL 진입 가능 구간
                
                # 안전장치 체크
                safety_pass, safety_reason = check_bottom_reversal_safety(df_daily, df_15m)
                
                if not safety_pass:
                    if DEBUG_MODE:
                        coin_name = ticker.replace('KRW-', '')
                        print(f"{Colors.YELLOW}[Bottom Safety] {coin_name}: {safety_reason}{Colors.ENDC}")
                    # 안전장치 실패 → NORMAL 모드로 진행
                else:
                    # 15분봉 BB 체크
                    if bb_position_15m <= zone_info['max_15m_bb']:
                        # 점수 계산
                        score_result = calculate_buy_score_bottom(df_15m, zone_info)
                        
                        if score_result['score'] >= zone_info['min_score']:
                            # BOTTOM REVERSAL 매수 신호!
                            reason = f"[{zone_info['zone']}] {score_result['score']:.0f}점\n"
                            reason += "\n".join(score_result['reasons'][:5])  # 상위 5개만
                            
                            return {
                                'signal': True,
                                'reason': reason,
                                'confidence': min(score_result['score'], 100),
                            ...
                            if DEBUG_MODE:
                                print(f"{Colors.YELLOW}[Bottom Score] {ticker}: {score_result['score']:.0f} < {zone_info['min_score']}{Colors.ENDC}")
                    else:
                        if DEBUG_MODE:
                            print(f"{Colors.YELLOW}[Bottom 15mBB] {ticker}: {bb_position_15m:.0f}% > {zone_info['max_15m_bb']}%{Colors.ENDC}")
        
        # ========================================
        # Step 3: NORMAL 모드 (기존 v7.5 로직)
        # ========================================
        
        # 일봉 필터 체크
        if df_daily is not None:
            filter_pass, filter_reason = check_daily_bb_filter(ticker)
        ...
        # 기존 v7.5 점수 계산
        score_info = calculate_buy_score_v75(df_15m)
        score = score_info['score']
        
        if score >= V75_BUY_MIN_SCORE:
            reason = f"[NORMAL] {score:.0f}점\n"
            reason += "\n".join(score_info['reasons'][:5])
        ...
        return {
            'signal': False,
            'reason': f'점수 부족 ({score:.0f}/{V75_BUY_MIN_SCORE})',
            'confidence': score,
            'entry_price': current_price,
            'bb_position': bb_position_15m,
            'bb_width_pct': bb_width_pct,
        ...
        if DEBUG_MODE:
            print(f"{Colors.RED}[v77 Buy Signal Error] {e}{Colors.ENDC}")
        traceback.print_exc()
...
def execute_buy(ticker, signal):
    ...
    try:
        with trade_lock:
            ...
            # Step 2: 목표 포지션 사이즈 (POSITION_SIZE_RATIO 파라미터 반영)
            ...
            if TEST_MODE:
                ...
                with held_coins_lock:
                    held_coins[ticker] = {
                        ...
                        'buy_mode': signal.get('mode', 'NORMAL')  # ✅ 신규 추가
                    }
...
def buy_thread_worker():
    ...
    [v7.7] BOTTOM REVERSAL 우선 매수 시스템
    """
    ...
    while not stop_event.is_set():
        try:
            ...
            for ticker in FIXED_STABLE_COINS:
                ...
                # ========================================
                # [v7.7] 통합 매수 신호 체크 (BOTTOM REVERSAL 우선)
                # ========================================
                ...
                if buy_signal['signal']:
                    ...
                    print(f"\n{Colors.CYAN}[BUY Thread] {coin_name} 매수 신호 [{mode}]!{Colors.ENDC}")
                    print(f"  신뢰도: {buy_signal.get('score', 0):.0f}점")
                    print(f"  15분봉 BB: {buy_signal['bb_position']:.1f}%")
                    
                    # BOTTOM REVERSAL 모드일 때 추가 정보 출력
                    if mode in ['EXTREME_BOTTOM', 'BOTTOM']:
                        daily_bb = buy_signal.get('daily_bb', 0)
                        print(f"  일봉 BB: {daily_bb:.1f}% ✅")
