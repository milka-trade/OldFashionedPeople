...
def trade_buy(ticker):
    ...
    최종 개선 매수 로직
    - 총자산의 10% 기반 동적 매수금액
    - 암호화폐 평가액 100만원 상한선 관리
    - 동적 적응형 조건 (변동성/시장국면 반영)
    - 신호 강도 기반 미세 조정
    - 다중 시간프레임 분석
    """
    ...
    def calculate_dynamic_thresholds(volatility, regime):
        """
        동적 임계값 계산
        - 변동성 높을수록 조건 강화
        - 상승장: 완화 / 하락장: 강화
        ...
        base_rsi_lower = 25
        base_rsi_upper = 50
        base_bb_threshold = 0.4
        
        # 변동성 조정 (0.7~1.5배)
        vol_factor = np.clip(volatility / 2.5, 0.7, 1.5)
        ...
        combined_factor = vol_factor * regime_factor
        
        return {
            'rsi_lower': max(15, base_rsi_lower * combined_factor),
            'rsi_upper': min(65, base_rsi_upper * (2 - combined_factor)),
            'bb_threshold': min(0.5, base_bb_threshold / combined_factor),
    ...
    def calculate_signal_strength(indicators, thresholds):
        ...
        if vol_ratio > 1.0:
            score += min(15, (vol_ratio - 1) * 30)
        
        # 5. 가격 모멘텀 점수 (0~10점)
        price_momentum = indicators['price_momentum']
        if 0 < price_momentum < 0.05:
            score += 10 * (price_momentum / 0.05)
        elif -0.02 < price_momentum < 0:
            # 약간의 하락도 허용 (반등 포착)
            score += 5
        
        return min(100, max(0, score))
    
    def calculate_position_size(signal_strength, total_asset_value, available_krw, available_space):
        """
        총자산 10% 기반 포지션 사이징 + 신호강도 미세 조정
        
        Parameters:
        - signal_strength: 신호 강도 (0~100)
        - total_asset_value: 총 자산 (암호화폐 + KRW)
        - available_krw: 사용 가능한 원화
        - available_space: 100만원 한도까지 남은 여유분
        """
        MIN_ORDER = 5000
        ...
        # 기본 매수금액 = 총자산의 10%
        base_position = total_asset_value * 0.10
        
        # 신호 강도에 따른 조정 (±20%)
        # 40점: -20% / 70점: 0% / 100점: +20%
        if signal_strength >= 70:
            strength_multiplier = 1.0 + (signal_strength - 70) / 150  # 1.0 ~ 1.2
        else:
            strength_multiplier = 0.8 + (signal_strength - 40) / 150  # 0.8 ~ 1.0
        
        adjusted_position = base_position * strength_multiplier
        
        # 제약 조건 적용
        position = min(adjusted_position, available_krw * 0.999, available_space)
        
        return position if position >= MIN_ORDER else 0
    ...
    print(f"💎 암호화폐 평가액: {crypto_value:,.0f}원")
    print(f"💰 KRW 잔고: {krw_balance:,.0f}원")
    print(f"📊 총 자산: {total_asset:,.0f}원")
    
    # ========== STEP 2: 100만원 상한선 체크 ==========
    CRYPTO_LIMIT = 1_000_000
    
    if crypto_value >= CRYPTO_LIMIT:
        limit_msg = f"⛔ 암호화폐 평가액이 한도에 도달했습니다!\n"
        limit_msg += f"   현재: {crypto_value:,.0f}원 / 한도: {CRYPTO_LIMIT:,.0f}원\n"
        limit_msg += f"   → 평가액이 {CRYPTO_LIMIT:,.0f}원 이하로 떨어질 때까지 매수 대기"
        print(limit_msg)
        send_discord_message(f"[매수중단] {ticker}\n{limit_msg}")
        return "Asset limit reached", None
    
    available_space = CRYPTO_LIMIT - crypto_value
    print(f"✅ 매수 가능 여유분: {available_space:,.0f}원")
    
    # ========== STEP 3: 기본 매수금액 계산 (총자산의 10%) ==========
    base_buy_amount = total_asset * 0.10
    print(f"💵 기본 매수금액 (총자산 10%): {base_buy_amount:,.0f}원")
    ...
    print(f"🌐 시장 국면: {market_regime.upper()}")
    
    # 변동성 계산
    volatility = calculate_market_volatility(closes_5m)
    print(f"📊 변동성 지수: {volatility:.2f}%")
    
    # 동적 임계값 계산
    thresholds = calculate_dynamic_thresholds(volatility, market_regime)
    print(f"🎯 동적 임계값:")
    print(f"   RSI: {thresholds['rsi_lower']:.1f} ~ {thresholds['rsi_upper']:.1f}")
    print(f"   BB: {thresholds['bb_threshold']:.2%}")
    print(f"   최소 안전점수: {thresholds['min_safety_score']}")
    ...
    # ========== STEP 8: 안전 검증 ==========
    safety_checks = {
        'RSI 과매도 회피': current_rsi > 15,
        'BB 밴드 이탈 방지': bb_position > -0.1,
        'EMA 지지 확인': closes_5m[-1] > ema_12 * 0.80,
        '극단 변동 회피': abs(price_momentum) < 0.2,
        '거래량 확인': volume_ratio > 0.5
    }
    ...
    basic_condition = (
        ...
        rsi_momentum > -2  # RSI가 급락하지 않음
    )
    
    # 신호 강도 조건
    signal_condition = signal_strength >= 40
    ...
    print(f"\n🔍 매수 조건 검토:")
    print(f"   기본 조건: {'✅' if basic_condition else '❌'}")
    print(f"   신호 강도: {'✅' if signal_condition else '❌'} ({signal_strength:.1f}점)")
    print(f"   안전 검증: {'✅' if safety_condition else '❌'} ({passed_checks}/{required_checks})")
    
    if not can_buy:
        fail_msg = f"❌ 매수 조건 미충족: {ticker}\n"
        if not basic_condition:
            fail_msg += f"   - RSI 범위 벗어남 ({current_rsi:.1f})\n"
        if not signal_condition:
            fail_msg += f"   - 신호 강도 부족 ({signal_strength:.1f}/40)\n"
    ...
    if buy_size < MIN_ORDER_AMOUNT:
        print(f"❌ 매수 금액 부족: {buy_size:.0f}원 < {MIN_ORDER_AMOUNT}원")
        return "Buy size too small", None
    
    # 신호강도 배율 계산
    if signal_strength >= 70:
        strength_mult = 1.0 + (signal_strength - 70) / 150
    else:
        strength_mult = 0.8 + (signal_strength - 40) / 150
    
    print(f"\n💵 최종 매수금액 계산:")
    print(f"   기본금액 (총자산 10%): {base_buy_amount:,.0f}원")
    print(f"   신호강도 배율: {strength_mult:.2f}x (강도 {signal_strength:.1f}점)")
    print(f"   조정 후: {base_buy_amount * strength_mult:,.0f}원")
    print(f"   최종 매수금액: {buy_size:,.0f}원")
    print(f"   (KRW 잔고: {krw_balance:,.0f}원 / 여유분: {available_space:,.0f}원 고려)")
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if price_change > 0.05:
                print(f"⚠️ 가격 급등 감지 ({price_change:+.2%}), 재확인 중...")
                time.sleep(2)
                continue
            ...
            # 매수 후 예상 평가액
            expected_crypto_value = crypto_value + buy_size
            expected_total_asset = expected_crypto_value + (krw_balance - buy_size)
            
            # 성공 메시지
            success_msg = f"✅ ★★★ 매수 성공 ★★★\n"
            success_msg += f"━━━━━━━━━━━━━━━━━━━━\n"
            success_msg += f"🪙 종목: {ticker}\n"
            success_msg += f"💰 매수가: {current_price:,.2f}원\n"
            success_msg += f"💵 매수금액: {buy_size:,.0f}원\n"
            success_msg += f"━━━━━━━━━━━━━━━━━━━━\n"
            success_msg += f"💪 신호강도: {signal_strength:.1f}/100 (배율 {strength_mult:.2f}x)\n"
            success_msg += f"📊 RSI: {current_rsi:.1f} (모멘텀: {rsi_momentum:+.1f})\n"
            success_msg += f"📉 BB위치: {bb_position:.1%}\n"
            success_msg += f"📈 거래량: {volume_ratio:.2f}x\n"
            success_msg += f"🌐 시장: {market_regime.upper()} / 변동성: {volatility:.1f}%\n"
            success_msg += f"━━━━━━━━━━━━━━━━━━━━\n"
            success_msg += f"💎 매수 전 총자산: {total_asset:,.0f}원\n"
            success_msg += f"   - 암호화폐: {crypto_value:,.0f}원\n"
            success_msg += f"   - KRW: {krw_balance:,.0f}원\n"
            success_msg += f"📊 매수 후 예상:\n"
            success_msg += f"   - 총자산: {expected_total_asset:,.0f}원\n"
            success_msg += f"   - 암호화폐: {expected_crypto_value:,.0f}원 / {CRYPTO_LIMIT:,.0f}원\n"
            success_msg += f"   - 한도 여유: {CRYPTO_LIMIT - expected_crypto_value:,.0f}원"
            ...
            print(f"⚠️ 매수 실행 오류 (시도 {attempt}): {e}")
