...
def evolution_80_sell_signal(df, buy_price, buy_time=None, held_info=None, df_5m=None, regime='NEUTRAL'):
    ...
    try:
        ...
        if engine_type == 'MOMENTUM':
            ...
            # A-3: 반등 실패 매도 [v25.1 수정: 홀드보호+진입BB 체크 추가]
            # 조건: BB 하단에서 매수한 게 아니라, 중간 이상에서 매수했는데
            #       BB 30% 이하로 재진입한 경우에만 반등실패 판정
            # 핵심: BB 하단 매수(entry_bb≤30%) 직후에는 BB가 여전히 ≤30%이므로
            #       이 경우 반등실패가 아니라 "아직 반등 전"임 → 홀딩해야 함
            if not in_hold_protection and buy_time and bb_position <= ENGA_SELL_BOUNCE_FAIL_BB:
                candles_held = (datetime.now() - buy_time).total_seconds() / 900
                # 진입 BB가 30% 초과였던 경우에만 (= BB가 올랐다가 다시 내려온 것)
                peak_bb = held_info.get('peak_bb_position', 0) if held_info else 0
                entry_bb_for_check = held_info.get('entry_bb_position', 50) if held_info else 50
                # 반등실패 = 진입BB > 30%(하단매수 아님) OR 피크BB > 40%(한번은 올랐었음)
                was_above = (entry_bb_for_check > ENGA_SELL_BOUNCE_FAIL_BB) or (peak_bb > 40)
                if was_above and candles_held <= ENGA_SELL_BOUNCE_FAIL_CANDLES and profit_pct < 0:
                    return {**base_response, 'signal': True,
                            'reason': (f'EngA_반등실패 (BB{bb_position:.0f}%≤{ENGA_SELL_BOUNCE_FAIL_BB}%, '
                                       f'{candles_held:.0f}봉, 진입BB{entry_bb_for_check:.0f}%, '
                                       f'피크BB{peak_bb:.0f}%, 수익{profit_pct:.2f}%)')}
            ...
            # [v25.0] 홀드 보호 구간이면 소진매도도 최소 수익 1.5% + BB 60%+ 필요
            if in_hold_protection:
                dyn_min = max(dyn_min, 1.5)

            if exhaustion['should_sell'] and ex_score >= ex_threshold and profit_pct >= dyn_min:
                # [v25.1] 홀드보호 중이면 BB 60% 이상에서만 소진매도 허용
                if in_hold_protection and bb_position < 60:
                    pass  # BB 상단 대기 → 소진매도 스킵
                else:
                    return {**base_response, 'signal': True,
...
# [v25.1 정리] evolution_76_sell_signal, evolution_70_sell_signal 래퍼 제거
# → 모든 매도는 evolution_80_sell_signal() 단일 함수로 통합
