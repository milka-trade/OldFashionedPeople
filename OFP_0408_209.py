...
def find_best_bollinger_ratio(ticker, ratios=[i / 100 for i in range(6, 11)], window=20, std_dev=2.5):
    ...
    Args:
        ...
        ratios (list): 볼린저 밴드 비율 리스트 (기본값: 0.03 ~ 0.1, 0.01 간격).
        window (int): 볼린저 밴드 윈도우 사이즈 (기본값: 20).
        std_dev (float): 볼린저 밴드 표준 편차 배수 (기본값: 2.5).
        rsi_period (int): RSI 기간 (기본값: 14).
        rsi_lower (int): RSI 매수 하한값 (기본값: 25).
        rsi_upper (int): RSI 매수 상한값 (기본값: 35).
    ...
    best_ratio = None
    best_return = 0  # 누적수익률 초기값은 0이 아닌 1로 설정해야 곱셈 연산에 영향이 없습니다.
    ...
    for ratio_threshold in ratios:
        ...
        # 백테스팅
        cumulative_return = 1.0  # 초기 누적 수익률 (곱셈을 위해 1로 초기화)
        ...
        for i in range(1, len(df) - 1): # 마지막 봉은 제외 (다음 봉 데이터 필요)
            ...
            #     band_diff = 0  # 또는 적절한 다른 값으로 설정
            # else:
            #     band_diff = (upper_band.iloc[i] - lower_band.iloc[i]) / lower_band.iloc[i]
...
def get_best_ticker():
    ...
    excluded_tickers = ["KRW-QI", "KRW-ONX", "KRW-ETHF", "KRW-ETHW", "KRW-PURSE", "KRW-BTC", "KRW-USDT", "KRW-BERA", "KRW-VTHO", "KRW-SBD", "KRW-JTO", "KRW-SCR", "KRW-VIRTUAL", "KRW-SOLVE", "KRW-IOST", "KRW-HIFI", "KRW-WAL", "KRW-ORCA", "KRW-CRO", "KRW-LOOM", "KRW-ARKM", "KRW-KAITO", "KRW-COW", "KRW-TRUMP"]  # 제외할 코인 리스트
    balances = upbit.get_balances()
    held_coins = []
