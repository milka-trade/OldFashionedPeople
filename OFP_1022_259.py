...
#     """
#     🚀 적응형 매수 시스템 v7.0 - 멀티타임프레임 기대값 혁신
    
#     🔥 핵심 개선 (10만원→10억 로직):
#     1. 15분봉 기반 실제 수익 잠재력 계산 (BB 폭 2~3배)
#     2. 동적 손실 계수 (과매도 구간 0.6, 일반 0.9)
#     3. 리스크 레벨별 차등 기대값 임계값
#     4. Kelly Criterion 승률×손익비 검증
#     5. 상용 자동매매 로직 적용: "저점 매수, 고점 매도"의 확률론적 접근
#     """    
#     # ===== STEP 1: 자산 현황 =====
...
#                 if volatility_1h > 0.08:
#                     fail_reason = "변동성폭발"
#                     fail_counts['변동성폭발'] += 1
            
#             # 🔥 [개선] 5분봉 반등확률로 1차 필터링 (기준 완화 48% → 45%)
#             if fail_reason is None:
#                 rebound = ind.get('rebound_potential')
#                 if rebound and rebound['probability'] < 0.45:
...
#     print(f"  └─ BB폭: 5m({ind['bb_5m_width']:.4f}) 15m({ind['bb_15m_width']:.4f})")
...
#     if rebound_5m:
#         print(f"  └─ 반등예측(5m): 확률({rebound_5m['probability']*100:.0f}%) 예상수익({rebound_5m['expected_gain']:.1f}%)")
#     if rebound_15m:
#         print(f"  └─ 반등예측(15m): 확률({rebound_15m['probability']*100:.0f}%) 예상수익({rebound_15m['expected_gain']:.1f}%)")
    
#     # ===== STEP 4: 🔥 혁신적 손실방지 시스템 =====
#     print("\n[STEP 4] 🔥 멀티타임프레임 기대값 분석")
...
#     # 🔥 [2단계] 혁신적 기대값 계산 - 멀티타임프레임
#     print("\n【2단계: 멀티타임프레임 기대값】")
    
#     # 🔥 [혁신 1] 15분봉 기반 실제 수익 잠재력 사용
...
#     expected_gain_15m = rebound_15m['expected_gain'] if rebound_15m else 0
    
#     # 🔥 타임프레임 가중 평균 (15분봉 70%, 5분봉 30%)
#     rebound_prob = rebound_prob_15m * 0.7 + rebound_prob_5m * 0.3
#     expected_gain = expected_gain_15m  # 수익은 15분봉 기준
    
#     loss_prob = 1 - rebound_prob
    
#     # 🔥 [혁신 2] 동적 손실 계수 - 과매도 구간 손실확률 낮음
#     if rsi_5m < 20 and bb_pos_5m < 0.20:
#         volatility_factor = 0.6  # 극과매도: 손실 60%만 반영
#         regime = "극과매도"
#     elif rsi_5m < 25 and bb_pos_5m < 0.25:
#         volatility_factor = 0.75  # 강과매도: 75% 반영
#         regime = "강과매도"
#     elif rsi_5m < 30 and bb_pos_5m < 0.30:
#         volatility_factor = 0.85  # 과매도: 85% 반영
#         regime = "과매도"
#     else:
#         volatility_factor = 0.95  # 일반: 95% 반영
#         regime = "일반"
    
#     # 🔥 [혁신 3] 15분봉 BB 위치 기반 손실 계산 (더 현실적)
#     expected_loss = bb_pos_15m * 8 * volatility_factor  # 5분봉 10→15분봉 8로 조정
    
#     # 기대값 = (수익 × 확률) - (손실 × 확률)
#     expected_value = (expected_gain * rebound_prob) - (expected_loss * loss_prob)
    
#     print(f"  └─ 시장상태: {regime} (손실계수 {volatility_factor:.2f})")
#     print(f"  └─ 반등확률: {rebound_prob*100:.1f}% (15m:{rebound_prob_15m*100:.0f}% / 5m:{rebound_prob_5m*100:.0f}%)")
#     print(f"  └─ 손실확률: {loss_prob*100:.1f}%")
#     print(f"  └─ 예상수익: +{expected_gain:.2f}% (15분봉 기준)")
#     print(f"  └─ 예상손실: -{expected_loss:.2f}% (BB{bb_pos_15m*100:.0f}% × 계수{volatility_factor:.2f})")
...
#     # 🔥 리스크별 필요 기대값
#     if risk_score >= 85:
#         min_expected_value = 0.2  # 초안전: 0.2% 이상
#         risk_path = "ULTRA_SAFE"
#         path_name = "🟢 초안전"
#         stop_loss_pct = 1.8
#     elif risk_score >= 75:
#         min_expected_value = 0.35  # 안전: 0.35% 이상
#         risk_path = "SAFE"
#         path_name = "🟡 안전"
#         stop_loss_pct = 2.2
#     elif risk_score >= 65:
#         min_expected_value = 0.5  # 균형: 0.5% 이상
#         risk_path = "BALANCED"
#         path_name = "🟠 균형"
#         stop_loss_pct = 2.8
#     else:
#         print(f"\n  └─ ❌ 리스크 점수 부족 ({risk_score}점 < 65점)")
#         send_discord_message(f"{selected_ticker} 리스크과다 ({risk_score}점)")
#         return "Risk score too low", None
    
#     print(f"\n【리스크 분석】")
#     print(f"  └─ RSI 안전성: {rsi_risk}/30점")
#     print(f"  └─ BB 안전성: {bb_risk}/30점")
#     print(f"  └─ 반등 확실성: {rebound_risk}/25점")
#     print(f"  └─ 일봉 안정성: {day_risk}/15점")
#     print(f"  └─ 총점: {risk_score}/100점 → {path_name}")
#     print(f"  └─ 필요 기대값: {min_expected_value:.2f}%")
    
#     # 🔥 기대값 검증
#     if expected_value < min_expected_value:
#         print(f"\n  └─ ❌ 기대값 부족 ({expected_value:.2f}% < {min_expected_value:.2f}%)")
...
#     # 🔥 [혁신 5] Kelly Criterion 검증 (상용 자동매매 핵심)
#     print("\n【Kelly Criterion 검증】")
    
#     # 승률 × 평균수익 vs 패율 × 평균손실
#     win_expectation = rebound_prob * expected_gain
#     loss_expectation = loss_prob * expected_loss
    
#     # Kelly 비율 = (승률 × 손익비 - 패율) / 손익비
#     if expected_loss > 0:
...
#     print(f"  └─ 승률×수익: {rebound_prob*100:.1f}% × {expected_gain:.2f}% = {win_expectation:.3f}")
#     print(f"  └─ 패율×손실: {loss_prob*100:.1f}% × {expected_loss:.2f}% = {loss_expectation:.3f}")
#     print(f"  └─ 손익비: {profit_loss_ratio:.2f}:1")
#     print(f"  └─ Kelly 비율: {kelly_ratio:.3f}")
    
#     if kelly_ratio < 0.05:
#         print(f"\n  └─ ⚠️ Kelly 비율 부족 ({kelly_ratio:.3f} < 0.05) → 우위 미약")
#         # Kelly는 경고만, 거부하지 않음
...
#     # 🔥 [개선] 필수 조건 완화
#     critical_conditions = {
#         f'반등확률 {(0.50 if risk_score >= 75 else 0.55)*100:.0f}%+': rebound_prob >= (0.50 if risk_score >= 75 else 0.55),
#         'BB위치 42% 이하': bb_pos_5m <= 0.42,
...
#     # 특별 보너스
#     if bb_pos_5m < 0.18 and rebound_prob >= 0.65 and rsi_5m < 22:
...
def trade_buy(ticker=None):
    """
    🚀 적응형 매수 시스템 v7.5 - 저점 민감도 최적화
    
    🔥 핵심 개선 (실전 최적화):
    1. 적응형 임계값: 과매도 구간 기준 70% 완화
    2. 누적 신호 점수: "아깝게 실패" 케이스 구제
    3. 저점 확신 가중치: 복합 저점 지표 시 기대값 부스트
    4. 소프트 거부: 경고 후 재평가 (즉시 거부 방지)
    5. 빈도 우선: 60% 승률 × 5회 > 70% 승률 × 1회
    """    
    # ===== STEP 1: 자산 현황 =====
    krw_balance = get_krw_balance(upbit)
    crypto_value = get_total_crypto_value(upbit)
    ...
    if ticker is None:
        ...
        for t in candidates:
            ...
            # 🛡️ [Hard Filter 1] 극단 RSI 차단 (기준 완화 15→12)
            if ind['rsi_5m'] < 12:  # 🔥 개선: 15→12
                fail_reason = "극단RSI"
                fail_counts['극단RSI'] += 1
            
            # 🛡️ [Hard Filter 2] 일봉 급등 제한 (기준 완화 4→5)
            elif ind['daily_change_from_open'] > 5.0:  # 🔥 개선: 4→5
                fail_reason = "일봉급등"
                fail_counts['일봉급등'] += 1
            ...
            # 🔥 [개선] 반등확률 기준 대폭 완화 (45% → 40%)
            if fail_reason is None:
                rebound = ind.get('rebound_potential')
                if rebound and rebound['probability'] < 0.40:  # 🔥 개선: 45→40
                    fail_reason = f"반등확률{rebound['probability']*100:.0f}%"
                    fail_counts['반등확률부족'] += 1
    ...
    # ===== STEP 4: 🔥 저점 민감도 최적화 시스템 =====
    print("\n[STEP 4] 🔥 저점 민감도 최적화 분석")
    ...
    # 🛡️ [1단계] Hard Filter 재확인 (완화된 기준)
    print("\n【1단계: Hard Filter】")
    hard_filters = {
        '극단RSI 회피': rsi_5m >= 12,  # 🔥 15→12
        'BB 절대한계': bb_pos_5m <= 0.45,  # 🔥 0.42→0.45
        '일봉 안정': ind['daily_change_from_open'] <= 5.0,  # 🔥 4→5
        '변동성 통제': abs(ind.get('price_reversal', {}).get('price_momentum', 0)) <= 0.08
    }
    ...
    # 🔥 [2단계] 저점 확신도 계산 (새로운 시스템)
    print("\n【2단계: 저점 확신도 분석】")
    
    confidence_score = 0
    confidence_factors = []
    
    # 🔥 복합 저점 지표 1: RSI 멀티타임프레임
    if rsi_5m < 25 and rsi_15m < 30:
        confidence_score += 20
        confidence_factors.append("멀티TF과매도")
    elif rsi_5m < 28 and rsi_15m < 33:
        confidence_score += 15
        confidence_factors.append("TF과매도")
    elif rsi_5m < 30:
        confidence_score += 10
        confidence_factors.append("단기과매도")
    
    # 🔥 복합 저점 지표 2: BB 포지션
    if bb_pos_5m < 0.20 and bb_pos_15m < 0.25:
        confidence_score += 20
        confidence_factors.append("멀티TF하단")
    elif bb_pos_5m < 0.25 and bb_pos_15m < 0.30:
        confidence_score += 15
        confidence_factors.append("TF하단")
    elif bb_pos_5m < 0.30:
        confidence_score += 10
        confidence_factors.append("단기하단")
    
    # 🔥 복합 저점 지표 3: 거래량 증가 (공포 매도 흡수)
    if vol_ratio >= 2.0:
        confidence_score += 15
        confidence_factors.append("거래량폭발")
    elif vol_ratio >= 1.5:
        confidence_score += 10
        confidence_factors.append("거래량증가")
    
    # 🔥 복합 저점 지표 4: 극단 조합
    if rsi_5m < 22 and bb_pos_5m < 0.22:
        confidence_score += 25
        confidence_factors.append("🔥극단저점")
    
    print(f"  └─ 저점 확신도: {confidence_score}/80점")
    if confidence_factors:
        print(f"  └─ 확신 요소: {', '.join(confidence_factors)}")
    
    # 🔥 확신도별 부스트 계수
    if confidence_score >= 60:
        confidence_multiplier = 1.30  # 30% 부스트
        confidence_level = "🔥 극고확신"
    elif confidence_score >= 45:
        confidence_multiplier = 1.20  # 20% 부스트
        confidence_level = "💪 고확신"
    elif confidence_score >= 30:
        confidence_multiplier = 1.10  # 10% 부스트
        confidence_level = "👍 확신"
    else:
        confidence_multiplier = 1.0
        confidence_level = "😐 보통"
    
    print(f"  └─ 확신 수준: {confidence_level} (부스트 {confidence_multiplier:.0%})")
    ...
    # 🔥 저점 확신도 부스트 적용
    rebound_prob = min(rebound_prob_raw * confidence_multiplier, 0.85)  # 최대 85%
    expected_gain = expected_gain_15m * confidence_multiplier
    
    loss_prob = 1 - rebound_prob
    
    # 동적 손실 계수
    if rsi_5m < 20 and bb_pos_5m < 0.20:
        volatility_factor = 0.55  # 🔥 0.6→0.55
        regime = "극과매도"
    elif rsi_5m < 25 and bb_pos_5m < 0.25:
        volatility_factor = 0.70  # 🔥 0.75→0.70
        regime = "강과매도"
    elif rsi_5m < 30 and bb_pos_5m < 0.30:
        volatility_factor = 0.80  # 🔥 0.85→0.80
        regime = "과매도"
    else:
        volatility_factor = 0.90  # 🔥 0.95→0.90
        regime = "일반"
    
    # 손실 계산
    expected_loss = bb_pos_15m * 8 * volatility_factor
    ...
    print(f"  └─ 부스트 반등확률: {rebound_prob*100:.1f}% (×{confidence_multiplier:.2f})")
    print(f"  └─ 손실확률: {loss_prob*100:.1f}%")
    print(f"  └─ 원시 예상수익: +{expected_gain_15m:.2f}%")
    print(f"  └─ 부스트 예상수익: +{expected_gain:.2f}% (×{confidence_multiplier:.2f})")
    print(f"  └─ 예상손실: -{expected_loss:.2f}%")
    print(f"  └─ 🎯 기대값: {expected_value:+.2f}%")
    
    # 🔥 [4단계] 리스크 분석 및 적응형 임계값
    print("\n【4단계: 리스크 분석】")
    ...
    print(f"  └─ 반등 확실성: {rebound_risk}/25점")
    print(f"  └─ 일봉 안정성: {day_risk}/15점")
    print(f"  └─ 총점: {risk_score}/100점")
    
    # 🔥 [혁신 1] 적응형 임계값 - 저점일수록 완화
    if risk_score >= 85:
        min_expected_value = 0.15  # 🔥 0.2→0.15
        risk_path = "ULTRA_SAFE"
        path_name = "🟢 초안전"
        stop_loss_pct = 1.8
    elif risk_score >= 75:
        min_expected_value = 0.25  # 🔥 0.35→0.25
        risk_path = "SAFE"
        path_name = "🟡 안전"
        stop_loss_pct = 2.2
    elif risk_score >= 65:
        min_expected_value = 0.35  # 🔥 0.5→0.35
        risk_path = "BALANCED"
        path_name = "🟠 균형"
        stop_loss_pct = 2.8
    elif risk_score >= 60:  # 🔥 새로운 구간
        min_expected_value = 0.45
        risk_path = "AGGRESSIVE"
        path_name = "🟠 공격"
        stop_loss_pct = 3.2
    else:
        print(f"\n  └─ ❌ 리스크 점수 부족 ({risk_score}점 < 60점)")
        send_discord_message(f"{selected_ticker} 리스크과다 ({risk_score}점)")
        return "Risk score too low", None
    ...
    bonus_applied = False
    original_ev = expected_value
    
    # 보너스 1: "아깝게 실패" 구제
    if min_expected_value * 0.85 <= expected_value < min_expected_value:
        gap = min_expected_value - expected_value
        bonus = gap * 1.5
        expected_value += bonus
        print(f"  └─ 🎁 근접 보너스: +{bonus:.3f}% (격차 {gap:.3f}%)")
        bonus_applied = True
    
    # 보너스 2: 고리스크 + 저기대값 조합
    if risk_score >= 80 and expected_value >= min_expected_value * 0.90:
        bonus = min_expected_value * 0.15
        expected_value += bonus
        print(f"  └─ 🎁 고안전 보너스: +{bonus:.3f}%")
        bonus_applied = True
    
    # 보너스 3: 극단 저점 특별 보너스
    if confidence_score >= 60:
        bonus = min_expected_value * 0.20
        expected_value += bonus
        print(f"  └─ 🎁 극단저점 보너스: +{bonus:.3f}%")
        bonus_applied = True
    
    if bonus_applied:
        print(f"  └─ 📊 기대값: {original_ev:.3f}% → {expected_value:.3f}%")
    else:
        print(f"  └─ 📊 보너스 없음 (기대값 {expected_value:.2f}%)")
    
    # 🔥 기대값 최종 검증
    if expected_value < min_expected_value:
        # 🔥 [혁신 3] 소프트 거부 (경고 후 재평가)
        deficit = min_expected_value - expected_value
        
        if deficit <= 0.08 and confidence_score >= 30:  # 격차 0.08% 이하 + 확신 있음
            print(f"\n  └─ ⚠️ 기대값 미달 ({expected_value:.3f}% < {min_expected_value:.2f}%)")
            print(f"  └─ 🔄 소프트 패스: 격차 {deficit:.3f}% + 확신도 {confidence_score}점")
            print(f"  └─ ✅ 예외적 진입 허용")
        else:
            print(f"\n  └─ ❌ 기대값 부족 ({expected_value:.3f}% < {min_expected_value:.2f}%)")
            print(f"  └─ ❌ 격차 과다: {deficit:.3f}% (허용: 0.08%)")
            send_discord_message(f"{selected_ticker} 기대값부족 ({expected_value:.2f}%)")
            return "Expected value too low", None
    ...
    if kelly_ratio < 0.03:  # 🔥 0.05→0.03
        print(f"  └─ ⚠️ Kelly 비율 낮음 ({kelly_ratio:.3f}) but 진행")
    ...
    # 🔥 [개선] 필수 조건 대폭 완화
    critical_conditions = {
        f'반등확률 {(0.45 if risk_score >= 75 else 0.50)*100:.0f}%+': rebound_prob >= (0.45 if risk_score >= 75 else 0.50),  # 🔥 0.50→0.45
        'BB위치 45% 이하': bb_pos_5m <= 0.45,  # 🔥 0.42→0.45
        'BB전환 또는 반등': (slope and slope['is_flattening']) or (reversal and reversal['is_reversing'])
    }
    ...
    if not all(critical_conditions.values()):
        # 🔥 [혁신 4] 2개 중 1개 실패는 경고만
        failed_count = sum(1 for passed in critical_conditions.values() if not passed)
        
        if failed_count == 1 and confidence_score >= 45:
            print(f"\n  └─ ⚠️ 필수 조건 1개 미달 but 확신도 {confidence_score}점")
            print(f"  └─ 🔄 예외적 진입 허용")
        else:
            print("\n  └─ ❌ 필수 조건 미충족 (복수 실패)")
            send_discord_message(f"{selected_ticker} 필수조건 미충족")
    ...
    if selected_score < min_score:
        # 🔥 격차 5점 이하 + 확신도 높으면 통과
        score_gap = min_score - selected_score
        
        if score_gap <= 5 and confidence_score >= 40:
            print(f"\n  └─ ⚠️ 신호 점수 근접 ({selected_score:.0f}점 vs {min_score}점, 격차 {score_gap}점)")
            print(f"  └─ 🔄 확신도 {confidence_score}점으로 보완 → 진입 허용")
        else:
            print(f"\n  └─ ❌ 신호 점수 부족 ({selected_score:.0f}점 < {min_score}점)")
            send_discord_message(f"{selected_ticker} 신호부족 ({selected_score}/{min_score}점)")
            return "Signal score insufficient", None
    ...
    # 특별 보너스
    if bb_pos_5m < 0.18 and rebound_prob >= 0.60 and rsi_5m < 22:  # 🔥 0.65→0.60
        print("\n  └─ 🔥 극단 기회: BB극하단 + 극과매도 + 고확률반등!")
        send_discord_message(f"🔥 {selected_ticker} 극단기회!")
    ...
    for attempt in range(1, 3):
        try:
            ...
            if price_change > 0.03:  # 🔥 0.025→0.03 (급등 허용폭 확대)
                print(f"  └─ ⚠️ 가격 급등 감지 ({price_change*100:.1f}%), 재확인 중...")
                time.sleep(2)
                continue
            ...
            success_msg += f"금액: {buy_size:,.0f}원 ({buy_size/total_asset*100:.1f}%)\n"
            success_msg += f"신호: {selected_score:.0f}점 | 리스크: {risk_score}점\n"
            success_msg += f"확신도: {confidence_score}점 ({confidence_level})\n"
            success_msg += f"반등확률: {rebound_prob*100:.0f}% | 기대값: +{expected_value:.2f}%\n"
            success_msg += f"손익비: {profit_loss_ratio:.2f}:1 | Kelly: {kelly_ratio:.3f}\n"
            success_msg += f"🛡️ 손절가: {stop_loss_price:,.2f}원 (-{stop_loss_pct:.1f}%)\n"
            
            if bonus_applied:
                success_msg += f"💰 보너스 적용: {original_ev:.2f}% → {expected_value:.2f}%\n"
