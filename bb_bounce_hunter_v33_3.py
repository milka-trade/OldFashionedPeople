...
# -*- coding: utf-8 -*-
"""
BB Bounce Hunter v33.2 — Price Predictor v5.1 매수+매도 양방향 연동
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
v33.1 기반 + 가격예측 모듈 매도 타이밍 연동 (v33.2)
 
★ v33.2 신규: 매도 예측기 연동 (4개 개입 포인트)
  [매도]
  1. Step 1 손절 타이트닝: 예측 SELL → 동적 손절선 +0.5%p 상향
  2. Step 2.7 가속 익절 (신규): 예측 SELL + 수익≥0.7% → 즉시 매도
  3. Step 3 안전익절 변조: 예측 SELL→BB조건완화, BUY→BB조건강화
  4. Step 3.5 홀드 보호 (신규): 예측 BUY → 안전익절 기준 +0.3%p
  5. Step 4 트레일 변조: 예측 SELL→간격 타이트, BUY→간격 확장 (상한 제한)
  
  ★ 안전 원칙:
  - Step 0(가속손절), Step 2(강제익절), Step 5(시간청산)은 예측기 영향 없음
  - 예측기 오류/타임아웃 시 기존 로직 100% 유지 (fail-safe)
  - 각 개입 모드 개별 ON/OFF 스위치

★ v33.1: 매수 예측기 연동 (기존 유지)
...
# ── v33.2 신규: 매도 예측기 연동 파라미터 ──────────────────────────────────
PREDICTOR_SELL_ENABLED        = True    # 매도 예측기 연동 마스터 스위치
PRED_SELL_STOP_TIGHTEN        = True    # Step 1: SELL 예측 시 손절선 강화
PRED_SELL_STOP_TIGHTEN_PCT    = 0.5     # 손절선 상향폭 (%p) — -3% → -2.5%
PRED_SELL_ACCEL_EXIT          = True    # Step 2.7: SELL 예측 가속 익절
PRED_SELL_ACCEL_MIN_PROFIT    = 0.7     # 가속 익절 최소 수익률 (%)
PRED_SELL_SAFE_MODIFY         = True    # Step 3: 안전익절 조건 변조
PRED_SELL_BB_EASE             = 55      # SELL 예측 시 BB 하한 완화
PRED_BUY_BB_STRICT            = 80      # BUY 예측 시 BB 하한 강화
PRED_BUY_HOLD_PROTECT         = True    # Step 3.5: BUY 예측 홀드 보호
PRED_BUY_SAFE_PROFIT_BOOST    = 0.3     # 홀드 보호 시 안전익절 기준 상향 (%p)
PRED_SELL_TRAIL_MODIFY        = True    # Step 4: 트레일링 간격 변조
PRED_SELL_TRAIL_TIGHTEN       = 0.1     # SELL 예측 → 트레일 간격 축소 (%p)
PRED_BUY_TRAIL_EXPAND         = 0.2     # BUY 예측 → 트레일 간격 확장 (%p)
PRED_BUY_TRAIL_MAX_GAP        = 1.5     # BUY 예측 트레일 확장 상한 (절대값 %)
...
def sell_signal(df, buy_price, buy_time=None, held_info=None):
    """
    ★★★ v33.2 매도 신호 — 탄력 트레일링 + 동적 목표 + 시간 기반 청산 + 가격예측 연동
 
    v33.1 대비 변경사항:
    ① Step 1: 예측 SELL → 동적 손절선 타이트닝 (+0.5%p)
    ② Step 2.7 신규: 예측 SELL + 수익≥0.7% → 가속 익절
    ③ Step 3: 예측 SELL→BB조건 완화, BUY→BB조건 강화
    ④ Step 3.5 신규: 예측 BUY → 안전익절 기준 +0.3%p (홀드 보호)
    ⑤ Step 4: 예측 SELL→트레일 타이트, BUY→트레일 확장 (상한 제한)
    ★ 안전 원칙: Step 0, 2, 2.5, 5는 예측기 영향 없음
 
    ════════════════════════════════════════════════════════
    Step 0: 가속 손절 (5분봉 — 최우선, 예측기 무관)
    Step 1: 동적 손절 (BB Width 연동) ← ★예측 SELL 타이트닝
    Step 2: 강제 익절 (5.0% — 긴급 안전밸브, 예측기 무관)
    Step 2.5: 과매수 조기 익절 (RSI>70+BB>70, 예측기 무관)
    Step 2.7: ★ 예측 가속 익절 (SELL+수익≥0.7%)
    Step 3: 안전 익절 (BBW 연동 동적 목표) ← ★예측 BB변조
    Step 3.5: ★ 예측 홀드 보호 (BUY→익절기준 상향)
    Step 4: ★ 탄력 트레일링 (구간별 자동 조정) ← ★예측 간격변조
    Step 5: ★ 시간 기반 청산 (예측기 무관)
    ════════════════════════════════════════════════════════
    """
    try:
        if df is None or len(df) < 5:
        ...
        # ★★★ v33.2: 매도 예측기 컨텍스트 조회 (fail-safe: 기본값 = 변조 없음)
        pred_ctx = {'signal': 'NEUTRAL', 'confidence': 'LOW', 'ok': False,
                    'stop_adjust': 0.0, 'safe_profit_adjust': 0.0,
                    'safe_bb_override': None, 'trail_adjust': 0.0,
                    'accel_exit': False, 'tag': ''}
        if ticker and PREDICTOR_SELL_ENABLED:
            try:
                pred_ctx = get_sell_prediction_context(ticker)
            except Exception:
                pass  # fail-safe: 예측기 오류 시 기존 로직 유지
 
        pred_tag = pred_ctx['tag']
 
        # ════════════════════════════════════════
        # Step 0: 가속 손절 (최우선, 예측기 무관)
        # ════════════════════════════════════════
        ...
        # ════════════════════════════════════════
        # Step 1: 동적 손절 (보유시간 무시) ← ★예측 SELL 타이트닝
        # ════════════════════════════════════════
        # ★ v33.2: 예측 SELL이면 손절선을 상향 (더 빨리 손절)
        _adjusted_stop = _dynamic_stop
        if pred_ctx['ok'] and pred_ctx['stop_adjust'] > 0:
            _adjusted_stop = _dynamic_stop + pred_ctx['stop_adjust']
            with predictor_stats_lock:
                predictor_stats['sell_tighten_count'] += 1
 
        if profit_pct <= _adjusted_stop:
            stop_tag = ""
            if _adjusted_stop != _dynamic_stop:
                stop_tag = f" 예측SELL강화({_dynamic_stop:.1f}→{_adjusted_stop:.1f}%)"
            return {**base, 'signal': True,
                    'reason': f'동적손절({profit_pct:.2f}%≤{_adjusted_stop:.1f}% '
                              f'BBW{entry_bbw:.1f}%{stop_tag})[{grade_str}]{pred_tag}'}
        ...
        # ════════════════════════════════════════
        # Step 2: 강제 익절 (v32: 5.0% — 긴급 안전밸브, 예측기 무관)
        # ════════════════════════════════════════
        ...
        # ════════════════════════════════════════
        # ★ Step 2.7: v33.2 예측 가속 익절 (SELL + 수익≥0.7%)
        # 예측기가 SELL이고 수익이 최소 기준 이상이면 즉시 매도
        # ════════════════════════════════════════
        if (pred_ctx['ok'] and pred_ctx['accel_exit'] and
                profit_pct >= PRED_SELL_ACCEL_MIN_PROFIT):
            with predictor_stats_lock:
                predictor_stats['sell_accel_count'] += 1
            return {**base, 'signal': True,
                    'reason': (f'예측가속익절({profit_pct:.2f}%≥{PRED_SELL_ACCEL_MIN_PROFIT}% '
                               f'예측SELL {pred_ctx["confidence"]})[{grade_str}]{pred_tag}')}
 
        # ════════════════════════════════════════
        # Step 3: 안전 익절 (v32: BBW 연동 동적 목표) ← ★예측 BB변조
        # ════════════════════════════════════════
        # ★ v33.2: 예측기에 따른 BB 하한 오버라이드
        _effective_bb_min = _safe_bb_min
        if pred_ctx['ok'] and pred_ctx['safe_bb_override'] is not None:
            _effective_bb_min = pred_ctx['safe_bb_override']
 
        # ★ v33.2: 예측 BUY → 안전익절 기준 상향 (홀드 보호)
        _effective_safe_profit = _safe_profit
        if pred_ctx['ok'] and pred_ctx['safe_profit_adjust'] > 0:
            _effective_safe_profit = _safe_profit + pred_ctx['safe_profit_adjust']
            with predictor_stats_lock:
                predictor_stats['sell_protect_count'] += 1
 
        if profit_pct >= _effective_safe_profit:
            rsi_dropping = current['rsi'] < prev['rsi']
            bb_high      = bb_pos >= _effective_bb_min
            ...
            if rsi_dropping and bb_high:
                reasons = [f"RSI↓{current['rsi']:.0f}", f"BB{bb_pos:.0f}%≥{_effective_bb_min}%"]
                if _5m_confirm:
                    reasons.append("5mRSI↓")
                if _5m_srsi_dead:
                    reasons.append("5mSRSI데드")
                if _effective_bb_min != _safe_bb_min:
                    reasons.append(f"BB변조({_safe_bb_min}→{_effective_bb_min})")
                if _effective_safe_profit != _safe_profit:
                    reasons.append(f"목표변조({_safe_profit:.1f}→{_effective_safe_profit:.1f}%)")
                return {**base, 'signal': True,
                        'reason': f'안전익절({profit_pct:.2f}%≥{_effective_safe_profit:.1f}% {"+".join(reasons)})[{grade_str}]{pred_tag}'}
 
            if _5m_srsi_dead and profit_pct >= _effective_safe_profit * 1.1:
                if df_5m is not None:
                    _5m_bb = df_5m.iloc[-1].get('bb_position', 50)
                    if _5m_bb >= 70:
                        return {**base, 'signal': True,
                                'reason': (f'SRSI데드익절({profit_pct:.2f}% '
                                           f'5mSRSI데드 5mBB{_5m_bb:.0f}%)[{grade_str}]{pred_tag}')}
 
            if _5m_confirm and profit_pct >= _effective_safe_profit * 1.2:
                if df_5m is not None:
                    _5m_bb = df_5m.iloc[-1].get('bb_position', 50)
                    if _5m_bb >= 75:
                        return {**base, 'signal': True,
                                'reason': (f'5m안전익절({profit_pct:.2f}%, '
                                           f'5mBB{_5m_bb:.0f}% 5mRSI↓)[{grade_str}]{pred_tag}')}
 
        # ════════════════════════════════════════
        # ★ Step 4: v32.0 탄력 트레일링 ← ★v33.2 예측 간격변조
        ...
        if held_info:
            ...
            if peak_price > buy_price:
                ...
                if trail_gap is not None:
                    # ★ v33.2: 예측기에 따른 트레일 간격 변조
                    original_gap = trail_gap
                    if pred_ctx['ok'] and pred_ctx['trail_adjust'] != 0:
                        # trail_gap은 음수, trail_adjust는 양수=확장/음수=타이트
                        adjusted_abs = abs(trail_gap) + pred_ctx['trail_adjust']
                        # 상한 제한: BUY 예측이어도 절대값 PRED_BUY_TRAIL_MAX_GAP 초과 불가
                        adjusted_abs = min(adjusted_abs, PRED_BUY_TRAIL_MAX_GAP)
                        # 하한 보호: 최소 0.1%는 유지
                        adjusted_abs = max(adjusted_abs, 0.1)
                        trail_gap = -adjusted_abs
                        with predictor_stats_lock:
                            predictor_stats['sell_trail_mod_count'] += 1
                    ...
                    if drawdown_from_peak >= abs(trail_gap):
                        ...
                        trail_mod_tag = ""
                        if trail_gap != original_gap:
                            trail_mod_tag = f" 예측변조({abs(original_gap):.1f}→{abs(trail_gap):.1f}%)"
                        return {**base, 'signal': True,
                                'reason': (f'탄력트레일(고점{peak_profit:.1f}%→{profit_pct:.1f}%, '
                                           f'-{drawdown_from_peak:.1f}%≥{abs(trail_gap):.1f}%'
                                           f'{_5m_tag}{trail_mod_tag})[{grade_str}]{pred_tag}')}
 
        # ════════════════════════════════════════
        # ★ Step 5: v32.0 시간 기반 청산 (예측기 무관)
        # ════════════════════════════════════════
        if buy_time:
            # 16시간 후 + 수익 > 0 → 시장가 청산
        ...
        stop_info = f" | 손절{_adjusted_stop:.1f}%"
        night_tag = " [야간]" if _entry_type == 'night_momentum' else ""
        time_tag = f" | {elapsed_hours:.1f}h" if buy_time else ""
        target_tag = f" | 목표{_effective_safe_profit:.1f}%"
        pred_hold_tag = f" {pred_tag}" if pred_tag else ""
        base['reason'] = (f'홀드(수익{profit_pct:.2f}%, BB{bb_pos:.0f}%'
                          f'{trail_info}{stop_info}{target_tag}{time_tag}{night_tag}{pred_hold_tag})[{grade_str}]')
        return base
