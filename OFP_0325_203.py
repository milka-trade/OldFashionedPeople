...
def filtered_tickers(tickers):
    ...
    for t in tickers:
        try:
            ...
            filtering_message += f"[cond1: {is_increasing_15}] band_diff15: {band_diff15[-1]} > {band_diff_15_margin} \n"
...
def get_best_ticker():
    ...
    for ticker in filtered_list:   # 조회할 코인 필터링
        stoch_RsiS = stoch_rsiS(ticker, window = 7, interval = min15)
...
def trade_sell(ticker):
    ...
    if cut_start <= cut_time <= cut_end:      # 매도 제한시간이면
        if cut_price :
            ...
            cutTimemsg += f"srsi_d15_7: {srsi_d[1]:,.2f} >> {srsi_d[2]:,.2f} > srsi_k15_7: {srsi_k[1]:,.2f} >> {srsi_k[2]:,.2f} \n \n"
            print(cutTimemsg)
