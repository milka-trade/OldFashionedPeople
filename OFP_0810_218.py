...
def filtered_tickers(tickers):
    ...
    for t in tickers:
        try:
            ...
            filtering_message += f"[bonus: {bonus_score}/6] RSI모멘: {rsi_momentum} | 거래량: {volume_surge}/{volume_above_avg} | 추세: {price_above_ma} | 변동성: {good_volatility} | 반등: {bounce_signal}\n"
...
def trade_buy(ticker):
    ...
    if market_rsi < 30:      # 극도 과매도
        ...
        risk_level = "LOW"
    elif market_rsi < 40:    # 과매도
        buy_ratio = 0.8      # 보통 매수
        risk_level = "MEDIUM"
    elif market_rsi < 50:    # 중립
        buy_ratio = 0.6      # 보수 매수
        risk_level = "MEDIUM"
    else:                    # 과매수
        buy_ratio = 0.4      # 최소 매수
