...
def get_user_input():
    ...
    return min_rate, sell_time, rsi_sell_s, rsi_sell_e, max_rate, band_diff_margin

# 함수 호출 및 결과 저장
min_rate, sell_time, rsi_sell_s, rsi_sell_e, max_rate, band_diff_margin = get_user_input()
...
def trade_buy(ticker):
    ...
    MIN_THRESHOLD = 200000  # 20만원
    if krw >= MIN_THRESHOLD:
        # 20만원 이상: 절반과 20만원 중 큰 금액
        half_amount = krw * 0.5
        buy_size = max(half_amount, MIN_THRESHOLD)
    else:
        # 20만원 미만: 전량 매수
