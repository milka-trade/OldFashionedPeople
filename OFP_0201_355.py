...
VERSION = "8.1 RECOVERY_HUNTER"  # 버전 변경

print(f"\n{Colors.BOLD}{Colors.CYAN}{'='*60}")
print(f"EVOLUTION {VERSION}")
print(f"{'='*60}")
print(f"{Colors.GREEN}[v8.1] RECOVERY HUNTER - 반등 초기 포착 강화{Colors.ENDC}")
print(f"   [매수] 트리거우선 → 일봉상태별 동적기준 → 모멘텀")
print(f"   [신규] CRASH_RECOVERY 트리거 (급락후반등)")
print(f"   [신규] 반등초기 보너스 +25점")
print(f"   [변경] 일봉양봉시 모멘텀기준 40점으로 완화")
print(f"")
print(f"{Colors.YELLOW}핵심 철학{Colors.ENDC}")
print(f"   '일봉 양봉 = 큰 방향 OK' → 15분봉 기준 완화")
print(f"   '반등 초기를 놓치지 않는다'")
print(f"")
...
def evolution_80_buy_signal(df_15m, df_5m, ticker):
    ...
    try:
        ...
        if not trigger['triggered']:
            base_response['reason'] = f"트리거대기 ({trigger['reason']})"
            return base_response
        ...
        daily_info = check_daily_momentum_confirmed(df_daily)
        base_response['daily_bb'] = daily_info.get('daily_bb', 50)
        
        daily_bullish = daily_info.get('is_bullish', False)
        ...
        if momentum['score'] < min_momentum:
            base_response['reason'] = f"모멘텀부족 ({momentum['score']}점 < {min_momentum}점, 일봉:{daily_status})"
        ...
        reason_parts = [
            f"트리거:{trigger['type']}",
            f"일봉:{daily_status}",
            f"모멘텀{momentum['score']}점/{min_momentum}점",
            f"BB{bb_position:.0f}%"
        ]
        ...
        return {
            ...
            'mode': 'MOMENTUM_V81',
            'market_condition': daily_status,
            'score': momentum['score'],
            'daily_bb': daily_info.get('daily_bb', 50),
            'momentum_score': momentum['score'],
            'trigger_type': trigger['type'],
            'is_recovery_start': momentum.get('is_recovery_start', False)
        }
        
    except Exception as e:
        if DEBUG_MODE:
            print(f"{Colors.RED}[v8.1 Buy Signal Error] {e}{Colors.ENDC}")
            traceback.print_exc()
