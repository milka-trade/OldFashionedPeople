...
ENGA_SELL_QUICK_PROFIT = 1.5              # 빠른 익절 수익 기준 (%)
ENGA_SELL_QUICK_BB_MIN = 55               # 빠른 익절 BB 기준 (%)
ENGA_SELL_MAX_PROFIT = 2.0                # 무조건 익절 수익률 (%)
ENGA_SELL_TRAILING_ACTIVATION = 1.0       # 트레일링 활성화 수익 (%)
ENGA_SELL_TRAILING_DISTANCE = 0.7         # 트레일링 거리 (%)
ENGA_SELL_BOUNCE_FAIL_CANDLES = 4         # 반등 실패 판정 봉 수
...
def evolution_80_sell_signal(df, buy_price, buy_time=None, held_info=None, df_5m=None, regime='NEUTRAL'):
    ...
    try:
        ...
        if engine_type == 'MOMENTUM':
            ...
            # ─── ENGINE A 전용 매도 로직 (BOUNCE) ──────────────────────
            
            # A-1: 빠른 익절 (BB 55%+ 도달 + 수익 1.5%+)
            ...
            # A-3: 반등 실패 매도 (매수 후 4봉 이내 BB 30% 이하 재진입)
        ...
        try:
            ...
            for ticker in active_coins:
                ...
                if buy_signal and buy_signal.get('signal', False):
                    ...
                    print(f"\n{Colors.CYAN}{'='*50}")
                    print(f"[BUY SIGNAL] {coin_name} {engine_emoji}{engine_tag} 매수!{time_tag}{dyn_tag}")
                    ...
                    if success:
                        print(f"{Colors.GREEN}[BUY] {coin_name} {engine_emoji}{engine_tag} 매수 완료!{time_tag}{dyn_tag}{Colors.ENDC}")
                    else:
