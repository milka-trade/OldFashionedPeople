...
def filtered_tickers(tickers):
    """🛡️ 천재적 리스크 관리 통합 시스템 - 진짜 바닥 vs 가짜 바닥 구분"""
       
    filtered_tickers = []
    selected_details = []
    
    # === 🚨 1단계: 시장 패닉 감지 시스템 ===
    def detect_market_panic():
        """실시간 시장 패닉 상황 감지"""
        major_tickers = ['KRW-BTC', 'KRW-ETH', 'KRW-XRP', 'KRW-ADA', 'KRW-SOL', 
                        'KRW-DOGE', 'KRW-AVAX', 'KRW-DOT', 'KRW-MATIC', 'KRW-LINK']
        
        panic_signals = {
            'market_crash_rate': 0,      # 전체 시장 급락률
            'sync_decline_ratio': 0,     # 동시 하락 비율
            'btc_flash_crash': False,    # BTC 급락
            'volume_explosion': 0,       # 거래량 폭발
            'panic_score': 0             # 종합 패닉 점수
        }
        ...
        for ticker in major_tickers:
            try:
                ...
                # 급락률 체크 (5분 내)
                if len(closes) >= 2:
                    decline_rate = (closes[-2] - closes[-1]) / closes[-2]
                    total_decline += decline_rate
                    
                    if decline_rate > 0.03:  # 3% 이상 하락
                        decline_count += 1
                        
                        # BTC 특별 체크
                        if ticker == 'KRW-BTC' and decline_rate > 0.03:
                            panic_signals['btc_flash_crash'] = True
                ...
                time.sleep(0.05)  # 빠른 체크
                
            except Exception:
                continue
        
        # 패닉 지표 계산
        analyzed_count = len(major_tickers) - decline_count  # 정상 분석된 개수
        if analyzed_count > 0:
            panic_signals['market_crash_rate'] = total_decline / analyzed_count
            panic_signals['sync_decline_ratio'] = decline_count / analyzed_count
            
        if volume_ratios:
            panic_signals['volume_explosion'] = np.mean(volume_ratios)
        
        # 🚨 종합 패닉 점수 계산 (0-100)
        panic_score = 0
        if panic_signals['market_crash_rate'] > 0.05:  # 평균 5% 이상 하락
            panic_score += 40
        elif panic_signals['market_crash_rate'] > 0.03:
            panic_score += 25
            
        if panic_signals['sync_decline_ratio'] > 0.8:  # 80% 이상 동시 하락
            panic_score += 30
        elif panic_signals['sync_decline_ratio'] > 0.6:
            panic_score += 15
            
        if panic_signals['btc_flash_crash']:
            panic_score += 20
            
        if panic_signals['volume_explosion'] > 3.0:  # 거래량 3배 이상
            panic_score += 10
            
        panic_signals['panic_score'] = panic_score
        
        return panic_signals
    
    # 🔍 패닉 상황 체크
    print("🚨 시장 패닉 감지 중...")
    panic_info = detect_market_panic()
    
    # 패닉 상황 판단
    is_market_panic = panic_info['panic_score'] >= 60
    is_caution_needed = panic_info['panic_score'] >= 30
    
    if is_market_panic:
        print(f"🚨 **시장 패닉 감지** (점수: {panic_info['panic_score']:.0f}) - 거래 중단!")
        print(f"   └ 시장급락: {panic_info['market_crash_rate']:.1%} | 동시하락: {panic_info['sync_decline_ratio']:.1%}")
        print(f"   └ BTC급락: {'Yes' if panic_info['btc_flash_crash'] else 'No'} | 거래량폭발: {panic_info['volume_explosion']:.1f}배")
        print("💡 시장 안정화까지 대기하는 것이 안전합니다.")
        return []
    
    # === 🎯 2단계: 바닥 확신도 측정 시스템 ===
    def calculate_bottom_confidence(ticker):
        """바닥 확신도 계산 (0-100점)"""
        try:
            ...
            confidence_details = {
                'technical_score': 0,    # 기술적 신호 (40점 만점)
                'structural_score': 0,   # 구조적 신호 (40점 만점)  
                'psychological_score': 0 # 심리적 신호 (20점 만점)
            }
            ...
            try:
                ...
                if rsi_data is not None and not rsi_data.empty:
                    ...
                    if len(rsi_data) >= 3:
                        rsi_trend = rsi_data.values[-3:]
                        # RSI 25 이하 + 상승 전환
                        if current_rsi <= 25 and rsi_trend[-1] > rsi_trend[-2]:
                            confidence_details['technical_score'] += 15
                        elif current_rsi <= 30 and rsi_trend[-1] > rsi_trend[-2]:
                            confidence_details['technical_score'] += 10
                        elif current_rsi <= 35:
                            confidence_details['technical_score'] += 5
            except Exception:
                pass
            
            # 2. 볼린저밴드 이탈 후 복귀 (15점)
            try:
                bands_df = get_bollinger_bands(ticker, interval=min5, window=20, std_dev=2.0)
                if bands_df is not None and not bands_df.empty and len(bands_df) >= 3:
                    lower_bands = bands_df['Lower_Band'].values[-3:]
                    recent_closes = closes[-3:]
                    
                    # 하단 이탈 후 복귀 패턴 체크
                    if (recent_closes[0] < lower_bands[0] and  # 이탈
                        recent_closes[-1] > lower_bands[-1]):    # 복귀
                        confidence_details['technical_score'] += 15
                    elif current_price <= lower_bands[-1] * 1.02:  # 하단 근처
                        confidence_details['technical_score'] += 8
            except Exception:
                pass
            ...
            if len(lows) >= 10:
                ...
                low_prices = lows[-10:]
                support_count = sum(1 for low in low_prices if abs(low - recent_low) / recent_low < 0.02)
                
                if support_count >= 3:
                    confidence_details['structural_score'] += 15
                elif support_count >= 2:
                    confidence_details['structural_score'] += 8
            
            # 2. 시장 전체 과매도와의 동조화 (15점)
            market_rsi = panic_info.get('market_rsi', 50)  # 앞서 계산된 시장 RSI 사용
            if market_rsi <= 30:
                confidence_details['structural_score'] += 15
            elif market_rsi <= 35:
                confidence_details['structural_score'] += 10
            elif market_rsi <= 40:
                confidence_details['structural_score'] += 5
            
            # 3. 하락 추세 둔화 (10점)
            if len(closes) >= 6:
                recent_slope = np.polyfit(range(6), closes[-6:], 1)[0]
                older_slope = np.polyfit(range(6), closes[-12:-6], 1)[0] if len(closes) >= 12 else recent_slope
                
                # 하락 속도 둔화 또는 횡보 전환
                if recent_slope > older_slope and recent_slope > -0.5:
                    confidence_details['structural_score'] += 10
                elif recent_slope > -0.2:  # 거의 횡보
                    confidence_details['structural_score'] += 5
            ...
            if len(closes) >= 15:
                ...
                # 3일 이상 하락 후 안정화
                if decline_days >= 6 and closes[-1] >= closes[-2]:  # 6캔들 = 30분
                    confidence_details['psychological_score'] += 10
                elif decline_days >= 3 and closes[-1] >= closes[-2]:
                    confidence_details['psychological_score'] += 5
            
            # 2. 패닉 이후 안정화 (10점)
            if panic_info['panic_score'] <= 20:  # 패닉 상황이 아님
                confidence_details['psychological_score'] += 10
            elif panic_info['panic_score'] <= 40:
                confidence_details['psychological_score'] += 5
            
            # === 최종 바닥 확신도 계산 ===
            total_confidence = (confidence_details['technical_score'] + 
                              confidence_details['structural_score'] + 
                              confidence_details['psychological_score'])
            
            return total_confidence, confidence_details
            
        except Exception:
            return 0, {}
    
    # === 🎯 3단계: 적응형 매수 시스템 ===
    def make_adaptive_decision(ticker, confidence_score, confidence_details, panic_score):
        """적응형 매수 결정"""
        
        # 기본 안전 조건
        if confidence_score < 40:
            return False, "LOW_CONFIDENCE", f"확신도 부족 ({confidence_score:.0f}점)"
        
        # 패닉 상황별 기준 조정
        if panic_score >= 30:  # 주의 상황
        ...
        # 매수 결정 및 등급
        if confidence_score >= required_confidence + 20:
            return True, f"{tier_prefix} PERFECT", f"완벽한 바닥 확신 ({confidence_score:.0f}점)"
        elif confidence_score >= required_confidence + 10:
            return True, f"{tier_prefix} EXCELLENT", f"우수한 바닥 신호 ({confidence_score:.0f}점)"
        elif confidence_score >= required_confidence:
            return True, f"{tier_prefix} GOOD", f"안전한 바닥 진입 ({confidence_score:.0f}점)"
        else:
            return False, "INSUFFICIENT", f"기준점 미달 ({confidence_score:.0f}/{required_confidence}점)"
    
    # === 🚀 메인 분석 루프 ===
    print(f"🔍 시장 상황: {'⚠️ 주의' if is_caution_needed else '✅ 정상'} (패닉점수: {panic_info['panic_score']:.0f})")
    print(f"📊 {len(tickers)}개 티커 바닥 확신도 분석 시작...")
    print("=" * 80)
    ...
    for i, ticker in enumerate(tickers):
        try:
            ...
            # 바닥 확신도 계산
            confidence_score, confidence_details = calculate_bottom_confidence(ticker)
            ...
            if confidence_score > 0:
                ...
                # 매수 결정
                should_buy, tier, reason = make_adaptive_decision(
                    ticker, confidence_score, confidence_details, panic_info['panic_score']
                )
                ...
                if should_buy:
                    ...
                    detail = {
                        ...
                        'tier': tier,
                        'reason': reason,
                        'technical_score': confidence_details['technical_score'],
                        'structural_score': confidence_details['structural_score'],
                        'psychological_score': confidence_details['psychological_score'],
                        'panic_adjusted': is_caution_needed
                    }
                    selected_details.append(detail)
                    
                    # 실시간 결과 출력
                    filtering_time = datetime.now().strftime('%H:%M:%S')
                    print(f"🎯 **{tier}**: {ticker} ({confidence_score:.0f}점) - {filtering_time}")
                    print(f"   └ 기술:{confidence_details['technical_score']:.0f} | 구조:{confidence_details['structural_score']:.0f} | 심리:{confidence_details['psychological_score']:.0f}")
                    print(f"   └ {reason}")
                    print("-" * 60)
    ...
    # === 🆘 긴급 상황 대응 ===
    if len(filtered_tickers) == 0 and high_confidence_count >= 1 and not is_market_panic:
        print("🆘 **긴급 기회 확보** - 기준 일시 완화...")
        
        # 상위 확신도 코인들 재검토
        emergency_candidates = []
        for ticker in tickers[:30]:  # 상위 30개만 재분석
            try:
                confidence_score, confidence_details = calculate_bottom_confidence(ticker)
                if confidence_score >= 35:  # 완화된 기준
        ...
        if emergency_candidates:
            best_candidate = max(emergency_candidates, key=lambda x: x['confidence_score'])
            
            filtered_tickers.append(best_candidate['ticker'])
            selected_details.append({
                'ticker': best_candidate['ticker'],
                'confidence_score': best_candidate['confidence_score'],
                'tier': '🆘 EMERGENCY',
                'reason': f"긴급 기회 확보 ({best_candidate['confidence_score']:.0f}점)",
                'technical_score': best_candidate['details']['technical_score'],
                'structural_score': best_candidate['details']['structural_score'],
                'psychological_score': best_candidate['details']['psychological_score'],
                'panic_adjusted': True
            })
            
            print(f"🆘 **긴급 기회**: {best_candidate['ticker']} ({best_candidate['confidence_score']:.0f}점)")
    
    # === 📊 최종 결과 요약 ===
    selection_rate = (len(filtered_tickers) / analyzed_count * 100) if analyzed_count > 0 else 0
    confidence_rate = (high_confidence_count / analyzed_count * 100) if analyzed_count > 0 else 0
    
    print("=" * 100)
    print(f"🎯 **분석 완료**: {analyzed_count}개 분석 → {len(filtered_tickers)}개 선정 ({selection_rate:.1f}%)")
    print(f"📈 **고확신**: {high_confidence_count}개 발견 ({confidence_rate:.1f}%)")
    print(f"🚨 **시장 상황**: 패닉점수 {panic_info['panic_score']:.0f} | {'위험' if is_market_panic else '주의' if is_caution_needed else '안전'}")
    
    if len(filtered_tickers) > 0:
        print(f"🛡️ **안전한 바닥 진입 기회 {len(filtered_tickers)}개 확보!**")
        
        # 디스코드 알림
        summary_msg = f"🛡️ 안전 진입 기회 {len(filtered_tickers)}개!\n"
        summary_msg += f"패닉점수: {panic_info['panic_score']:.0f} | 확신도: {confidence_rate:.1f}%\n"
        
        for detail in selected_details:
            summary_msg += f"{detail['ticker']} {detail['tier']} ({detail['confidence_score']:.0f}점)\n"
        
        summary_msg += f"{datetime.now().strftime('%m/%d %H:%M:%S')} 완료"
        
        try:
            send_discord_message(summary_msg)
            print("📱 안전 진입 알림 전송 완료")
        except Exception as e:
            print(f"📱 알림 전송 실패: {e}")
    else:
        if is_market_panic:
            print("🚨 시장 패닉 상황으로 안전을 위해 거래를 중단합니다.")
        elif high_confidence_count > 0:
            print(f"💡 {high_confidence_count}개 관심 종목 발견, 추가 관찰 필요")
        else:
            print("💡 현재 안전한 바닥 진입 기회가 부족합니다.")
        
        print("🛡️ 안전 제일 - 더 나은 기회를 기다리는 중...")
