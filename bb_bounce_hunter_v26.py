...
def calculate_coin_status_for_report(ticker):
    try:
        ...
        if df_15m is not None and len(df_15m) >= 20:
            ...
            rsi15 = c.get('rsi', 50); srsi_k = c.get('srsi_k', 50)
...
# ============================================================================
# SECTION 13: ★★★ 매수 신호 v26.0 — 듀얼 타임프레임 + 크래시 리커버리
# ============================================================================
...
def buy_signal(ticker):
    """
    ★★★ v26.0 매수 신호 — 듀얼 타임프레임 + 크래시 리커버리

    Phase A: 자격 검증 (15분봉 BB 하단 + BB 폭 + 일봉)
    Phase B: 반등 확인 (15분봉 점수 + 5분봉 점수 듀얼)
      매수: 15m≥2 AND 5m≥2, 또는 합산≥5
    ...
    try:
        ...
        if not is_bull:
            ...
            phase_a_fail_reason = f"일봉거부: {daily_reason}"

        # A-2: BB 하단 위치
        if phase_a_passed and bb_pos > _bb_max:
            phase_a_passed = False
        ...
        return {
            'signal': True,
            'reason': (f"[{grade}]BB{bb_pos:.0f}% 폭{bb_width:.1f}% | "
                       f"15m({score_15m}점:{d15}) 5m({score_5m}점:{d5}) "
                       f"합{total_score} | {daily_reason}"),
            'entry_price': current['close'],
            'bb_position': bb_pos,
            'bb_width_pct': bb_width,
            'market_grade': grade,
...
# ============================================================================
# SECTION 17: 매수 스레드 (v26.0: 듀얼 타임프레임)
# ============================================================================
    ...
    while not stop_event.is_set():
        try:
            ...
            for ticker in FIXED_STABLE_COINS:
                ...
                if sig['signal']:
                    coin_name = ticker.replace('KRW-', '')
                    entry_tag = "🔄CR" if sig.get('entry_type') == 'crash_recovery' else "📈"
                    print(f"\n{Colors.CYAN}{'='*55}")
