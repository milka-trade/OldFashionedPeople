...
def get_rsi_bear_diver(ticker, period=14, interval='minute5', lookback=20, min_data_points=50):
    ...
    Returns:
        dict: {
            ...
            'current_price': float,         # 현재 가격
            'current_rsi': float,          # 현재 RSI
            'signal_strength': float,      # 신호 강도 (0-100) - 제거됨
            'price_high': float,           # 가격 고점
            'rsi_high': float,            # RSI 고점
            'divergence_bars': int,        # 다이버전스 발생 구간
            'overbought_level': bool,     # 과매수 구간 여부
            'rsi_declining': bool         # RSI 하락 추세 여부
        }
    """
    try:
        # 충분한 데이터 확보를 위해 더 많은 캔들 가져오기
        ...
        divergence_bars = 0
        overbought_level = current_rsi > 70
        ...
        for i in range(lookback, min(len(df), lookback + 10)):  # 최대 10개 구간 검사
            ...
            # 3. RSI가 과매수 구간(70 이상)에 있거나 있었음
            # 4. RSI가 70 아래로 떨어지기 시작했음
            
            price_condition = current_price >= price_max * 0.98  # 2% 허용 오차
            rsi_condition = current_rsi < rsi_max - 3  # RSI 3포인트 이상 하락
            overbought_condition = rsi_max > 70  # 과매수 구간에서 시작
            rsi_declining = current_rsi < 70  # 과매수 구간에서 벗어남
            
            if price_condition and rsi_condition and overbought_condition:
                is_divergence = True
        ...
        # RSI 하락 추세 확인
        rsi_declining_trend = False
        if is_divergence and len(df) >= 3:
            recent_rsi = df['rsi'].tail(3).values
            rsi_declining_trend = recent_rsi[-3] > recent_rsi[-2] > recent_rsi[-1]
        
        # 볼륨 확인을 위한 추가 조건 (선택사항)
        volume_confirmation = False
        if len(df) >= 5:
            recent_volume = df['volume'].tail(5).mean()
            prev_volume = df['volume'].iloc[-10:-5].mean() if len(df) >= 10 else recent_volume
            volume_confirmation = recent_volume > prev_volume * 1.1  # 거래량 10% 증가
        ...
        return {
            ...
            'rsi_declining': rsi_declining_trend,
            'volume_confirmation': volume_confirmation,
            'timestamp': datetime.now()
        }
    
    except Exception as e:
        print(f"Error in get_rsi_bear_diver: {e}")
        return None
...
def filtered_tickers(tickers):
    ...
    for t in tickers:
        try:
            ...
            is_increasing = band_diff[-1] > band_diff_margin
            ...
            # rsiD = ta_rsiD.values
            # rsiD_range =  25 < rsiD[-1] < 75 # rsiD[-2] < rsiD[-1] and
            ...
            # filtering_message += f"[cond1: {rsiD_range}] 30 < rsiD: {rsiD[-2]:,.2f} > {rsiD[-1]:,.2f} < 70 \n"
            ...
            # filtering_message += f"[cond2: {is_increasing}] band_diff: {band_diff[-1]:,.4f} > BD_Margin: {band_diff_margin} \n"
            filtering_message += f"[cond3: {rsi_range}] {rsi_buy_s} < rsi: {rsi[-2]:,.2f} > {rsi[-1]:,.2f} < {rsi_buy_e} \n"
...
def trade_sell(ticker):
    ...
    rsi_decling = (rsi_sell_s <= rsi[-1] <= rsi_sell_e) and rsi[-3] > rsi[-2] > rsi[-1]

    bear_result = get_rsi_bear_diver(ticker)

    if bear_result is None:
        return {"signal": "NO_DATA", "message": "데이터를 가져올 수 없습니다"}
    
    is_sell_signal = bear_result['is_bearish_divergence']
    ...
    if profit_rate >= min_rate:
        while attempts < max_attempts:       
            ...
            if profit_rate > max_rate or (is_sell_signal and rsi_decling):
                sell_order = upbit.sell_market_order(ticker, buyed_amount)
                sellmsg = f"[!!목표가달성!!]:[{ticker}] / 수익률: {profit_rate:,.2f}%  / 현재가: {cur_price:,.1f} \n"
                sellmsg += f"sell_signal:{is_sell_signal} / bear_result: {bear_result['is_bearish_divergence']} \n"
                sellmsg += f"RSI: {rsi[-3]:,.3f} >> {rsi[-2]:,.3f} >> {rsi[-1]:,.3f} \n"
                print(sellmsg)
        ...
        #     if sell_price:
                ...
                cut_message += f"RSI: {rsi[-3]:,.3f} >> {rsi[-2]:,.3f} >> {rsi[-1]:,.3f} \n"
                print(cut_message)
