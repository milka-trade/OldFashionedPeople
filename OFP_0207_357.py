...
# === v13.0 일봉 모멘텀 설정 ===
DAILY_MOMENTUM_ENABLED = True          # 일봉 모멘텀 체크 활성화
DAILY_LOOKBACK = 3                      # 일봉 확인 기간 (최근 3일)
DAILY_BULLISH_CHANGE_MIN = 0.0          # 양봉 진행 중 최소 조건 (현재가 >= 시가)
DAILY_ATR_MIN_RATIO = 0.5              # 일봉 ATR 최소 변동성 비율 (%)
...
# ========================================
# [v10.0] BOUNCE HUNTER Parameters
# ========================================
...
def calculate_momentum_strength(df_15m, df_daily=None):
    """
    [v10.0] 보고서용 모멘텀 강도 측정 (0-100점)
    
    ⚠️ 매수 판단과 분리됨 - 순수 보고서 표시용
    
    구성요소:
    1. RSI 방향 (25점)
    2. 가격 방향 (25점)
    3. MACD 방향 (25점)
    4. Stochastic RSI 위치 (25점)
    
    Returns:
        dict: {score, details, rsi, volume_ratio, bb_width, is_recovery_start}
    """
    try:
        score = 0
        details = []
        ...
        if df_15m is None or len(df_15m) < 20:
            return {'score': 0, 'details': ['데이터 부족'], 'rsi': 50,
                    'volume_ratio': 1.0, 'bb_width': 2.0, 'is_recovery_start': False}
        ...
        if current['rsi'] > prev['rsi']:
            score += 25
            details.append(f"RSI상승 {rsi_now:.0f} (+25)")
        elif rsi_now >= 40:
            score += 10
            details.append(f"RSI양호 {rsi_now:.0f} (+10)")
        
        # 2. 가격 방향 (25점)
        if current['close'] > prev['close']:
            score += 25
            details.append("가격상승 (+25)")
        elif current['close'] > current['open']:
            score += 12
            details.append("양봉 (+12)")
        
        # 3. MACD 방향 (25점)
        if 'macd_hist' in current.index:
            hist_now = current['macd_hist']
            hist_prev = prev['macd_hist']
            if hist_now > hist_prev:
                score += 25
                details.append("MACD개선 (+25)")
            elif hist_now > 0:
                score += 12
                details.append("MACD양수 (+12)")
        
        # 4. Stochastic RSI (25점)
        if 'stoch_rsi_k' in current.index:
            k_now = current['stoch_rsi_k']
            if k_now > prev.get('stoch_rsi_k', 50):
                score += 25
                details.append(f"S-RSI상승 {k_now:.0f} (+25)")
            elif k_now >= 50:
                score += 10
                details.append(f"S-RSI양호 {k_now:.0f} (+10)")
        
        # 거래량 / BB 정보 (보고서용)
        vol_ratio = current['volume_ratio'] if 'volume_ratio' in current.index else 1.0
        bb_width = current['bb_width'] if 'bb_width' in current.index else 2.0
        bb_position = current['bb_position'] if 'bb_position' in current.index else 50
        ...
        return {
            ...
            'is_recovery_start': (bb_position < 35 and is_bullish),
            'bb_position': bb_position
        }
        ...
        return {
            'score': 0, 'details': [f'계산오류: {e}'], 'rsi': 50,
            'volume_ratio': 1.0, 'bb_width': 2.0, 'is_recovery_start': False
        }
...
def detect_downtrend_15m(df_15m):
    """
    [v10.0] Phase 2: 15분봉 위치 필터 (조정 구간 확인)
    
    ⚠️ 함수명 유지, 역할 변경:
    기존: 하락 추세면 차단 → 신규: BB 하단 근처인지 확인
    
    반환 형식은 호환성 유지하되 의미 변경:
    - is_downtrend=True → 매수 불가 (BB 위치 부적합)
    - is_downtrend=False → 매수 가능 (BB 하단 근처)
    
    조건:
    - BB 위치: 5% ~ 35% (V10_15M_BB_MIN ~ V10_15M_BB_MAX)
    - BB 폭: ≥ 1.5% (V10_15M_BB_WIDTH_MIN)
    - RSI: ≥ 20 (V10_15M_RSI_MIN)
    """
    try:
        if df_15m is None or len(df_15m) < 5:
            return {
                'is_downtrend': True,  # 데이터 부족 → 매수 불가
                'reason': '데이터 부족',
                'bb_position': 50,
                'bb_width': 0,
                'rsi': 50
        ...
        # ========================================
        # 모든 체크 통과 → BB 하단 근처, 매수 가능 구간
        # ========================================
        return {
            'is_downtrend': False,  # 매수 가능!
            'reason': f'BB하단구간 OK (BB:{bb_position:.0f}%, 폭:{bb_width:.1f}%, RSI:{rsi:.0f})',
            'bb_position': bb_position,
            'bb_width': bb_width,
            'rsi': rsi
        }
