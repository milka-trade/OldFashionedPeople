...
def filtered_tickers(tickers):
    ...
    for t in tickers:
        try:
            ...
            filtering_message += f"[cond1: {is_increasing}] band_diff: {band_diff[-1]:,.4f} > BD_Margin: {band_diff_margin} \n"
            filtering_message += f"[cond2: {rsi_range}] {rsi_buy_s} < rsi: {rsi[-2]:,.2f} > {rsi[-1]:,.2f} < {rsi_buy_e} \n"
            filtering_message += f"[cond3: bullish_diver:{divergence_result['is_bullish_divergence']}] \n"
            filtering_message += f"[cond4: diver_bull: {divergence_result['is_bullish_divergence']} / 다이버전스 구간:{divergence_result['divergence_bars']}개 캔들\n"
...
def trade_buy(ticker):
    ...
    if krw >= min_krw :
        while attempt < max_retries:
            print(f"[가격 확인 중]: {ticker}  현재가: {cur_price:,.2f} / 시도: {attempt} - 최대: {max_retries}")
            
            if cur_price < last_ema:
                buy_attempts = 3
                for i in range(buy_attempts):
                    try:
                        buy_order = upbit.buy_market_order(ticker, buy_size)
                        buyedmsg = f"매수 성공: {ticker} / 현재가 :{cur_price:,.2f} \n"
                        buyedmsg += f" {rsi_buy_s} < rsi: {rsi[-2]:,.2f} >> {rsi[-1]:,.2f} < {rsi_buy_e} \n"
                        print(buyedmsg)
...
def trade_sell(ticker):
    ...
    if divergence_result is None:
            print(f"[{ticker}] 다이버전스 분석 실패")
            return False

    diver_bear = divergence_result['is_bearish_divergence']
    ...
    if profit_rate >= min_rate:
        while attempts < max_attempts:       
            ...
            if profit_rate > max_rate:
                ...
                sellmsg += f"rsi_downing: {rsi_downing} / {rsi_sell_s} < rsi: {rsi[-2]:,.2f} >> {rsi[-1]:,.2f} < {rsi_sell_e} \n" 
                sellmsg += f"diver_bear: {divergence_result['is_bearish_divergence']} / 다이버전스 구간:{divergence_result['divergence_bars']}개 캔들\n"
                print(sellmsg)
            ...
            if diver_bear :   
                ...
                middlemsg += f"rsi_downing: {rsi_downing} / {rsi_sell_s} < rsi: {rsi[-2]:,.2f} >> {rsi[-1]:,.2f} < {rsi_sell_e} \n" 
                middlemsg += f"diver_bear: {divergence_result['is_bearish_divergence']} / 다이버전스 구간:{divergence_result['divergence_bars']}개 캔들\n"
                print(middlemsg)
                send_discord_message(middlemsg)
            else:
                middleFailmsg = f"[매도 감시중]:[{ticker}] / 수익률: {profit_rate:,.2f}%  / 현재가: {cur_price:,.1f} \n"
                middleFailmsg += f"rsi_downing: {rsi_downing} / {rsi_sell_s} < rsi: {rsi[-2]:,.2f} >> {rsi[-1]:,.2f} < {rsi_sell_e} \n" 
                middleFailmsg  += f"diver_bear: {divergence_result['is_bearish_divergence']} / 다이버전스 구간:{divergence_result['divergence_bars']}개 캔들\n"
                print(middleFailmsg)
