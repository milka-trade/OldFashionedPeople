...
def calculate_coin_status_for_report(ticker):
    ...
    try:
        ...
        return {
            ...
            'is_dynamic': is_dynamic, 'match_type': match_type,
            # 레거시 호환
...
def send_enhanced_statistics_report():
    ...
    try:
        ...
        dyn_section = ""
        if DYNAMIC_COIN_ENABLED:
            with dynamic_coins_lock: dyn_list = list(dynamic_coins_cache)
            wd = [d for d in dyn_list if d['ticker'] not in hs]
            if wd:
                dyn_section = f"\n\n🔄 **동적 {len(wd)}개**"
                for dc in wd:
                    tk = dc['ticker']; cn = dc['coin']
                    st = calculate_coin_status_for_report(tk)
                    mt = dc.get('match_type','?')[:1]
                    dyn_section += (
                        f"\n🔹`{cn:>4} {st.get('cur_price_str','-'):>7} "
                        f"{st['d_change']:+.1f}% BB{st['bb15']:2.0f} "
                        f"W{st['bw15']:3.1f} R{st['rsi15']:2.0f} "
                        f"SR{st['srsi_k']:2.0f}{st['srsi_direction']} {mt}`"
                    )
            else:
                dyn_section = f"\n\n🔄 동적 `0개`"

        msg = f"\n{'─'*25}\n{header}{mkt_section}{hold_section}{watch_section}{dyn_section}\n{'─'*25}"
...
def evolution_80_buy_signal(df_15m, df_5m, ticker):
    ...
    try:
        ...
        base_response = {
            ...
            'mode': 'BOUNCE_V22', 'market_condition': 'NORMAL', 'score': 0,
        ...
        return {
            ...
            'mode': 'BOUNCE_V22', 'market_condition': 'BOUNCE_CONFIRMED',
...
def evolution_80_sell_signal(df, buy_price, buy_time=None, held_info=None, df_5m=None, regime='NEUTRAL'):
    """
    [v22.0] BUAE 매도 + 반등의날 홀드 모드

    반등의날 매수 코인:
    Step 4.5(신규): BB>65+RSI>55 → 강제 홀드
    Step 5: BUAE BB존 85% (기존 70%)
    Step 6: 트레일링 2.5%, 최소익절 2.0%, 소진임계+2

    Step 1~4: 기존 v20.0 100% 동일 (안전장치 유지)
    """
    try:
        ...
        if is_rev_day and profit_pct > 0:
            ...
            if bb_position >= REVERSAL_DAY_HOLD_BB and rsi_now >= REVERSAL_DAY_HOLD_RSI:
                base_response['reason'] = (
                    ...
                    f'수익{profit_pct:.2f}% [{rev_strength}]')
                return base_response

        # ===== Step 5: BUAE 핵심 — BB 상단 추세강도 + 5분봉 판정 =====
        # ★ 반등의 날이면 BB존 85% (기존 70%)
        ...
        if bb_position >= buae_bb_zone:
            ...
            if trend_grade == 'STRONG' and not micro_weak:
                base_response['reason'] = (
                    ...
                    f'5분봉{micro["detail"]}, BB{bb_position:.0f}%, 수익{profit_pct:.2f}%)')
                return base_response
            ...
            if trend_grade == 'MODERATE' and not micro_weak:
                if QPL_ENABLED and profit_pct > 0:
                    ...
                    if profit_pct >= QPL_TIER1_PROFIT and qpl_count >= 3:
                        ...
                        return {**base_response, 'signal': True,
                                'reason': (f'QPL강화 (수익{profit_pct:.2f}%, {sigs}, MODERATE+5분강세)')}
                pass
        ...
        # ★ 반등의날이면 트레일링 거리 강제 상향
        if is_rev_day and trailing['distance'] < REVERSAL_DAY_TRAILING_DISTANCE:
            # 트레일링 거리를 반등의날 값으로 덮어쓰기
            peak_p = trailing.get('peak_profit', 0)
            drawdown = peak_p - profit_pct if peak_p > 0 else 0
            if drawdown >= REVERSAL_DAY_TRAILING_DISTANCE and profit_pct > 0.3:
                trailing['should_sell'] = True
                trailing['distance'] = REVERSAL_DAY_TRAILING_DISTANCE
            else:
                trailing['should_sell'] = False
        ...
        if profit_pct < dyn_min:
            base_response['reason'] = (
                ...
                f'소진{ex_score}/{ex_threshold}, {t_info}, {ppf_info}{rev_info})')
        else:
            base_response['reason'] = (
        ...
        if DEBUG_MODE:
            print(f"{Colors.RED}[v22.0 BUAE Sell Error] {e}{Colors.ENDC}")
            traceback.print_exc()
