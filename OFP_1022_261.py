...
def trade_buy(ticker=None):
    """
    🚀 적응형 매수 시스템 v8.1 - 정밀 반등 타이밍 포착
    
    🔥 핵심 개선:
    1. BB 15분 하단 필수 조건 (0.35 이하만 선정)
    2. 반등 시작 감지 (하락 멈춤 + 상승 전환)
    3. 출력 최소화 (2줄로 축약)
    4. 가짜 신호 제거 (BB 상단 코인 배제)
    5. 수익 예측 정확도 향상
    """    
    # ===== STEP 1: 자산 현황 =====
    ...
    if ticker is None:
        ...
        for t in candidates:
            ...
            # 🔥 [필수 조건 1] 가격 범위
            if not (100 <= analysis['current_price']):
                continue
            
            # 🔥 [필수 조건 2] BB 15분 하단 (0.35 이하만)
            bb_15m_pos = ind['bb_15m_pos']
            if bb_15m_pos > 0.35:
                continue
            
            # 🔥 [필수 조건 3] RSI 과매도 (40 이하)
            rsi_5m = ind['rsi_5m']
            if rsi_5m > 40:
                continue
            
            # 🔥 [필수 조건 4] BB 5분도 하단 (0.45 이하)
            ...
            rebound_prob = rebound_15m['probability'] * 0.7 + rebound_5m['probability'] * 0.3
            expected_gain = rebound_15m['expected_gain']
            
            # 반등 확률 최소 기준 (50% 이상)
            if rebound_prob < 0.50:
                continue
            ...
            # === BB 하단 이탈 보너스 ===
            if bb_15m_pos < 0:
                bb_bonus = abs(bb_15m_pos) * 3.0
            elif bb_15m_pos < 0.15:
                bb_bonus = (0.15 - bb_15m_pos) * 2.5 + 0.30
            elif bb_15m_pos < 0.25:
                bb_bonus = (0.25 - bb_15m_pos) * 2.0 + 0.15
            else:
                bb_bonus = (0.35 - bb_15m_pos) * 1.0
            ...
            expected_loss = max(bb_15m_pos, 0.10) * 5 * volatility_factor
        ...
        # 🔥 간결한 출력 (2줄)
        print(f"\n🎯 [{selected_ticker}] BB15m:{best['bb_15m_pos']*100:.0f}% RSI:{best['rsi_5m']:.0f} 반등:{best['rebound_prob']*100:.0f}% 기대값:+{best['expected_value']:.2f}% 손익비:{best['profit_loss_ratio']:.1f}:1")
        print(f"   반등징후:{best['rebound_score']}점 거래량:{best['vol_ratio']:.1f}x 예상수익:+{best['expected_gain']:.1f}% 예상손실:-{best['expected_loss']:.1f}%")
        ...
        if bb_15m_pos > 0.35 or rsi_5m > 40:
            print(f"⚠️ [{ticker}] BB15m:{bb_15m_pos*100:.0f}% RSI:{rsi_5m:.0f} - 조건 미달")
        ...
        volatility_factor = 0.35 if bb_15m_pos < 0.15 else (0.50 if bb_15m_pos < 0.25 else 0.65)
        expected_loss = max(bb_15m_pos, 0.10) * 5 * volatility_factor
        loss_prob = 1 - rebound_prob
    ...
    # BB 위치 최종 확인
    if bb_15m_pos > 0.40:
        print(f"❌ BB 위치 높음: {bb_15m_pos*100:.0f}%")
        return "BB position too high", None
    ...
    for attempt in range(1, 3):
        try:
            ...
            success_msg += f"BB15m:{bb_15m_pos*100:.0f}% RSI:{rsi_5m:.0f} 반등:{rebound_prob*100:.0f}%\n"
