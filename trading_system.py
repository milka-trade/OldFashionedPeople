...
class KiwoomAPI:
    ...
    def _on_event_connect(self, err_code):
        if err_code == 0:
            ...
            server_name = "실서버" if server_type == "1" else "모의투자"
            logger_api.info(
                f"로그인 성공 | 서버: {server_name} | 계좌: {self.account_no}"
            )
        else:
            self.connected = False
    ...
    def get_account_balance(self):
        ...
        self.set_input_value("비밀번호", config.ACCOUNT_PASSWORD)
