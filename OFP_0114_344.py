...
def execute_buy(ticker, signal):
    ...
    [v343.1 개선사항]
    - FIXED_STABLE_COINS 진입 시점 검증 추가
    - 관리 대상 외 코인 매수 차단
    """
    # ============================================================
    ...
    try:
        with trade_lock:
            ...
            # ========================================
            # ✅ [v343.1 핵심 추가] 진입 시점 검증
            # ========================================
            # 봇 관리 대상 코인만 매수 허용
            # - FIXED_STABLE_COINS에 없는 코인은 매수 차단
            # - sync 함수와 일관성 확보
            # - 이중 안전장치 역할
            if ticker not in FIXED_STABLE_COINS:
                coin_name = ticker.replace('KRW-', '')
                print(f"{Colors.RED}[Buy Failed] {coin_name}는 관리 대상 외 코인입니다.{Colors.ENDC}")
                print(f"  현재 관리 대상: {', '.join([t.replace('KRW-', '') for t in FIXED_STABLE_COINS])}")
...
**현재 관리 대상:**
`{', '.join([t.replace('KRW-', '') for t in FIXED_STABLE_COINS])}`

**안내:**
이 코인을 거래하려면 코드에서 `FIXED_STABLE_COINS` 리스트에 추가하세요.
...
"""
            ...
            # Step 2: 목표 포지션 사이즈 (POSITION_SIZE_RATIO 파라미터 반영) ✅ FIXED
            ...
            # 매수 정보 출력
            coin_name = ticker.replace('KRW-', '')
            print(f"{Colors.CYAN}[Buy Info] {coin_name}")
            print(f"  총자산: {total_assets:,.0f}원 | 목표포지션: {target_position_size:,.0f}원 | 실제매수: {buy_amount:,.0f}원{Colors.ENDC}")
