...
# -*- coding: utf-8 -*-
"""
BB Bounce Hunter v30.0 — 3-Tier 낙폭 기반 진입 + 야간 모멘텀 + 일봉 컨텍스트
 
★ v30.0 핵심 개선 (90일 실데이터 기반):
  [매수]
  1. 3-Tier 낙폭 분기 진입 시스템
     - T2 대형낙폭(-4%+): 승률 67~79% — 기준 완화하여 적극 포착
     - T1 중간낙폭(-2~-4%): 승률 42~49% — RSI 스윗스팟 + 볼륨 필수
     - T3 과매도급락(RSI<30+낙폭-3%+볼륨): 승률 63% — 별도 패스트트랙
  2. RSI 스윗스팟(20~45) 필수 조건화 — 패닉셀 구간 제외
  3. 야간(22-06시) 모멘텀 별도 진입 트랙 — BB 위치 무관 상승 감지
  4. 일봉 컨텍스트 보너스 — 일봉 RSI<30/BB<25 시 진입 기준 완화
  5. PlungeScore cum4 임계값 상향 — 더 의미있는 급락만 감지
 
  [매도]
  6. RSI>70+BB>75 시 안전익절 조건 강화 (기존 RSI 하락 요구→과매수 즉반응)
  7. 5분봉 SRSI 데드크로스 확인으로 매도 정확도 향상
 
★ 데이터 근거 (ETH/XRP/SOL/BCH/SUI 15분봉 90일):
  - BB하단 단순진입: 2%승률 13.6%
  - T1 중간낙폭+RSI+볼륨: 42.0%
  - T2 대형낙폭: 67.1%
  - T3 과매도+낙폭+볼륨: 63.6%
  - RSI 20~45 스윗스팟: 평균최대 +2.53~3.11%
  - 일봉 RSI<30 → 다음날 15분봉 반등 승률 24.6% (전체 14.4%)
"""
...
# ============================================================================
# ★ 시장 등급별 파라미터 테이블 (v30.0)
# 동적 손절 공식: stop_loss = -min(max(BB_Width × mult, floor), ceil)
# ============================================================================
MARKET_GRADE_PARAMS = {
    'HIGH': {
        'BUY_BB_MAX':            35,     # v30: T2 포착 위해 +5 완화
        'BUY_BB_WIDTH_MIN':     1.5,
        'SELL_SAFE_PROFIT':     1.2,
        'SELL_SAFE_BB_MIN':      70,     # v30: 75→70으로 완화 (빠른 반응)
    ...
    'MID': {
        'BUY_BB_MAX':            30,     # v30: T2 포착 위해 25→30 완화
        'BUY_BB_WIDTH_MIN':     1.0,
        'SELL_SAFE_PROFIT':     1.2,
        'SELL_SAFE_BB_MIN':      68,     # v30: 70→68
    ...
    'LOW': {
        'BUY_BB_MAX':            25,     # v30: T2 포착 위해 20→25 완화
...
# ★ v30.0: 코인별 급락 임계값 — cum4 상향 조정 (T1 기준 -2.0%로 통일)
# 기존 -1.0~-1.4% → -2.0~-2.5% (실제 의미있는 급락만 감지)
COIN_DROP_THRESHOLDS = {
    'KRW-XRP':  {'cum4': -2.0, 'daily': -4.5},   # -1.1→-2.0 / -3.9→-4.5
    'KRW-SOL':  {'cum4': -2.0, 'daily': -5.0},   # -1.2→-2.0 / -4.5→-5.0
    'KRW-ADA':  {'cum4': -2.0, 'daily': -5.0},   # -1.3→-2.0 / -4.4→-5.0
    'KRW-ETH':  {'cum4': -2.0, 'daily': -4.5},   # -1.0→-2.0 / -4.1→-4.5
    'KRW-LINK': {'cum4': -2.0, 'daily': -4.5},   # -1.1→-2.0 / -4.0→-4.5
    'KRW-BCH':  {'cum4': -2.0, 'daily': -4.5},   # -1.1→-2.0 / -3.8→-4.5
    'KRW-SUI':  {'cum4': -2.5, 'daily': -5.5},   # -1.4→-2.5 / -4.9→-5.5 (고변동)
}
 
# ── v28.0: 추세 모멘텀 진입 (Trend Momentum Entry) 파라미터 ──
...
# ── ★ v30.0 신규: 3-Tier 낙폭 진입 파라미터 ──────────────────────────────
# Tier 분류 기준: 최근 4봉(1시간) 누적 낙폭
TIER_DROP_T1_MIN  = -4.0    # T1 중간낙폭: -4.0% ~ -2.0%
TIER_DROP_T1_MAX  = -2.0
TIER_DROP_T2      = -4.0    # T2 대형낙폭: -4.0% 이하
 
# T1 중간낙폭 조건
TIER1_BB_MAX      = 22      # BB Position 상한 (엄격)
TIER1_RSI_MIN     = 20      # RSI 스윗스팟 하한
TIER1_RSI_MAX     = 45      # RSI 스윗스팟 상한 (핵심 필터!)
TIER1_VOL_MIN     = 1.0     # 최소 볼륨 배수
TIER1_SCORE_MIN   = 4       # 15m+5m 합산 최소 점수
 
# T2 대형낙폭 조건 (완화)
TIER2_BB_MAX      = 30      # BB Position 상한 (완화)
TIER2_RSI_MAX     = 50      # RSI 상한 (T1보다 완화 — 대형낙폭 후 RSI 넓음)
TIER2_SCORE_MIN   = 3       # 합산 최소 점수 (완화)
 
# T3 과매도+급락 패스트트랙
TIER3_RSI_MAX     = 32      # 과매도 기준
TIER3_DROP_MIN    = -3.0    # 최소 낙폭
TIER3_VOL_MIN     = 1.5     # 볼륨 필수 (과매도 저점 확인)
TIER3_SCORE_MIN   = 3       # 합산 최소 점수
 
# ── ★ v30.0 신규: 야간 모멘텀 진입 파라미터 ─────────────────────────────
# 야간 정의: 22:00~06:00 KST
NIGHT_HOUR_START  = 22      # 야간 시작 시각 (KST)
NIGHT_HOUR_END    = 6       # 야간 종료 시각 (KST, 미만)
# 야간 모멘텀 진입 조건
NIGHT_MOM_RISE_PCT    = 0.5     # 단일봉 최소 상승률 (%)
NIGHT_MOM_RSI_MIN     = 30      # RSI 최소 (극단적 과매도 제외)
NIGHT_MOM_RSI_MAX     = 70      # RSI 최대 (과매수 제외)
NIGHT_MOM_BB_MAX      = 65      # BB Position 상한 (고점 진입 방지)
NIGHT_MOM_VOL_MIN     = 0.8     # 최소 볼륨 배수
NIGHT_MOM_CONFIRM     = True    # 연속 2봉 상승 확인 여부
# 야간 목표 수익률 (강제익절 낮춤 — 짧게)
NIGHT_FORCE_PROFIT    = 1.8     # 야간 강제익절 (주간 2.5% 대비 낮춤)
 
# ── ★ v30.0 신규: 일봉 컨텍스트 보너스 파라미터 ────────────────────────
DAY_CONTEXT_RSI_LOW   = 32      # 일봉 RSI 이하 → 진입 기준 완화
DAY_CONTEXT_BB_LOW    = 25      # 일봉 BB Position 이하 → 기준 완화
DAY_CONTEXT_SCORE_BONUS = 1     # 조건 충족 시 점수 보너스
 
# ── ★ v30.0 신규: 매도 강화 파라미터 ────────────────────────────────────
# 과매수 구간 조기 감지 (BB>75 + RSI>70 → 안전익절 즉시 발동)
SELL_OVERBOUGHT_BB    = 75      # BB Position 과매수 기준
SELL_OVERBOUGHT_RSI   = 70      # RSI 과매수 기준
# 기존 안전익절 추가 조건: RSI>70이면 BB 기준 낮춰 더 빠르게 반응
SELL_FAST_RSI_THRESH  = 70      # 이 RSI 이상이면 BB 기준 완화
SELL_FAST_BB_MIN      = 70      # RSI>70 시 이 BB 이상이면 익절
...
def run_sharp_drop_scanner():
    ...
    실행 흐름:
      1. ticker API 1회 호출 → 당일 낙폭 빠른 파악 (스냅샷 캐시 공유)
    ...
    # Step 1: 스냅샷 (캐시 공유 — 추세 스캐너와 동일 호출 절약)
...
def buy_signal(ticker):
    """
    ★★★ v30.0 매수 신호 — 3-Tier 낙폭 분기 + 야간 모멘텀 + 일봉 컨텍스트
 
    ════════════════════════════════════════════════════════
    흐름도:
    ① 시간대 확인 → 야간(22-06시)이면 야간 모멘텀 트랙 분기
    ② Phase A: 자격 검증 (BB 위치·폭) + 일봉 컨텍스트 완화
    ③ Phase A 실패 → Phase C 크래시 리커버리
    ④ Phase B: 4봉 낙폭 Tier 판별 → Tier별 진입 기준 적용
       - T2(낙폭<-4%): 완화된 기준, 승률 67~79%
       - T1(낙폭 -2~-4%): RSI 스윗스팟+볼륨 필수, 승률 42~49%
       - T3(RSI<32+낙폭<-3%+볼륨): 패스트트랙, 승률 63%
       - 기본: 기존 듀얼 타임프레임 유지
    ════════════════════════════════════════════════════════
    """
    try:
        ...
        # ════════════════════════════════════════
        # ★ v30.0: 시간대 확인 — 야간 모멘텀 트랙
        # ════════════════════════════════════════
        kst_hour = (datetime.now().hour + 9) % 24  # UTC → KST
        ...
        if is_night:
            ...
            if night_ok:
                return {
                    'signal': True,
                    'reason': f"[야간모멘텀][{grade}] {night_reason}",
                    'entry_price': current['close'],
                    'bb_position': bb_pos,
                    'bb_width_pct': bb_width,
                    'market_grade': grade,
            ...
            # 야간이지만 모멘텀 미충족 — 기존 로직으로 계속 진행
            # (야간에도 BB 하단 급락 반등은 가능)
 
        # ════════════════════════════════════════
        # ★ v30.0: 일봉 컨텍스트 조회 (보너스 점수)
        # ════════════════════════════════════════
        day_ctx = get_daily_context(ticker)
        day_bonus = day_ctx['bonus']
 
        # ════════════════════════════════════════
        # Phase A: 자격 검증 (BB 위치·폭)
        # 일봉 저점권이면 BB_MAX +5 완화
        # ════════════════════════════════════════
        phase_a_passed = True
        phase_a_fail_reason = ""
 
        # A-1: BB 하단 위치 (일봉 저점권 완화)
        bb_max_effective = _bb_max + (5 if day_ctx['is_low_bb'] else 0)
        if bb_pos > bb_max_effective:
            phase_a_passed = False
            phase_a_fail_reason = (f"BB{bb_pos:.0f}%>{bb_max_effective}%[{grade}]"
                                   f"{'(일봉저점+5)' if day_ctx['is_low_bb'] else ''}")
        ...
        # ════════════════════════════════════════
        # ★ v30.0: 4봉 누적 낙폭 계산 → Tier 판별
        # ════════════════════════════════════════
        drop_4 = 0.0
        if len(df_15m) >= 5:
            p4_close = df_15m.iloc[-5]['close']
            if p4_close > 0:
                drop_4 = (current['close'] - p4_close) / p4_close * 100
 
        # 현재봉 볼륨 배수 (5분봉 없을 때 15분봉으로 대체)
        vol_ratio_now = current.get('vol_ratio', 1.0)
        if vol_ratio_now is None or vol_ratio_now != vol_ratio_now:  # NaN 처리
            vol_ratio_now = 1.0
 
        # 5분봉 볼륨 보조 확인
        vol_5m = 1.0
        if df_5m is not None and len(df_5m) >= 5:
            vol_5m = df_5m.iloc[-1].get('vol_ratio', 1.0) or 1.0
 
        # ════════════════════════════════════════
        # Phase B-0: ★ T3 과매도 패스트트랙
        # 조건: RSI<32 + 낙폭<-3% + 볼륨>1.5x
        # 승률 63.6% — 가장 강력한 반등 신호
        # ════════════════════════════════════════
        if (current['rsi'] < TIER3_RSI_MAX and
            drop_4 < TIER3_DROP_MIN and
            vol_ratio_now >= TIER3_VOL_MIN):
 
            score_15m, details_15m = score_15m_bounce(df_15m)
            score_5m, details_5m = (score_5m_bounce(df_5m)
                                    if df_5m is not None and len(df_5m) >= 10
                                    else (0, []))
            total_score = score_15m + score_5m + day_bonus
            ...
            if total_score >= TIER3_SCORE_MIN:
                d15 = '+'.join(details_15m)
                d5  = '+'.join(details_5m) if details_5m else 'N/A'
                day_tag = f" 일봉보너스+{day_bonus}" if day_bonus > 0 else ""
                return {
                    'signal': True,
                    'reason': (f"[T3-과매도][{grade}]BB{bb_pos:.0f}% 낙폭{drop_4:.1f}% "
                               f"RSI{current['rsi']:.0f} 볼륨{vol_ratio_now:.1f}x{day_tag} | "
                               f"15m({score_15m}:{d15}) 5m({score_5m}:{d5}) 합{total_score}"),
                    'entry_price': current['close'],
                    'bb_position': bb_pos,
                    'bb_width_pct': bb_width,
                    'market_grade': grade,
                    'entry_type': 'tier3_oversold',
                }
            else:
                base['reason'] = (f"T3점수부족 RSI{current['rsi']:.0f} 낙폭{drop_4:.1f}% "
                                  f"합산{total_score}<{TIER3_SCORE_MIN}")
                return base
 
        # ════════════════════════════════════════
        # Phase B-1: ★ T2 대형낙폭 (-4%+ 이하)
        # 완화된 기준: BB<30, RSI<50, score≥3
        # 승률 67~79% — 기회 있을 때 적극 진입
        # ════════════════════════════════════════
        if drop_4 <= TIER_DROP_T2:
            if bb_pos <= TIER2_BB_MAX and current['rsi'] < TIER2_RSI_MAX:
                score_15m, details_15m = score_15m_bounce(df_15m)
                score_5m, details_5m = (score_5m_bounce(df_5m)
                                        if df_5m is not None and len(df_5m) >= 10
                                        else (0, []))
                ...
                if total_score >= TIER2_SCORE_MIN:
                    d15 = '+'.join(details_15m)
                    d5  = '+'.join(details_5m) if details_5m else 'N/A'
                    day_tag = f" 일봉+{day_bonus}({day_ctx['reason']})" if day_bonus > 0 else ""
                    return {
                        'signal': True,
                        'reason': (f"[T2-대형낙폭][{grade}]BB{bb_pos:.0f}% 낙폭{drop_4:.1f}% "
                                   f"RSI{current['rsi']:.0f}{day_tag} | "
                                   f"15m({score_15m}:{d15}) 5m({score_5m}:{d5}) 합{total_score}"),
                        'entry_price': current['close'],
                        'bb_position': bb_pos,
                        'bb_width_pct': bb_width,
                        'market_grade': grade,
                        'entry_type': 'tier2_large_drop',
                    }
                else:
                    base['reason'] = (f"T2점수부족 낙폭{drop_4:.1f}% RSI{current['rsi']:.0f} "
                                      f"합산{total_score}<{TIER2_SCORE_MIN}")
                    return base
            else:
                # T2 낙폭이지만 BB/RSI 조건 미달 → T3 패스트트랙도 못 탄 경우
                base['reason'] = (f"T2조건미달 BB{bb_pos:.0f}%>{TIER2_BB_MAX}% "
                                  f"or RSI{current['rsi']:.0f}≥{TIER2_RSI_MAX}")
                return base
 
        # ════════════════════════════════════════
        # Phase B-2: ★ T1 중간낙폭 (-2~-4%)
        # RSI 스윗스팟(20~45) 필수 + 볼륨 필수
        # 승률 42~49%
        # ════════════════════════════════════════
        if TIER_DROP_T1_MIN < drop_4 <= TIER_DROP_T1_MAX:
            # RSI 스윗스팟 필수 체크
            if not (TIER1_RSI_MIN <= current['rsi'] <= TIER1_RSI_MAX):
                base['reason'] = (f"T1 RSI스윗스팟미달 RSI{current['rsi']:.0f} "
                                  f"(필요:{TIER1_RSI_MIN}~{TIER1_RSI_MAX})")
                return base
 
            # 볼륨 필수 체크 (15분봉 or 5분봉)
            effective_vol = max(vol_ratio_now, vol_5m)
            if effective_vol < TIER1_VOL_MIN:
                base['reason'] = f"T1 볼륨미달 {effective_vol:.1f}x<{TIER1_VOL_MIN}x"
                return base
            ...
            # T1은 표준 기준 (일봉 보너스로 완화 가능)
            t1_threshold = TIER1_SCORE_MIN - day_bonus  # 보너스만큼 기준 완화
            t1_threshold = max(t1_threshold, 3)  # 최소 3점 유지
 
            if score_15m >= BUY_BOUNCE_MIN_15M and score_5m >= BUY_BOUNCE_MIN_5M:
                buy_ok = True
            elif total_score >= t1_threshold:
                buy_ok = True
            else:
                buy_ok = False
            ...
            if buy_ok:
                d15 = '+'.join(details_15m)
                d5  = '+'.join(details_5m) if details_5m else 'N/A'
                day_tag = f" 일봉+{day_bonus}" if day_bonus > 0 else ""
                return {
                    'signal': True,
                    'reason': (f"[T1-중간낙폭][{grade}]BB{bb_pos:.0f}% 낙폭{drop_4:.1f}% "
                               f"RSI{current['rsi']:.0f}{day_tag} | "
                               f"15m({score_15m}:{d15}) 5m({score_5m}:{d5}) 합{total_score}"),
                    'entry_price': current['close'],
                    'bb_position': bb_pos,
                    'bb_width_pct': bb_width,
                    'market_grade': grade,
                    'entry_type': 'tier1_mid_drop',
                }
            else:
                base['reason'] = (f"T1점수부족 낙폭{drop_4:.1f}% RSI{current['rsi']:.0f} "
                                  f"볼륨{effective_vol:.1f}x 합산{total_score}<{t1_threshold}")
                return base
 
        # ════════════════════════════════════════
        # Phase B-기본: 소폭낙폭 or 낙폭 불충분
        # 기존 듀얼 타임프레임 (RSI 스윗스팟 보너스 포함)
        # 더 엄격한 기준 적용
        # ════════════════════════════════════════
        score_15m, details_15m = score_15m_bounce(df_15m)
        score_5m = 0
        ...
        # 소폭 낙폭 기본 로직: RSI 범위 추가 제한 (20~50)
        rsi_ok = 20 <= current['rsi'] <= 50
        if not rsi_ok:
            base['reason'] = (f"기본RSI범위외 RSI{current['rsi']:.0f}(필요20~50) "
                              f"낙폭{drop_4:.1f}%[{grade}]")
            return base
        ...
        if not buy_confirmed:
            ...
            base['reason'] = (f"반등부족 15m({score_15m}:{d15}) "
                              f"5m({score_5m}:{d5}) 합산{total_score}[{grade}] "
                              f"낙폭{drop_4:.1f}%")
            return base
        ...
        return {
            'signal': True,
            'reason': (f"[기본][{grade}]BB{bb_pos:.0f}% 폭{bb_width:.1f}%{decel_tag}"
                       f"{day_tag} 낙폭{drop_4:.1f}% | "
                       f"15m({score_15m}:{d15}) 5m({score_5m}:{d5}) 합{total_score}"),
            'entry_price': current['close'],
            'bb_position': bb_pos,
            'bb_width_pct': bb_width,
...
def sell_signal(df, buy_price, buy_time=None, held_info=None):
    ...
    Step 2: 강제 익절 (진입 유형별 목표 수익)
    ★Step 2.5: 과매수 조기 익절 (RSI>70 + BB>70 시 빠른 반응) [v30.0 신규]
    Step 3: 안전 익절 (RSI↓ + BB 위치 + 5분봉 SRSI 데드크로스) [v30.0 강화]
    Step 4: 트레일링 (5분봉 보조)
    """
    try:
        if df is None or len(df) < 5:
        ...
        # ════════════════════════════════════════
        # ★ Step 2.5: v30.0 신규 — 과매수 조기 익절
        # RSI>70 + BB>70 시 최소 수익 달성하면 빠른 익절
        # (데이터: BB>85+RSI>65 → 17.1% 확률 즉각 -1.5% 하락)
        # ════════════════════════════════════════
                    ...
                    if _5m_bb >= 70:
                        return {**base, 'signal': True,
                                'reason': (f'SRSI데드익절({profit_pct:.2f}% '
                                           f'5mSRSI데드크로스 5mBB{_5m_bb:.0f}%)[{grade_str}]')}
        ...
        base['reason'] = (f'홀드(수익{profit_pct:.2f}%, BB{bb_pos:.0f}%'
                          f'{trail_info}{stop_info}{night_tag})[{grade_str}]')
        return base
        ...
        try:
            ...
            if DEBUG_MODE and iteration % 6 == 0:
                ...
                print(f"{Colors.MAGENTA}[BUY] KRW:{krw_balance_now:,.0f}원 | "
                      ...
                      f"급락:{wl_count}종 추세:{tme_count}종{Colors.ENDC}")

            # ══════════════════════════════════════════════
            # ★ 코인별 매수 검토 — 경로 1: 급락반등 (기존)
            # ══════════════════════════════════════════════
            for ticker in FIXED_STABLE_COINS:
                ...
                if sig['signal']:
                    ...
                    entry_tag  = "🔄CR" if entry_type == 'crash_recovery' else f"📉WL({watch_info.get('stage1_score', 0):.1f})"
            ...
            for ticker in FIXED_STABLE_COINS:
                ...
                # 급락 감시 중인 코인은 추세 경로 스킵 (반등 로직 우선)
                in_plunge, _ = is_in_watchlist(ticker)
                if in_plunge:
                    continue
                ...
                if trend_info:
                    ...
                    if detected_price > 0 and cur_price_now > 0:
                        ...
                        if advance > TREND_MAX_ADVANCE_PCT:
                            coin_name = ticker.replace('KRW-', '')
                            if DEBUG_MODE:
                                print(f"{Colors.YELLOW}[TME] {coin_name} 이미 {advance:.1f}% 상승 → 늦은진입 차단{Colors.ENDC}")
                            continue
