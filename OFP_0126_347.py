...
"""
================================================================================
EVOLUTION v8.0 "MOMENTUM WAVE" - THREADED EDITION [FIXED]
================================================================================
...
[v8.0 FIXED - 2025.01.26]
✅ TIME_EXIT 완전 제거 - 시간 제약 없이 모멘텀만으로 판단
✅ 디스코드 알람 v7.6 스타일 복구 - 상세한 포트폴리오 정보
✅ 시장 분석 리포트 추가 - 보유/관심 코인 통합 분석
...
DEBUG_MODE = True
TEST_MODE = False
VERSION = "8.0 MOMENTUM_WAVE [FIXED]"
...
# ================================================================================
# SECTION 3: v8.0 MOMENTUM WAVE Parameters [FIXED]
# ================================================================================
...
print(f"{'='*60}")
print(f"{Colors.GREEN}🚀 MOMENTUM WAVE TRADING [FIXED]{Colors.ENDC}")
...
print(f"{Colors.RED}[FIXED] ❌ TIME_EXIT 제거 (모멘텀만으로 판단){Colors.ENDC}")
print(f"{Colors.GREEN}[FIXED] ✅ 디스코드 알람 v7.6 스타일 복구{Colors.ENDC}")
print(f"{'='*60}{Colors.ENDC}\n")


# ================================================================================
# SECTION 6: Discord Functions [FIXED - v7.6 Style]
# ================================================================================
...
def send_buy_notification(ticker, signal, buy_amount, total_balance):
    ...
    try:
        ...
        # 기울기 정보
        slope_str = f" | 기울기`{signal.get('slope', 0):+.2f}%`" if signal.get('slope') else ""
...
└ 📊 `BB {signal['bb_position']:.0f}%` | **사유:** {signal['reason'].split('(')[0]}"""
...
def check_reentry_cooldown(ticker):
    try:
        ...
        for key in ['STOP_LOSS', 'TARGET', 'TRAILING', 'MOMENTUM']:
            if key in sell_reason:
                cooldown_key = key + ('_REACHED' if key == 'TARGET' else '_STOP' if key == 'TRAILING' else '_EXIT' if key == 'MOMENTUM' else '')
                break
...
# ================================================================================
# SECTION 11: Sell Logic [FIXED - TIME_EXIT 제거]
# ================================================================================
...
def get_adaptive_targets(df, buy_mode):
    """
    [v8.0 FIXED] max_hold_minutes 제거
    파동 크기와 매수 모드에 따른 적응형 목표 수익률 설정
    """
    try:
        ...
        # 파동별 기본 설정 (시간 제한 제거)
        targets = {
            'SMALL': (TARGET_PROFIT_SMALL, STOP_LOSS_TIGHT, 0.8),
            'MID': (TARGET_PROFIT_MID, STOP_LOSS_DEFAULT, TRAILING_START),
            'LARGE': (TARGET_PROFIT_LARGE, STOP_LOSS_WIDE, 2.0)
        }
        target, stop, trailing = targets.get(wave_category, targets['MID'])
        ...
        return {
            'target_profit': TARGET_PROFIT_MID,
...
def momentum_wave_sell_signal(df, buy_price, buy_time=None, held_info=None):
    """
    [v8.0 FIXED] TIME_EXIT 완전 제거 - 모멘텀만으로 판단
...
# ================================================================================
# SECTION 12: Market Analysis [FIXED - v7.6 Style]
# ================================================================================

def generate_market_summary_v8():
    """
    [v8.0 FIXED] v7.6 스타일 시장 분석 추가
    보유코인 + 관심코인 통합 분석
    """
    try:
        target_coins = set()
            ...
            for ticker in held_tickers:
                ...
                with held_coins_lock:
                    if ticker in held_coins:
                        ...
                        mode_emoji = {'REVERSAL': '🔄', 'PULLBACK': '📉', 'EXPLOSION': '💥'}.get(buy_mode, '✅')
                        profit_emoji = "📈" if profit_pct >= 0 else "📉"
                        
                        message += f"\n{profit_emoji} **{coin_name}** {mode_emoji} `{current['close']:,.0f}원`"
                        message += f" | `{profit_pct:+.2f}%` | ⏱`{hold_duration}`"
                        message += f"\n└ BB`{current['bb_position']:.0f}%` | RSI`{current['rsi']:.0f}` | 기울기`{slope:+.1f}%`"
        
        # 관심 코인 분석
        if fixed_tickers:
            message += "\n\n**[관심코인]**"
            for ticker in fixed_tickers[:5]:  # 최대 5개만
                df = get_candles(ticker, interval='15', count=50)
                if df is None or len(df) < 20:
                    continue
                ...
                signal_emoji = '🟢' if slope > 0.5 else '🔴' if slope < -0.5 else '🟡'
                
                message += f"\n{signal_emoji} **{coin_name}** `{current['close']:,.0f}원`"
                message += f" | BB`{current['bb_position']:.0f}%` | RSI`{current['rsi']:.0f}` | 기울기`{slope:+.1f}%`"
...
def send_enhanced_statistics_report():
    ...
    try:
        ...
        # 시장 분석 (보유코인+관심코인 통합)
        market_summary = generate_market_summary_v8()
