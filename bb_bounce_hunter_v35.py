...
# -*- coding: utf-8 -*-
"""
BB Bounce Hunter v34.0 — TrendSell 매도 로직 전면 재설계
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
v33.2 기반 → 매도 로직만 전면 교체 (매수 로직 완전 유지)

★ v34.0 핵심 변경: TrendSell (추세 전환 기반 매도) 적용
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  [제거]
  - Step 3 안전익절 (BB Position 기반) → 볼린저밴드로 인한 찔끔 매도 원인
  - Step 4 탄력 트레일링 (0.3~1.0%) → 노이즈에 의한 과도한 손절 원인
  - Step 2.7 예측 가속익절 → TrendSell로 흡수
  - Step 3.5 예측 홀드보호 → TrendSell로 흡수
  - 동적 손절 (BBW 연동 -1.5~2.5%) → 너무 타이트, 대형 코인 노이즈 범위 내 손절 원인

  [신규 TrendSell 구조 — 주식 TrendSellEngine 이식]
  Step 0: 가속 손절 (5분봉 3연속음봉+RSI<25) ← 기존 유지
  Step 1: 절대 손절 (XRP:-4%, ETH:-3%, SOL:-3.5%) ← 코인별 느슨하게
  Step 2: 강제 익절 (5.0%, 긴급 안전밸브) ← 기존 유지
  Step 3: ★NEW EMA 추세 전환 (EMA5<EMA10 + 3봉+ 보유 + 수익>0.3%)
  Step 4: ★NEW RSI 과매수 반전 (RSI≥70 후 RSI<60, 수익>0%)
  Step 5: ★NEW 탄력 트레일링 (XRP:2~3%, ETH:3~4.5%, SOL:2.5~3.5%)
  Step 6: ★NEW 오전 과열 안전 청산 (8~9시 + RSI≥60 + 수익>0.5%)
  Step 7: 시간 청산 (16h 수익>0 / 24h 무조건) ← 기존 유지

  ★ 설계 철학 변경:
  - v33: "BB Position이 오르면 팔아" (BB 기반) → 올라가면 즉시 팔아버리는 구조
  - v34: "추세가 꺾였을 때 팔아" (추세 전환 기반) → 급등 구간 버티고 고점 후 매도

  ★ 백테스트 실증 (XRP/ETH 30일, 15분봉):
  - XRP: 승률 74.4%, 평균수익 +0.25% (v33 대비 대폭 개선)
  - ETH: 승률 76.8~79.7%, 평균수익 +0.23~0.36%
  - 손절 회복력: XRP -3% 이하 터치 후 29% 회복 → -4% 느슨한 손절 근거
  - ETH -3% 이하 후 회복률 0% → -3% 엄격 유지

★ v33.2 유지: 매수 예측기 연동 (매수 로직 전체 불변)
★ v33.1 유지: 매수 로직 전체 (BB+RSI+5분봉+시간대별 전략 등)
"""
...
DEBUG_MODE = True
TEST_MODE = False
VERSION = "35.0 BB_BOUNCE_HUNTER — Cheap-Buy-High-Sell"
 
# 거래 대상 (고정 7개)
FIXED_STABLE_COINS = [
    "KRW-ETH", "KRW-XRP", "KRW-SOL", "KRW-BTC" 
...
PREDICTOR_TIMEOUT_SEC    = 180        # get_prediction() 호출 타임아웃 (초)
                                      # ★ pkl 없을 때 신규 학습 최대 2~3분 소요
                                      # ★ pkl 있을 때는 통상 1~3초 → 여유 충분
...
# ── v33.2 신규: 매도 예측기 연동 파라미터 (v34: 매수 예측기만 유지) ─────────
PREDICTOR_SELL_ENABLED        = False   # v34: 매도 예측기 비활성 (TrendSell로 대체)
PRED_SELL_STOP_TIGHTEN        = False
PRED_SELL_STOP_TIGHTEN_PCT    = 0.5
PRED_SELL_ACCEL_EXIT          = False
PRED_SELL_ACCEL_MIN_PROFIT    = 0.7
PRED_SELL_SAFE_MODIFY         = False
PRED_SELL_BB_EASE             = 55
PRED_BUY_BB_STRICT            = 80
PRED_BUY_HOLD_PROTECT         = False
PRED_BUY_SAFE_PROFIT_BOOST    = 0.3
PRED_SELL_TRAIL_MODIFY        = False
PRED_SELL_TRAIL_TIGHTEN       = 0.1
PRED_BUY_TRAIL_EXPAND         = 0.2
PRED_BUY_TRAIL_MAX_GAP        = 1.5

# ============================================================================
# ★ v34.0 TrendSell 전용 상수 (매도 전략 핵심 파라미터)
# ============================================================================

# ─── EMA 추세 전환 (Step 3) ──────────────────────────────────────────────────
TREND_SELL_EMA_SHORT   = 5       # 단기 EMA 기간 (15분봉 기준)
TREND_SELL_EMA_LONG    = 10      # 장기 EMA 기간
TREND_SELL_MIN_PROFIT  = 0.3     # EMA 크로스 발동 최소 수익률 (%)
TREND_SELL_MIN_BARS    = 3       # 매수 후 최소 보유 봉수 (15분봉 기준)

# ─── RSI 과매수 반전 (Step 4) ────────────────────────────────────────────────
RSI_OB_ENTER    = 70.0           # RSI 과매수 진입 임계 (이 이상이면 was_overbought=True)
RSI_OB_COOL     = 60.0           # RSI 반전 임계 (이 미만으로 하락 시 매도)
                                  # 데이터: XRP 47회, ETH 57회 / 30일 = 주요 청산 신호

# ─── 탄력 트레일링 (Step 5) — 코인별 수익 구간 차등 ─────────────────────────
# 형식: [(최소수익%, 트레일간격%)]  수익 클수록 간격 확장 (더 버팀)
# 수익 0% 미만 구간(0.0, 999): 트레일 비활성 → 손절은 Step 1이 담당
TRAIL_TIERS = {
    'KRW-XRP': [           # XRP: 소폭 움직임 → 2~3% 간격
        (3.0, 3.0),        # 3%+ 수익 → 고점 대비 -3.0% 허용
        (2.0, 2.5),        # 2%+ → -2.5%
        (1.0, 2.0),        # 1%+ → -2.0%
        (0.0, 999),        # 수익 없음 → 트레일 비활성
    ],
    'KRW-ETH': [           # ETH: 대형 변동성 → 3~4.5% 간격 (백테스트 최적)
        (4.0, 4.5),        # 4%+ → -4.5%
        (3.0, 4.0),        # 3%+ → -4.0%
        (2.0, 3.5),        # 2%+ → -3.5%
        (1.0, 3.0),        # 1%+ → -3.0%
...
TRAIL_TIERS_DEFAULT = [    # 기타 코인 (BTC 등)
    (3.0, 3.0),
    (2.0, 2.5),
    (1.0, 2.0),
    (0.0, 999),
...
# ─── 절대 손절 (Step 1) — 코인별 느슨하게 ────────────────────────────────────
# 근거: XRP -3% 이하 후 29% 회복 → -4% / ETH -3% 후 회복률 0% → -3% 엄격 유지
ABS_STOP_LOSS = {
    'KRW-XRP':  -4.0,     # XRP: 회복력 있음 → 느슨하게
    'KRW-ETH':  -3.0,     # ETH: 회복률 0% → 엄격하게
    'KRW-SOL':  -3.5,     # SOL: 중간
    'KRW-BTC':  -3.0,     # BTC: 대형이지만 보수적
}
ABS_STOP_LOSS_DEFAULT = -3.5   # 기타 코인 기본값

# ─── 오전 과열 안전 청산 (Step 6) ────────────────────────────────────────────
# 오전 8~9시: 9시 장초반 직전 → RSI 과열 상태 + 수익 있으면 선제 청산
# (9시 무조건 청산 ❌ → RSI 조건부 청산 ✅, 데이터상 9시 청산 평균 0.3% 불과)
MORNING_SAFE_SELL_HOUR_START = 8     # 오전 8시부터 적용
MORNING_SAFE_SELL_HOUR_END   = 9     # 오전 9시 전까지
MORNING_SAFE_SELL_RSI_MIN    = 60.0  # RSI 60 이상 (과열 판단)
MORNING_SAFE_SELL_PROFIT_MIN = 0.5   # 수익 0.5% 이상일 때만

# ============================================================================
# ★★★ v35.0 Cheap-Buy-High-Sell 전용 파라미터 (광현 요청 반영) ★★★
# ============================================================================
# 설계 철학:
#   - "싸게 사서 비싸게 판다" — v34의 조기 매도/허위 손절 문제 해결
#   - 매수: 당일 저점권만 진입 (VWAP + 당일저가 + 15분RSI/BB)
#   - 매도: 수익 1.5% 미만은 대부분 매도 차단 (Profit-Ladder)
#   - 손절: 2-Bar 확정 룰 (순간 터치 허위손절 회피)
#   - 급등: Rally Guard — 급등 중 매도 신호 일시 차단
# ============================================================================

# ─── [매수] 당일 저점 게이트 ──────────────────────────────────────────────
# 목적: "싸게 사기" 강제. 모든 Tier 매수 시 공통 선행 필터.
DAILY_LOW_GATE_ENABLED     = True      # 당일 저점 게이트 ON/OFF (False면 v34 동일)
DAILY_LOW_VWAP_GAP_PCT     = 0.3       # VWAP 대비 허용 하한 — 현재가 ≤ VWAP×(1-0.003)
DAILY_LOW_FROM_LOW_PCT     = 1.5       # 당일 저가 대비 허용 상한 — 현재가 ≤ 저가×(1+0.015)
                                        # 둘 중 하나만 만족해도 통과 (OR 조건, 너무 엄격하지 않게)
DAILY_LOW_GATE_EXCEPT_DEEP = True      # 일봉 RSI<30 또는 일봉BB<15일 때는 게이트 면제 (초저점)

# ─── [매수] 하루 최대 매수 건수 제한 ──────────────────────────────────────
# 목적: "하루 1~2건 품질 매매" 강제. v34에서는 일평균 19건 → v35 목표 1~3건.
DAILY_BUY_COUNT_LIMIT      = 3         # 하루 최대 매수 건수 (초과 시 모든 매수 차단)
                                        # 새벽/오전/오후 골고루 분산시키고 싶으면 2로 낮출 것

# ─── [매도] Profit-Ladder 수익 게이트 ──────────────────────────────────
# 목적: +1.5% 미만 조기매도 원천 차단. v34 Step3 (EMA전환 @ 0.3%)의 가장 큰 문제.
PROFIT_LADDER_ENABLED      = True
PROFIT_GATE_EMA_CROSS      = 1.2       # Step3 EMA전환 매도 최소 수익 (v34: 0.3 → v35: 1.5)
PROFIT_GATE_RSI_REVERT     = 1.2       # Step4 RSI과매수반전 매도 최소 수익 (v34: 0.0 → v35: 1.2)
PROFIT_GATE_TRAIL_ACTIVATE = 1.5       # Step5 트레일링 활성화 최소 고점수익 (신규 게이트)
                                        # 고점 수익이 1.5% 이상 올라간 적 있어야 트레일링 작동
                                        # = "비싸게 가본 적 없으면 매도 금지"

# ─── [매도] Rally Guard (급등 보호) ─────────────────────────────────────
# 목적: 새벽 급등, 미국장 진입 급등 시 조기 매도 차단.
# 조건: 최근 N분간 누적 상승률이 임계치 초과 시 발동 → M분간 매도 신호 차단
# 차단 제외: 절대손절(Step1), 강제익절(Step2) — 안전 관련은 유지
RALLY_GUARD_ENABLED        = True
RALLY_GUARD_LOOKBACK_MIN   = 20        # 최근 20분 (5분봉 4개) 누적 상승률 검사
RALLY_GUARD_THRESHOLD_PCT  = 1.5       # 20분간 +1.5% 이상 상승 시 발동
RALLY_GUARD_DURATION_MIN   = 15        # 발동 후 30분간 매도 신호 차단
RALLY_GUARD_DAWN_ONLY      = False     # True면 새벽(00~08시)에만 작동. False면 24시간.
                                        # 새벽+미국장만 특화하려면 True 권장

# ─── [매도] 2-Bar 손절 확정 (허위 손절 필터) ─────────────────────────────
# 목적: 순간 -3~4% 터치 후 원복하는 Long Wick 허위 손절 회피.
# 방식: 손절선 터치 후, 직전 5분봉 종가도 손절선 이하여야 최종 손절 확정.
# 예외: 긴급 손절(-5% 이하)은 즉시 발동 (안전장치).
STOPLOSS_2BAR_ENABLED      = True
STOPLOSS_EMERGENCY_PCT     = -5.0      # 이 이하면 2-Bar 룰 무시하고 즉시 손절

# ─── [매도] 오전 폭락 선제 청산 확장 ─────────────────────────────────────
# 목적: 국내장 개장(09:00) 전후 하락 대응. v34(08~09시)에서 07:30~09:30으로 확장.
MORNING_EXIT_EXT_START_HM  = (7, 30)   # 시작: 07:30
MORNING_EXIT_EXT_END_HM    = (9, 30)   # 종료: 09:30
MORNING_EXIT_EXT_PROFIT    = 1.0       # 수익 1.0% 이상일 때만 적용 (기존 0.5%보다 상향)
MORNING_EXIT_EXT_RSI       = 55.0      # RSI 55 이상 (기존 60보다 낮춰서 더 많이 잡힘)
# 09:00~09:30 급락 감지 (국내장 개장 직후 투매)
MORNING_CRASH_DETECT_START_HM = (9, 0)
MORNING_CRASH_DETECT_END_HM   = (9, 30)
MORNING_CRASH_DROP_PCT     = -1.0      # 직전 10분간 -1% 이상 하락 시 수익>0이면 즉시 청산

# ─── [매도] 시간 청산 재설정 (v34 대비 완화) ──────────────────────────────
# v34: 16h 수익>0 청산, 24h 무조건
# v35: 24h 수익>0 청산, 48h 무조건 (추세 지속 시 더 오래 보유)
V35_HOLD_PROFIT_EXIT_HOURS = 24        # v34 HOLD_PROFIT_EXIT_HOURS(16) 대체
V35_HOLD_MAX_EXIT_HOURS    = 48        # v34 HOLD_MAX_EXIT_HOURS(24) 대체

# ─── [매수 차단 시간대 확장] ────────────────────────────────────────────
# v34: 08:59~09:15 매수 차단
# v35: 08:50~09:30 확장 (국내장 개장 혼란기 전체)
V35_BUY_BLOCK_START_HM     = (8, 50)
V35_BUY_BLOCK_END_HM       = (9, 30)
V35_BUY_BLOCK_EXTEND       = True      # True면 v35 확장 시간대 적용, False면 v34 유지
...
predictor_stats = {
    ...
    # ★ v33.2 신규: 매도 예측기 통계
    'sell_accel_count': 0,    # 예측기 가속 익절 횟수
    'sell_protect_count': 0,  # 예측기 홀드 보호 횟수
    'sell_tighten_count': 0,  # 예측기 손절 강화 횟수
    'sell_trail_mod_count': 0,# 예측기 트레일 변조 횟수
}
...
def add_indicators(df):
    try:
        ...
        # ★ v34.0: TrendSell용 EMA 추가 (EMA5, EMA10)
        df['ema5']  = df['close'].ewm(span=TREND_SELL_EMA_SHORT, adjust=False).mean()
        df['ema10'] = df['close'].ewm(span=TREND_SELL_EMA_LONG,  adjust=False).mean()
        return df
    except Exception as e:
        if DEBUG_MODE:
...
def get_daily_context(ticker):
    """
    ★ v35.0: 일봉 컨텍스트 + 당일 저점 정보 (VWAP, 당일 저가 추가)

    v34 대비 신규 필드:
      - 'vwap': 당일 VWAP (거래량 가중 평균가)
      - 'today_low': 당일 저가
      - 'today_high': 당일 고가
      - 'pct_from_low': 당일 저가 대비 현재가 % (+0.5면 저가 대비 +0.5% 위치)
      - 'pct_from_vwap': VWAP 대비 현재가 % (음수면 VWAP 아래)
      - 'daily_low_gate_pass': 당일 저점 게이트 통과 여부
      - 'daily_low_gate_reason': 게이트 통과/실패 사유

    반환 예:
      {
        'rsi': 35.2, 'bb_position': 22.1, 'is_oversold': True, 'is_low_bb': True,
        'pct_from_20d_high': -12.3, 'bonus': 2, 'reason': '일봉RSI35↓+일봉BB22↓',
        'vwap': 2175.3, 'today_low': 2168.0, 'today_high': 2195.0,
        'pct_from_low': 0.32, 'pct_from_vwap': -0.15,
        'daily_low_gate_pass': True, 'daily_low_gate_reason': 'VWAP아래(-0.15%)',
      }
    """
    # ── 기본값 (오류 시 fail-safe로 반환) ─────────────────────────────────
    default = {
        'rsi': 50, 'bb_position': 50, 'is_oversold': False,
        'is_low_bb': False, 'pct_from_20d_high': 0, 'bonus': 0, 'reason': '일봉없음',
        'vwap': 0, 'today_low': 0, 'today_high': 0,
        'pct_from_low': 0, 'pct_from_vwap': 0,
        'daily_low_gate_pass': True,  # ★ fail-safe: 데이터 없으면 통과 (v34 거동 유지)
        'daily_low_gate_reason': '일봉데이터없음(fail-safe통과)',
    }
    ...
    try:
        ...
        today_close = float(c['close'])
        today_low   = float(c['low'])
        today_high  = float(c['high'])
        today_open  = float(c['open'])

        # ─── 20일 고점 대비 낙폭 ──────────────────────────────────────
        high_20d = df_d['high'].iloc[-20:].max() if len(df_d) >= 20 else df_d['high'].max()
        pct_from_hi = (today_close - high_20d) / high_20d * 100 if high_20d > 0 else 0

        # ─── ★ v35 신규: 당일 VWAP 근사 계산 ──────────────────────────
        # 일봉은 1개 캔들 = 하루 전체이므로 단순 VWAP 근사:
        # VWAP ≈ (high + low + close) / 3 (전형적 가격 평균)
        # 더 정확하려면 15분봉 누적 계산 필요하지만, 일봉으로도 "당일 평균 가격대" 추정 충분
        vwap = (today_high + today_low + today_close) / 3 if today_close > 0 else today_close

        # ─── ★ v35 신규: 저가/VWAP 대비 현재 위치 ──────────────────────
        pct_from_low  = (today_close - today_low)  / today_low  * 100 if today_low  > 0 else 0
        pct_from_vwap = (today_close - vwap)       / vwap       * 100 if vwap       > 0 else 0

        # ─── 일봉 보너스 계산 ─────────────────────────────────────────
        is_oversold = rsi_d < DAY_CONTEXT_RSI_LOW
        is_low_bb   = bb_pos_d < DAY_CONTEXT_BB_LOW
        is_deep_dd  = pct_from_hi < -30
        ...
        # ─── ★ v35 신규: 당일 저점 게이트 판정 ──────────────────────────
        # 아래 조건 중 하나만 만족하면 '저점권'으로 인정 (OR, 너무 엄격하지 않게):
        #   (a) 현재가 ≤ VWAP × (1 - 0.3%) → VWAP 아래 0.3% 이상
        #   (b) 현재가 ≤ 당일저가 × (1 + 1.5%) → 당일 저가 대비 +1.5% 이내
        # 추가 면제:
        #   (c) 일봉 RSI<30 또는 일봉 BB<15 → 초저점 구간, 게이트 면제 (버스 놓치지 않기)
        gate_pass = False
        gate_reasons = []

        if DAILY_LOW_GATE_ENABLED:
            # 조건 (a): VWAP 아래
            cond_a = pct_from_vwap <= -DAILY_LOW_VWAP_GAP_PCT
            # 조건 (b): 당일 저가 근접
            cond_b = pct_from_low  <= DAILY_LOW_FROM_LOW_PCT
            # 조건 (c): 일봉 초저점 면제
            cond_c = DAILY_LOW_GATE_EXCEPT_DEEP and (rsi_d < 30 or bb_pos_d < 15)

            if cond_a:
                gate_pass = True
                gate_reasons.append(f"VWAP아래({pct_from_vwap:.2f}%)")
            if cond_b:
                gate_pass = True
                gate_reasons.append(f"저가근접(+{pct_from_low:.2f}%)")
            if cond_c:
                gate_pass = True
                gate_reasons.append(f"일봉초저점면제(RSI{rsi_d:.0f}/BB{bb_pos_d:.0f})")

            if not gate_pass:
                gate_reasons.append(
                    f"저점권아님(VWAP{pct_from_vwap:+.2f}%, 저가+{pct_from_low:.2f}%)"
                )
        else:
            # 게이트 비활성화 시 항상 통과 (v34 거동 유지)
            gate_pass = True
            gate_reasons.append("게이트OFF")
        ...
        return {
            ...
            'daily_low_gate_pass': gate_pass,
            'daily_low_gate_reason': gate_reason_str,
        }

    except Exception as e:
        if DEBUG_MODE:
            print(f"{Colors.YELLOW}[get_daily_context] {ticker} 오류: {e}{Colors.ENDC}")
        # 오류 시 fail-safe 통과 (v34 거동 유지)
        default['daily_low_gate_reason'] = f'오류:{e}(fail-safe통과)'
        return default
...
def buy_signal(ticker):
    """
    ★★★ v35.0 매수 신호 — "싸게 사기" 게이트 + 하루 매수 건수 제한

    v34 대비 핵심 변경:
    ① ★ 당일 저점 게이트 (DAILY_LOW_GATE_ENABLED)
       - VWAP 아래 또는 당일 저가 근접일 때만 진입 허용
       - 야간 모멘텀 / 크래시 리커버리 제외 (급등 추격은 예외)
    ② ★ 하루 매수 건수 제한 (DAILY_BUY_COUNT_LIMIT)
       - 하루 3건 초과 시 모든 매수 차단 → 품질 우선 강제
    ③ ★ 매수 차단 시간대 확장 (V35_BUY_BLOCK_EXTEND)
       - 08:50~09:30 (국내장 개장 혼란기)
    ④ v34 매수 로직 (Tier A/B/C, 야간, CR, 보너스) 모두 유지

    ════════════════════════════════════════════════════════
    흐름도 (v35):
    ① 매수 차단 시간대 체크 (08:50~09:30 차단)
    ② 하루 매수 건수 체크 (3건 초과 시 차단)
    ③ 일봉 컨텍스트 조회 (VWAP, 당일저가 포함)
    ④ 야간 모멘텀 트랙 (저점 게이트 면제 — 급등 포착용)
    ⑤ 당일 저점 게이트 (저점권 아니면 이하 모두 차단, CR 제외)
    ⑥ Tier A/C/B 평가 (v34 동일)
    ⑦ 크래시 리커버리 (게이트 면제)
    ════════════════════════════════════════════════════════
    """
    try:
        ...
        # ════════════════════════════════════════
        # ★ v35 STEP A: 매수 차단 시간대 확장 체크
        # ════════════════════════════════════════
        now = datetime.now()
        now_hm = (now.hour, now.minute)

        if V35_BUY_BLOCK_EXTEND:
            # v35: 08:50~09:30 확장 차단
            block_start = V35_BUY_BLOCK_START_HM
            block_end   = V35_BUY_BLOCK_END_HM
            if block_start <= now_hm < block_end:
                base['reason'] = (f"매수차단(국내장개장혼란기 "
                                  f"{block_start[0]:02d}:{block_start[1]:02d}~"
                                  f"{block_end[0]:02d}:{block_end[1]:02d})")
                return base
        else:
            # v34 기본 차단 유지 (08:59~09:15)
            if ((now.hour == BUY_BLOCK_START_HOUR and now.minute >= BUY_BLOCK_START_MINUTE) or
                (now.hour == BUY_BLOCK_END_HOUR   and now.minute <  BUY_BLOCK_END_MINUTE)):
                base['reason'] = "매수차단(업비트정산시간)"
                return base

        # ════════════════════════════════════════
        # ★ v35 STEP B: 하루 매수 건수 제한
        # ════════════════════════════════════════
        if daily_buy_count >= DAILY_BUY_COUNT_LIMIT:
            base['reason'] = (f"일일매수한도도달 "
                              f"({daily_buy_count}/{DAILY_BUY_COUNT_LIMIT}건, "
                              f"품질우선전략)")
            return base
        ...
        # ════════════════════════════════════════
        # ★ v35 STEP C: 일봉 컨텍스트 선행 조회
        # (저점 게이트를 야간/CR 전에 확인하기 위해 앞당김)
        # ════════════════════════════════════════
        day_ctx = get_daily_context(ticker)
        day_bonus = day_ctx['bonus']
        gate_pass = day_ctx.get('daily_low_gate_pass', True)
        gate_reason = day_ctx.get('daily_low_gate_reason', '')

        # ════════════════════════════════════════
        # 야간 모멘텀 트랙 (★ v35: 저점 게이트 면제 — 급등 포착용)
        # ════════════════════════════════════════
        kst_hour = now.hour
        ...
        # ════════════════════════════════════════
        # ★ v35 STEP D: 당일 저점 게이트 — "싸게 사기" 강제
        # Tier A/B/C 매수는 이 게이트를 반드시 통과해야 함
        # (야간 모멘텀/CR은 위에서/아래에서 각각 예외 처리)
        # ════════════════════════════════════════
        if DAILY_LOW_GATE_ENABLED and not gate_pass:
            # 저점권 아님 → Tier 매수 모두 차단, CR만 아래서 시도
            base['reason'] = (f"저점게이트미통과[{grade}][{tz_label}] "
                              f"VWAP{day_ctx.get('pct_from_vwap', 0):+.2f}% "
                              f"저가+{day_ctx.get('pct_from_low', 0):.2f}% "
                              f"({gate_reason})")
            # ★ 중요: 게이트 실패 시에도 크래시 리커버리는 시도 (폭락장 기회 포착)
            # 아래 CR 블록으로 점프
            gate_failed_skip_tier = True
        else:
            gate_failed_skip_tier = False
        ...
        # 저점 태그 (게이트 통과 시 매수 사유에 추가)
        low_tag = ""
        if gate_pass and DAILY_LOW_GATE_ENABLED:
            low_tag = f" 저점권({gate_reason})"

        # ════════════════════════════════════════
        # Tier A/B/C 평가 (v35: 게이트 통과 시에만 진행)
        # ════════════════════════════════════════
        if not gate_failed_skip_tier:
            ...
            if (bb_pos <= tier_a_bb_max and
                ...
                return {
                    'signal': True,
                    'reason': (f"[Tier-A고승률][{grade}][{tz_label}] BB{bb_pos:.0f}% "
                               f"RSI{rsi:.0f}↑ BBW{bb_width:.1f}%{vol_tag}"
                               f"{low_tag}{bonus_str} {confirm_5m_tag}"),
                    'entry_price': current['close'],
                    'bb_position': bb_pos,
                    'bb_width_pct': bb_width,
                    'market_grade': grade,
            ...
            # ────────────────────────────────────────
            tier_b_bb_max = (DIRECT_TIER_B_BB_MAX + tme_bb_extend + tz_bb_adjust +
                             (5 if day_ctx['is_low_bb'] else 0))
            ...
            if (DIRECT_TIER_C_ENABLED and
                ...
                if not confirm_5m_pass:
                    ...
                    if score_15m + total_bonus < max(3 + tz_score_adjust, 2):
                        ...
                        return {
                            'signal': True,
                            'reason': (f"[Tier-C반전][{grade}][{tz_label}] BB{bb_pos:.0f}% "
                                       f"RSI{rsi:.0f}↑ SRSI{srsi_k:.0f}>D "
                                       f"BBW{bb_width:.1f}% 직전음봉→양봉 "
                                       f"15m보완{score_15m}점{vol_tag}{low_tag}{bonus_str}"),
                            'entry_price': current['close'],
                            'bb_position': bb_pos,
                            'bb_width_pct': bb_width,
                            'market_grade': grade,
                    ...
                    return {
                        ...
                        'reason': (f"[Tier-C반전][{grade}][{tz_label}] BB{bb_pos:.0f}% "
                                   f"RSI{rsi:.0f}↑ SRSI{srsi_k:.0f}>D "
                                   f"BBW{bb_width:.1f}% 직전음봉→양봉"
                                   f"{vol_tag}{low_tag}{bonus_str} {confirm_5m_tag}"),
                        'entry_price': current['close'],
                        'bb_position': bb_pos,
                        'bb_width_pct': bb_width,
                        'market_grade': grade,
            ...
            if (bb_pos <= tier_b_bb_max and
                ...
                if not confirm_5m_pass and DIRECT_5M_CONFIRM:
                    ...
                    if score_15m + total_bonus < min_score:
                        ...
                        return {
                            'signal': True,
                            'reason': (f"[Tier-B표준][{grade}][{tz_label}] BB{bb_pos:.0f}% "
                                       f"RSI{rsi:.0f}↑ SRSI{srsi_k:.0f}>D "
                                       f"BBW{bb_width:.1f}% 15m보완({score_15m}점:{d15})"
                                       f"{vol_tag}{low_tag}{bonus_str}"),
                            'entry_price': current['close'],
                            'bb_position': bb_pos,
                            'bb_width_pct': bb_width,
                            'market_grade': grade,
                    ...
                    return {
                        ...
                        'reason': (f"[Tier-B표준][{grade}][{tz_label}] BB{bb_pos:.0f}% "
                                   f"RSI{rsi:.0f}↑ SRSI{srsi_k:.0f}>D "
                                   f"BBW{bb_width:.1f}%{vol_tag}{low_tag}{bonus_str} "
                                   f"{confirm_5m_tag}"),
                        'entry_price': current['close'],
                        'bb_position': bb_pos,
                        'bb_width_pct': bb_width,
        ...
        if df_5m is not None and len(df_5m) >= 10:
            ...
            if cr_ok:
                ...
                if score_5m >= 2:
                    ...
                    return {
                        'signal': True,
                        'reason': (f"[CR][{grade}][{tz_label}] {cr_reason} | "
                                   f"5m({score_5m}점:{detail_str}) (저점게이트면제)"),
                        'entry_price': current['close'],
                        'bb_position': bb_pos,
                        'bb_width_pct': bb_width,
                        'market_grade': grade,
        ...
        # v35: 게이트 실패가 주요 원인이면 최우선 표시
        if gate_failed_skip_tier:
            fail_parts.append(f"저점게이트실패({gate_reason})")

        if bb_pos > tier_b_bb_max if 'tier_b_bb_max' in dir() else 100:
            try:
                fail_parts.append(f"BB{bb_pos:.0f}%>{tier_b_bb_max}%")
            except Exception:
                fail_parts.append(f"BB{bb_pos:.0f}%")
        ...
        if DEBUG_MODE:
            print(f"{Colors.RED}[Buy Signal v35] {ticker} 오류: {e}{Colors.ENDC}")
            traceback.print_exc()
        return {'signal': False, 'reason': f'오류: {e}',
                'entry_price': 0, 'bb_position': 50, 'bb_width_pct': 0}
...
def sell_signal(df, buy_price, buy_time=None, held_info=None):
    """
    ★★★ v35.0 매도 신호 — "비싸게 팔기" Profit-Ladder + Rally Guard + 2-Bar 손절

    v34 대비 핵심 변경:
    ① ★ Profit-Ladder 수익 게이트
       - Step 3 EMA전환: 0.3% → 1.5% (조기매도 원천 차단)
       - Step 4 RSI반전: 0% → 1.2%
       - Step 5 트레일: 고점수익 1.5% 이상 올라간 적 있어야 작동
    ② ★ Rally Guard: 20분간 +1.5% 급등 감지 시 30분간 매도 차단
       (새벽 급등, 미국장 진입 추격용)
    ③ ★ 2-Bar 손절 확정: 순간 손절선 터치는 무시, 2봉 연속 확정돼야 손절
       (긴급 -5% 이하는 즉시 발동)
    ④ ★ 오전 폭락 선제 청산 확장: 07:30~09:30 + RSI≥55 + 수익≥1.0%
    ⑤ ★ 09:00~09:30 급락 감지: 10분간 -1% 하락 시 수익>0이면 즉시 청산
    ⑥ ★ 시간 청산 완화: 16h→24h 수익청산, 24h→48h 강제청산

    ══════════════════════════════════════════════════════════════
    Step 0: 가속 손절 (5분봉 3연속음봉+RSI<25)          ← 기존 유지
    Step 1: ★v35 2-Bar 확정 손절 (긴급 -5%는 즉시)
    Step 2: 강제 익절 (5.0%, 긴급 안전밸브)             ← 기존 유지
    ★ Rally Guard 체크: 급등 중이면 Step 3~7 모두 스킵
    Step 3: ★v35 EMA 전환 + 수익≥1.5% 게이트
    Step 4: ★v35 RSI 반전 + 수익≥1.2% 게이트
    Step 5: ★v35 트레일링 + 고점수익≥1.5% 활성화
    Step 6: ★v35 오전 폭락 선제 청산 (확장 07:30~09:30)
    Step 6b: ★v35 09:00~09:30 급락 감지 청산
    Step 7: ★v35 시간 청산 (24h 수익>0 / 48h 무조건)
    ══════════════════════════════════════════════════════════════
    """
    try:
        if df is None or len(df) < 5:
        ...
        profit_pct    = ((current_price - buy_price) / buy_price) * 100
        bb_pos        = current.get('bb_position', 50)
        bb_width      = current.get('bb_width', 2.0)
        ...
        # ─── 코인별 절대 손절선 결정 ──────────────────────────────
        _abs_stop = ABS_STOP_LOSS.get(ticker, ABS_STOP_LOSS_DEFAULT)

        # ─── 코인별 탄력 트레일 티어 결정 ────────────────────────
        _trail_tiers = TRAIL_TIERS.get(ticker, TRAIL_TIERS_DEFAULT)

        # ─── 보유 시간 계산 ───────────────────────────────────────
        now_ts = time.time()
        now_dt = datetime.now()
        if held_info and 'buy_time' in held_info and held_info['buy_time']:
            buy_dt = held_info['buy_time']
            if isinstance(buy_dt, datetime):
                elapsed_sec = (now_dt - buy_dt).total_seconds()
            else:
                elapsed_sec = now_ts - float(buy_dt)
        else:
            elapsed_sec = 0.0
        elapsed_hours = elapsed_sec / 3600
        now_hour   = now_dt.hour
        now_minute = now_dt.minute
        now_hm     = (now_hour, now_minute)

        min_hold_active = elapsed_sec < _min_hold_sec

        # ─── peak_price 관리 ──────────────────────────────────────
        if held_info is not None:
            peak_price = held_info.get('peak_price', buy_price)
            if 'was_overbought' not in held_info:
                held_info['was_overbought'] = False
            was_overbought = held_info['was_overbought']
        else:
        ...
        if current_price > peak_price:
            ...
            if held_info is not None:
                held_info['peak_price'] = peak_price

        peak_pnl = (peak_price - buy_price) / buy_price * 100 if buy_price > 0 else 0.0
        drawdown = (peak_price - current_price) / peak_price * 100 if peak_price > 0 else 0.0

        # ─── 공통 반환 베이스 ─────────────────────────────────────
        base = {
            'signal': False,
            'reason': f'홀딩({profit_pct:.2f}%,고점{peak_pnl:.1f}%)[{grade_str}]',
        ...
        df_5m = None
        if held_info is not None:
            df_5m = held_info.get('df_5m')
        if df_5m is None:
            try:
                df_5m = get_candles_5m(ticker) if ticker else None
            except Exception:
                df_5m = None

        # ════════════════════════════════════════════════════════
        # Step 0: 가속 손절 (5분봉 최우선 — 기존 유지)
        # ════════════════════════════════════════════════════════
        if df_5m is not None:
            _entry_type = held_info.get('entry_type', 'normal') if held_info else 'normal'
            triggered, reason_0 = check_accel_stop(df_5m, entry_type=_entry_type)
            if triggered and profit_pct < 0:
                return {**base, 'signal': True, 'reason': reason_0}

        # ════════════════════════════════════════════════════════
        # ★ Step 1 (v35): 2-Bar 확정 손절
        # - 긴급 손절(-5% 이하): 즉시 발동 (안전장치)
        # - 절대 손절선 터치: 직전 5분봉 종가도 손절선 이하여야 확정
        # ════════════════════════════════════════════════════════
        # (a) 긴급 손절: STOPLOSS_EMERGENCY_PCT(-5%) 이하 즉시 발동
        if profit_pct <= STOPLOSS_EMERGENCY_PCT:
            return {**base, 'signal': True,
                    'reason': f'긴급손절({profit_pct:.2f}%≤{STOPLOSS_EMERGENCY_PCT}%)[{grade_str}]'}

        # (b) 절대 손절선 터치 — 2-Bar 확정 룰
        if profit_pct <= _abs_stop:
            if STOPLOSS_2BAR_ENABLED and df_5m is not None and len(df_5m) >= 2:
                # 직전 5분봉 종가 기준 수익률 계산
                prev_5m_close = float(df_5m.iloc[-2]['close'])
                prev_5m_profit = ((prev_5m_close - buy_price) / buy_price) * 100

                if prev_5m_profit <= _abs_stop:
                    # 2봉 연속 손절선 이하 → 확정 손절
                    return {**base, 'signal': True,
                            'reason': (f'절대손절확정({profit_pct:.2f}%≤{_abs_stop}%, '
                                       f'직전봉{prev_5m_profit:.2f}%도이하)[{grade_str}]')}
                else:
                    # 1봉만 이하 → 허위 손절 가능성, 보류
                    base['reason'] = (f'손절선터치(1봉만,허위가능 '
                                      f'현{profit_pct:.2f}%/직전{prev_5m_profit:.2f}%)'
                                      f'[{grade_str}]')
                    return base
            else:
                # 2-Bar 비활성 또는 데이터 부족 → v34 거동 (즉시 손절)
                return {**base, 'signal': True,
                        'reason': f'절대손절({profit_pct:.2f}%≤{_abs_stop}%)[{grade_str}]'}

        # ─── 최소 보유 시간 중이면 이하 스킵 ──────────────────────
        if min_hold_active:
            remaining = int((_min_hold_sec - elapsed_sec) / 60) + 1
            base['reason'] = (f'최소보유 대기({remaining}분, '
                              f'수익{profit_pct:.2f}%)[{grade_str}]')
            return base

        # ════════════════════════════════════════════════════════
        # Step 2: 강제 익절 (5.0% — 기존 유지)
        # ════════════════════════════════════════════════════════
        ...
        # ════════════════════════════════════════════════════════
        # ★ Rally Guard 체크 (v35 신규)
        # 조건: 최근 N분간 누적 상승률 ≥ 임계치
        # 발동: Step 3~7 모두 차단, Step 1/2(손절/강제익절)는 위에서 이미 처리됨
        # 안전 예외: 오전 07:30~09:30 폭락 감지 Step 6b는 Rally Guard 무시 (국내장 폭락 보호)
        # ════════════════════════════════════════════════════════
        rally_guard_active = False
        rally_guard_reason = ""

        if (RALLY_GUARD_ENABLED and df_5m is not None and len(df_5m) >= 5):
            # 새벽 한정 옵션 확인
            if RALLY_GUARD_DAWN_ONLY:
                is_dawn = 0 <= now_hour < 8
            else:
                is_dawn = True  # 24시간 작동

            if is_dawn:
                # 최근 LOOKBACK_MIN분 = 5분봉 (LOOKBACK/5)개
                lookback_bars = max(1, RALLY_GUARD_LOOKBACK_MIN // 5)
                if len(df_5m) > lookback_bars:
                    past_close = float(df_5m.iloc[-(lookback_bars + 1)]['close'])
                    recent_rise = (current_price - past_close) / past_close * 100 \
                                  if past_close > 0 else 0.0

                    if recent_rise >= RALLY_GUARD_THRESHOLD_PCT:
                        # 급등 감지 → held_info에 종료 시각 저장
                        if held_info is not None:
                            guard_until = held_info.get('rally_guard_until')
                            new_until = now_ts + RALLY_GUARD_DURATION_MIN * 60
                            # 기존에 활성화된 가드가 더 길면 유지, 아니면 갱신
                            if guard_until is None or guard_until < new_until:
                                held_info['rally_guard_until'] = new_until
                                guard_until = new_until
                            rally_guard_active = now_ts < guard_until
                            rally_guard_reason = (f"급등{recent_rise:+.1f}%/"
                                                  f"{RALLY_GUARD_LOOKBACK_MIN}분,"
                                                  f"차단{int((guard_until-now_ts)/60)}분남음")
                    else:
                        # 급등 아님 → 기존 가드가 아직 유효한지만 확인
                        if held_info is not None:
                            guard_until = held_info.get('rally_guard_until', 0)
                            if guard_until and now_ts < guard_until:
                                rally_guard_active = True
                                rally_guard_reason = (f"가드유지,"
                                                      f"{int((guard_until-now_ts)/60)}분남음")

        # ─── EMA 값 추출 ───────────────────────────────────────
        ema5  = current.get('ema5',  None)
        ema10 = current.get('ema10', None)
        cur_rsi = current.get('rsi', None)
        hold_bars = max(1, int(elapsed_sec / 900))

        # ════════════════════════════════════════════════════════
        # ★ Step 6b (v35): 09:00~09:30 급락 감지 청산 (Rally Guard 무시)
        # 국내장 개장 직후 투매 대응 — 안전 최우선
        # 조건: 09:00~09:30 + 10분간 -1% 이상 하락 + 현재 수익 > 0
        # ════════════════════════════════════════════════════════
        crash_start = MORNING_CRASH_DETECT_START_HM
        crash_end   = MORNING_CRASH_DETECT_END_HM
        if crash_start <= now_hm < crash_end and profit_pct > 0 and \
           df_5m is not None and len(df_5m) >= 3:
            # 최근 10분 = 5분봉 2개
            past_close_10m = float(df_5m.iloc[-3]['close'])
            drop_10m = (current_price - past_close_10m) / past_close_10m * 100 \
                       if past_close_10m > 0 else 0
            if drop_10m <= MORNING_CRASH_DROP_PCT:
                return {**base, 'signal': True,
                        'reason': (f'오전급락청산({profit_pct:.2f}%,10분{drop_10m:+.1f}%≤'
                                   f'{MORNING_CRASH_DROP_PCT}%)[{grade_str}]')}

        # ════════════════════════════════════════════════════════
        # Rally Guard 작동 중이면 Step 3~7 전체 차단
        # ════════════════════════════════════════════════════════
        if rally_guard_active:
            base['reason'] = (f'RallyGuard작동({profit_pct:.2f}%,고점{peak_pnl:.1f}%,'
                              f'{rally_guard_reason})[{grade_str}]')
            return base

        # ════════════════════════════════════════════════════════
        # ★ Step 3 (v35): EMA 추세 전환 + Profit-Ladder 게이트
        # v34: 수익 ≥ 0.3%  →  v35: 수익 ≥ PROFIT_GATE_EMA_CROSS (기본 1.5%)
        # ════════════════════════════════════════════════════════
        _ema_gate = PROFIT_GATE_EMA_CROSS if PROFIT_LADDER_ENABLED else TREND_SELL_MIN_PROFIT
        if (ema5 is not None and ema10 is not None and
                not (ema5 != ema5) and not (ema10 != ema10) and
                hold_bars >= TREND_SELL_MIN_BARS and
                profit_pct >= _ema_gate):
            if ema5 < ema10:
                return {**base, 'signal': True,
                        'reason': (f'EMA추세전환({profit_pct:.2f}%≥{_ema_gate}% '
                                   f'EMA5:{ema5:.2f}<EMA10:{ema10:.2f} '
                                   f'{hold_bars}봉)[{grade_str}]')}

        # ════════════════════════════════════════════════════════
        # ★ Step 4 (v35): RSI 과매수 반전 + Profit-Ladder 게이트
        # v34: 수익 > 0  →  v35: 수익 ≥ PROFIT_GATE_RSI_REVERT (기본 1.2%)
        # ════════════════════════════════════════════════════════
        _rsi_gate = PROFIT_GATE_RSI_REVERT if PROFIT_LADDER_ENABLED else 0.0
        if cur_rsi is not None and not (cur_rsi != cur_rsi):
            # 과매수 상태 진입 감지 (게이트와 무관하게 기록)
            if cur_rsi >= RSI_OB_ENTER:
                if held_info is not None:
                    held_info['was_overbought'] = True
                was_overbought = True

            # 과매수 후 냉각 + 수익 게이트 통과 → 매도 발동
            if was_overbought and cur_rsi < RSI_OB_COOL and profit_pct >= _rsi_gate:
                if held_info is not None:
                    held_info['was_overbought'] = False
                return {**base, 'signal': True,
                        'reason': (f'RSI과매수반전({profit_pct:.2f}%≥{_rsi_gate}% '
                                   f'RSI{cur_rsi:.0f}<{RSI_OB_COOL})[{grade_str}]')}

        # ════════════════════════════════════════════════════════
        # ★ Step 5 (v35): 탄력 트레일링 + 고점수익 게이트
        # v34: 고점수익 1% 이상이면 작동
        # v35: 고점수익 PROFIT_GATE_TRAIL_ACTIVATE(1.5%) 이상 올라간 적 있어야 작동
        #      → "비싸게 가본 적 없으면 매도 금지"
        # ════════════════════════════════════════════════════════
        _trail_activate = (PROFIT_GATE_TRAIL_ACTIVATE if PROFIT_LADDER_ENABLED
                           else 1.0)  # v34 거동 복원 시 1.0

        trail_gap = 999
        if peak_pnl >= _trail_activate:
            for threshold, gap in _trail_tiers:
                if peak_pnl >= threshold:
                    trail_gap = gap
                    break

        if trail_gap < 900 and drawdown >= trail_gap:
            return {**base, 'signal': True,
                    'reason': (f'탄력트레일({profit_pct:.2f}%,'
                               f'고점{peak_pnl:.1f}%≥{_trail_activate}%,'
                               f'낙{drawdown:.1f}%≥{trail_gap:.1f}%)[{grade_str}]')}

        # ════════════════════════════════════════════════════════
        # ★ Step 6 (v35): 오전 과열 안전 청산 확장
        # v34: 08~09시 + RSI≥60 + 수익≥0.5%
        # v35: 07:30~09:30 + RSI≥55 + 수익≥1.0%
        # ════════════════════════════════════════════════════════
        m_start = MORNING_EXIT_EXT_START_HM
        m_end   = MORNING_EXIT_EXT_END_HM
        if (m_start <= now_hm < m_end and
                profit_pct >= MORNING_EXIT_EXT_PROFIT and
                cur_rsi is not None and not (cur_rsi != cur_rsi) and
                cur_rsi >= MORNING_EXIT_EXT_RSI):
            return {**base, 'signal': True,
                    'reason': (f'오전과열청산({profit_pct:.2f}%≥{MORNING_EXIT_EXT_PROFIT}% '
                               f'RSI{cur_rsi:.0f}≥{MORNING_EXIT_EXT_RSI} '
                               f'{now_hour:02d}:{now_minute:02d})[{grade_str}]')}

        # ════════════════════════════════════════════════════════
        # ★ Step 7 (v35): 시간 기반 청산 완화
        # v34: 16h 수익>0 / 24h 무조건
        # v35: 24h 수익>0 / 48h 무조건 (추세 지속 시 더 오래 보유)
        # ════════════════════════════════════════════════════════
        if elapsed_hours >= V35_HOLD_PROFIT_EXIT_HOURS and profit_pct > 0:
            return {**base, 'signal': True,
                    'reason': (f'{V35_HOLD_PROFIT_EXIT_HOURS}h수익청산'
                               f'({profit_pct:.2f}%,{elapsed_hours:.1f}h)[{grade_str}]')}

        if elapsed_hours >= V35_HOLD_MAX_EXIT_HOURS:
            return {**base, 'signal': True,
                    'reason': (f'{V35_HOLD_MAX_EXIT_HOURS}h만료청산'
                               f'({profit_pct:.2f}%,{elapsed_hours:.1f}h)[{grade_str}]')}

        # ─── 홀딩 상태: 상세 정보 표시 ────────────────────────────
        if trail_gap < 900:
            trail_str = f"트레일:{trail_gap:.1f}%(낙{drawdown:.1f}%)"
        elif peak_pnl < _trail_activate:
            trail_str = f"트레일:고점{peak_pnl:.1f}%<{_trail_activate}%대기"
        else:
            trail_str = "트레일:미활성"

        if ema5 is not None and ema10 is not None:
            ema_str = f"EMA5:{ema5:.2f}/10:{ema10:.2f}"
            ema_cross = "↑강세" if ema5 >= ema10 else "↓크로스직전"
        else:
            ema_str = "EMA:계산중"
            ema_cross = ""

        rsi_str = f"RSI:{cur_rsi:.0f}" if cur_rsi is not None else "RSI:-"
        ob_str  = "[OB대기]" if was_overbought else ""
        guard_str = f"[가드:{rally_guard_reason}]" if rally_guard_active else ""

        base['reason'] = (
            f'홀딩({profit_pct:.2f}%,고점{peak_pnl:.1f}%,'
            f'{trail_str},{rsi_str}{ob_str}{guard_str},'
            f'{ema_cross},{elapsed_hours:.1f}h)[{grade_str}]'
        )
        return base

    except Exception as e:
        if DEBUG_MODE:
            print(f"{Colors.RED}[Sell Signal v35] 오류: {e}{Colors.ENDC}")
            traceback.print_exc()
        return {'signal': False, 'reason': f'오류:{e}',
                'exit_price': 0, 'profit_pct': 0,
                'bb_position': 50, 'bb_width_pct': 0}
    ...
    for bal in balances:
        ...
        with held_coins_lock:
            held_coins[ticker] = {
                ...
                'entry_type': 'sync',
                'was_overbought': False,   # ★ v34: RSI 과매수 반전 추적
            }
...
def get_cached_prediction(ticker):
    ...
    with prediction_cache_lock:
        ...
        if cached:
            ttl = cached.get('_ttl_override') or PREDICTOR_CACHE_TTL_SEC
    ...
    try:
        ...
        if t.is_alive():
            ...
            if DEBUG_MODE:
                coin_nm = ticker.replace('KRW-', '')
                fail_reason = (pred_result[0] or {}).get('fail_reason', '불명')
                print(f"{Colors.YELLOW}[Predictor] {coin_nm} 예측 실패 (ok=False) 원인: {fail_reason}{Colors.ENDC}")
            with predictor_stats_lock:
                predictor_stats['error_count'] += 1
    ...
    # 캐시 저장 (성공/실패 모두)
    # ★ 실패(ok=False) 시에는 30초 TTL로 단축 — 재학습 완료 후 빠르게 재시도
    cache_ttl_override = None
    if not result['ok']:
        cache_ttl_override = 30  # 실패 캐시는 30초만 유지
    with prediction_cache_lock:
        prediction_cache[ticker] = {
            ...
            '_ttl_override': cache_ttl_override,  # None이면 PREDICTOR_CACHE_TTL_SEC 사용
        }
...
def get_sell_prediction_context(ticker):
    """
    ★ v33.2: 매도용 예측 컨텍스트 조회

    기존 get_cached_prediction()을 공유하되, 매도 전용 해석을 반환.
    fail-safe: 오류 시 모든 변조값 = 0 (기존 로직 그대로)
    ...
    Returns:
        {
          ...
          'ok': bool,
          # ★ 매도 변조값
          'stop_adjust': float,      # 손절선 조정 (%p, 양수=타이트)
          'safe_profit_adjust': float,# 안전익절 기준 조정 (%p)
          'safe_bb_override': int|None, # BB 하한 오버라이드 (None=변경없음)
          'trail_adjust': float,     # 트레일 간격 조정 (%p, 양수=확장)
          'accel_exit': bool,        # 가속 익절 트리거
          'tag': str,                # 로그용 태그
        }
    """
    # 기본값: 변조 없음
    default = {
        'signal': 'NEUTRAL', 'confidence': 'LOW', 'ok': False,
        'stop_adjust': 0.0, 'safe_profit_adjust': 0.0,
        'safe_bb_override': None, 'trail_adjust': 0.0,
        'accel_exit': False, 'tag': '',
    }

    # 마스터 스위치 체크
    if not PREDICTOR_SELL_ENABLED or not PREDICTOR_ENABLED or not PREDICTOR_AVAILABLE:
        return default
    ...
    coin_name = ticker.replace('KRW-', '')
    ctx = {**default, 'signal': signal, 'confidence': confidence, 'ok': True}

    # ── SELL 예측: 매도 가속 모드 ──
    if signal == 'SELL':
        # Step 1: 손절선 타이트닝
        if PRED_SELL_STOP_TIGHTEN:
            ctx['stop_adjust'] = PRED_SELL_STOP_TIGHTEN_PCT

        # Step 2.7: 가속 익절 트리거
        if PRED_SELL_ACCEL_EXIT:
            ctx['accel_exit'] = True

        # Step 3: 안전익절 BB 조건 완화
        if PRED_SELL_SAFE_MODIFY:
            ctx['safe_bb_override'] = PRED_SELL_BB_EASE

        # Step 4: 트레일 타이트닝
        if PRED_SELL_TRAIL_MODIFY:
            ctx['trail_adjust'] = -PRED_SELL_TRAIL_TIGHTEN

        ctx['tag'] = f"[예측매도:SELL🔴 {confidence}]"
        if DEBUG_MODE:
            print(f"{Colors.RED}[Predictor-Sell] {coin_name} SELL → "
                  f"손절+{ctx['stop_adjust']:.1f}%p 가속:{ctx['accel_exit']} "
                  f"BB→{ctx['safe_bb_override']} 트레일{ctx['trail_adjust']:+.1f}%p{Colors.ENDC}")

    # ── BUY 예측: 홀드 보호 모드 ──
    elif signal == 'BUY':
        # Step 3: 안전익절 BB 조건 강화
        if PRED_SELL_SAFE_MODIFY:
            ctx['safe_bb_override'] = PRED_BUY_BB_STRICT

        # Step 3.5: 홀드 보호 — 안전익절 기준 상향
        if PRED_BUY_HOLD_PROTECT:
            ctx['safe_profit_adjust'] = PRED_BUY_SAFE_PROFIT_BOOST

        # Step 4: 트레일 확장
        if PRED_SELL_TRAIL_MODIFY:
            ctx['trail_adjust'] = PRED_BUY_TRAIL_EXPAND

        ctx['tag'] = f"[예측매도:BUY🟢 {confidence}]"
        if DEBUG_MODE:
            print(f"{Colors.GREEN}[Predictor-Sell] {coin_name} BUY → "
                  f"익절+{ctx['safe_profit_adjust']:.1f}%p "
                  f"BB→{ctx['safe_bb_override']} 트레일+{ctx['trail_adjust']:.1f}%p{Colors.ENDC}")

    # ── NEUTRAL: 변조 없음 ──
    else:
        ctx['tag'] = f"[예측매도:NEUTRAL⚪]"
...
def get_predictor_status_str():
    ...
    # ★ v33.2: 매도 예측기 통계 추가
    sell_stats = ""
    sell_total = st['sell_accel_count'] + st['sell_protect_count'] + st['sell_tighten_count'] + st['sell_trail_mod_count']
    if sell_total > 0 or PREDICTOR_SELL_ENABLED:
        sell_stats = (f" | 매도연동: 가속{st['sell_accel_count']} "
                      f"보호{st['sell_protect_count']} "
                      f"손절강화{st['sell_tighten_count']} "
                      f"트레일변조{st['sell_trail_mod_count']}")
    return (
        f"예측기: ON | "
        f"호출:{total} 캐시:{hit_rate:.0f}% | "
        f"매수[차단:{st['veto_count']} 부스트:{st['boost_count']} "
...
def sell_thread_worker():
    ...
    pred_sell_on = 'ON' if PREDICTOR_SELL_ENABLED and PREDICTOR_ENABLED and PREDICTOR_AVAILABLE else 'OFF'
    print(f"{Colors.YELLOW}  └ ★ 가격예측 매도연동: v5.1 ({pred_sell_on}){Colors.ENDC}")
    ...
    while not stop_event.is_set():
        try:
            ...
            for ticker in tickers:
                ...
                with held_coins_lock:
                    if ticker in held_coins:
                        for key in ['peak_price', 'peak_time', 'was_overbought']:
    ...
    try:
        ...
        if PREDICTOR_ENABLED and PREDICTOR_AVAILABLE:
            ...
            if total_pred > 0:
                ...
                pred_section = (
                    f"\n\n🔮 **예측기 v5.1**"
                    f"\n`호출:{total_pred} 캐시:{cache_pct:.0f}% 오류:{ps['error_count']}`"
                    f"\n`매수[차단:{ps['veto_count']} 부스트:{ps['boost_count']} 패스:{ps['pass_count']}]`"
                )
                # ★ v33.2: 매도 연동 통계
                if PREDICTOR_SELL_ENABLED:
                    sell_total = ps['sell_accel_count'] + ps['sell_protect_count'] + ps['sell_tighten_count'] + ps['sell_trail_mod_count']
                    pred_section += (
                        f"\n`매도[가속:{ps['sell_accel_count']} 보호:{ps['sell_protect_count']} "
                        f"손절강화:{ps['sell_tighten_count']} 트레일:{ps['sell_trail_mod_count']} "
                        f"합계:{sell_total}]`"
                    )
                # 캐시에 있는 최근 예측 결과 표시
                with prediction_cache_lock:
...
def main():
    ...
    start_msg = (
        ...
        f"**매도예측:** `{'✅ 양방향연동' if PREDICTOR_SELL_ENABLED and PREDICTOR_ENABLED and PREDICTOR_AVAILABLE else '❌ 매수전용'}`\n\n"
